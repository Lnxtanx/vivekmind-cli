...
function InsightApp({ data }) {
    ...
    const performExport = async () => {
        ...
        try {
            ...
            link.href = imgData;
            link.download = `vivekmind-insights-card-${new Date().toISOString().slice(0, 10)}.png`;
            link.click();
        }
        catch (error) {
    ...
    return (_jsxs("div", { children: [_jsx("header", { className: "insights-header", children: _jsxs("div", { className: "header-content", children: [_jsxs("div", { className: "header-title-section", children: [_jsx("h1", { className: "header-title", children: "VivekMind Insights" }), _jsxs("p", { className: "header-subtitle", children: [data.totalMessages
                                            ? `${data.totalMessages.toLocaleString()} messages across ${data.totalSessions?.toLocaleString()} sessions`
                                            : 'Your personalized coding journey and patterns', dateRangeStr && ` · ${dateRangeStr}`] })] }), _jsx(ExportCardButton, { onExport: handleExportWithTheme })] }) }), data.qualitative && (_jsxs(_Fragment, { children: [_jsx(AtAGlance, { qualitative: data.qualitative }), _jsx(NavToc, {})] })), _jsx(StatsRow, { data: data }), data.qualitative && (_jsx(_Fragment, { children: _jsx(ProjectAreas, { qualitative: data.qualitative, topGoals: data.topGoals, topTools: data.topTools }) })), data.qualitative && (_jsx(_Fragment, { children: _jsx(InteractionStyle, { qualitative: data.qualitative, insights: data }) })), data.qualitative && (_jsxs(_Fragment, { children: [_jsx(ImpressiveWorkflows, { qualitative: data.qualitative, primarySuccess: data.primarySuccess, outcomes: data.outcomes }), _jsx(FrictionPoints, { qualitative: data.qualitative, satisfaction: data.satisfaction, friction: data.friction }), _jsx(Improvements, { qualitative: data.qualitative }), _jsx(FutureOpportunities, { qualitative: data.qualitative }), _jsx(MemorableMoment, { qualitative: data.qualitative })] })), _jsx(ShareCard, { data: data, theme: cardTheme })] }));
}
