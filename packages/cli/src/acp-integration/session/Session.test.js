...
function createStreamWithChunks(chunks) {
    return (async function* () {
...
function expectCompressBeforeSend(compressMock, sendMock, callIndex) {
    expect(compressMock.mock.invocationCallOrder.length).toBeGreaterThan(callIndex);
    expect(sendMock.mock.invocationCallOrder.length).toBeGreaterThan(callIndex);
    expect(compressMock.mock.invocationCallOrder[callIndex]).toBeLessThan(sendMock.mock.invocationCallOrder[callIndex]);
}
describe('Session', () => {
    let mockChat;
    let mockConfig;
    ...
    beforeEach(() => {
        ...
        switchModelSpy = vi
            .fn()
            .mockImplementation(async (authType, modelId) => {
        ...
        session = new Session('test-session-id', mockConfig, mockClient, mockSettings);
    });
    afterEach(() => {
        ...
        mockGeminiClient = undefined;
        mockToolRegistry = undefined;
        vi.restoreAllMocks();
        vi.clearAllTimers();
    });
    describe('setMode', () => {
        ...
        ])('maps %s mode', async (modeId, expected) => {
    ...
    describe('setModel', () => {
        it('sets model via config and returns current model', async () => {
            ...
            expect(mockConfig.switchModel).toHaveBeenCalledWith(AuthType.USE_OPENAI, 'qwen3-coder-plus', undefined);
        });
        it('rejects empty/whitespace model IDs', async () => {
            await expect(session.setModel({
                sessionId: 'test-session-id',
                modelId: '   ',
    ...
    describe('sendAvailableCommandsUpdate', () => {
        it('sends available_commands_update from getAvailableCommands()', async () => {
            ...
            expect(getAvailableCommandsSpy).toHaveBeenCalledWith(mockConfig, expect.any(AbortSignal), 'acp');
        ...
        it('swallows errors and does not throw', async () => {
            getAvailableCommandsSpy.mockRejectedValueOnce(new Error('Command discovery failed'));
            await expect(session.sendAvailableCommandsUpdate()).resolves.toBeUndefined();
    ...
    describe('prompt', () => {
        describe('auto-compress', () => {
            it('runs automatic compression before sending an ACP prompt', async () => {
                ...
                expect(mockGeminiClient.tryCompressChat).toHaveBeenCalledWith('test-session-id########1', false, expect.any(AbortSignal));
                const sendMessageStream = mockChat.sendMessageStream;
                expectCompressBeforeSend(mockGeminiClient.tryCompressChat, sendMessageStream, 0);
            });
            it('uses the current chat after automatic compression replaces it', async () => {
                ...
                expect(compressedChat.sendMessageStream).toHaveBeenCalledWith('qwen3-code-plus', {
            ...
            it('emits an ACP-visible update when automatic compression succeeds', async () => {
                ...
                expect(mockClient.sessionUpdate).toHaveBeenCalledWith({
                    ...
                    update: {
                        ...
                        content: {
                            type: 'text',
                            text: 'IMPORTANT: This conversation approached the input token limit for qwen3-code-plus. ' +
            ...
            it('continues sending when automatic compression fails', async () => {
                mockGeminiClient.tryCompressChat.mockRejectedValueOnce(new Error('compression rate limited'));
                mockChat.sendMessageStream = vi
                    .fn()
                    .mockResolvedValue(createEmptyStream());
                await session.prompt({
                ...
                expect(mockGeminiClient.tryCompressChat).toHaveBeenCalledWith('test-session-id########1', false, expect.any(AbortSignal));
                expect(mockChat.sendMessageStream).toHaveBeenCalledWith('qwen3-code-plus', {
                    message: expect.any(Array),
                    config: { abortSignal: expect.any(AbortSignal) },
                }, 'test-session-id########1');
            });
            it('does not use global UI telemetry when compression fails before local token counts exist', async () => {
                ...
                vi.spyOn(core.uiTelemetryService, 'getLastPromptTokenCount').mockReturnValue(101);
                mockGeminiClient.tryCompressChat.mockRejectedValueOnce(new Error('compression rate limited'));
                mockChat.sendMessageStream = vi
                    .fn()
                    .mockResolvedValue(createEmptyStream());
                ...
                expect(mockClient.sessionUpdate).not.toHaveBeenCalledWith(expect.objectContaining({
            ...
            it('returns cancelled when automatic compression is aborted', async () => {
                mockConfig.getSessionTokenLimit = vi.fn().mockReturnValue(100);
                mockGeminiClient.tryCompressChat.mockImplementation(async (_promptId, _force, signal) => new Promise((_, reject) => {
                ...
                expect(mockClient.sessionUpdate).not.toHaveBeenCalledWith({
                    ...
                    update: {
                        ...
                        content: {
                            type: 'text',
                            text: 'Session token limit exceeded: 101 tokens > 100 limit. ' +
            ...
            it('uses compression token info instead of global UI telemetry for the session limit', async () => {
                mockConfig.getSessionTokenLimit = vi.fn().mockReturnValue(100);
                vi.spyOn(core.uiTelemetryService, 'getLastPromptTokenCount').mockReturnValue(999);
                mockGeminiClient.tryCompressChat.mockResolvedValueOnce({
            ...
            it('falls back to the previous prompt token count when compression returns zero token info', async () => {
                ...
                mockChat.sendMessageStream = vi
                    .fn()
                    .mockResolvedValueOnce(createStreamWithChunks([
                    {
            ...
            it('resets the session-local token count when the active chat instance changes', async () => {
                ...
                mockGeminiClient.tryCompressChat
                    ...
                    .mockRejectedValueOnce(new Error('compression unavailable'));
                mockChat.sendMessageStream = vi.fn().mockResolvedValueOnce(createStreamWithChunks([
                    {
                        type: core.StreamEventType.CHUNK,
                        value: {
                            usageMetadata: {
            ...
            it('also runs automatic compression before tool response follow-up sends', async () => {
                ...
                expect(mockGeminiClient.tryCompressChat).toHaveBeenNthCalledWith(2, 'test-session-id########1', false, expect.any(AbortSignal));
                const sendMessageStream = mockChat.sendMessageStream;
                expectCompressBeforeSend(mockGeminiClient.tryCompressChat, sendMessageStream, 1);
            });
            ...
            it('runs automatic compression before Stop-hook continuation sends', async () => {
                ...
                mockConfig.hasHooksForEvent = vi
                    .fn()
                    .mockImplementation((eventName) => eventName === 'Stop');
                ...
                expect(mockChat.sendMessageStream).toHaveBeenCalledTimes(2);
                expect(mockGeminiClient.tryCompressChat).toHaveBeenNthCalledWith(2, 'test-session-id########1_stop_hook_1', false, expect.any(AbortSignal));
                const sendMessageStream = mockChat.sendMessageStream;
                expectCompressBeforeSend(mockGeminiClient.tryCompressChat, sendMessageStream, 1);
            });
            it('skips automatic compression after the first Stop-hook continuation', async () => {
                ...
                expect(mockGeminiClient.tryCompressChat).toHaveBeenCalledTimes(2);
                expect(mockGeminiClient.tryCompressChat).toHaveBeenNthCalledWith(2, 'test-session-id########1_stop_hook_1', false, expect.any(AbortSignal));
                expect(mockGeminiClient.tryCompressChat).not.toHaveBeenCalledWith('test-session-id########1_stop_hook_2', false, expect.any(AbortSignal));
                const sendMessageStream = mockChat.sendMessageStream;
                expect(sendMessageStream.mock.calls[2]?.[2]).toBe('test-session-id########1_stop_hook_2');
            });
            ...
            it('runs automatic compression before cron-fired ACP prompt sends', async () => {
                ...
                expect(mockGeminiClient.tryCompressChat).toHaveBeenNthCalledWith(1, 'test-session-id########1', false, expect.any(AbortSignal));
                expect(mockGeminiClient.tryCompressChat).toHaveBeenNthCalledWith(2, expect.stringMatching(/^test-session-id########cron\d+$/), false, expect.any(AbortSignal));
                const sendMessageStream = mockChat.sendMessageStream;
                expectCompressBeforeSend(mockGeminiClient.tryCompressChat, sendMessageStream, 1);
            });
            it('stops cron-fired ACP prompt before sending when the session token limit is exceeded', async () => {
                ...
                const sessionUpdateMock = mockClient.sessionUpdate;
                const tokenLimitDiagnosticCount = () => sessionUpdateMock.mock.calls.filter((call) => {
                    const notification = call[0];
                    return (notification.update?.sessionUpdate === 'agent_message_chunk' &&
                        notification.update.content?.type === 'text' &&
                        notification.update.content.text?.includes('Session token limit exceeded'));
                }).length;
            ...
            it('does not auto-compress slash commands handled without a model send', async () => {
                vi.mocked(nonInteractiveCliCommands.handleSlashCommand).mockResolvedValueOnce({
                    type: 'message',
                    messageType: 'info',
        ...
        it('passes resolved paths to read_many_files tool', async () => {
            const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'qwen-acp-session-'));
        ...
        it('runs prompt inside runtime output dir context', async () => {
            ...
            session = new Session('test-session-id', mockConfig, mockClient, mockSettings);
            const runWithRuntimeBaseDirSpy = vi.spyOn(core.Storage, 'runWithRuntimeBaseDir');
            try {
                mockChat.sendMessageStream = vi
                    .fn()
                    .mockResolvedValue(createEmptyStream());
                ...
                await session.prompt(promptRequest);
                expect(runWithRuntimeBaseDirSpy).toHaveBeenCalledWith(runtimeDir, process.cwd(), expect.any(Function));
            }
            finally {
        ...
        it('hides allow-always options when confirmation already forbids them', async () => {
            ...
            expect(mockClient.requestPermission).toHaveBeenCalledWith(expect.objectContaining({
                options: [
                    expect.objectContaining({ kind: 'allow_once' }),
                    expect.objectContaining({ kind: 'reject_once' }),
                ],
            }));
            const options = mockClient.requestPermission
        ...
        it('allows info confirmation tools in plan mode', async () => {
            ...
            expect(onConfirmSpy).toHaveBeenCalledWith(core.ToolConfirmationOutcome.ProceedOnce, { answers: undefined });
        ...
        it('respects permission-request hook allow decisions without opening ACP permission dialog', async () => {
            ...
            expect(mockClient.requestPermission).not.toHaveBeenCalled();
            expect(onConfirmSpy).toHaveBeenCalledWith(core.ToolConfirmationOutcome.ProceedOnce);
        ...
        describe('hooks', () => {
            describe('UserPromptSubmit hook', () => {
                it('fires UserPromptSubmit hook before sending prompt', async () => {
                    ...
                    expect(messageBus.request).toHaveBeenCalledWith(expect.objectContaining({
            ...
            describe('StopFailure hook', () => {
                it('fires StopFailure hook when API error occurs during sendMessageStream', async () => {
                    ...
                    expect(mockFireStopFailureEvent).toHaveBeenCalledWith('rate_limit', 'Rate limit exceeded');
                });
        ...
        describe('tool call concurrency', () => {
            it('runs multiple Agent tool calls concurrently (issue #2516)', async () => {
                ...
                mockToolRegistry.getTool.mockImplementation((name) => name === core.ToolNames.AGENT ? agentTool : undefined);
                mockConfig.getApprovalMode = vi
                    .fn()
                    .mockReturnValue(ApprovalMode.DEFAULT);
                mockConfig.getPermissionManager = vi.fn().mockReturnValue(null);
                ...
                const followUp = sendMessageStream.mock.calls[1][1];
        ...
        describe('system reminders', () => {
            ...
            const stubEmptySubagents = () => {
                mockConfig['getSubagentManager'] = vi.fn().mockReturnValue({
                    listSubagents: vi.fn().mockResolvedValue([]),
                });
                // ensureTool is called on the result of getToolRegistry(); add it.
                mockToolRegistry.ensureTool = vi.fn().mockResolvedValue(true);
            };
            it('prepends plan-mode reminder when approval mode is PLAN (#1151)', async () => {
                ...
                const reminderPart = capture.parts.find((p) => p.text && p.text.includes('Plan mode is active'));
                ...
                const userIdx = capture.parts.findIndex((p) => p.text === 'research this');
            ...
            it('does not prepend plan-mode reminder in default approval mode', async () => {
                ...
                const hasPlanReminder = capture.parts.some((p) => p.text && p.text.includes('Plan mode is active'));
            ...
            it('prepends subagent reminder when user-level subagents exist', async () => {
                ...
                const reminder = capture.parts.find((p) => p.text &&
