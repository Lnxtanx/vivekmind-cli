 ...
 */
import { AuthType, ApprovalMode, CompressionStatus, convertToFunctionResponse, createDebugLogger, DiscoveredMCPTool, StreamEventType, ToolConfirmationOutcome, logToolCall, logUserPrompt, getErrorStatus, UserPromptEvent, readManyFiles, Storage, ToolNames, fireNotificationHook, firePermissionRequestHook, firePreToolUseHook, firePostToolUseHook, firePostToolUseFailureHook, injectPermissionRulesIfMissing, NotificationType, persistPermissionOutcome, createHookOutput, generateToolUseId, MessageBusType, getPlanModeSystemReminder, getSubagentSystemReminder, getArenaSystemReminder, evaluatePermissionFlow, needsConfirmation, isPlanModeBlocked, } from '@vivekmind/core';
...
export class Session {
    ...
    async #executePrompt(params, pendingSend) {
        return Storage.runWithRuntimeBaseDir(this.runtimeBaseDir, this.config.getWorkingDir(), async () => {
            ...
            while (nextMessage !== null) {
                ...
                try {
                    const sendResult = await this.#sendMessageStreamWithAutoCompression(promptId, nextMessage?.parts ?? [], pendingSend.signal);
                ...
                if (functionCalls.length > 0) {
                    const toolResponseParts = await this.runToolCalls(pendingSend.signal, promptId, functionCalls);
            ...
            return this.#handleStopHookLoop(pendingSend, promptId, hooksEnabled, messageBus);
        });
    }
    /**
    ...
    async #handleStopHookLoop(pendingSend, promptId, hooksEnabled, messageBus) {
        ...
        while (stopHookIterationCount < MAX_STOP_HOOK_ITERATIONS) {
            ...
            if (stopOutput?.isBlockingDecision() ||
                ...
                if (stopHookIterationCount > 1) {
                    await this.messageEmitter.emitStopHookLoop(stopHookIterationCount, stopHookReasons, response.stopHookCount ?? 1);
                }
                ...
                while (nextMessage !== null) {
                    ...
                    const streamStartTime = Date.now();
                    try {
                        const continueSendResult = await this.#sendMessageStreamWithAutoCompression(promptId + '_stop_hook_' + stopHookIterationCount, nextMessage?.parts ?? [], pendingSend.signal, { skipCompression: stopHookIterationCount > 1 });
                        if (!continueSendResult.responseStream) {
                            this.#preserveUnsentMessageHistory(nextMessage, continueSendResult.stopReason === 'cancelled');
        ...
        if (stopHookIterationCount >= MAX_STOP_HOOK_ITERATIONS) {
            debugLogger.warn(`Stop hook loop reached maximum iterations (${MAX_STOP_HOOK_ITERATIONS}), forcing stop`);
        }
        return { stopReason: 'end_turn' };
    }
    ...
    async #sendMessageStreamWithAutoCompression(promptId, message, abortSignal, options = {}) {
        ...
        const responseStream = await this.#getCurrentChat().sendMessageStream(this.config.getModel(), {
            message,
            config: {
                abortSignal,
            },
     ...
     */
    async #executeCronPrompt(prompt) {
        return Storage.runWithRuntimeBaseDir(this.runtimeBaseDir, this.config.getWorkingDir(), async () => {
            ...
            try {
                ...
                while (nextMessage !== null) {
                    ...
                    if (functionCalls.length > 0) {
                        const toolResponseParts = await this.runToolCalls(ac.signal, promptId, functionCalls);
                        nextMessage = { role: 'user', parts: toolResponseParts };
                    }
                }
            }
    ...
    #stopCronAfterTokenLimit() {
        ...
        void this.#emitAgentDiagnosticMessageSafely('Cron jobs disabled for the rest of this session due to token limit. Restart the session to re-enable.', 'Failed to emit cron-disabled diagnostic');
    }
    async sendAvailableCommandsUpdate() {
        const abortController = new AbortController();
        try {
            // Load commands available in ACP mode
            const slashCommands = await getAvailableCommands(this.config, abortController.signal, 'acp');
            ...
            const availableCommands = slashCommands.map((cmd) => {
                const acceptsInput = cmd.kind !== CommandKind.BUILT_IN ||
                    cmd.completion != null ||
    ...
    async setModel(params) {
        ...
        await this.config.switchModel(selectedAuthType, parsed.modelId, selectedAuthType !== previousAuthType &&
    ...
    async runToolCalls(abortSignal, promptId, functionCalls) {
        ...
        // `VIVEKMIND_CODE_MAX_TOOL_CONCURRENCY` (default 10). Results are returned
        // in input order regardless of resolution order.
        const runBounded = async (calls) => {
            const parsed = parseInt(process.env['VIVEKMIND_CODE_MAX_TOOL_CONCURRENCY'] || '', 10);
    ...
    async runTool(abortSignal, promptId, fc) {
        ...
        const earlyErrorResponse = async (error, toolName = fc.name ?? 'unknown_tool') => {
        ...
        if (!tool) {
            return earlyErrorResponse(new Error(`Tool "${fc.name}" not found in registry.`));
        }
        // ---- L1: Tool enablement check ----
        const pm = this.config.getPermissionManager?.();
        if (pm && !(await pm.isToolEnabled(fc.name))) {
            return earlyErrorResponse(new Error(`VivekMind requires permission to use "${fc.name}", but that permission was declined.`), fc.name);
        }
        ...
        try {
            ...
            if (isAgentTool && taskEventEmitter) {
                ...
                subAgentCleanupFunctions = subSubAgentTracker.setup(taskEventEmitter, abortSignal);
            }
            ...
            const flowResult = await evaluatePermissionFlow(this.config, invocation, fc.name, toolParams);
            const { finalPermission, pmForcedAsk, pmCtx, denyMessage } = flowResult;
            // ---- L5: ApprovalMode overrides ----
            const isPlanMode = approvalMode === ApprovalMode.PLAN;
            if (finalPermission === 'deny') {
                return earlyErrorResponse(new Error(denyMessage ?? `Tool "${fc.name}" is denied.`), fc.name);
            }
            ...
            if (needsConfirmation(finalPermission, approvalMode, fc.name)) {
                ...
                if (isPlanModeBlocked(isPlanMode, isExitPlanModeTool, isAskUserQuestionTool, confirmationDetails)) {
                    return earlyErrorResponse(new Error(`Plan mode is active. The tool "${fc.name}" cannot be executed because it modifies the system. ` +
                        'Please use the exit_plan_mode tool to present your plan and exit plan mode before making changes.'), fc.name);
                }
                const messageBus = this.config.getMessageBus?.();
                const hooksEnabled = !this.config.getDisableAllHooks?.();
                let hookHandled = false;
                if (hooksEnabled && messageBus) {
                    const hookResult = await firePermissionRequestHook(messageBus, fc.name, args, String(approvalMode));
                    if (hookResult.hasDecision) {
                        ...
                        if (hookResult.shouldAllow) {
                            ...
                            return earlyErrorResponse(new Error(hookResult.denyMessage ||
                                `Permission denied by hook for "${fc.name}"`), fc.name);
                        }
                    }
                }
                ...
                if (approvalMode === ApprovalMode.AUTO_EDIT &&
                    ...
                    if (hooksEnabled && messageBus) {
                        void fireNotificationHook(messageBus, `VivekMind needs your permission to use ${fc.name}`, NotificationType.PermissionPrompt, 'Permission needed');
                    }
                    const params = {
                        sessionId: this.sessionId,
                    ...
                    if (outcome === ToolConfirmationOutcome.ProceedAlways ||
                        ...
                        await persistPermissionOutcome(outcome, confirmationDetails, this.config.getOnPersistPermissionRule?.(), this.config.getPermissionManager?.(), { answers: output.answers });
                    }
            ...
            if (hooksEnabledForTool && messageBusForTool) {
                const preHookResult = await firePreToolUseHook(messageBusForTool, fc.name, args, toolUseId, permissionMode, abortSignal);
                if (!preHookResult.shouldProceed) {
                    ...
                    await this.messageEmitter.emitAgentMessage(`🚫 **PreToolUse blocked**: ${fc.name} - ${blockReason}`);
            ...
            if (hooksEnabledForTool && messageBusForTool && !toolResult.error) {
                ...
                const postHookResult = await firePostToolUseHook(messageBusForTool, fc.name, args, toolResponse, toolUseId, permissionMode, abortSignal);
                ...
                if (postHookResult.shouldStop) {
                    ...
                    debugLogger.info(`PostToolUse hook requested stop for ${fc.name}: ${stopMessage}`);
                ...
                const failureHookResult = await firePostToolUseFailureHook(messageBusForTool, toolUseId, fc.name ?? 'unknown_tool', args, toolResult.error.message, false, // not an interrupt
                permissionMode, abortSignal);
                // Log additional context if provided
                if (failureHookResult.additionalContext) {
                    debugLogger.debug(`PostToolUseFailure hook additional context for ${fc.name}: ${failureHookResult.additionalContext}`);
                }
            }
            ...
            if (hooksEnabledForError && messageBusForError) {
                const failureHookResult = await firePostToolUseFailureHook(messageBusForError, toolUseId, fc.name ?? 'unknown_tool', args, error.message, isInterrupt, String(approvalMode), abortSignal);
                // Log additional context if provided
                if (failureHookResult.additionalContext) {
                    debugLogger.debug(`PostToolUseFailure hook additional context for ${fc.name}: ${failureHookResult.additionalContext}`);
                }
            ...
            await this.toolCallEmitter.emitError(callId, fc.name ?? 'unknown_tool', error);
        ...
        switch (result.type) {
            ...
            case 'stream_messages': {
                ...
                for await (const msg of result.messages) {
                    ...
                    await this.messageEmitter.emitAgentMessage((msg.content || '').replace(/\n/g, '  \n'));
            ...
            case 'no_command':
                ...
                return this.#resolvePrompt(originalPrompt, new AbortController().signal);
            default: {
    ...
    async #resolvePrompt(message, abortSignal) {
        ...
        const pathSpecsToRead = atPathCommandParts.map((part) => part.fileData.fileUri);
