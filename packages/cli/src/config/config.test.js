...
import { ToolNames, DEFAULT_VIVEKMIND_MODEL, OutputFormat, NativeLspService, Storage, } from '@vivekmind/core';
import { loadCliConfig, parseArguments } from './config.js';
import * as ServerConfig from '@vivekmind/core';
...
vi.mock('read-package-up', () => ({
    readPackageUp: vi.fn(() => Promise.resolve({
        packageJson: {
...
describe('parseArguments', () => {
    ...
    it('should throw an error when both --prompt and --prompt-interactive are used together', async () => {
        ...
        expect(mockWriteStderrLine).toHaveBeenCalledWith(expect.stringContaining('Cannot use both --prompt (-p) and --prompt-interactive (-i) together'));
    ...
    it('should throw an error when both --yolo and --approval-mode are used together', async () => {
        ...
        mockWriteStderrLine.mockClear();
        await expect(parseArguments()).rejects.toThrow('process.exit called');
        expect(mockWriteStderrLine).toHaveBeenCalledWith(expect.stringContaining('Cannot use both --yolo (-y) and --approval-mode together. Use --approval-mode=yolo instead.'));
        mockExit.mockRestore();
    });
    ...
    it('should throw an error when include-partial-messages is used without stream-json output', async () => {
        ...
        mockWriteStderrLine.mockClear();
        await expect(parseArguments()).rejects.toThrow('process.exit called');
        expect(mockWriteStderrLine).toHaveBeenCalledWith(expect.stringContaining('--include-partial-messages requires --output-format stream-json'));
        mockExit.mockRestore();
    });
    ...
    it('should reject invalid --approval-mode values', async () => {
        ...
        mockWriteStderrLine.mockClear();
        await expect(parseArguments()).rejects.toThrow('process.exit called');
        expect(mockWriteStderrLine).toHaveBeenCalledWith(expect.stringContaining('Invalid values:'));
        mockExit.mockRestore();
    });
...
describe('loadCliConfig', () => {
    ...
    it('should reset context file names to QWEN.md and AGENTS.md by default', async () => {
        ...
        const argv = await parseArguments();
        const settings = {};
        const setGeminiMdFilenameSpy = vi.spyOn(ServerConfig, 'setGeminiMdFilename');
    ...
    it('should use context.fileName from settings when provided', async () => {
        process.argv = ['node', 'script.js'];
        const argv = await parseArguments();
        const settings = { context: { fileName: 'CUSTOM_CONTEXT.md' } };
    ...
    describe('Proxy configuration', () => {
        ...
        it('should set proxy from settings when present', async () => {
            process.argv = ['node', 'script.js'];
            const argv = await parseArguments();
            const settings = { proxy: 'http://localhost:7890' };
            const config = await loadCliConfig(settings, argv);
            expect(config.getProxy()).toBe('http://localhost:7890');
        });
        it('should normalize proxy from settings when scheme is omitted', async () => {
            process.argv = ['node', 'script.js'];
            const argv = await parseArguments();
            const settings = { proxy: 'localhost:7890' };
            const config = await loadCliConfig(settings, argv);
            expect(config.getProxy()).toBe('http://localhost:7890');
        });
        ...
        it('should prioritize CLI flag over settings proxy', async () => {
            process.argv = ['node', 'script.js', '--proxy', 'http://localhost:7890'];
            const argv = await parseArguments();
            const settings = { proxy: 'http://localhost:7891' };
            const config = await loadCliConfig(settings, argv);
            expect(config.getProxy()).toBe('http://localhost:7890');
        });
    });
...
describe('mergeExcludeTools', () => {
    ...
    it('should handle settings with excludeTools but no extensions', async () => {
        ...
        const config = await loadCliConfig(settings, argv, undefined, []);
        expect(config.getPermissionsDeny()).toEqual(expect.arrayContaining(['tool1', 'tool2']));
...
describe('Approval mode tool exclusion logic', () => {
    ...
    it('should throw an error for invalid approval mode values in loadCliConfig', async () => {
        ...
        const settings = {};
        await expect(loadCliConfig(settings, invalidArgv, undefined, [])).rejects.toThrow('Invalid approval mode: invalid_mode. Valid values are: plan, default, auto-edit, yolo');
    });
});
...
describe('loadCliConfig model selection', () => {
    ...
    it.skip('uses the default gemini model if nothing is set', async () => {
        ...
        expect(config.getModel()).toBe(DEFAULT_VIVEKMIND_MODEL);
    });
...
describe('loadCliConfig approval mode', () => {
    ...
    it('should throw when approval mode in settings is invalid', async () => {
        ...
        await expect(loadCliConfig(settings, argv, undefined, [])).rejects.toThrow('Invalid approval mode: invalid_mode. Valid values are: plan, default, auto-edit, yolo');
    });
...
describe('loadCliConfig fileFiltering', () => {
    ...
    const testCases = [
        ...
        {
            property: 'respectVivekMindIgnore',
            getter: (c) => c.getFileFilteringRespectVivekMindIgnore(),
            value: true,
        },
        {
            property: 'respectVivekMindIgnore',
...
describe('Output format', () => {
    ...
    it('should use the format from settings', async () => {
        process.argv = ['node', 'script.js'];
        const argv = await parseArguments();
        const config = await loadCliConfig({ output: { format: OutputFormat.JSON } }, argv, undefined, []);
...
describe('parseArguments with positional prompt', () => {
    ...
    it('should throw an error when both a positional prompt and the --prompt flag are used', async () => {
        ...
        mockWriteStderrLine.mockClear();
        await expect(parseArguments()).rejects.toThrow('process.exit called');
        expect(mockWriteStderrLine).toHaveBeenCalledWith(expect.stringContaining('Cannot use both a positional prompt and the --prompt (-p) flag together'));
        mockExit.mockRestore();
    });
...
describe('Telemetry configuration via environment variables', () => {
    it('should prioritize VIVEKMIND_TELEMETRY_ENABLED over settings', async () => {
        vi.stubEnv('VIVEKMIND_TELEMETRY_ENABLED', 'true');
        process.argv = ['node', 'script.js'];
        const argv = await parseArguments();
        const settings = { telemetry: { enabled: false } };
        const config = await loadCliConfig(settings, argv, undefined, []);
    ...
    it('should prioritize VIVEKMIND_TELEMETRY_TARGET over settings', async () => {
        vi.stubEnv('VIVEKMIND_TELEMETRY_TARGET', 'gcp');
        process.argv = ['node', 'script.js'];
        const argv = await parseArguments();
        const settings = {
    ...
    it('should throw when VIVEKMIND_TELEMETRY_TARGET is invalid', async () => {
        vi.stubEnv('VIVEKMIND_TELEMETRY_TARGET', 'bogus');
        process.argv = ['node', 'script.js'];
        const argv = await parseArguments();
        const settings = {
            telemetry: { target: 'gcp' },
        };
        await expect(loadCliConfig(settings, argv, undefined, [])).rejects.toThrow(/Invalid telemetry configuration: .*Invalid telemetry target/i);
        vi.unstubAllEnvs();
    });
    it('should prioritize VIVEKMIND_TELEMETRY_OTLP_ENDPOINT over settings and default env var', async () => {
        vi.stubEnv('OTEL_EXPORTER_OTLP_ENDPOINT', 'http://default.env.com');
        vi.stubEnv('VIVEKMIND_TELEMETRY_OTLP_ENDPOINT', 'http://gemini.env.com');
        process.argv = ['node', 'script.js'];
        const argv = await parseArguments();
        const settings = {
    ...
    it('should prioritize VIVEKMIND_TELEMETRY_OTLP_PROTOCOL over settings', async () => {
        vi.stubEnv('VIVEKMIND_TELEMETRY_OTLP_PROTOCOL', 'http');
        process.argv = ['node', 'script.js'];
        const argv = await parseArguments();
    ...
    it('should prioritize VIVEKMIND_TELEMETRY_LOG_PROMPTS over settings', async () => {
        vi.stubEnv('VIVEKMIND_TELEMETRY_LOG_PROMPTS', 'false');
        process.argv = ['node', 'script.js'];
        const argv = await parseArguments();
        const settings = { telemetry: { logPrompts: true } };
        const config = await loadCliConfig(settings, argv, undefined, []);
    ...
    it('should prioritize VIVEKMIND_TELEMETRY_OUTFILE over settings', async () => {
        vi.stubEnv('VIVEKMIND_TELEMETRY_OUTFILE', '/gemini/env/telemetry.log');
        process.argv = ['node', 'script.js'];
        const argv = await parseArguments();
        const settings = {
    ...
    it('should prioritize VIVEKMIND_TELEMETRY_USE_COLLECTOR over settings', async () => {
        vi.stubEnv('VIVEKMIND_TELEMETRY_USE_COLLECTOR', 'true');
        process.argv = ['node', 'script.js'];
        const argv = await parseArguments();
    ...
    it('should use settings value when VIVEKMIND_TELEMETRY_ENABLED is not set', async () => {
        vi.stubEnv('VIVEKMIND_TELEMETRY_ENABLED', undefined);
        process.argv = ['node', 'script.js'];
        const argv = await parseArguments();
        const settings = { telemetry: { enabled: true } };
        const config = await loadCliConfig(settings, argv, undefined, []);
    ...
    it('should use settings value when VIVEKMIND_TELEMETRY_TARGET is not set', async () => {
        vi.stubEnv('VIVEKMIND_TELEMETRY_TARGET', undefined);
        process.argv = ['node', 'script.js'];
        const argv = await parseArguments();
        const settings = {
            telemetry: { target: 'local' },
    ...
    it("should treat VIVEKMIND_TELEMETRY_ENABLED='1' as true", async () => {
        vi.stubEnv('VIVEKMIND_TELEMETRY_ENABLED', '1');
        process.argv = ['node', 'script.js'];
        const argv = await parseArguments();
        const config = await loadCliConfig({}, argv, undefined, []);
        expect(config.getTelemetryEnabled()).toBe(true);
    });
    it("should treat VIVEKMIND_TELEMETRY_ENABLED='0' as false", async () => {
        vi.stubEnv('VIVEKMIND_TELEMETRY_ENABLED', '0');
        process.argv = ['node', 'script.js'];
        const argv = await parseArguments();
        const config = await loadCliConfig({ telemetry: { enabled: true } }, argv, undefined, []);
        expect(config.getTelemetryEnabled()).toBe(false);
    });
    it("should treat VIVEKMIND_TELEMETRY_LOG_PROMPTS='1' as true", async () => {
        vi.stubEnv('VIVEKMIND_TELEMETRY_LOG_PROMPTS', '1');
        process.argv = ['node', 'script.js'];
        const argv = await parseArguments();
        const config = await loadCliConfig({}, argv, undefined, []);
        expect(config.getTelemetryLogPromptsEnabled()).toBe(true);
    });
    it("should treat VIVEKMIND_TELEMETRY_LOG_PROMPTS='false' as false", async () => {
        vi.stubEnv('VIVEKMIND_TELEMETRY_LOG_PROMPTS', 'false');
        process.argv = ['node', 'script.js'];
        const argv = await parseArguments();
        const config = await loadCliConfig({ telemetry: { logPrompts: true } }, argv, undefined, []);
        expect(config.getTelemetryLogPromptsEnabled()).toBe(false);
    });
});
describe('sandbox image resolution precedence', () => {
    ...
    beforeEach(() => {
        ...
        vi.mocked(os.homedir).mockReturnValue('/mock/home/user');
        vi.stubEnv('GEMINI_API_KEY', 'test-api-key');
        delete process.env['VIVEKMIND_SANDBOX_IMAGE'];
    });
    afterEach(() => {
        process.argv = originalArgv;
        vi.unstubAllEnvs();
    ...
    it('uses --sandbox-image over env and settings', async () => {
        vi.stubEnv('VIVEKMIND_SANDBOX_IMAGE', 'env-image');
        process.argv = [
            'node',
            'script.js',
            '--sandbox',
    ...
    it('uses VIVEKMIND_SANDBOX_IMAGE over tools.sandboxImage', async () => {
        vi.stubEnv('VIVEKMIND_SANDBOX_IMAGE', 'env-image');
...
describe('loadCliConfig runtimeOutputDir', () => {
    const originalArgv = process.argv;
    const originalRuntimeEnv = process.env['VIVEKMIND_RUNTIME_DIR'];
    beforeEach(() => {
        process.argv = ['node', 'script.js'];
    ...
    afterEach(() => {
        ...
        if (originalRuntimeEnv !== undefined) {
            process.env['VIVEKMIND_RUNTIME_DIR'] = originalRuntimeEnv;
        }
        else {
            delete process.env['VIVEKMIND_RUNTIME_DIR'];
        }
    ...
    it('should resolve relative runtimeOutputDir against cwd', async () => {
        const argv = await parseArguments();
        const settings = {
            advanced: { runtimeOutputDir: '.vivekmind' },
        };
    ...
    it('should let VIVEKMIND_RUNTIME_DIR env var take priority over settings', async () => {
    ...
    it('should reset runtime base dir on subsequent load when runtimeOutputDir is absent', async () => {
        ...
        await loadCliConfig({ advanced: { runtimeOutputDir: firstRuntimeDir } }, argv);
