...
function getSandboxCommand(sandbox) {
    ...
    const environmentConfiguredSandbox = process.env['VIVEKMIND_SANDBOX']?.toLowerCase().trim() ?? '';
    sandbox =
    ...
    if (typeof sandbox === 'string' && sandbox) {
        ...
        throw new FatalSandboxError(`Missing sandbox command '${sandbox}' (from VIVEKMIND_SANDBOX)`);
    }
    ...
    if (sandbox === true) {
        throw new FatalSandboxError('VIVEKMIND_SANDBOX is true but failed to determine command for sandbox; ' +
            'install docker or podman or specify command in VIVEKMIND_SANDBOX');
    }
    return '';
}
export async function loadSandboxConfig(settings, argv) {
    ...
    const image = argv.sandboxImage ??
        process.env['VIVEKMIND_SANDBOX_IMAGE'] ??
