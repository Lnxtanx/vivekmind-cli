 ...
 */
function findModelConfig(modelProviders, authType, modelId) {
...
export function validateAuthMethod(authMethod, config) {
    ...
    if (authMethod === AuthType.VIVEKMIND_OAUTH) {
        ...
        return t('VivekMind OAuth free tier was discontinued on 2026-04-15. Run /auth to switch to Coding Plan, OpenRouter, Fireworks AI, or another provider.');
    }
    ...
    if (authMethod === AuthType.USE_BEDROCK) {
        if (!process.env['AWS_ACCESS_KEY_ID'] || !process.env['AWS_SECRET_ACCESS_KEY']) {
            return t('AWS credentials not found. Please set AWS_ACCESS_KEY_ID and AWS_SECRET_ACCESS_KEY environment variables.');
        }
        return null;
    }
