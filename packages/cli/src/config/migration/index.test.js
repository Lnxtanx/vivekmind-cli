...
import { runMigrations, needsMigration, ALL_MIGRATIONS, MigrationScheduler, } from './index.js';
...
describe('Migration Framework Integration', () => {
    describe('runMigrations', () => {
        ...
        it('should migrate V2 settings to V3', () => {
            ...
            const settings = result.settings;
            expect(settings['$version']).toBe(3);
            expect(settings['general']['enableAutoUpdate']).toBe(true);
            expect(settings['general']['disableAutoUpdate']).toBeUndefined();
        });
    ...
    describe('MigrationScheduler with all migrations', () => {
        it('should execute full migration chain', () => {
            ...
            expect(settings['general']['enableAutoUpdate']).toBe(false);
            expect(settings['ui']['accessibility']['enableLoadingPhrases']).toBe(false);
        });
    });
    ...
    describe('migration chain integrity', () => {
        ...
        it('should be acyclic (no version appears as fromVersion more than once)', () => {
            const fromVersionCounts = new Map();
            for (const migration of ALL_MIGRATIONS) {
    ...
    describe('invalid version handling', () => {
        ...
        it('should not migrate non-numeric version with already-migrated shape (normalized by loader)', () => {
            ...
            expect(needsMigration(settingsWithInvalidVersionButMigratedShape)).toBe(false);
            // No migrations should execute
            const result = runMigrations(settingsWithInvalidVersionButMigratedShape, 'user');
            expect(result.executedMigrations).toHaveLength(0);
        });
