...
import { FatalConfigError, VIVEKMIND_DIR, getErrorMessage, Storage, createDebugLogger, } from '@vivekmind/core';
        ...
        try {
            ...
            for (const templatePath of templateCandidates) {
                ...
                if (fs.existsSync(normalizedPath)) {
                    ...
                    debugLogger.info(`Auto-seeded settings.json from ${normalizedPath}`);
                    break;
                }
            }
        }
        catch (seedError) {
            debugLogger.warn(`Failed to auto-seed settings.json: ${getErrorMessage(seedError)}`);
        }
    }
