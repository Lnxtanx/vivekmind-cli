 ...
 */
import { createDebugLogger, appendToLastTextPart, } from '@vivekmind/core';
...
export class SkillCommandLoader {
    ...
    async loadCommands(_signal) {
        ...
        try {
            ...
            debugLogger.debug(`Loaded ${userSkills.length} user + ${projectSkills.length} project + ${extensionSkills.length} extension skill(s) as slash commands`);
