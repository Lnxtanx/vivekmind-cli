...
describe('FileCommandLoader', () => {
    ...
    describe('Extension Command Loading', () => {
        it('loads commands from active extensions', async () => {
            const userCommandsDir = Storage.getUserCommandsDir();
            const projectCommandsDir = new Storage(process.cwd()).getProjectCommandsDir();
            const extensionDir = path.join(process.cwd(), '.vivekmind/extensions/test-ext');
            mock({
                [userCommandsDir]: {
        ...
        it('only loads commands from active extensions', async () => {
            const extensionDir1 = path.join(process.cwd(), '.vivekmind/extensions/active-ext');
            const extensionDir2 = path.join(process.cwd(), '.vivekmind/extensions/inactive-ext');
            mock({
                [extensionDir1]: {
                    'vivekmind-extension.json': JSON.stringify({
                        name: 'active-ext',
        ...
        it('handles missing extension commands directory gracefully', async () => {
            const extensionDir = path.join(process.cwd(), '.vivekmind/extensions/no-commands');
            mock({
                [extensionDir]: {
                    'vivekmind-extension.json': JSON.stringify({
                        name: 'no-commands',
