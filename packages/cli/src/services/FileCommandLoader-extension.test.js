...
describe('FileCommandLoader - Extension Commands Support', () => {
    ...
    const projectCommandsDir = path.join(projectRoot, '.vivekmind', 'commands');
    afterEach(() => {
        mock.restore();
    });
    it('should load commands from extension with config.commands path', async () => {
        const extensionDir = path.join(projectRoot, '.vivekmind', 'extensions', 'test-ext');
        ...
        mock({
            ...
            [projectCommandsDir]: {},
            [extensionDir]: {
                'vivekmind-extension.json': JSON.stringify(extensionConfig),
                'custom-cmds': {
    ...
    it('should load commands from extension with multiple commands paths', async () => {
        const extensionDir = path.join(projectRoot, '.vivekmind', 'extensions', 'multi-ext');
        const extensionConfig = {
            name: 'multi-ext',
            version: '1.0.0',
    ...
    it('should fallback to default "commands" directory when config.commands not specified', async () => {
        const extensionDir = path.join(projectRoot, '.vivekmind', 'extensions', 'default-ext');
        const extensionConfig = {
            name: 'default-ext',
            version: '1.0.0',
        };
    ...
    it('should handle extension without commands directory gracefully', async () => {
        const extensionDir = path.join(projectRoot, '.vivekmind', 'extensions', 'no-cmds-ext');
        const extensionConfig = {
            name: 'no-cmds-ext',
            version: '1.0.0',
        };
    ...
    it('should set extensionName property for extension commands', async () => {
        const extensionDir = path.join(projectRoot, '.vivekmind', 'extensions', 'prefix-ext');
        const extensionConfig = {
            name: 'prefix-ext',
            version: '1.0.0',
        };
    ...
    it('should load commands from multiple extensions in alphabetical order', async () => {
        const ext1Dir = path.join(projectRoot, '.vivekmind', 'extensions', 'ext-b');
        const ext2Dir = path.join(projectRoot, '.vivekmind', 'extensions', 'ext-a');
        mock({
            [userCommandsDir]: {},
            [projectCommandsDir]: {},
            [ext1Dir]: {
                'vivekmind-extension.json': JSON.stringify({
                    name: 'ext-b',
                    version: '1.0.0',
                }),
                commands: {
