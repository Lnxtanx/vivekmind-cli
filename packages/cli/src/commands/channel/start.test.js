...
const mockProxyAgent = vi.hoisted(() => vi.fn((url) => ({ proxyUrl: url })));
...
vi.mock('@vivekmind/channel-base', () => ({
...
describe('resolveProxy', () => {
    it('prefers the CLI proxy over settings and environment proxies', () => {
        ...
        const proxy = resolveProxy('http://cli.example.com:8080', 'http://settings.example.com:8080');
    ...
    it('prefers settings.proxy over environment proxies', () => {
        ...
        expect(mockProxyAgent).toHaveBeenCalledWith('http://settings.example.com:8080');
    });
...
describe('startCommand.handler', () => {
    it('loads settings.merged.proxy when no CLI proxy is provided', async () => {
        ...
        expect(mockCreateChannel).toHaveBeenCalledWith('telegram', mockParsedChannelConfig, expect.any(Object), expect.objectContaining({ proxy: settingsProxy }));
    });
});
