import { PairingStore } from '@vivekmind/channel-base';
...
export const pairingListCommand = {
    ...
    handler: (argv) => {
        ...
        for (const req of pending) {
            ...
            writeStdoutLine(`  Code: ${req.code}  Sender: ${req.senderName} (${req.senderId})  ${ago}m ago`);
        }
    },
};
export const pairingApproveCommand = {
    ...
    handler: (argv) => {
        ...
        if (!request) {
            writeStderrLine(`No pending request found for code "${argv.code.toUpperCase()}". It may have expired.`);
            process.exit(1);
        }
        writeStdoutLine(`Approved: ${request.senderName} (${request.senderId}) can now use channel "${argv.name}".`);
    },
};
