 ...
 */
export function resolveProxy(cliProxy, settingsProxy) {
    const proxyUrl = normalizeProxyUrl(cliProxy ||
        settingsProxy ||
...
async function startSingle(name, proxy) {
    ...
    let config;
    try {
        config = await parseChannelConfig(name, channelsConfig[name]);
    }
    catch (err) {
    ...
    const attachDisconnectHandler = (b) => {
        b.on('disconnected', async () => {
            ...
            const recentCrashes = crashTimestamps.filter((ts) => now - ts < CRASH_WINDOW_MS);
            if (recentCrashes.length > MAX_CRASH_RESTARTS) {
                writeStderrLine(`[Channel] Bridge crashed ${recentCrashes.length} times in ${CRASH_WINDOW_MS / 1000}s. Giving up.`);
                channel.disconnect();
                router.clearAll();
                removeServiceInfo();
                process.exit(1);
            }
            writeStderrLine(`[Channel] Bridge crashed (${recentCrashes.length}/${MAX_CRASH_RESTARTS} in window). Restarting in ${RESTART_DELAY_MS / 1000}s...`);
...
async function startAll(proxy) {
    ...
    for (const [name, raw] of Object.entries(channelsConfig)) {
        try {
            ...
            writeStderrLine(`Error in channel "${name}": ${err instanceof Error ? err.message : String(err)}`);
            process.exit(1);
        }
    }
    const cliEntryPath = findCliEntryPath();
    ...
    for (const { name, config } of parsed) {
        channels.set(name, await createChannel(name, config, bridge, { router, proxy }));
    }
    registerToolCallDispatch(bridge, router, channels);
...
export const startCommand = {
    ...
    handler: async (argv) => {
        const settings = loadSettings(process.cwd());
        const proxy = resolveProxy(argv['proxy'], settings.merged.proxy);
        if (argv.name) {
