...
describe('extensions enable command', () => {
    it('should fail if no name is provided', () => {
        const validationParser = yargs([])
            ...
            .fail(false)
            .locale('en');
        expect(() => validationParser.parse('enable')).toThrow('Not enough non-option arguments: got 0, need at least 1');
    });
    it('should fail if invalid scope is provided', () => {
        const validationParser = yargs([])
            ...
            .fail(false)
            .locale('en');
        expect(() => validationParser.parse('enable test-extension --scope=invalid')).toThrow(/Invalid scope: invalid/);
    });
    it('should accept valid scope values', () => {
        ...
        expect(() => parser.parse('enable my-extension --scope=user')).not.toThrow();
    });
});
