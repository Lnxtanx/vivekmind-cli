...
async function handleNew(args) {
    try {
        if (args.template) {
            ...
            await writeFile(join(args.path, 'vivekmind-extension.json'), JSON.stringify(manifest, null, 2));
            writeStdoutLine(`Successfully created new extension at ${args.path}.`);
        }
        writeStdoutLine(`You can install this using "vivekmind extensions link ${args.path}" to test it out.`);
    }
    catch (error) {
