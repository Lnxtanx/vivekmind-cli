...
import { Config, ExtensionManager } from '@vivekmind/core';
...
describe('mcp reconnect command', () => {
    ...
    describe('reconnect specific server', () => {
        it('should successfully reconnect a specific server', async () => {
            ...
            const handler = reconnectCommand.handler;
            await handler({ 'server-name': 'test-server', all: false });
            expect(mockWriteStdoutLine).toHaveBeenCalledWith('Reconnecting to server "test-server"...');
            expect(mockToolRegistry.discoverToolsForServer).toHaveBeenCalledWith('test-server');
            expect(mockWriteStdoutLine).toHaveBeenCalledWith('Successfully reconnected to server "test-server".');
        });
        it('should print error when server not found', async () => {
            ...
            expect(mockWriteStderrLine).toHaveBeenCalledWith('Error: Server "nonexistent-server" not found in configuration.');
        ...
        it('should print error when reconnection fails', async () => {
            ...
            mockToolRegistry.discoverToolsForServer.mockRejectedValue(new Error('Connection refused'));
            const handler = reconnectCommand.handler;
            await handler({ 'server-name': 'test-server', all: false });
            expect(mockWriteStderrLine).toHaveBeenCalledWith('Failed to reconnect to server "test-server": Connection refused');
            expect(mockProcessExit).toHaveBeenCalledWith(1);
        });
    });
    describe('reconnect all servers', () => {
        it('should successfully reconnect all servers', async () => {
            ...
            expect(mockWriteStdoutLine).toHaveBeenCalledWith('Reconnecting to all MCP servers...\n');
            expect(mockToolRegistry.discoverToolsForServer).toHaveBeenCalledWith('server-one');
            expect(mockToolRegistry.discoverToolsForServer).toHaveBeenCalledWith('server-two');
            expect(mockWriteStdoutLine).toHaveBeenCalledWith('✓ server-one: Reconnected successfully');
            expect(mockWriteStdoutLine).toHaveBeenCalledWith('✓ server-two: Reconnected successfully');
        });
        it('should print message when no servers configured', async () => {
            ...
            const handler = reconnectCommand.handler;
            await handler({ 'server-name': undefined, all: true });
            expect(mockWriteStdoutLine).toHaveBeenCalledWith('No MCP servers configured.');
        });
        it('should report failure for individual servers when reconnecting all', async () => {
            ...
            await handler({ 'server-name': undefined, all: true });
            expect(mockWriteStdoutLine).toHaveBeenCalledWith('✓ server-one: Reconnected successfully');
            expect(mockWriteStdoutLine).toHaveBeenCalledWith('✗ server-two: Failed - Timeout');
        });
    });
});
//# sourceMappingURL=reconnect.test.js.map