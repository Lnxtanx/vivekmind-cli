...
const { mockRefreshAuth, mockSetValue, mockForScope, mockBackupSettingsFile, mockLoadCliConfig, } = vi.hoisted(() => {
...
vi.mock('./openrouterOAuth.js', () => ({
    ...
    applyOpenRouterModelsConfiguration: vi.fn(async ({ settings, apiKey }) => {
        ...
        settings.setValue('user', 'security.auth.selectedType', AuthType.USE_OPENAI);
...
import { applyOpenRouterModelsConfiguration, runOpenRouterOAuthLogin, } from './openrouterOAuth.js';
describe('handleVivekMindAuth openrouter', () => {
    ...
    it('stores OpenRouter key and model provider config', async () => {
        ...
        expect(mockSetValue).toHaveBeenCalledWith('user', 'env.OPENROUTER_API_KEY', 'or-key-123');
        expect(mockSetValue).toHaveBeenCalledWith('user', 'security.auth.selectedType', AuthType.USE_OPENAI);
        expect(mockSetValue).toHaveBeenCalledWith('user', 'model.name', 'openai/gpt-4o-mini:free');
        const modelProvidersCall = mockSetValue.mock.calls.find((call) => call[1] === `modelProviders.${AuthType.USE_OPENAI}`);
    ...
    it('uses OAuth flow when key is not provided', async () => {
        ...
        expect(mockSetValue).toHaveBeenCalledWith('user', 'env.OPENROUTER_API_KEY', 'oauth-key-123');
