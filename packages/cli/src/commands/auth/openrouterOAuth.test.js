...
import { buildOpenRouterAuthorizationUrl, createOpenRouterOAuthSession, createOAuthState, createPkcePair, exchangeAuthCodeForApiKey, fetchOpenRouterModels, getOpenRouterModelsWithFallback, getPreferredOpenRouterModelId, mergeOpenRouterConfigs, OPENROUTER_DEFAULT_MODELS, OPENROUTER_MODELS_URL, OPENROUTER_OAUTH_AUTHORIZE_URL, OPENROUTER_OAUTH_EXCHANGE_URL, runOpenRouterOAuthLogin, selectRecommendedOpenRouterModels, startOAuthCallbackListener, applyOpenRouterModelsConfiguration, } from './openrouterOAuth.js';
...
describe('openrouterOAuth', () => {
    ...
    it('builds OpenRouter authorization URL with required params', () => {
        ...
        expect(parsed.origin + parsed.pathname).toBe(OPENROUTER_OAUTH_AUTHORIZE_URL);
        expect(parsed.searchParams.get('callback_url')).toBe('http://localhost:3000/openrouter/callback');
    ...
    it('exchanges auth code for API key', async () => {
        ...
        expect(fetchMock).toHaveBeenCalledWith(OPENROUTER_OAUTH_EXCHANGE_URL, expect.objectContaining({
            method: 'POST',
    ...
    it('throws when exchange response does not contain key', async () => {
        ...
        await expect(exchangeAuthCodeForApiKey({
            code: 'auth-code-123',
            codeVerifier: 'verifier-123',
        })).rejects.toThrow('no key was returned');
    });
    it('resolves callback code without waiting for server close completion', async () => {
        const listener = startOAuthCallbackListener('http://localhost:3100/openrouter/callback', 5000, 'state-123');
        await listener.ready;
        const codePromise = listener.waitForCode;
        await new Promise((resolve, reject) => {
            const req = request('http://localhost:3100/openrouter/callback?code=fast-code-123&state=state-123', (res) => {
                res.resume();
                res.on('end', resolve);
            });
            req.on('error', reject);
    ...
    it('rejects callback codes with mismatched OAuth state', async () => {
        const listener = startOAuthCallbackListener('http://localhost:3101/openrouter/callback', 5000, 'expected-state');
        await listener.ready;
        const codePromise = listener.waitForCode.catch((error) => error);
        await new Promise((resolve, reject) => {
            const req = request('http://localhost:3101/openrouter/callback?code=fast-code-123&state=wrong-state', (res) => {
        ...
        await expect(codePromise).resolves.toEqual(expect.objectContaining({
            message: expect.stringContaining('Invalid OAuth state'),
        }));
    }, 15_000);
    it('creates a reusable OAuth session for manual fallback links', () => {
        const session = createOpenRouterOAuthSession('http://localhost:3000/openrouter/callback', {
            codeVerifier: 'verifier-123',
    ...
    it('returns OAuth result without waiting for slow listener close', async () => {
        ...
        const listener = {
            ...
            close: vi.fn(() => new Promise((resolve) => {
        ...
        const resultPromise = runOpenRouterOAuthLogin('http://localhost:3000/openrouter/callback', {
            openBrowser,
    ...
    it('passes the session state to the OAuth callback listener', async () => {
        ...
        expect(startListener).toHaveBeenCalledWith('http://localhost:3000/openrouter/callback', expect.any(Number), 'state-123');
    });
    it('records wait and exchange timings during OAuth login', async () => {
        ...
        const result = await runOpenRouterOAuthLogin('http://localhost:3000/openrouter/callback', {
            openBrowser,
            startListener: () => listener,
            exchangeApiKey,
            now,
        });
        expect(openBrowser).toHaveBeenCalledWith(expect.stringContaining('https://openrouter.ai/auth'));
    ...
    it('allows cancelling OAuth wait with process signals after opening the browser', async () => {
        ...
        const signalTarget = {
            ...
            removeListener: vi.fn((_event, _handler) => undefined),
        };
        const listener = {
            ready: Promise.resolve(),
        ...
        await expect(resultPromise).rejects.toThrow('OpenRouter OAuth cancelled by user (SIGINT) while waiting for browser authorization.');
        expect(exchangeApiKey).not.toHaveBeenCalled();
        expect(listener.close).toHaveBeenCalled();
        expect(signalTarget.removeListener).toHaveBeenCalledWith('SIGINT', sigintHandler);
        expect(signalTarget.removeListener).toHaveBeenCalledWith('SIGTERM', sigtermHandler);
    });
    ...
    it('fetches dynamic OpenRouter text models with free-first ordering', async () => {
        ...
        expect(fetchMock).toHaveBeenCalledWith(OPENROUTER_MODELS_URL, expect.objectContaining({ method: 'GET' }));
    ...
    it('applies OpenRouter configuration to settings and reloads providers', async () => {
        ...
        const fetchSpy = vi
            .spyOn(await import('./openrouterOAuth.js'), 'getOpenRouterModelsWithFallback')
            .mockResolvedValue([
            {
        ...
        expect(settings.setValue).toHaveBeenCalledWith(expect.anything(), 'env.OPENROUTER_API_KEY', 'or-key-123');
        const modelProvidersCall = vi
            .mocked(settings.setValue)
            .mock.calls.find((call) => call[1] === `modelProviders.${AuthType.USE_OPENAI}`);
        expect(modelProvidersCall).toBeDefined();
        expect(modelProvidersCall?.[2]).toEqual(expect.arrayContaining([
    ...
    it('prefers the default OpenRouter model when it remains enabled', () => {
        expect(getPreferredOpenRouterModelId([
            { id: 'anthropic/claude-3.7-sonnet' },
            { id: 'openai/gpt-4o-mini' },
    ...
    it('falls back to default models when dynamic fetch fails', async () => {
        ...
        await expect(getOpenRouterModelsWithFallback()).resolves.toEqual(OPENROUTER_DEFAULT_MODELS);
    });
...
//# sourceMappingURL=openrouterOAuth.test.js.map