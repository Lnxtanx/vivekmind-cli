...
describe('showAuthStatus', () => {
    ...
    it('should show message when no authentication is configured', async () => {
        ...
        expect(writeStdoutLine).toHaveBeenCalledWith(expect.stringContaining('No authentication method configured'));
        expect(writeStdoutLine).toHaveBeenCalledWith(expect.stringContaining('vivekmind auth openrouter'));
        expect(writeStdoutLine).toHaveBeenCalledWith(expect.stringContaining('vivekmind auth vivekmind-oauth'));
        expect(writeStdoutLine).toHaveBeenCalledWith(expect.stringContaining('vivekmind auth coding-plan'));
    ...
    it('should show VivekMind OAuth status when configured', async () => {
        ...
        await showAuthStatus();
        expect(writeStdoutLine).toHaveBeenCalledWith(expect.stringContaining('VivekMind OAuth'));
        expect(writeStdoutLine).toHaveBeenCalledWith(expect.stringContaining('Free tier (discontinued 2026-04-15)'));
        expect(writeStdoutLine).toHaveBeenCalledWith(expect.stringContaining('No longer available'));
        expect(process.exit).toHaveBeenCalledWith(0);
    });
    it('should show Coding Plan status when configured with API key', async () => {
        ...
        await showAuthStatus();
        expect(writeStdoutLine).toHaveBeenCalledWith(expect.stringContaining('Alibaba Cloud Coding Plan'));
        expect(writeStdoutLine).toHaveBeenCalledWith(expect.stringContaining('API key configured'));
        expect(process.exit).toHaveBeenCalledWith(0);
    });
    it('should show OpenRouter status when configured with API key', async () => {
        ...
        await showAuthStatus();
        expect(writeStdoutLine).toHaveBeenCalledWith(expect.stringContaining('OpenRouter'));
        expect(writeStdoutLine).toHaveBeenCalledWith(expect.stringContaining('openai/gpt-4o-mini'));
        expect(writeStdoutLine).toHaveBeenCalledWith(expect.stringContaining('API key configured'));
        expect(process.exit).toHaveBeenCalledWith(0);
    });
    it('should show OpenRouter as incomplete when API key is missing', async () => {
        ...
        await showAuthStatus();
        expect(writeStdoutLine).toHaveBeenCalledWith(expect.stringContaining('OpenRouter (Incomplete)'));
        expect(writeStdoutLine).toHaveBeenCalledWith(expect.stringContaining('vivekmind auth openrouter'));
    });
    it('should show Coding Plan as incomplete when API key is missing', async () => {
        ...
        await showAuthStatus();
        expect(writeStdoutLine).toHaveBeenCalledWith(expect.stringContaining('Incomplete'));
        expect(writeStdoutLine).toHaveBeenCalledWith(expect.stringContaining('API key not found'));
    });
    it('should show Coding Plan when detected via modelProviders entry (no codingPlan.region)', async () => {
        ...
        expect(writeStdoutLine).toHaveBeenCalledWith(expect.stringContaining('Alibaba Cloud Coding Plan'));
        expect(writeStdoutLine).toHaveBeenCalledWith(expect.stringContaining('API key configured'));
        expect(writeStdoutLine).toHaveBeenCalledWith(expect.stringContaining('中国 (China)'));
        expect(writeStdoutLine).not.toHaveBeenCalledWith(expect.stringContaining('OpenAI-compatible Provider'));
        expect(process.exit).toHaveBeenCalledWith(0);
    });
    it('should not fall back to stale Coding Plan metadata when model selection is unmatched', async () => {
        ...
        await showAuthStatus();
        expect(writeStdoutLine).toHaveBeenCalledWith(expect.stringContaining('OpenAI-compatible Provider'));
        expect(writeStdoutLine).not.toHaveBeenCalledWith(expect.stringContaining('Alibaba Cloud Coding Plan'));
    });
    ...
    it('should show Coding Plan region for global', async () => {
        ...
        await showAuthStatus();
        expect(writeStdoutLine).toHaveBeenCalledWith(expect.stringContaining('Global'));
    });
    it('should show current model name', async () => {
        ...
        await showAuthStatus();
        expect(writeStdoutLine).toHaveBeenCalledWith(expect.stringContaining('qwen3.5-plus'));
    });
    it('should show config version (truncated)', async () => {
        ...
        await showAuthStatus();
        expect(writeStdoutLine).toHaveBeenCalledWith(expect.stringContaining('abc123de...'));
    });
    it('should handle errors and exit with code 1', async () => {
        ...
        await showAuthStatus();
        expect(writeStderrLine).toHaveBeenCalledWith(expect.stringContaining('Failed to check authentication status'));
    ...
    describe('OpenAI-compatible provider (no Coding Plan)', () => {
        ...
        it('should show OpenAI-compatible status with OPENAI_API_KEY', async () => {
            ...
            await showAuthStatus();
            expect(writeStdoutLine).toHaveBeenCalledWith(expect.stringContaining('OpenAI-compatible Provider'));
            expect(writeStdoutLine).toHaveBeenCalledWith(expect.stringContaining('gpt-4o'));
            expect(writeStdoutLine).toHaveBeenCalledWith(expect.stringContaining('API key configured'));
            expect(writeStdoutLine).not.toHaveBeenCalledWith(expect.stringContaining('Alibaba Cloud Coding Plan'));
            expect(process.exit).toHaveBeenCalledWith(0);
        });
        it('should show OpenAI-compatible status with custom envKey from modelProviders', async () => {
            ...
            await showAuthStatus();
            expect(writeStdoutLine).toHaveBeenCalledWith(expect.stringContaining('OpenAI-compatible Provider'));
            expect(writeStdoutLine).toHaveBeenCalledWith(expect.stringContaining('custom-model'));
            expect(writeStdoutLine).toHaveBeenCalledWith(expect.stringContaining('https://custom-api.example.com/v1'));
            expect(writeStdoutLine).toHaveBeenCalledWith(expect.stringContaining('API key configured'));
            expect(process.exit).toHaveBeenCalledWith(0);
        });
        it('should show OpenAI-compatible status with settings.security.auth.apiKey', async () => {
            ...
            await showAuthStatus();
            expect(writeStdoutLine).toHaveBeenCalledWith(expect.stringContaining('OpenAI-compatible Provider'));
            expect(writeStdoutLine).toHaveBeenCalledWith(expect.stringContaining('https://my-provider.example.com/v1'));
            expect(writeStdoutLine).toHaveBeenCalledWith(expect.stringContaining('API key configured'));
            expect(process.exit).toHaveBeenCalledWith(0);
        });
        it('should show incomplete when no API key is found for OpenAI-compatible provider', async () => {
            ...
            await showAuthStatus();
            expect(writeStdoutLine).toHaveBeenCalledWith(expect.stringContaining('OpenAI-compatible Provider (Incomplete)'));
            expect(writeStdoutLine).toHaveBeenCalledWith(expect.stringContaining('API key not found'));
            expect(writeStdoutLine).not.toHaveBeenCalledWith(expect.stringContaining('Alibaba Cloud Coding Plan'));
        });
        it('should detect API key via default model when model.name is unset', async () => {
            ...
            await showAuthStatus();
            expect(writeStdoutLine).toHaveBeenCalledWith(expect.stringContaining('OpenAI-compatible Provider'));
            expect(writeStdoutLine).toHaveBeenCalledWith(expect.stringContaining('https://default-api.example.com/v1'));
            expect(writeStdoutLine).toHaveBeenCalledWith(expect.stringContaining('API key configured'));
            expect(process.exit).toHaveBeenCalledWith(0);
        });
        ...
        it('should not bind to unrelated provider entry when model.name does not match', async () => {
            ...
            expect(writeStdoutLine).not.toHaveBeenCalledWith(expect.stringContaining('https://other-api.example.com/v1'));
            expect(process.exit).toHaveBeenCalledWith(0);
        });
        ...
        it('should show OpenAI-compatible when stale Coding Plan key exists but active model is generic', async () => {
            ...
            await showAuthStatus();
            expect(writeStdoutLine).toHaveBeenCalledWith(expect.stringContaining('OpenAI-compatible Provider'));
            expect(writeStdoutLine).toHaveBeenCalledWith(expect.stringContaining('spark-v4'));
            expect(writeStdoutLine).toHaveBeenCalledWith(expect.stringContaining('API key configured'));
            expect(writeStdoutLine).not.toHaveBeenCalledWith(expect.stringContaining('Alibaba Cloud Coding Plan'));
            expect(process.exit).toHaveBeenCalledWith(0);
        });
