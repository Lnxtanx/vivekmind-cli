...
describe('InteractiveSelector', () => {
    ...
    describe('select', () => {
        it('should reject if raw mode is not available', async () => {
            ...
            const selector = new InteractiveSelector(mockOptions, mockPrompt);
            await expect(selector.select()).rejects.toThrow('Raw mode not available. Please run in an interactive terminal.');
            // Restore
        ...
        it('should select first option with Enter key', async () => {
            ...
            stdin.setEncoding = mockSetEncoding;
            stdin.removeListener = mockRemoveListener;
            stdin.on = mockOn;
    ...
    describe('renderMenu', () => {
        ...
        it('should calculate correct total lines', () => {
            const selector = new InteractiveSelector(mockOptions, mockPrompt);
            // Access private method for testing
            selector.calculateTotalLines();
            // Expected: 4 (prompt + empty + empty + instructions) + 3 (options) = 7
            expect(selector.calculateTotalLines()).toBe(7);
        });
...
//# sourceMappingURL=interactiveSelector.test.js.map