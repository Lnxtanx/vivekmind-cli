 ...
 */
function findRootId(startId, byId) {
    const seen = new Set();
    let cur = startId;
    while (true) {
        if (seen.has(cur))
 ...
 */
function isReviewWorthShowing(body) {
    const trimmed = (body ?? '').trim();
    if (trimmed.length === 0)
        return false;
    if (/^No issues found\.?\s*LGTM/i.test(trimmed))
        return false;
    return true;
}
function buildMarkdown(prNumber, ownerRepo, meta, inline, issue, reviews) {
    ...
    if (meaningfulReviews.length > 0) {
        ...
        for (const r of meaningfulReviews) {
            ...
            parts.push(`- **@${r.user?.login ?? '?'}** [${r.state ?? 'COMMENTED'}]${date ? ` ${date}` : ''}: ${snippet(r.body)}`);
        }
        parts.push('');
    }
    ...
    if (repliedRoots.length > 0 || issue.length > 0) {
        ...
        if (repliedRoots.length > 0) {
            ...
            for (const root of sortedRoots) {
                ...
                parts.push(`**\`${root.path ?? '?'}\`:${root.line ?? '?'}** — initiated by @${root.user?.login ?? '?'}`);
                parts.push('');
    ...
    if (openRoots.length > 0) {
        ...
        for (const c of openRoots) {
            parts.push(`- \`${c.path ?? '?'}\`:${c.line ?? '?'} by @${c.user?.login ?? '?'}: ${snippet(c.body)}`);
        }
        parts.push('');
    }
    return parts.join('\n');
...
async function runPrContext(args) {
    ...
    const issue = ghApiAll(`repos/${owner}/${repo}/issues/${prNumber}/comments`);
    ...
    const meaningfulReviewCount = reviews.filter((r) => isReviewWorthShowing(r.body)).length;
    writeStdoutLine(`Wrote PR context to ${out} (${inline.length} inline, ${issue.length} issue comments, ${meaningfulReviewCount}/${reviews.length} review summaries)`);
}
