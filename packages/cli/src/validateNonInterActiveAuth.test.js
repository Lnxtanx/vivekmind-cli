...
import { AuthType, OutputFormat } from '@vivekmind/core';
...
function createMockConfig(overrides) {
    const baseModelsConfig = {
        ...
        getCurrentAuthType: vi.fn().mockReturnValue(AuthType.VIVEKMIND_OAUTH),
...
describe('validateNonInterActiveAuth', () => {
    ...
    beforeEach(() => {
        ...
        originalEnvQwenOauth = process.env['VIVEKMIND_OAUTH'];
        ...
        delete process.env['VIVEKMIND_OAUTH'];
    ...
    afterEach(() => {
        ...
        if (originalEnvQwenOauth !== undefined) {
            process.env['VIVEKMIND_OAUTH'] = originalEnvQwenOauth;
        }
        else {
            delete process.env['VIVEKMIND_OAUTH'];
        }
    ...
    it('exits if validateAuthMethod fails for default auth type', async () => {
        ...
        vi.spyOn(auth, 'validateAuthMethod').mockReturnValue('Missing API key for authentication');
        ...
        try {
            await validateNonInteractiveAuth(undefined, nonInteractiveConfig, mockSettings);
            expect.fail('Should have exited');
        }
        catch (e) {
            expect(e.message).toContain('process.exit(1) called');
        }
        expect(mockWriteStderrLine).toHaveBeenCalledWith(expect.stringContaining('Missing API key'));
    ...
    it('exits with error for VIVEKMIND_OAUTH (free tier discontinued)', async () => {
        const nonInteractiveConfig = createMockConfig({
            refreshAuth: refreshAuthMock,
            getModelsConfig: vi.fn().mockReturnValue({
                getModel: vi.fn().mockReturnValue('default-model'),
        ...
        await expect(validateNonInteractiveAuth(undefined, nonInteractiveConfig, mockSettings)).rejects.toThrow('process.exit(1)');
        expect(mockWriteStderrLine).toHaveBeenCalledWith(expect.stringContaining('discontinued'));
    });
    ...
    it('skips validation if useExternalAuth is true', async () => {
        ...
        await validateNonInteractiveAuth(true, // useExternalAuth = true
        ...
        expect(refreshAuthMock).toHaveBeenCalledWith(AuthType.VIVEKMIND_OAUTH);
    });
    ...
    it('exits if currentAuthType does not match enforcedAuthType', async () => {
        mockSettings.merged.security.auth.enforcedType = AuthType.VIVEKMIND_OAUTH;
        process.env['OPENAI_API_KEY'] = 'fake-key';
        const nonInteractiveConfig = createMockConfig({
            refreshAuth: refreshAuthMock,
            getModelsConfig: vi.fn().mockReturnValue({
        ...
        expect(mockWriteStderrLine).toHaveBeenCalledWith('The configured auth type is vivekmind-oauth, but the current auth type is openai. Please re-authenticate with the correct type.');
        expect(processExitSpy).toHaveBeenCalledWith(1);
    });
    describe('JSON output mode', () => {
        ...
        it('emits error result and exits when enforced auth mismatches current auth', async () => {
            ...
            expect(emitResultMock).toHaveBeenCalledWith({
                isError: true,
                errorMessage: expect.stringContaining('The configured auth type is vivekmind-oauth, but the current auth type is openai.'),
                durationMs: 0,
                apiDurationMs: 0,
                numTurns: 0,
                usage: undefined,
