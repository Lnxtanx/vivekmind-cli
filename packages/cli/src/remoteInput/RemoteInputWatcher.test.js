...
describe('RemoteInputWatcher', () => {
    ...
    it('forwards submit commands to the registered submit fn', async () => {
        ...
        fs.appendFileSync(inputFile, JSON.stringify({ type: 'submit', text: 'hello' }) + '\n');
    ...
    it('dispatches confirmation_response immediately, bypassing the queue', async () => {
        ...
        fs.appendFileSync(inputFile, JSON.stringify({
    ...
    it('retries queued submits when the TUI signals it has become idle', async () => {
        ...
        watcher.setSubmitFn((text) => {
            if (busy)
                return false; // simulate TUI rejecting because it is responding
            accepted.push(text);
            return true;
        });
        fs.appendFileSync(inputFile, JSON.stringify({ type: 'submit', text: 'queued' }) + '\n');
    ...
    it('skips malformed JSON lines without throwing', async () => {
        ...
        fs.appendFileSync(inputFile, JSON.stringify({ type: 'submit', text: 'after-bad-line' }) + '\n');
        await watcher.checkForNewInput();
    ...
    it('stops watching after shutdown', async () => {
        ...
        watcher.shutdown();
        fs.appendFileSync(inputFile, JSON.stringify({ type: 'submit', text: 'too-late' }) + '\n');
        // checkForNewInput should be a no-op after shutdown (active=false)
        await watcher.checkForNewInput();
        expect(submitted).toEqual([]);
    });
});
//# sourceMappingURL=RemoteInputWatcher.test.js.map