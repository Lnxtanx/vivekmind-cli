...
describe('ControlDispatcher', () => {
    ...
    describe('dispatch', () => {
        ...
        it('should route get_context_usage request to system controller', async () => {
            ...
            vi.mocked(mockSystemController.handleRequest).mockResolvedValue(mockResponse);
            await dispatcher.dispatch(request);
            expect(mockSystemController.handleRequest).toHaveBeenCalledWith(request.request, 'req-ctx');
            expect(mockContext.streamJson.send).toHaveBeenCalledWith({
                type: 'control_response',
                response: {
                    subtype: 'success',
    ...
    describe('handleControlResponse', () => {
        ...
        it('should handle response for non-existent request in debug mode', () => {
            ...
            expect(() => dispatcherWithDebug.handleControlResponse(response)).not.toThrow();
        });
    });
    ...
    describe('markInputClosed', () => {
        it('should reject all pending outgoing requests when input closes', () => {
            ...
            expect(reject1).toHaveBeenCalledWith(expect.objectContaining({ message: 'Input closed' }));
            expect(reject2).toHaveBeenCalledWith(expect.objectContaining({ message: 'Input closed' }));
        ...
        it('should mark input closed in debug mode without throwing', () => {
            ...
            const reject = vi.fn();
            const timeoutId = setTimeout(() => { }, 1000);
            dispatcherWithDebug.registerOutgoingRequest(requestId, 'SystemController', resolve, reject, timeoutId);
