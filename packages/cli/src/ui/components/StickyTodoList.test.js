...
import { getStickyTodoMaxVisibleItems, STICKY_TODO_MAX_VISIBLE_ITEMS, } from '../utils/todoSnapshot.js';
...
describe('StickyTodoList', () => {
    it('keeps each task number attached to the original task after sorting', () => {
        ...
        const { lastFrame } = render(_jsx(StickyTodoList, { todos: todos, width: 60 }));
        ...
        expect(lines.find((line) => line.includes('Run core tests')) ?? '').toContain('3.');
        expect(lines.find((line) => line.includes('Run cli tests')) ?? '').toContain('2.');
        expect(lines.find((line) => line.includes('Summarize results')) ?? '').toContain('1.');
        expect(output.indexOf('Run core tests')).toBeLessThan(output.indexOf('Run cli tests'));
        expect(output.indexOf('Run cli tests')).toBeLessThan(output.indexOf('Summarize results'));
    });
    it('keeps long todo lists compact with a hidden item summary', () => {
        const todos = [
            {
                id: 'active',
                content: 'This active task has a very long description that should not wrap across multiple rows in the sticky panel',
                status: 'in_progress',
            },
            {
                id: 'pending-1',
        ...
        const { lastFrame } = render(_jsx(StickyTodoList, { todos: todos, width: 42, maxVisibleItems: 2 }));
        const output = lastFrame() ?? '';
    ...
    it('sizes the number column for original todo numbers after sorting', () => {
        const todos = makeTodos(10).map((todo, index) => ({
            ...todo,
            status: index === 9 ? 'in_progress' : 'completed',
        }));
        const { lastFrame } = render(_jsx(StickyTodoList, { todos: todos, width: 24, maxVisibleItems: 1 }));
        const output = lastFrame() ?? '';
    ...
    it('falls back to the maximum visible item count for non-finite maxVisibleItems', () => {
        ...
        for (const maxVisibleItems of [Number.NaN, Number.POSITIVE_INFINITY]) {
            const { lastFrame, unmount } = render(_jsx(StickyTodoList, { todos: todos, width: 42, maxVisibleItems: maxVisibleItems }));
            const output = lastFrame() ?? '';
...
//# sourceMappingURL=StickyTodoList.test.js.map