...
import { ApprovalMode, Storage, createDebugLogger, } from '@vivekmind/core';
...
export const InputPrompt = ({ buffer, onSubmit, userMessages, onClearScreen, config, slashCommands, commandContext, placeholder, focus = true, suggestionsWidth, shellModeActive, setShellModeActive, approvalMode, onEscapePromptChange, onToggleShortcuts, showShortcuts, onSuggestionsVisibilityChange, vimHandleInput, isEmbeddedShellFocused, promptSuggestion, onPromptSuggestionDismiss, }) => {
    ...
    const { entries: bgEntries, dialogOpen: bgDialogOpen, pillFocused: bgPillFocused, } = useBackgroundTaskViewState();
    ...
    const handleSubmitAndClear = useCallback((submittedValue) => {
        ...
        if (pendingPastes.size > 0) {
            ...
            finalValue = finalValue.replace(placeholderRegex, (matchedPlaceholder) => pendingPastes.get(matchedPlaceholder) ?? matchedPlaceholder);
        ...
        if (isOnCursorLine && cursorVisualColAbsolute === cpLen(lineText)) {
            ...
            if (ghostText && showCursorOpt && ghostText.text.length > 0) {
                if (ghostText.showCursorBeforeText) {
                    renderedLine.push(_jsx(Text, { children: chalk.inverse(' ') }, "ghost-cursor"));
                    ...
                    renderedLine.push(_jsx(Text, { children: chalk.inverse(firstChar) }, "ghost-cursor"));
                    if (rest.length > 0) {
                ...
                renderedLine.push(_jsx(Text, { children: showCursorOpt ? chalk.inverse(' ') + '\u200B' : ' \u200B' }, `cursor-end-${cursorVisualColAbsolute}`));
            }
        }
    ...
    const shouldUseExportSuggestions = !commandSearchActive && !reverseSearchActive;
    ...
    const prefixNode = (_jsx(Box, { children: shellModeActive ? (_jsx(Text, { color: statusColor ?? theme.text.accent, children: reverseSearchActive ? `(r:) ` : `! ` })) : commandSearchActive ? (_jsx(Text, { color: theme.text.accent, children: "(r:) " })) : showYoloStyling ? (_jsx(Text, { color: statusColor ?? theme.text.accent, children: "* " })) : (_jsxs(Box, { children: [_jsx(Text, { color: "#E53E3E", bold: true, children: "vivekmind" }), _jsx(Text, { color: "white", children: '> ' })] })) }));
    return (_jsxs(_Fragment, { children: [attachments.length > 0 && (_jsxs(Box, { marginLeft: 2, marginBottom: 0, children: [_jsx(Text, { color: theme.text.secondary, children: t('Attachments: ') }), attachments.map((att, idx) => (_jsxs(Text, { color: isAttachmentMode && idx === selectedAttachmentIndex
                            ...
                            : theme.text.secondary, children: ["[", att.filename, "]", idx < attachments.length - 1 ? ' ' : ''] }, att.id)))] })), _jsx(BaseTextInput, { buffer: buffer, onSubmit: handleSubmitAndClear, onKeypress: handleInput, showCursor: showCursor, placeholder: followup.state.isVisible && followup.state.suggestion
                    ? followup.state.suggestion
                    : placeholder, prefix: prefixNode, borderColor: borderColor, topRightLabel: uiState.sessionName || undefined, isActive: !isEmbeddedShellFocused, renderLine: renderLineWithHighlighting }), shouldShowSuggestions && (_jsx(Box, { marginLeft: 2, marginRight: 2, children: _jsx(SuggestionsDisplay, { suggestions: suggestionDisplayProps.suggestions, activeIndex: suggestionDisplayProps.activeIndex, isLoading: suggestionDisplayProps.isLoading, width: suggestionsWidth, scrollOffset: suggestionDisplayProps.scrollOffset, userInput: buffer.text, mode: buffer.text.startsWith('/') &&
                        !reverseSearchActive &&
                        !commandSearchActive
                        ? 'slash'
                        : 'reverse', expandedIndex: expandedSuggestionIndex }) })), attachments.length > 0 && !shouldShowSuggestions && (_jsx(Box, { marginLeft: 2, marginRight: 2, children: _jsx(Text, { color: theme.text.secondary, children: isAttachmentMode
