...
describe('PluginChoicePrompt', () => {
    ...
    describe('rendering', () => {
        it('renders marketplace name in title', () => {
            const { lastFrame } = render(_jsx(PluginChoicePrompt, { marketplaceName: "test-marketplace", plugins: [{ name: 'plugin1' }], onSelect: onSelect, onCancel: onCancel, terminalWidth: terminalWidth }));
            expect(lastFrame()).toContain('test-marketplace');
        });
        it('renders plugin names', () => {
            const { lastFrame } = render(_jsx(PluginChoicePrompt, { marketplaceName: "test", plugins: [
                    { name: 'plugin1', description: 'First plugin' },
                    { name: 'plugin2', description: 'Second plugin' },
                ], onSelect: onSelect, onCancel: onCancel, terminalWidth: terminalWidth }));
        ...
        it('renders help text', () => {
            const { lastFrame } = render(_jsx(PluginChoicePrompt, { marketplaceName: "test", plugins: [{ name: 'plugin1' }], onSelect: onSelect, onCancel: onCancel, terminalWidth: terminalWidth }));
    ...
    describe('scrolling behavior', () => {
        ...
        it('shows "more below" indicator for long lists', () => {
            ...
            const { lastFrame } = render(_jsx(PluginChoicePrompt, { marketplaceName: "test", plugins: plugins, onSelect: onSelect, onCancel: onCancel, terminalWidth: terminalWidth }));
    ...
    describe('keyboard navigation', () => {
        it('registers keypress handler', () => {
            render(_jsx(PluginChoicePrompt, { marketplaceName: "test", plugins: [{ name: 'plugin1' }], onSelect: onSelect, onCancel: onCancel, terminalWidth: terminalWidth }));
        ...
        it('calls onCancel when escape is pressed', () => {
            ...
            keypressHandler({ name: 'escape', sequence: '\x1b' });
            expect(onCancel).toHaveBeenCalled();
        });
        it('calls onSelect with plugin name when enter is pressed', () => {
            render(_jsx(PluginChoicePrompt, { marketplaceName: "test", plugins: [{ name: 'test-plugin' }], onSelect: onSelect, onCancel: onCancel, terminalWidth: terminalWidth }));
            const keypressHandler = mockedUseKeypress.mock.calls[0][0];
            keypressHandler({ name: 'return', sequence: '\r' });
            expect(onSelect).toHaveBeenCalledWith('test-plugin');
        });
        it('calls onSelect with correct plugin when number key 1-9 is pressed', () => {
            render(_jsx(PluginChoicePrompt, { marketplaceName: "test", plugins: [
                    { name: 'plugin1' },
                    { name: 'plugin2' },
                    { name: 'plugin3' },
                ], onSelect: onSelect, onCancel: onCancel, terminalWidth: terminalWidth }));
    ...
    describe('selection indicator', () => {
        it('shows selection indicator for first plugin by default', () => {
            const { lastFrame } = render(_jsx(PluginChoicePrompt, { marketplaceName: "test", plugins: [{ name: 'plugin1' }, { name: 'plugin2' }], onSelect: onSelect, onCancel: onCancel, terminalWidth: terminalWidth }));
            expect(lastFrame()).toContain('❯');
        });
    });
});
//# sourceMappingURL=PluginChoicePrompt.test.js.map