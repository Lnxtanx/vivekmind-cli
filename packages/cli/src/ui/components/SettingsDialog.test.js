...
import { getDialogSettingKeys, getSettingDefinition, saveModifiedSettings, TEST_ONLY, } from '../../utils/settingsUtils.js';
...
describe('SettingsDialog', () => {
    ...
    describe('Settings Toggling', () => {
        it('should toggle setting with Enter key', async () => {
            ...
            expect(vi.mocked(saveModifiedSettings)).toHaveBeenCalledWith(new Set(['general.vimMode']), {
                general: {
                    vimMode: true,
                },
        ...
        describe('enum values', () => {
            it('toggles enum values with the enter key', async () => {
                ...
                expect(vi.mocked(saveModifiedSettings)).toHaveBeenCalledWith(new Set(['tools.approvalMode']), expect.objectContaining({
    ...
    describe('Specific Settings Behavior', () => {
        ...
        it('should show restart prompt for restart-required settings', async () => {
            ...
            expect(lastFrame()).not.toContain('To see changes, VivekMind must be restarted');
            unmount();
        });
        it('should keep restart prompt when switching scopes', async () => {
            const settings = createMockSettings();
            const onSelect = vi.fn();
            const { stdin, lastFrame, unmount } = render(_jsx(KeypressProvider, { kittyProtocolEnabled: false, children: _jsx(SettingsDialog, { settings: settings, onSelect: onSelect }) }));
            await waitFor(() => {
                expect(lastFrame()).toContain('Tool Approval Mode');
            });
            ...
            await waitFor(() => {
                expect(lastFrame()).toContain('To see changes, VivekMind must be restarted');
            });
            // Switch scopes; restart prompt should remain visible.
            await press(TerminalKeys.TAB);
            await press('2');
            await waitFor(() => {
                expect(lastFrame()).toContain('To see changes, VivekMind must be restarted');
            });
    ...
    describe('Output Language', () => {
        it('treats empty output language as auto', async () => {
            ...
            const { stdin, unmount, lastFrame } = render(_jsx(KeypressProvider, { kittyProtocolEnabled: false, children: _jsx(SettingsDialog, { settings: settings, onSelect: () => { } }) }));
            await waitFor(() => {
                expect(lastFrame()).toContain('Settings');
            });
            ...
            const outputLanguageIndex = getDialogSettingKeys().indexOf('general.outputLanguage');
            ...
            await waitFor(() => {
                const outputLanguageCall = vi
                    ...
                    .mock.calls.find((call) => call[0].has('general.outputLanguage'));
    ...
    describe('Complex User Interactions', () => {
        it('should handle complete user workflow: navigate, toggle, change scope, exit', async () => {
            ...
            expect(lastFrame()).toContain('(Use Enter to select, Tab to configure scope)'); // Help text
