...
export function VivekMindOAuthProgress({
  onTimeout,
  onCancel,
  deviceAuth,
  authStatus,
  authMessage,
}: VivekMindOAuthProgressProps): React.JSX.Element {
    ...
    return (
      ...
      >
        <Text bold color={theme.status.error}>
          {t('VivekMind OAuth Authentication Timeout')}
        </Text>
  ...
  if (authStatus === 'error') {
    return (
      ...
      >
        <Text bold color={theme.status.error}>
          {t('VivekMind OAuth Authentication Error')}
        </Text>
  ...
  if (!deviceAuth) {
    return (
      <Box
        ...
        width="100%"
      >
        <Text bold>{t('VivekMind OAuth Authentication')}</Text>

        <Box marginTop={1} flexDirection="column">
          <Text>{t('Waiting for VivekMind OAuth authentication...')}</Text>
          <Text>
