 ...
 */
function getAuthDisplayType(authType, baseUrl) {
    if (!authType) {
        return '—';
    }
...
export const AppHeader = ({ version }) => {
    ...
    const authDisplayType = getAuthDisplayType(authType, contentGeneratorConfig?.baseUrl);
    return (_jsx(Box, { flexDirection: "column", children: showBanner && (_jsx(Header, { version: version, authDisplayType: authDisplayType, model: model, workingDirectory: targetDir })) }));
};
