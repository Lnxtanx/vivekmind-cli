...
export function DescriptiveRadioButtonSelect({ items, initialIndex = 0, onSelect, onHighlight, isFocused = true, showNumbers = false, showScrollArrows = false, maxItemsToShow = 10, itemGap = 0, }) {
    return (_jsx(BaseSelectionList, { items: items, initialIndex: initialIndex, onSelect: onSelect, onHighlight: onHighlight, isFocused: isFocused, showNumbers: showNumbers, showScrollArrows: showScrollArrows, maxItemsToShow: maxItemsToShow, itemGap: itemGap, renderItem: (item, { titleColor }) => (_jsxs(Box, { flexDirection: "column", children: [typeof item.title === 'string' ? (_jsx(Text, { color: titleColor, children: item.title })) : (item.title), typeof item.description === 'string' ? (_jsx(Text, { color: theme.text.secondary, children: item.description })) : (item.description)] }, item.key)) }));
}
