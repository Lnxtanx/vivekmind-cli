...
describe('HooksDisabledStep', () => {
    ...
    it('should render disabled title', () => {
        const { lastFrame } = render(_jsx(HooksDisabledStep, { configuredHooksCount: 2 }));
    ...
    it('should show singular form for single hook', () => {
        const { lastFrame } = render(_jsx(HooksDisabledStep, { configuredHooksCount: 1 }));
        expect(lastFrame()).toContain('1 configured hook');
    });
    it('should show zero hooks message', () => {
        const { lastFrame } = render(_jsx(HooksDisabledStep, { configuredHooksCount: 0 }));
    ...
    it('should show explanation items', () => {
        ...
        expect(output).toContain('Tool operations will proceed without hook validation');
    });
    ...
    it('should handle large hook counts', () => {
        const { lastFrame } = render(_jsx(HooksDisabledStep, { configuredHooksCount: 100 }));
        expect(lastFrame()).toContain('100 configured hooks');
    });
});
//# sourceMappingURL=HooksDisabledStep.test.js.map