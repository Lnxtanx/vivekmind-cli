...
describe('HooksManagementDialog', () => {
    ...
    describe('Initial rendering', () => {
        ...
        it('should render with border', async () => {
            const { lastFrame, unmount } = renderWithProviders(_jsx(HooksManagementDialog, { onClose: mockOnClose }));
    ...
    describe('Keyboard navigation - HOOKS_LIST step', () => {
        ...
        it('should not go above first item when pressing up', async () => {
            const { unmount } = renderWithProviders(_jsx(HooksManagementDialog, { onClose: mockOnClose }));
            await new Promise((resolve) => setTimeout(resolve, 100));
