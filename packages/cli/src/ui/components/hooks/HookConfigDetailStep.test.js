...
import { HookEventName, HooksConfigSource, HookType, } from '@vivekmind/core';
...
describe('HookConfigDetailStep', () => {
    const createMockHookEvent = () => ({
        ...
        shortDescription: 'Right before VivekMind concludes its response',
        description: '',
        exitCodes: [
    ...
    it('should render source for Extensions with path', () => {
        const hookEvent = createMockHookEvent();
        const hookConfig = createMockHookConfig(HooksConfigSource.Extensions, 'ralph-wiggum', '/Users/test/.vivekmind/extensions/ralph-wiggum');
        const { lastFrame } = render(_jsx(HookConfigDetailStep, { hookEvent: hookEvent, hookConfig: hookConfig }));
