...
describe('getPillLabel', () => {
    ...
    it('uses singular form for one running monitor', () => {
        expect(getPillLabel([monitorEntry({ monitorId: 'mon-a' })])).toBe('1 monitor');
    });
    ...
    it('uses paused form when only paused entries remain', () => {
        expect(getPillLabel([agentEntry({ agentId: 'a', status: 'paused' })])).toBe('1 local agent paused');
    });
    it('uses generic done form when all entries are terminal', () => {
        expect(getPillLabel([agentEntry({ agentId: 'a', status: 'completed' })])).toBe('1 task done');
        expect(getPillLabel([
...
//# sourceMappingURL=BackgroundTasksPill.test.js.map