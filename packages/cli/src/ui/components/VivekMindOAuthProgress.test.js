...
import { VivekMindOAuthProgress } from './VivekMindOAuthProgress.js';
...
describe('VivekMindOAuthProgress', () => {
    ...
    const renderComponent = (props = {}) => render(_jsx(VivekMindOAuthProgress, { onTimeout: mockOnTimeout, onCancel: mockOnCancel, ...props }));
    beforeEach(() => {
        vi.clearAllMocks();
        vi.useFakeTimers();
    ...
    describe('Loading state (no deviceAuth)', () => {
        it('should render loading state when deviceAuth is not provided', () => {
            ...
            expect(output).toContain('Waiting for VivekMind OAuth authentication...');
        ...
        it('should render loading state with single border', () => {
            ...
            expect(output).toContain('VivekMind OAuth Authentication');
    ...
    describe('Authenticated state (with deviceAuth)', () => {
        ...
        it('should format time correctly', () => {
            ...
            const { lastFrame } = render(_jsx(VivekMindOAuthProgress, { onTimeout: mockOnTimeout, onCancel: mockOnCancel, deviceAuth: deviceAuthWithCustomTime }));
            const output = lastFrame();
    ...
    describe('Timer functionality', () => {
        it('should countdown and call onTimeout when timer expires', async () => {
            ...
            const { rerender } = render(_jsx(VivekMindOAuthProgress, { onTimeout: mockOnTimeout, onCancel: mockOnCancel, deviceAuth: deviceAuthWithShortTime }));
            // Advance timer by 1 second
            vi.advanceTimersByTime(1000);
            rerender(_jsx(VivekMindOAuthProgress, { onTimeout: mockOnTimeout, onCancel: mockOnCancel, deviceAuth: deviceAuthWithShortTime }));
        ...
        it('should update time remaining display', async () => {
            const { lastFrame, rerender } = render(_jsx(VivekMindOAuthProgress, { onTimeout: mockOnTimeout, onCancel: mockOnCancel, deviceAuth: mockDeviceAuth }));
            // Initial time should be 5:00
            expect(lastFrame()).toContain('Time remaining: 5:00');
            // Advance by 1 second
            vi.advanceTimersByTime(1000);
            rerender(_jsx(VivekMindOAuthProgress, { onTimeout: mockOnTimeout, onCancel: mockOnCancel, deviceAuth: mockDeviceAuth }));
        ...
        it('should use default 300 second timeout when deviceAuth is null', () => {
            const { lastFrame } = render(_jsx(VivekMindOAuthProgress, { onTimeout: mockOnTimeout, onCancel: mockOnCancel }));
    ...
    describe('User interactions', () => {
        it('should call onCancel when ESC key is pressed', () => {
            render(_jsx(VivekMindOAuthProgress, { onTimeout: mockOnTimeout, onCancel: mockOnCancel, deviceAuth: mockDeviceAuth }));
        ...
        it('should call onCancel when ESC is pressed in loading state', () => {
            render(_jsx(VivekMindOAuthProgress, { onTimeout: mockOnTimeout, onCancel: mockOnCancel }));
            // Simulate ESC key press
            if (keypressHandler) {
                keypressHandler({
                    name: 'escape',
    ...
    describe('Props changes', () => {
        it('should display initial timer value from deviceAuth', () => {
            ...
            const { lastFrame } = render(_jsx(VivekMindOAuthProgress, { onTimeout: mockOnTimeout, onCancel: mockOnCancel, deviceAuth: deviceAuthWith10Min }));
            expect(lastFrame()).toContain('Time remaining: 10:00');
        });
        it('should reset to loading state when deviceAuth becomes null', () => {
            const { rerender, lastFrame } = render(_jsx(VivekMindOAuthProgress, { onTimeout: mockOnTimeout, onCancel: mockOnCancel, deviceAuth: mockDeviceAuth }));
            // Initially shows waiting for authorization
            expect(lastFrame()).toContain('Waiting for authorization');
            rerender(_jsx(VivekMindOAuthProgress, { onTimeout: mockOnTimeout, onCancel: mockOnCancel }));
            expect(lastFrame()).toContain('Waiting for VivekMind OAuth authentication...');
    ...
    describe('Timeout state', () => {
        it('should render timeout state when authStatus is timeout', () => {
            ...
            const output = lastFrame();
            expect(output).toContain('VivekMind OAuth Authentication Timeout');
...
//# sourceMappingURL=VivekMindOAuthProgress.test.js.map