...
export const DialogManager = ({ addItem, terminalWidth, }) => {
    ...
    if (uiState.codingPlanUpdateRequest) {
        return (_jsx(ConsentPrompt, { prompt: uiState.codingPlanUpdateRequest.prompt, onConfirm: uiState.codingPlanUpdateRequest.onConfirm, terminalWidth: terminalWidth }));
    }
    ...
    if (uiState.isManageModelsDialogOpen) {
        return (_jsx(ManageModelsDialog, { config: config, onClose: uiActions.closeManageModelsDialog }));
    }
    ...
    if (uiState.isAuthenticating) {
        if (uiState.pendingAuthType === AuthType.USE_OPENAI &&
            ...
            return (_jsx(ExternalAuthProgress, { title: uiState.externalAuthState.title, message: uiState.externalAuthState.message, detail: uiState.externalAuthState.detail, onCancel: () => {
        ...
        // VivekMind OAuth remains as a separate flow
        if (uiState.pendingAuthType === AuthType.VIVEKMIND_OAUTH) {
            return (_jsx(VivekMindOAuthProgress, { deviceAuth: uiState.vivekmindAuthState.deviceAuth || undefined, authStatus: uiState.vivekmindAuthState.authStatus, authMessage: uiState.vivekmindAuthState.authMessage, onTimeout: () => {
                    uiActions.onAuthError('VivekMind OAuth authentication timed out.');
                    uiActions.cancelAuthentication();
                    uiActions.setAuthState(AuthState.Updating);
                }, onCancel: () => {
                    uiActions.cancelAuthentication();
    ...
    if (uiState.isDeleteDialogOpen) {
        return (_jsx(SessionPicker, { sessionService: config.getSessionService(), currentBranch: uiState.branchName, onSelect: uiActions.handleDelete, onCancel: uiActions.closeDeleteDialog, title: t('Delete Session') }));
    }
    if (uiState.isRewindSelectorOpen) {
        return (_jsx(RewindSelector, { history: uiState.history, onRewind: uiActions.handleRewindConfirm, onCancel: uiActions.closeRewindSelector }));
    }
