...
import { getDialogSettingKeys, setPendingSettingValue, getDisplayValue, saveModifiedSettings, getSettingDefinition, isDefaultValue, requiresRestart, getRestartRequiredFromModified, getDefaultValue, setPendingSettingValueAny, getNestedValue, getEffectiveValue, } from '../../utils/settingsUtils.js';
...
export function SettingsDialog({ settings, onSelect, onRestartRequest, availableTerminalHeight, config, }) {
    ...
    const activeDescription = mode === 'settings' && items[activeSettingIndex]?.description
    ...
    return (_jsxs(Box, { borderStyle: "round", borderColor: theme.border.default, flexDirection: "column", padding: 1, width: "100%", children: [mode === 'settings' ? (_jsxs(Box, { flexDirection: "column", flexGrow: 1, children: [_jsxs(Text, { bold: mode === 'settings', wrap: "truncate", children: [mode === 'settings' ? '> ' : '  ', t('Settings')] }), _jsx(Box, { height: 1 }), showScrollUp && _jsx(Text, { color: theme.text.secondary, children: "\u25B2" }), visibleItems.map((item, idx) => {
                        const isActive = mode === 'settings' && activeSettingIndex === idx + scrollOffset;
                        ...
                        return (_jsxs(Box, { flexDirection: "row", alignItems: "center", children: [_jsx(Box, { minWidth: 2, flexShrink: 0, children: _jsx(Text, { color: isActive ? theme.status.success : theme.text.secondary, children: isActive ? '●' : '' }) }), _jsx(Box, { flexGrow: 1, flexShrink: 1, children: _jsxs(Text, { color: isActive ? theme.status.success : theme.text.primary, wrap: "truncate", children: [item.label, scopeMessage && (_jsxs(Text, { color: theme.text.secondary, children: [" ", scopeMessage] }))] }) }), _jsx(Box, { marginLeft: 1, flexShrink: 0, children: _jsx(Text, { color: isActive
                                            ? theme.status.success
                                            : shouldBeGreyedOut
                                                ? theme.text.secondary
                                                : theme.text.primary, wrap: "truncate", children: displayValue }) })] }, item.value));
                    }), showScrollDown && _jsx(Text, { color: theme.text.secondary, children: "\u25BC" })] })) : (_jsx(ScopeSelector, { onSelect: handleScopeSelect, onHighlight: handleScopeHighlight, isFocused: mode === 'scope', initialScope: selectedScope })), activeDescription && mode === 'settings' && (_jsx(Box, { marginTop: 1, children: _jsx(Text, { color: theme.text.secondary, wrap: "truncate-end", italic: true, children: activeDescription }) })), _jsx(Box, { marginTop: activeDescription && mode === 'settings' ? 0 : 1, children: _jsx(Text, { color: theme.text.secondary, wrap: "truncate", children: mode === 'settings'
                        ? t('(Use Enter to select, Tab to configure scope)')
                        : t('(Use Enter to apply scope, Tab to go back)') }) }), showRestartPrompt && (_jsx(Text, { color: theme.status.warning, children: t('To see changes, VivekMind must be restarted. Press r to exit and apply changes now.') }))] }));
}
