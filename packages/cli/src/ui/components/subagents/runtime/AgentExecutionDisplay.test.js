...
function makeRunningData(toolCount) {
    return {
        type: 'task_execution',
...
function makeCompletedData(toolCount) {
    return {
...
describe('<AgentExecutionDisplay />', () => {
    ...
    it('bounds expanded detail by the assigned visual height', () => {
        const { lastFrame } = render(_jsx(AgentExecutionDisplay, { data: makeRunningData(8), availableHeight: 8, childWidth: 40, config: makeFakeConfig() }));
        act(() => {
    ...
    it('keeps the rendered running frame within availableHeight', () => {
        ...
        const { lastFrame } = render(_jsx(AgentExecutionDisplay, { data: makeRunningData(8), availableHeight: availableHeight, childWidth: 80, config: makeFakeConfig() }));
        act(() => {
            keypressHandler?.({ ctrl: true, name: 'e' });
        });
        expect(visualRowCount(lastFrame() ?? '')).toBeLessThanOrEqual(availableHeight);
    });
    it('does not respond to ctrl+e when another running subagent has focus', () => {
        ...
        const { lastFrame } = render(_jsx(AgentExecutionDisplay, { data: makeRunningData(2), availableHeight: 20, childWidth: 80, config: makeFakeConfig(), isFocused: false }));
    ...
    it('survives the running → completed transition while expanded', () => {
        ...
        const { lastFrame, rerender } = render(_jsx(AgentExecutionDisplay, { data: makeRunningData(8), availableHeight: availableHeight, childWidth: 80, config: makeFakeConfig() }));
        ...
        rerender(_jsx(AgentExecutionDisplay, { data: makeCompletedData(8), availableHeight: availableHeight, childWidth: 80, config: makeFakeConfig() }));
...
//# sourceMappingURL=AgentExecutionDisplay.test.js.map