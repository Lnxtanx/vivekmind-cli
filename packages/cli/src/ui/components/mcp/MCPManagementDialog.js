...
import { getMCPServerStatus, DiscoveredMCPTool, MCPOAuthTokenStorage, createDebugLogger, } from '@vivekmind/core';
...
export const MCPManagementDialog = ({ onClose, }) => {
    ...
    const renderStepHeader = useCallback(() => {
        ...
        switch (currentStep) {
            ...
            case MCP_MANAGEMENT_STEPS.TOOL_DETAIL:
                headerText = (_jsxs(Box, { flexDirection: "column", children: [_jsxs(Box, { children: [_jsx(Text, { color: theme.text.accent, bold: true, children: selectedTool?.name || t('Tool Detail') }), selectedTool?.annotations?.destructiveHint && (_jsx(Text, { color: theme.status.error, children: '[destructive]' })), selectedTool?.annotations?.idempotentHint && (_jsx(Text, { color: theme.status.warning, children: '[idempotent]' })), selectedTool?.annotations?.readOnlyHint && (_jsx(Text, { color: theme.status.success, children: '[read-only]' })), selectedTool?.annotations?.openWorldHint && (_jsx(Text, { color: theme.text.primary, children: '[open-world]' }))] }), _jsx(Text, { color: theme.text.secondary, children: selectedTool?.serverName || t('Server') })] }));
                break;
    ...
    const renderStepContent = useCallback(() => {
        ...
        switch (currentStep) {
            ...
            case MCP_MANAGEMENT_STEPS.SERVER_DETAIL:
                return (_jsx(ServerDetailStep, { server: selectedServer, onViewTools: handleViewTools, onReconnect: handleReconnect, onDisable: handleDisable, onAuthenticate: handleAuthenticate, onClearAuth: handleClearAuth, onBack: handleNavigateBack }));
    ...
    return (_jsx(Box, { flexDirection: "column", width: boxWidth, children: _jsxs(Box, { borderStyle: "single", borderColor: theme.border.default, flexDirection: "column", width: boxWidth, gap: 1, paddingLeft: 1, paddingRight: 1, children: [renderStepHeader(), renderStepContent(), renderStepFooter()] }) }));
};
