...
describe('MCPHealthPill / getPillLabel', () => {
    ...
    it('uses singular form for one offline server', () => {
        expect(getPillLabel(snapshot({ totalCount: 1, disconnectedCount: 1 }))).toBe('1 MCP offline');
    });
    it('uses plural form for multiple offline servers', () => {
        expect(getPillLabel(snapshot({ totalCount: 3, disconnectedCount: 3 }))).toBe('3 MCPs offline');
    });
    it('counts only disconnected when mixed with connecting/connected', () => {
        expect(getPillLabel(snapshot({
            totalCount: 4,
            disconnectedCount: 1,
            connectingCount: 1,
            connectedCount: 2,
...
//# sourceMappingURL=MCPHealthPill.test.js.map