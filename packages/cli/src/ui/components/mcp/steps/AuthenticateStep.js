...
import { MCPOAuthProvider, MCPOAuthTokenStorage, getErrorMessage, } from '@vivekmind/core';
 ...
 */
function wrapForMultiplexer(osc) {
    if (process.env['TMUX']) {
...
export const AuthenticateStep = ({ server, onBack, }) => {
    ...
    return (_jsxs(Box, { flexDirection: "column", gap: 1, children: [_jsx(Box, { children: _jsxs(Text, { color: theme.text.secondary, children: [t('Server:'), " ", server.name] }) }), messages.length > 0 && (_jsx(Box, { flexDirection: "column", children: messages.map((msg, i) => (_jsx(Text, { color: theme.text.secondary, children: msg }, i))) })), authState === 'error' && errorMessage && (_jsx(Box, { children: _jsx(Text, { color: theme.status.error, children: errorMessage }) })), authUrl && (_jsx(Box, { children: _jsx(Text, { color: theme.text.accent, children: osc8Hyperlink(authUrl) }) })), _jsxs(Box, { flexDirection: "column", children: [authState === 'authenticating' && (_jsx(Text, { color: theme.text.secondary, children: t('Authenticating... Please complete the login in your browser.') })), authState === 'authenticating' && authUrl && (_jsx(Text, { bold: copyState.status === 'idle', color: copyState.status === 'copied'
