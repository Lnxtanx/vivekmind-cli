...
import { ToolConfirmationOutcome } from '@vivekmind/core';
...
describe('<AskUserQuestionDialog />', () => {
    describe('rendering', () => {
        it('renders single question with options', () => {
            ...
            const { lastFrame } = renderWithProviders(_jsx(AskUserQuestionDialog, { confirmationDetails: details, onConfirm: onConfirm }));
    ...
    describe('single-select interaction', () => {
        it('selects an option with Enter and submits immediately for single question', async () => {
            const onConfirm = vi.fn();
            const details = createConfirmationDetails();
            const { stdin, unmount } = renderWithProviders(_jsx(AskUserQuestionDialog, { confirmationDetails: details, onConfirm: onConfirm }));
            await wait();
    ...
    describe('multi-select interaction', () => {
        ...
        it('toggles options with Space', async () => {
            ...
            const { stdin, lastFrame, unmount } = renderWithProviders(_jsx(AskUserQuestionDialog, { confirmationDetails: details, onConfirm: onConfirm }));
            await wait();
    ...
    describe('multiple questions', () => {
        ...
        it.skipIf(process.platform === 'win32')('shows unanswered questions as (not answered) in Submit tab', async () => {
            const onConfirm = vi.fn();
            const details = createConfirmationDetails({
                questions: [
                    createSingleQuestion({ header: 'Q1' }),
    ...
    describe('focus behavior', () => {
        it('does not respond to keys when isFocused is false', async () => {
            const onConfirm = vi.fn();
            const details = createConfirmationDetails();
            const { stdin, unmount } = renderWithProviders(_jsx(AskUserQuestionDialog, { confirmationDetails: details, isFocused: false, onConfirm: onConfirm }));
            await wait();
            stdin.write('\r'); // Enter
            await wait();
            stdin.write('\u001B'); // Escape
