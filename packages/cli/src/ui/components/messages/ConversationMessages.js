...
const PrefixedMarkdownMessage = ({ text, prefix, prefixColor, isPending, availableTerminalHeight, contentWidth, ariaLabel, textColor, }) => {
    const prefixWidth = getPrefixWidth(prefix);
    return (_jsxs(Box, { flexDirection: "row", children: [_jsx(Box, { width: prefixWidth, children: _jsx(Text, { color: prefixColor, "aria-label": ariaLabel, children: prefix }) }), _jsx(Box, { flexGrow: 1, flexDirection: "column", children: _jsx(MarkdownDisplay, { text: text, isPending: isPending, availableTerminalHeight: availableTerminalHeight, contentWidth: contentWidth - prefixWidth, textColor: textColor }) })] }));
};
const ContinuationMarkdownMessage = ({ text, isPending, availableTerminalHeight, contentWidth, basePrefix, textColor, }) => {
    const prefixWidth = getPrefixWidth(basePrefix);
    return (_jsx(Box, { flexDirection: "column", paddingLeft: prefixWidth, children: _jsx(MarkdownDisplay, { text: text, isPending: isPending, availableTerminalHeight: availableTerminalHeight, contentWidth: contentWidth - prefixWidth, textColor: textColor }) }));
};
...
export const AssistantMessage = ({ text, isPending, availableTerminalHeight, contentWidth, }) => (_jsx(PrefixedMarkdownMessage, { text: text, prefix: "\u2726", prefixColor: theme.text.accent, ariaLabel: SCREEN_READER_MODEL_PREFIX, isPending: isPending, availableTerminalHeight: availableTerminalHeight, contentWidth: contentWidth }));
export const AssistantMessageContent = ({ text, isPending, availableTerminalHeight, contentWidth }) => (_jsx(ContinuationMarkdownMessage, { text: text, isPending: isPending, availableTerminalHeight: availableTerminalHeight, contentWidth: contentWidth, basePrefix: "\u2726" }));
