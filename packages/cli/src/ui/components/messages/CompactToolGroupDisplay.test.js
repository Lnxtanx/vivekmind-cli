...
vi.mock('../shared/ToolStatusIndicator.js', () => ({
    ToolStatusIndicator: () => _jsx(Text, { children: "\u2022" }),
    STATUS_INDICATOR_WIDTH: 2,
}));
const NOW = 1_700_000_000_000;
function shellTool(overrides = {}) {
    return {
        callId: 'c1',
        name: 'Shell',
...
describe('<CompactToolGroupDisplay /> — shell timeout plumbing', () => {
    ...
    it('surfaces shell timeoutMs inline via ToolElapsedTime', () => {
        ...
        const { lastFrame } = render(_jsx(CompactToolGroupDisplay, { toolCalls: [tool], contentWidth: 80 }));
    ...
    it('ignores non-ansi resultDisplay shapes', () => {
        ...
        const { lastFrame, rerender } = render(_jsx(CompactToolGroupDisplay, { toolCalls: [tool], contentWidth: 80 }));
        vi.advanceTimersByTime(5_000);
        rerender(_jsx(CompactToolGroupDisplay, { toolCalls: [tool], contentWidth: 80 }));
...
describe('<CompactToolGroupDisplay /> — summary label', () => {
    it('renders default header (active tool name + count) when no compactLabel is provided', () => {
        ...
        const { lastFrame } = render(_jsx(CompactToolGroupDisplay, { toolCalls: tools, contentWidth: 80 }));
    ...
    it('replaces header with compactLabel when provided', () => {
        ...
        const { lastFrame } = render(_jsx(CompactToolGroupDisplay, { toolCalls: tools, contentWidth: 80, compactLabel: "Searched in auth/" }));
        const frame = lastFrame();
    ...
    it('shows tool count suffix only when multiple tools are present', () => {
        ...
        const { lastFrame } = render(_jsx(CompactToolGroupDisplay, { toolCalls: tools, contentWidth: 80, compactLabel: "Read config.json" }));
        const frame = lastFrame();
        expect(frame).toContain('Read config.json');
        expect(frame).not.toContain('tools');
    });
    it('renders nothing for empty tool calls', () => {
        const { lastFrame } = render(_jsx(CompactToolGroupDisplay, { toolCalls: [], contentWidth: 80 }));
...
//# sourceMappingURL=CompactToolGroupDisplay.test.js.map