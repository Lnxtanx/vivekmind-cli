...
describe('InputPrompt', () => {
    ...
    describe('prompt suggestions', () => {
        it('does not accept the prompt suggestion on shift+tab', async () => {
            const { stdin, unmount } = renderWithProviders(_jsx(InputPrompt, { ...props, promptSuggestion: "commit this" }));
            await wait(350);
    ...
    describe('clipboard image paste', () => {
        ...
        describeConditional('should handle Ctrl+V when clipboard has an image', async () => {
            vi.mocked(clipboardUtils.clipboardHasImage).mockResolvedValue(true);
            vi.mocked(clipboardUtils.saveClipboardImage).mockResolvedValue('/Users/mochi/.vivekmind/tmp/clipboard-123.png');
            const { stdin, unmount } = renderWithProviders(_jsx(InputPrompt, { ...props }));
            await wait();
            // Send Ctrl+V
        ...
        it('should handle Cmd+V when clipboard has an image', async () => {
            vi.mocked(clipboardUtils.clipboardHasImage).mockResolvedValue(true);
            vi.mocked(clipboardUtils.saveClipboardImage).mockResolvedValue('/Users/mochi/.vivekmind/tmp/clipboard-456.png');
            const { stdin, unmount } = renderWithProviders(_jsx(InputPrompt, { ...props }));
            await wait();
        ...
        it('should insert image path at cursor position with proper spacing', async () => {
            const imagePath = '/Users/mochi/.vivekmind/tmp/clipboard-456.png';
            vi.mocked(clipboardUtils.clipboardHasImage).mockResolvedValue(true);
    ...
    it('should keep export format suggestions visible after arrow navigation fills input', async () => {
        ...
        const TestHarness = () => {
            ...
            return _jsx(InputPrompt, { ...props, buffer: buffer });
        };
        const { stdin, lastFrame, unmount } = renderWithProviders(_jsx(TestHarness, {}));
        await wait();
        stdin.write('\u001B[B');
        await wait();
    ...
    it('should not overwrite /export html with extra args when Down is pressed', async () => {
        ...
        props.buffer.setText.mockClear();
    ...
    it('should submit directly on Enter after arrow-navigate + backspace + retype to perfect match', async () => {
        ...
        unmount();
        const { unmount: unmountAfterEdit } = renderWithProviders(_jsx(InputPrompt, { ...props }));
        await wait();
        // Retype: /memor → /memory.
        props.buffer.setText('/memory');
        unmountAfterEdit();
        const { stdin: stdinFinal, unmount: unmountFinal } = renderWithProviders(_jsx(InputPrompt, { ...props }));
        await wait();
    ...
    describe('cursor-based completion trigger', () => {
        it('should trigger completion when cursor is after @ without spaces', async () => {
            ...
            expect(mockedUseCommandCompletion).toHaveBeenCalledWith(mockBuffer, path.join('test', 'project', 'src'), mockSlashCommands, mockCommandContext, false, expect.any(Object), 
    ...
    describe('command search (Ctrl+R when not in shell)', () => {
        it('passes newest-first user history to command search', async () => {
            ...
            const { unmount } = renderWithProviders(_jsx(InputPrompt, { ...props }));
            await wait();
            const commandSearchCall = mockedUseReverseSearchCompletion.mock.calls.find(([, history]) => Array.isArray(history) &&
                history.length === 3 &&
    ...
    describe('large paste placeholder', () => {
        it('should create placeholder for paste > 1000 characters', async () => {
            ...
            expect(mockBuffer.insert).toHaveBeenCalledWith('[Pasted Content 1001 chars]', { paste: false });
        ...
        it('should create placeholder for paste > 10 lines', async () => {
            ...
            expect(mockBuffer.insert).toHaveBeenCalledWith(expect.stringMatching(/\[Pasted Content \d+ chars\]/), { paste: false });
            unmount();
        });
        it('should use sequential IDs for multiple pastes of same size', async () => {
            ...
            expect(mockBuffer.insert).toHaveBeenCalledWith('[Pasted Content 1001 chars]', { paste: false });
            expect(mockBuffer.insert).toHaveBeenCalledWith('[Pasted Content 1001 chars] #2', { paste: false });
            unmount();
        });
        ...
        it('should write expanded placeholder content to shell history', async () => {
            ...
            stdin.write('\r');
            await wait();
            expect(mockShellHistory.addCommandToHistory).toHaveBeenCalledWith(largeContent);
            expect(props.onSubmit).toHaveBeenCalledWith(largeContent);
            unmount();
        });
        it('should reuse placeholder ID after deletion', async () => {
            // Set up mocks that actually update buffer state
            vi.mocked(mockBuffer.insert).mockImplementation((text) => {
                mockBuffer.text += text;
                mockBuffer.lines = [mockBuffer.text];
                mockBuffer.cursor = [0, mockBuffer.text.length];
            });
            vi.mocked(mockBuffer.replaceRangeByOffset).mockImplementation((start, end, replacement) => {
                mockBuffer.text =
        ...
        it('should handle mixed pastes with different character counts', async () => {
            ...
            expect(mockBuffer.insert).toHaveBeenCalledWith('[Pasted Content 1001 chars]', { paste: false });
            expect(mockBuffer.insert).toHaveBeenCalledWith('[Pasted Content 1500 chars]', { paste: false });
            expect(mockBuffer.insert).toHaveBeenCalledWith('[Pasted Content 1001 chars] #2', { paste: false });
            unmount();
        });
    });
    ...
    describe('Ctrl+Y retry shortcut', () => {
        ...
        it('should NOT trigger retry on Ctrl+R (reverse search)', async () => {
            ...
            expect(mockBuffer.handleInput).not.toHaveBeenCalledWith(expect.objectContaining({
                ctrl: true,
                name: 'y',
            }));
            unmount();
    ...
    describe('queue input editing', () => {
        ...
        it('should pop queued messages into input on Up arrow when queue is non-empty', async () => {
            ...
            expect(props.buffer.setText).toHaveBeenCalledWith('queued msg 1\n\nqueued msg 2');
            unmount();
        });
        it('should prepend queued messages before existing input text', async () => {
            ...
            expect(props.buffer.moveToOffset).toHaveBeenCalledWith('queued msg'.length + 1);
            unmount();
        });
