 ...
 */
function useDreamRunning(projectRoot) {
    const config = useConfig();
    const subscribe = useCallback((onStoreChange) => config.getMemoryManager().subscribe(onStoreChange), [config]);
    const getSnapshot = useCallback(() => config
        .getMemoryManager()
        .listTasksByType('dream', projectRoot)
        .some((task) => task.status === 'pending' || task.status === 'running'), [config, projectRoot]);
...
export const Footer = () => {
    ...
    if (dreamRunning) {
        rightItems.push({
            key: 'dream',
            node: _jsx(Text, { color: theme.text.secondary, children: t('✦ dreaming') }),
        });
    }
