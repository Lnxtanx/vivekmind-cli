...
import { MAIN_SOURCE } from '@vivekmind/core';
...
const renderWithMockedStats = (metrics, modelPricing) => {
    ...
    return render(_jsx(SettingsContext.Provider, { value: mockSettings, children: _jsx(ModelStatsDisplay, {}) }));
};
describe('<ModelStatsDisplay />', () => {
    ...
    describe('Subagent source attribution', () => {
        ...
        describe('Cost estimation', () => {
            const makeCore = (prompt, candidates, thoughts) => ({
                api: { totalRequests: 1, totalErrors: 0, totalLatencyMs: 100 },
                tokens: {
                    prompt,
                    candidates,
            ...
            const renderCostTest = (models, pricing) => renderWithMockedStats({ models, tools: baseTools, files: baseFiles }, pricing);
            it('should not display Cost section when no pricing and no thoughts', () => {
                const { lastFrame } = renderCostTest({ 'gemini-2.5-pro': mainOnly(makeCore(10, 20, 0)) }, {});
            ...
            it('should include thoughts tokens in cost calculation (regression)', () => {
                const { lastFrame } = renderCostTest({ 'gemini-2.5-pro': mainOnly(makeCore(10, 20, 5)) }, {
                    'gemini-2.5-pro': {
                        inputPerMillionTokens: 1,
                        outputPerMillionTokens: 2,
                    },
            ...
            it('should include thoughts tokens — larger numbers to expose difference', () => {
                ...
                const { lastFrame } = renderCostTest({ 'gemini-2.5-pro': mainOnly(makeCore(1000, 2000, 500)) }, {
                    'gemini-2.5-pro': {
                        inputPerMillionTokens: 1,
                        outputPerMillionTokens: 2,
                    },
