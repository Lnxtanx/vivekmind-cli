...
const noApprovals = new Map();
function toolCallMsg(callId, toolName, opts) {
...
function toolResultMsg(callId, toolName, opts) {
...
describe('agentMessagesToHistoryItems — role mapping', () => {
    it('maps user message', () => {
        const items = agentMessagesToHistoryItems([msg('user', 'hello')], noApprovals);
        expect(items).toHaveLength(1);
        expect(items[0]).toMatchObject({ type: 'user', text: 'hello' });
    });
    it('maps plain assistant message', () => {
        const items = agentMessagesToHistoryItems([msg('assistant', 'response')], noApprovals);
        expect(items[0]).toMatchObject({ type: 'gemini', text: 'response' });
    });
    it('maps thought assistant message', () => {
        const items = agentMessagesToHistoryItems([msg('assistant', 'thinking...', { thought: true })], noApprovals);
    ...
    it('maps assistant message with error metadata', () => {
        const items = agentMessagesToHistoryItems([msg('assistant', 'oops', { metadata: { error: true } })], noApprovals);
        expect(items[0]).toMatchObject({ type: 'error', text: 'oops' });
    });
    it('maps info message with no level → type info', () => {
        const items = agentMessagesToHistoryItems([msg('info', 'note')], noApprovals);
    ...
    it.each([
        ...
        ['success', 'success'],
        ['error', 'error'],
    ])('maps info message with level=%s', (level, expectedType) => {
        const items = agentMessagesToHistoryItems([msg('info', 'text', { metadata: { level } })], noApprovals);
        expect(items[0]).toMatchObject({ type: expectedType });
    });
    it('maps unknown info level → type info', () => {
        const items = agentMessagesToHistoryItems([msg('info', 'x', { metadata: { level: 'verbose' } })], noApprovals);
    ...
    it('skips unknown roles without crashing', () => {
        const items = agentMessagesToHistoryItems([
            ...
            { role: 'unknown', content: 'x', timestamp: 0 },
            msg('user', 'after'),
        ], noApprovals);
...
describe('agentMessagesToHistoryItems — tool grouping', () => {
    it('merges a tool_call + tool_result pair into one tool_group', () => {
        const items = agentMessagesToHistoryItems([toolCallMsg('c1', 'read_file'), toolResultMsg('c1', 'read_file')], noApprovals);
        expect(items).toHaveLength(1);
...
describe('agentMessagesToHistoryItems — tool status', () => {
    it('Executing: tool_call with no result yet', () => {
        const items = agentMessagesToHistoryItems([toolCallMsg('c1', 'shell')], noApprovals);
        const group = items[0];
    ...
    it('Confirming: tool_call present in pendingApprovals', () => {
        ...
        const items = agentMessagesToHistoryItems([toolCallMsg('c1', 'shell')], approvals);
        const group = items[0];
...
describe('agentMessagesToHistoryItems — tool metadata', () => {
    ...
    it('forwards description from tool_call', () => {
        const items = agentMessagesToHistoryItems([toolCallMsg('c1', 'read', { description: 'reading src/index.ts' })], noApprovals);
        const group = items[0];
...
describe('agentMessagesToHistoryItems — liveOutputs', () => {
    it('uses liveOutput as resultDisplay for Executing tools', () => {
        ...
        const items = agentMessagesToHistoryItems([toolCallMsg('c1', 'shell')], noApprovals, liveOutputs);
        const group = items[0];
...
describe('agentMessagesToHistoryItems — shellPids', () => {
    it('sets ptyId for Executing tools with a known PID', () => {
        ...
        const items = agentMessagesToHistoryItems([toolCallMsg('c1', 'shell')], noApprovals, undefined, shellPids);
        const group = items[0];
    ...
    it('does not set ptyId for completed tools', () => {
        ...
        ], noApprovals, undefined, shellPids);
        const group = items[0];
...
describe('agentMessagesToHistoryItems — executionStartTimes', () => {
    it('sets executionStartTime for Executing tools with a known start time', () => {
        ...
        const items = agentMessagesToHistoryItems([toolCallMsg('c1', 'shell')], noApprovals, undefined, undefined, starts);
        const group = items[0];
    ...
    it('does not set executionStartTime for completed tools', () => {
        ...
        const items = agentMessagesToHistoryItems([
            toolCallMsg('c1', 'shell'),
            toolResultMsg('c1', 'shell', { success: true }),
        ], noApprovals, undefined, undefined, starts);
        const group = items[0];
...
describe('agentMessagesToHistoryItems — ID stability', () => {
    ...
    it('IDs from a prefix of messages are stable when more messages are appended', () => {
        const base = [msg('user', 'u'), msg('assistant', 'a')];
        const before = agentMessagesToHistoryItems(base, noApprovals);
        const after = agentMessagesToHistoryItems([...base, msg('info', 'i')], noApprovals);
        expect(after[0].id).toBe(before[0].id);
        expect(after[1].id).toBe(before[1].id);
        expect(after[2].id).toBe(2);
    });
});
//# sourceMappingURL=agentHistoryAdapter.test.js.map