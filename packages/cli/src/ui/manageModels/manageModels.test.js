...
import { fetchManageModelsCatalog, getEnabledModelIdsForSource, saveManageModelsSelection, } from './manageModels.js';
const { mockFetchOpenRouterModels, mockMergeOpenRouterConfigs, mockIsOpenRouterConfig, } = vi.hoisted(() => ({
...
describe('manageModels', () => {
    ...
    it('fetchManageModelsCatalog maps OpenRouter models into catalog entries', async () => {
        ...
        expect(catalog.entries[0]?.badges).toEqual(expect.arrayContaining(['free', 'vision', 'long-context']));
    });
    it('getEnabledModelIdsForSource only returns OpenRouter-enabled ids', () => {
        mockIsOpenRouterConfig.mockImplementation((config) => config.baseUrl?.includes('openrouter') ?? false);
        const settings = {
            merged: {
                modelProviders: {
                    [AuthType.USE_OPENAI]: [
    ...
    it('saveManageModelsSelection merges selected OpenRouter models and reloads config', async () => {
        ...
        expect(settings.setValue).toHaveBeenCalledWith(expect.anything(), `modelProviders.${AuthType.USE_OPENAI}`, [
            { id: 'openai/gpt-4o-mini', baseUrl: 'https://openrouter.ai/api/v1' },
            { id: 'custom/model', baseUrl: 'https://example.com/v1' },
        ]);
...
//# sourceMappingURL=manageModels.test.js.map