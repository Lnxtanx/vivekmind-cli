...
describe('bugCommand', () => {
    ...
    it('should generate the default GitHub issue URL', async () => {
        ...
        const qwenCodeLine = GIT_COMMIT_INFO && !['N/A'].includes(GIT_COMMIT_INFO)
...
Proxy: no proxy
Memory Usage: 100 MB`;
        const expectedUrl = 'https://github.com/QwenLM/qwen-code/issues/new?template=bug_report.yml&title=A%20test%20bug&info=%0A' +
...
Proxy: no proxy
Memory Usage: 100 MB`;
        const expectedUrl = 'https://github.com/QwenLM/qwen-code/issues/new?template=bug_report.yml&title=OpenAI%20bug&info=%0A' +
            encodeURIComponent(expectedInfo) +
            '%0A';
        expect(mockContext.ui.addItem).toHaveBeenCalledWith({
            type: 'info',
