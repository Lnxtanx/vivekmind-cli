 ...
 */
import { getResponseText, SESSION_TITLE_MAX_LENGTH, stripTerminalControlSequences, tryGenerateSessionTitle, } from '@vivekmind/core';
 ...
 */
function extractConversationText(history) {
    const texts = [];
 ...
 */
async function generateKebabTitle(config, signal) {
    try {
 ...
 */
function autoFailureMessage(reason) {
    switch (reason) {
        case 'no_fast_model':
            return t('/rename --auto requires a fast model. Configure one with `/model --fast <model>`.');
        case 'empty_history':
        ...
        case 'model_error':
            return t('The fast model could not generate a title (rate limit, auth, or network error). Check debug log or try again.');
        case 'no_client':
        ...
        if (auto) {
            ...
            if (!config.getFastModel()) {
                return {
                    type: 'message',
                    messageType: 'error',
                    content: t('/rename --auto requires a fast model. Configure one with `/model --fast <model>`.'),
                };
            }
            if (positional) {
                return {
            ...
            let outcome;
            try {
                outcome = await tryGenerateSessionTitle(config, context.abortSignal ?? new AbortController().signal);
            }
            finally {
                clearInterval(timer);
