...
describe('statuslineCommand', () => {
    ...
    it('should return submit_prompt with default prompt when no args', () => {
        ...
        expect(result).toHaveProperty('content.0.text', expect.stringContaining('PS1'));
    });
    it('should use user-provided args as the prompt', () => {
        if (!statuslineCommand.action) {
            throw new Error('statusline command must have an action');
        }
        const result = statuslineCommand.action(mockContext, 'show model name and git branch');
        expect(result).toEqual({
            type: 'submit_prompt',
            content: [
                {
...
//# sourceMappingURL=statuslineCommand.test.js.map