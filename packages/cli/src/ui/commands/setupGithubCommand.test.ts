...
describe('setupGithubCommand', async () => {
  ...
  it('returns a tool action to download github workflows and handles paths', async () => {
    ...
    const expectedSubstrings = [
      `set -eEuo pipefail`,
      `fakeOpenCommand "https://github.com/VivekMindLM/vivekmind-cli-action`,
    ];
    ...
    if (gitignoreExists) {
      ...
      expect(gitignoreContent).toContain('.vivekmind/');
...
describe('updateGitignore', () => {
  ...
  it('does not add duplicate entries', async () => {
    const gitignorePath = path.join(scratchDir, '.gitignore');
    const existingContent = '.vivekmind/\nsome-other-file\ngha-creds-*.json\n';
    await fs.writeFile(gitignorePath, existingContent);
  ...
  it('adds only missing entries when some already exist', async () => {
    ...
    // Should not duplicate .vivekmind/ entry
    expect((content.match(/\.vivekmind\//g) || []).length).toBe(1);
  });

  it('does not get confused by entries in comments or as substrings', async () => {
    const gitignorePath = path.join(scratchDir, '.gitignore');
    const existingContent = [
      '# This is a comment mentioning .vivekmind/ folder',
      'my-app.vivekmind/config',
    ...
    expect(content).toContain('.vivekmind/');
    expect(content).toContain('gha-creds-*.json');
    ...
    const lines = content
      ...
      .filter((line) => line);
    expect(lines).toContain('.vivekmind/');
    expect(lines).toContain('gha-creds-*.json');
    expect(lines).toContain('my-app.vivekmind/config');
