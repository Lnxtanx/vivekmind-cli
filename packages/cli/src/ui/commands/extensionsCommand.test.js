...
describe('extensionsCommand', () => {
        ...
        mockContext = createMockCommandContext({
            services: {
                config: {
                    ...
                    getExtensionManager: () => mockExtensionManager,
                },
            },
            ui: {
    ...
    describe('manage', () => {
        const manageAction = extensionsCommand.subCommands?.find((cmd) => cmd.name === 'manage')?.action;
        if (!manageAction) {
    ...
    describe('install', () => {
        const installAction = extensionsCommand.subCommands?.find((cmd) => cmd.name === 'install')?.action;
        if (!installAction) {
            throw new Error('Install action not found');
        }
        const mockParseInstallSource = parseInstallSource;
        // Create a real ExtensionManager mock that passes instanceof check
        let realMockExtensionManager;
        beforeEach(() => {
            vi.resetAllMocks();
