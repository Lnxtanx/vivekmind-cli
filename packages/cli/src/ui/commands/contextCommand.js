...
import { DiscoveredMCPTool, uiTelemetryService, getCoreSystemPrompt, DEFAULT_TOKEN_LIMIT, ToolNames, buildSkillLlmContent, } from '@vivekmind/core';
...
export async function collectContextData(config, showDetails) {
    ...
    const compressionThreshold = config.getChatCompression()?.contextPercentageThreshold ??
        DEFAULT_COMPRESSION_THRESHOLD;
    const autocompactBuffer = compressionThreshold > 0
...
export function formatContextUsageText(data) {
    ...
    if (isEstimated) {
        ...
        lines.push(fmtCategoryRow('Autocompact buffer', breakdown.autocompactBuffer, contextWindowSize));
        lines.push('');
...
export const contextCommand = {
    ...
    action: async (context, args) => {
        const showDetails = args?.trim().toLowerCase() === 'detail' ||
