        ...
        if (!sessionStartTime) {
            if (context.executionMode !== 'interactive') {
                return {
                    type: 'message',
                    messageType: 'error',
                    content: t('Session start time is unavailable, cannot calculate stats.'),
                };
            }
            context.ui.addItem({
            ...
            action: (context) => {
                if (context.executionMode !== 'interactive') {
                    ...
                    for (const [modelName, modelMetrics] of Object.entries(metrics.models)) {
                        lines.push(`${modelName}: prompt=${modelMetrics.tokens.prompt}, output=${modelMetrics.tokens.candidates}, cached=${modelMetrics.tokens.cached}`);
