...
import { ArenaManager, ArenaEventType, isTerminalStatus, isSuccessStatus, ArenaSessionStatus, AuthType, createDebugLogger, stripStartupContext, } from '@vivekmind/core';
...
function buildArenaExecutionInput(parsed, config) {
    ...
    if (parsed.models.length < 2) {
        return {
            type: 'message',
            messageType: 'error',
            content: 'Arena requires at least 2 models. Use --models model1,model2 to specify.\n' +
...
function executeArenaCommand(config, ui, input) {
    ...
    const handleSessionUpdate = (event) => {
        ...
        if (event.message.startsWith(attachHintPrefix)) {
            ...
            addAndRecordArenaMessage(MessageType.INFO, `Arena panes are running in tmux. Attach with: \`${command}\``);
            return;
        }
    ...
    const handleAgentError = (event) => {
        ...
        addAndRecordArenaMessage(MessageType.ERROR, `[${label}] failed: ${event.error}`);
    };
...
export const arenaCommand = {
    ...
    subCommands: [
        {
            name: 'start',
            description: 'Start an Arena session with multiple models competing on the same task',
            kind: CommandKind.BUILT_IN,
            supportedModes: ['interactive'],
            action: async (context, args) => {
                ...
                if (executionMode !== 'interactive') {
                    return {
                        type: 'message',
                        messageType: 'error',
                        content: 'Arena is not supported in non-interactive mode. Use interactive mode to start an Arena session.',
                    };
                }
                ...
                if (existingManager) {
                    return {
                        type: 'message',
                        messageType: 'error',
                        content: 'An Arena session exists. Use /arena stop or /arena select to end it before starting a new one.',
                    };
                }
            ...
            action: async (context) => {
                ...
                if (executionMode !== 'interactive') {
                    return {
                        type: 'message',
                        messageType: 'error',
                        content: 'Arena is not supported in non-interactive mode. Use interactive mode to stop an Arena session.',
                    };
                }
        ...
        },
        {
            name: 'select',
            altNames: ['choose'],
            description: 'Select a model result and merge its diff into the current workspace',
            kind: CommandKind.BUILT_IN,
            supportedModes: ['interactive'],
            action: async (context, args) => {
                const executionMode = context.executionMode ?? 'interactive';
