 ...
 */
import { buildBackgroundEntryLabel, } from '@vivekmind/core';
...
export const tasksCommand = {
    ...
    action: async (context) => {
        ...
        const agentEntries = config
            .getBackgroundTaskRegistry()
            .getAll()
            .map((entry) => ({ ...entry, kind: 'agent' }));
        const shellEntries = config
            .getBackgroundShellRegistry()
            .getAll()
            .map((entry) => ({ ...entry, kind: 'shell' }));
        const monitorEntries = config
            .getMonitorRegistry()
            .getAll()
            .map((entry) => ({ ...entry, kind: 'monitor' }));
        const entries = [...agentEntries, ...shellEntries, ...monitorEntries].sort((a, b) => a.startTime - b.startTime);
        if (entries.length === 0) {
            return {
                type: 'message',
                messageType: 'info',
