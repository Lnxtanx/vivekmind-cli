...
import { IdeClient, ideContextStore, createDebugLogger, getErrorMessage, getAllGeminiMdFilenames, ShellExecutionService, Storage, SessionEndReason, SessionStartSource, generatePromptSuggestion, logPromptSuggestion, PromptSuggestionEvent, logSpeculation, SpeculationEvent, startSpeculation, acceptSpeculation, abortSpeculation, IDLE_SPECULATION, ApprovalMode, ConditionalRulesRegistry, ToolConfirmationOutcome, } from '@vivekmind/core';
import { buildResumedHistoryItems } from './utils/resumeHistoryUtils.js';
import { getStickyTodos, getStickyTodoMaxVisibleItems, getStickyTodosLayoutKey, getStickyTodosRenderKey, } from './utils/todoSnapshot.js';
...
import { readManyFiles } from '../utils/modelConfigUtils.js';
...
export const AppContainer = (props) => {
    ...
    const { codingPlanUpdateRequest, dismissCodingPlanUpdate } = useCodingPlanUpdates(settings, config, historyManager.addItem);
    ...
    useEffect(() => {
        (async () => {
            ...
            if (hookSystem) {
                hookSystem
                    .fireSessionStartEvent(sessionStartSource, config.getModel() ?? '', String(config.getApprovalMode()))
                    .then(() => {
    ...
    const { setAuthState, authError, onAuthError, isAuthDialogOpen, isAuthenticating, pendingAuthType, setPendingAuthType, externalAuthState, vivekmindAuthState, handleAuthSelect, handleCodingPlanSubmit, handleAlibabaStandardSubmit, handleOpenRouterSubmit, handleCustomApiKeySubmit, handleBedrockCredentialsSubmit, handleVertexCredentialsSubmit, openAuthDialog, cancelAuthentication, } = useAuthCommand(settings, config, historyManager.addItem, refreshStatic);
    ...
    const { isModelDialogOpen, isFastModelMode, openModelDialog, closeModelDialog, } = useModelCommand();
    const { isManageModelsDialogOpen, openManageModelsDialog, closeManageModelsDialog, } = useManageModelsCommand();
    ...
    const { isDeleteDialogOpen, openDeleteDialog, closeDeleteDialog, handleDelete, } = useDeleteCommand({
        config,
    ...
    const { handleSlashCommand, slashCommands, pendingHistoryItems: pendingSlashCommandHistoryItems, btwItem, setBtwItem, cancelBtw, commandContext, shellConfirmationRequest, confirmationRequest, } = useSlashCommandProcessor(config, settings, historyManager.addItem, historyManager.clearItems, historyManager.loadHistory, remountStaticHistory, toggleVimEnabled, isProcessing, setIsProcessing, isIdleRef, setGeminiMdFileCount, slashCommandActions, extensionsUpdateStateInternal, isConfigInitialized, logger, setSessionName);
    ...
    const performMemoryRefresh = useCallback(async () => {
        ...
        try {
            const { memoryContent, fileCount, conditionalRules, projectRoot } = await loadHierarchicalGeminiMemory(process.cwd(), settings.merged.context?.loadFromIncludeDirectories
    ...
    const { messageQueue, addMessage, clearQueue, popAllMessages, drainQueue, popNextSegment, } = useMessageQueue();
    ...
    useEffect(() => {
        ...
        for (const tc of pendingToolCalls) {
            if (tc.status === 'awaiting_approval' &&
                ...
                dualOutput.emitPermissionRequest(requestId, tc.request.name, tc.request.callId, tc.request.args);
            }
        }
    ...
    const handleFinalSubmit = useCallback((submittedValue, attachments) => {
        ...
        if (attachments && attachments.length > 0) {
            ...
            // readManyFiles handles binary/text detection and returns PartListUnion
            const result = readManyFiles(filePaths, config.getTargetDir());
            // Add the user text as the final part
            const fullContent = [
                ...result.contentParts,
                submittedValue,
            ];
            void submitQuery(fullContent);
            return;
        }
        // Process attachments if present
        if (attachments && attachments.length > 0) {
        ...
        if (['exit', 'quit', ':q', ':q!', ':wq', ':wq!'].includes(submittedValue.trim())) {
                        ...
                        if (msg.role === 'user' && msg.parts) {
                            ...
                            if (toolCalls.length > 0) {
                                ...
                                const tools = toolCalls.map((tc, i) => {
                                    ...
                                    const resultText = typeof resp === 'object' && resp
                                        ? (resp['output'] ??
                                            JSON.stringify(resp))
                                        : String(resp ?? '');
                                    return {
    ...
    const [compactMode, setCompactMode] = useState(settings.merged.ui?.compactMode ?? false);
    ...
    const stickyTodoMaxVisibleItems = getStickyTodoMaxVisibleItems(terminalHeight);
    ...
    const availableTerminalHeight = Math.max(0, terminalHeight - controlsHeight - staticExtraHeight - 2 - tabBarHeight);
    ...
    const handleRewindConfirm = useCallback((userItem) => {
        const geminiClient = config.getGeminiClient();
        if (!geminiClient)
            return;
        ...
        const apiTruncateIndex = computeApiTruncationIndex(originalHistory, userItem.id, apiHistory);
    ...
    const currentCandidatesTokens = Object.values(sessionStats.metrics?.models ?? {}).reduce((acc, model) => acc + (model.tokens?.candidates ?? 0), 0);
    const { elapsedTime, currentLoadingPhrase, taskStartTokens } = useLoadingIndicator(streamingState, settings.merged.ui?.customWittyPhrases, currentCandidatesTokens);
    ...
    const compactModeValue = useMemo(() => ({ compactMode, setCompactMode }), [compactMode, setCompactMode]);
    return (_jsx(UIStateContext.Provider, { value: uiState, children: _jsx(UIActionsContext.Provider, { value: uiActions, children: _jsx(ConfigContext.Provider, { value: config, children: _jsx(AppContext.Provider, { value: {
                    ...
                    }, children: _jsx(CompactModeProvider, { value: compactModeValue, children: _jsx(ShellFocusContext.Provider, { value: isFocused, children: _jsx(App, {}) }) }) }) }) }) }));
};
