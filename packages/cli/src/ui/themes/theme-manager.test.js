...
import { themeManager, DEFAULT_THEME, AUTO_THEME_NAME, } from './theme-manager.js';
...
describe('ThemeManager', () => {
    ...
    describe('auto theme detection', () => {
        it('should select VivekMind Dark when terminal is detected as dark', () => {
            ...
            expect(themeManager.getActiveTheme().name).toBe('VivekMind Dark');
        });
        it('should select VivekMind Light when terminal is detected as light', () => {
            vi.mocked(detectModule.detectTerminalTheme).mockReturnValue('light');
            const result = themeManager.setActiveTheme(AUTO_THEME_NAME);
            expect(result).toBe(true);
            expect(themeManager.getActiveTheme().name).toBe('VivekMind Light');
        });
        it('should always return true for auto theme', () => {
            expect(themeManager.setActiveTheme(AUTO_THEME_NAME)).toBe(true);
        });
        it('should resolve async auto theme with VivekMind Light for light', async () => {
            vi.mocked(detectModule.detectTerminalThemeAsync).mockResolvedValue('light');
            await themeManager.resolveAutoThemeAsync();
            expect(themeManager.getActiveTheme().name).toBe('VivekMind Light');
        });
        it('should resolve async auto theme with VivekMind Dark for dark', async () => {
            vi.mocked(detectModule.detectTerminalThemeAsync).mockResolvedValue('dark');
            await themeManager.resolveAutoThemeAsync();
            expect(themeManager.getActiveTheme().name).toBe('VivekMind Dark');
        });
