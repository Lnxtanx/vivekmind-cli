...
function createToolGroup(id, tools, isUserInitiated) {
    return {
        type: 'tool_group',
        id,
        tools,
...
function createTool(callId, name, status, resultDisplay, ptyId) {
    return {
        callId,
        name,
...
describe('mergeCompactToolGroups', () => {
    ...
    it('returns single non-tool-group unchanged', () => {
        const items = [{ type: 'user', id: 1, text: 'hello' }];
        expect(mergeCompactToolGroups(items)).toEqual(items);
    });
    ...
    it('does NOT merge user-initiated tool_group', () => {
        const items = [
            createToolGroup(1, [createTool('c1', 'Shell', ToolCallStatus.Success)], false),
            createToolGroup(2, [createTool('c2', 'Shell', ToolCallStatus.Success)], true), // user-initiated
            createToolGroup(3, [createTool('c3', 'Shell', ToolCallStatus.Success)], false),
        ];
        const merged = mergeCompactToolGroups(items);
        expect(merged.length).toBe(3);
    ...
    it('merges 8 tool_groups each separated by a gemini_thought', () => {
        ...
        const items = [];
        for (let n = 0; n < 8; n++) {
            items.push(createToolGroup(n * 2 + 1, [
    ...
    it('drops trailing tool_use_summary after a single absorbed tool_group', () => {
        ...
        const merged = mergeCompactToolGroups(items, false, undefined, new Set(['c1']));
        expect(merged.length).toBe(1);
        expect(merged[0].id).toBe(1);
        expect(merged[0].type).toBe('tool_group');
    });
    ...
    it('merges tool_groups separated by tool_use_summary (hidden in compact)', () => {
        ...
        const merged = mergeCompactToolGroups(items, false, undefined, new Set(['c1', 'c2']));
        expect(merged.length).toBe(1);
        expect(merged[0].id).toBe(1);
        if (isToolGroup(merged[0])) {
            expect(merged[0].tools.map((t) => t.callId)).toEqual(['c1', 'c2']);
    ...
    it('handles complex sequence with mixed force-expand and mergeable', () => {
        const items = [
            ...
            createToolGroup(5, [createTool('c5', 'Shell', ToolCallStatus.Success)], true), // user-initiated
...
//# sourceMappingURL=mergeCompactToolGroups.test.js.map