 ...
 */
export function isForceExpandGroup(item, embeddedShellFocused, activeShellPtyId) {
...
function isHiddenInCompactMode(item) {
    return (item.type === 'gemini_thought' ||
 ...
 */
export function mergeCompactToolGroups(items, embeddedShellFocused = false, activeShellPtyId = undefined, absorbedCallIds = new Set()) {
    const result = [];
    let i = 0;
    while (i < items.length) {
        ...
        if (item.type === 'tool_use_summary') {
            const allAbsorbed = item.precedingToolUseIds.length > 0 &&
...
//# sourceMappingURL=mergeCompactToolGroups.js.map