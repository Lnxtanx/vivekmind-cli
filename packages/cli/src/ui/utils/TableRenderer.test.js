...
describe('<TableRenderer />', () => {
    const renderTable = (headers, rows, contentWidth = 80, aligns) => {
        const { lastFrame } = renderWithProviders(_jsx(TableRenderer, { headers: headers, rows: rows, contentWidth: contentWidth, aligns: aligns }));
        return lastFrame() ?? '';
    };
    const getVisibleLines = (output) => output
        .split('\n')
        .map((line) => line.replace(/\r/g, ''))
        .filter((line) => line.length > 0);
    const expectAllLinesToHaveSameVisibleWidth = (output) => {
    ...
    it('keeps all rendered lines at the same visible width for mixed content', () => {
        const output = renderTable(['项目', 'ANSI', 'Markdown'], [['中文内容', '\u001b[31mRed\u001b[0m Blue', '**bold** and `code`']], 42, ['left', 'center', 'right']);
        expectAllLinesToHaveSameVisibleWidth(output);
    });
    it('handles CJK characters with correct column alignment', () => {
        const output = renderTable(['项目', '描述'], [['名称', '这是一个很长的描述']]);
    ...
    it('handles mixed CJK and ASCII content', () => {
        const output = renderTable(['Feature', '功能'], [
            ['Speed', '速度很快'],
            ['Quality', '质量很高'],
        ]);
    ...
    it('wraps long cell content instead of truncating', () => {
        ...
        const output = renderTable(['Col'], [[longText]], 30);
    ...
    it('handles multiple columns with mixed alignment', () => {
        const output = renderTable(['Left', 'Center', 'Right'], [['L', 'C', 'R']], 40, ['left', 'center', 'right']);
        expect(output).toContain('Left');
        expect(output).toContain('Center');
        expect(output).toContain('Right');
    });
    it('handles tables wider than terminal width', () => {
        const output = renderTable(['Column A', 'Column B', 'Column C'], [
            ['AAAA', 'BBBB', 'CCCC'],
            ['DDDD', 'EEEE', 'FFFF'],
        ], 30);
    ...
    it('renders CJK-heavy table that would previously be misaligned', () => {
        ...
        const output = renderTable(['对比项', 'Claude Code', 'VivekMind'], [
            ['性能', '优秀', '优秀'],
            ['中文支持', '一般', '很好'],
            ['开源', '否', '是'],
        ], 50);
        ...
        expect(output).toContain('VivekMind');
    ...
    it('handles rows with fewer columns than headers', () => {
        const output = renderTable(['A', 'B', 'C'], [['only-one']]);
        expect(output).toContain('only-one');
    });
    it('wraps content for very narrow terminals with many columns', () => {
        const output = renderTable(['Col1', 'Col2', 'Col3', 'Col4', 'Col5'], [['LongValue1', 'LongValue2', 'LongValue3', 'LongValue4', 'LongValue5']], 20);
    ...
    it('handles cell content that is all CJK', () => {
        const output = renderTable(['项目名', '状态'], [
            ['数据库连接测试', '成功'],
            ['缓存压力测试', '失败'],
        ], 40);
    ...
    it('wraps complex ANSI-colored content without losing segments', () => {
        const colorful = '\u001b[31m红色\u001b[0m and \u001b[32mgreen\u001b[0m then \u001b[34mblue文本\u001b[0m';
    ...
    it('keeps markdown cells readable while preserving layout', () => {
        const output = renderTable(['名称', '描述'], [['**加粗**', '`code` 和 普通文本']], 40);
    ...
    it('handles ANSI and markdown mixed across different columns', () => {
        ...
        const output = renderTable(['ANSI', 'Markdown'], [[blue, '**bold** text']], 50);
        expect(output).toContain('\u001b[34m');
        expect(output).toContain('Blue');
        expect(output).toContain('bold');
    });
    it('renders markdown links as readable plain text in cells', () => {
        const output = renderTable(['Name', 'Link'], [['Doc', '[VivekMind](https://example.com/path)']], 60);
        expect(output).toContain('VivekMind');
        expect(output).not.toContain('[VivekMind](');
    });
    it('renders inline code and bold text readably in the same cell', () => {
        const output = renderTable(['Desc'], [['Use `npm test` with **care**']], 40);
    ...
    it('does not collapse content when multiple markdown syntaxes coexist', () => {
        const output = renderTable(['Mixed'], [['**bold** _italic_ `code` [link](https://a.b)']], 60);
        expect(output).toContain('bold');
    ...
    it('handles all-ANSI cell content', () => {
        const colorful = '\u001b[31mR\u001b[0m\u001b[32mG\u001b[0m\u001b[34mB\u001b[0m';
    ...
    it('falls back safely under extremely narrow width', () => {
        const output = renderTable(['HeaderA', 'HeaderB'], [['ValueA', 'ValueB']], 2);
        expect(output).toBeDefined();
        expect(output).not.toContain('NaN');
    });
    ...
    it('keeps CJK + ANSI + wrapping stable near width boundary', () => {
        ...
        const output = renderTable(['项目', '结果说明'], [[cyan, '这是一个接近边界宽度的说明文本']], 26);
    ...
    it('keeps alignment stable with mixed widths near boundary', () => {
        const output = renderTable(['短', 'LongHeader'], [['中文', 'abcdefghi']], 24, ['center', 'right']);
    ...
    it('renders vertical fallback with CJK labels readably', () => {
        const output = renderTable(['字段一', '字段二', '字段三'], [['很长的值一', '很长的值二', '很长的值三']], 10);
    ...
    it('stays stable across multiple content widths', () => {
        for (const width of [8, 10, 12, 16, 20, 30, 40, 60]) {
            const output = renderTable(['项目', '状态', '说明'], [
                [
                    '中文ABC',
                    '\u001b[33mWARN\u001b[0m',
