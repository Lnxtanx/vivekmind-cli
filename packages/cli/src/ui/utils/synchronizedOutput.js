...
export function terminalSupportsSynchronizedOutput(env = process.env) {
    if (env['VIVEKMIND_CODE_DISABLE_SYNCHRONIZED_OUTPUT'] === '1' ||
        env['VIVEKMIND_CODE_SYNCHRONIZED_OUTPUT'] === '0') {
        return false;
    }
    if (env['VIVEKMIND_CODE_FORCE_SYNCHRONIZED_OUTPUT'] === '1' ||
        env['VIVEKMIND_CODE_SYNCHRONIZED_OUTPUT'] === '1') {
        return true;
    }
