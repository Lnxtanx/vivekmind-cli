 ...
 */
function recolorAfterResets(text, colorCode) {
 ...
 */
function renderMarkdownToAnsi(text) {
 ...
 */
export const TableRenderer = ({ headers, rows, contentWidth, aligns, }) => {
    ...
    const rowMetrics = rows.map((row) => Array.from({ length: colCount }, (_, i) => computeMetrics(row[i] || '')));
    ...
    const availableWidth = Math.max(contentWidth - borderOverhead - SAFETY_MARGIN, colCount * MIN_COLUMN_WIDTH);
    ...
    if (totalIdeal <= availableWidth) {
        ...
        columnWidths = minColumnWidths.map((w) => Math.max(Math.floor(w * scaleFactor), MIN_COLUMN_WIDTH));
    ...
    const getAlign = (colIndex) => aligns?.[colIndex] ?? 'left';
    ...
    function renderRowLines(renderedCells, isHeader) {
        const cellLines = renderedCells.map((cell, colIndex) => wrapText(cell, columnWidths[colIndex], { hard: needsHardWrap }));
    ...
    if (useVerticalFormat) {
        return (_jsx(Box, { marginY: 1, children: _jsx(Text, { children: renderVerticalFormat() }) }));
    }
    ...
    rowMetrics.forEach((row, rowIndex) => {
        tableLines.push(...renderRowLines(row.map((m) => m.rendered), false));
    ...
    return (_jsx(Box, { flexDirection: "column", marginY: 1, children: _jsx(Text, { children: tableLines.join('\n') }) }));
};
