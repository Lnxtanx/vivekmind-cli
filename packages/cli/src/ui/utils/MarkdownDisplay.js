...
const MarkdownDisplayInternal = ({ text, isPending, availableTerminalHeight, contentWidth, textColor = theme.text.primary, }) => {
    if (!text)
    ...
    const parseTableAligns = (line) => line
        .split(/(?<!\\)\|/)
        .map((cell) => cell.trim())
    ...
    lines.forEach((line, index) => {
        ...
        if (inCodeBlock) {
            ...
            if (fenceMatch &&
                ...
                addContentBlock(_jsx(RenderCodeBlock, { content: codeBlockContent, lang: codeBlockLang, isPending: isPending, availableTerminalHeight: availableTerminalHeight, contentWidth: contentWidth }, key));
                inCodeBlock = false;
                codeBlockContent = [];
                codeBlockLang = null;
                codeBlockFence = '';
        ...
        if (codeFenceMatch) {
            ...
            if (tableHeaders.length > 0 && tableRows.length > 0) {
                addContentBlock(_jsx(RenderTable, { headers: tableHeaders, rows: tableRows, contentWidth: contentWidth, aligns: tableAligns }, `table-${contentBlocks.length}`));
            }
            inTable = false;
            tableRows = [];
            tableHeaders = [];
    ...
    if (inCodeBlock) {
        addContentBlock(_jsx(RenderCodeBlock, { content: codeBlockContent, lang: codeBlockLang, isPending: isPending, availableTerminalHeight: availableTerminalHeight, contentWidth: contentWidth }, "line-eof"));
    }
...
const RenderCodeBlockInternal = ({ content, lang, isPending, availableTerminalHeight, contentWidth, }) => {
...
const RenderTableInternal = ({ headers, rows, contentWidth, aligns, }) => (_jsx(TableRenderer, { headers: headers, rows: rows, contentWidth: contentWidth, aligns: aligns }));
