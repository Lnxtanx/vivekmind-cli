...
import { getTerminalRedrawStatsSnapshot, installTerminalRedrawOptimizer, optimizeMultilineEraseLines, resetTerminalRedrawStats, } from './terminalRedrawOptimizer.js';
const ESC = '\u001B[';
...
describe('optimizeMultilineEraseLines', () => {
    it('collapses repeated cursor-up movement without erasing below', () => {
        ...
        expect(optimizeMultilineEraseLines(input)).toBe(`${ESC}2A${ERASE_LINE}${CURSOR_DOWN_ONE}${ERASE_LINE}${CURSOR_DOWN_ONE}${ERASE_LINE}${ESC}2A${CURSOR_LEFT}next frame`);
    });
    ...
    it('optimizes each multiline erase sequence in a chunk', () => {
        ...
        expect(optimizeMultilineEraseLines(`${first}a${second}b`)).toBe(`${first}a${ESC}2A${ERASE_LINE}${CURSOR_DOWN_ONE}${ERASE_LINE}${CURSOR_DOWN_ONE}${ERASE_LINE}${ESC}2A${CURSOR_LEFT}b`);
    });
...
describe('installTerminalRedrawOptimizer', () => {
    ...
    it('optimizes string writes and restores the original writer', () => {
        ...
        const input = `${ERASE_LINE}${CURSOR_UP_ONE}${ERASE_LINE}${CURSOR_UP_ONE}${ERASE_LINE}${CURSOR_LEFT}`;
        stdout.write(input);
        expect(write).toHaveBeenCalledWith(`${ESC}2A${ERASE_LINE}${CURSOR_DOWN_ONE}${ERASE_LINE}${CURSOR_DOWN_ONE}${ERASE_LINE}${ESC}2A${CURSOR_LEFT}`, undefined, undefined);
        restore();
    ...
    it('tracks write, byte, clear, and erase optimization counters', () => {
        ...
        const stdout = { write };
        installTerminalRedrawOptimizer(stdout);
        stdout.write(`${ERASE_LINE}${CURSOR_UP_ONE}${ERASE_LINE}${CURSOR_UP_ONE}${ERASE_LINE}${CURSOR_LEFT}`);
        stdout.write(Buffer.from('ok'));
        stdout.write('\u001B[2J\u001B[3J\u001B[H');
        expect(getTerminalRedrawStatsSnapshot()).toEqual({
            stdoutWriteCount: 3,
            stdoutBytes: Buffer.byteLength(`${ESC}2A${ERASE_LINE}${CURSOR_DOWN_ONE}${ERASE_LINE}${CURSOR_DOWN_ONE}${ERASE_LINE}${ESC}2A${CURSOR_LEFT}`) +
    ...
    it('can be disabled for terminal compatibility fallback', () => {
        vi.stubEnv('VIVEKMIND_CODE_LEGACY_ERASE_LINES', '1');
        const write = vi.fn(() => true);
        const stdout = { write };
        const restore = installTerminalRedrawOptimizer(stdout);
        expect(stdout.write).toBe(write);
...
//# sourceMappingURL=terminalRedrawOptimizer.test.js.map