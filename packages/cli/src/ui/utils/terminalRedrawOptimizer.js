...
const MULTILINE_ERASE_LINES_PATTERN = new RegExp(`(?:${escapeRegExp(ERASE_LINE + CURSOR_UP_ONE)})+${escapeRegExp(ERASE_LINE + CURSOR_LEFT)}`, 'g');
...
const terminalRedrawStats = {
    stdoutWriteCount: 0,
    stdoutBytes: 0,
    clearTerminalCount: 0,
    eraseLinesOptimizedCount: 0,
};
export function getTerminalRedrawStatsSnapshot() {
    return { ...terminalRedrawStats };
}
export function resetTerminalRedrawStats() {
...
function optimizeMultilineEraseLinesWithCount(output) {
    ...
    const optimizedOutput = output.replace(MULTILINE_ERASE_LINES_PATTERN, (sequence) => {
 ...
 */
export function optimizeMultilineEraseLines(output) {
    return optimizeMultilineEraseLinesWithCount(output).output;
}
export function installTerminalRedrawOptimizer(stdout) {
    if (process.env['VIVEKMIND_CODE_LEGACY_ERASE_LINES'] === '1') {
        return () => { };
    }
    const originalWrite = stdout.write;
    const optimizedWrite = function (chunk, encodingOrCallback, callback) {
        const optimizedResult = typeof chunk === 'string'
        ...
        if (typeof optimizedChunk === 'string' ||
            ...
            terminalRedrawStats.stdoutBytes += getChunkByteLength(optimizedChunk, encodingOrCallback);
            if (typeof optimizedChunk === 'string') {
                terminalRedrawStats.clearTerminalCount += countOccurrences(optimizedChunk, ansiEscapes.clearTerminal);
            }
        }
        if (optimizedResult) {
        ...
        return originalWrite.call(this, optimizedChunk, encodingOrCallback, callback);
    };
...
//# sourceMappingURL=terminalRedrawOptimizer.js.map