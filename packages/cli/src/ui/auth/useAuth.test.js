...
import { useAuthCommand, generateCustomApiKeyEnvKey, normalizeCustomModelIds, maskApiKey, } from './useAuth.js';
import { OPENROUTER_OAUTH_CALLBACK_URL, applyOpenRouterModelsConfiguration, createOpenRouterOAuthSession, runOpenRouterOAuthLogin, } from '../../commands/auth/openrouterOAuth.js';
...
vi.mock('../../commands/auth/openrouterOAuth.js', () => ({
    ...
    runOpenRouterOAuthLogin: vi.fn(() => new Promise(() => undefined)),
}));
...
describe('useAuthCommand', () => {
    ...
    it('cancels OpenRouter OAuth wait and reopens the auth dialog', async () => {
        ...
        expect(result.current.isAuthenticating).toBe(true);
        expect(createOpenRouterOAuthSession).toHaveBeenCalledWith(OPENROUTER_OAUTH_CALLBACK_URL);
        expect(runOpenRouterOAuthLogin).toHaveBeenCalledWith(OPENROUTER_OAUTH_CALLBACK_URL, expect.objectContaining({
    ...
    it('cleans up UI state when OpenRouter OAuth rejects with AbortError', async () => {
        ...
        const config = createConfig();
        const addItem = vi.fn();
        vi.mocked(runOpenRouterOAuthLogin).mockRejectedValueOnce(new DOMException('OpenRouter OAuth cancelled.', 'AbortError'));
        const { result } = renderHook(() => useAuthCommand(settings, config, addItem));
        await act(async () => {
    ...
    it('adds /model and /manage-models guidance after OpenRouter auth succeeds', async () => {
        ...
        expect(applyOpenRouterModelsConfiguration).toHaveBeenCalledWith(expect.objectContaining({
            settings: expect.anything(),
            config: expect.anything(),
            apiKey: 'oauth-key-123',
            reloadConfig: true,
        }));
        expect(addItem).toHaveBeenCalledWith(expect.objectContaining({ text: 'Successfully configured OpenRouter.' }), expect.any(Number));
        expect(addItem).toHaveBeenCalledWith(expect.objectContaining({ text: 'Use /model to switch models.' }), expect.any(Number));
...
describe('generateCustomApiKeyEnvKey', () => {
    it('generates env key from openai protocol and base URL', () => {
        const key = generateCustomApiKeyEnvKey('openai', 'https://api.openai.com/v1');
    ...
    it('generates env key from anthropic protocol and base URL', () => {
        const key = generateCustomApiKeyEnvKey('anthropic', 'https://api.anthropic.com/v1');
        expect(key).toBe('VIVEKMIND_CUSTOM_API_KEY_ANTHROPIC_HTTPS_API_ANTHROPIC_COM_V1');
    });
    it('generates env key from gemini protocol and base URL', () => {
        const key = generateCustomApiKeyEnvKey('gemini', 'https://generativelanguage.googleapis.com');
        expect(key).toBe('VIVEKMIND_CUSTOM_API_KEY_GEMINI_HTTPS_GENERATIVELANGUAGE_GOOGLEAPIS_COM');
    });
    it('handles localhost URLs', () => {
        const key = generateCustomApiKeyEnvKey('openai', 'http://localhost:11434/v1');
        expect(key).toBe('VIVEKMIND_CUSTOM_API_KEY_OPENAI_HTTP_LOCALHOST_11434_V1');
    });
    it('normalizes trailing slashes and special chars', () => {
        const key = generateCustomApiKeyEnvKey('openai', 'https://openrouter.ai/api/v1/');
