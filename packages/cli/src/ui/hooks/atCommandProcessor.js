...
import { getErrorMessage, isNodeError, Storage, unescapePath, readManyFiles, } from '@vivekmind/core';
 ...
 */
export async function handleAtCommand({ query, config, onDebugMessage, messageId: userMessageTimestamp, signal, addItem, }) {
    const commandParts = parseAllAtCommands(query);
    const atPathCommandParts = commandParts.filter((part) => part.type === 'atPath');
    const addToolGroup = (result) => {
        if (!addItem)
            return;
    ...
    for (const atPathPart of atPathCommandParts) {
        ...
        if (!absolutePathName.startsWith(projectTempDir) &&
        ...
        const vivekMindIgnored = respectFileIgnore.respectVivekMindIgnore &&
            fileDiscovery.shouldIgnoreFile(pathName, {
    ...
    try {
        ...
        const toolCallDisplays = result.files.map((file, index) => ({
