...
import { AuthType, isCodingPlanConfig, getCodingPlanConfig, CodingPlanRegion, CODING_PLAN_ENV_KEY, } from '@vivekmind/core';
 ...
 */
export function useCodingPlanUpdates(settings, config, addItem) {
     ...
     */
    const executeUpdate = useCallback(async (region = CodingPlanRegion.CHINA) => {
        try {
            const persistScope = getPersistScopeForModelSelection(settings);
            // Get current configs
            const currentConfigs = settings.merged.modelProviders?.[AuthType.USE_OPENAI] || [];
            // Filter out all Coding Plan configs (since they are mutually exclusive)
            // Keep only non-Coding-Plan user custom configs
            const nonCodingPlanConfigs = currentConfigs.filter((cfg) => !isCodingPlanConfig(cfg['baseUrl'], cfg['envKey']));
            ...
            const previousModelStillAvailable = newConfigs.some((cfg) => cfg.id === previousModel);
            ...
            if (previousModelStillAvailable && activeModel === previousModel) {
                ...
                addItem({
                    type: 'info',
                    text: t('{{region}} configuration updated successfully. Model switched to "{{model}}".', { region: t('Alibaba Cloud Coding Plan'), model: activeModel }),
                }, Date.now());
            }
            addItem({
                type: 'info',
    ...
    const checkForUpdates = useCallback(() => {
        ...
        if (savedVersion !== currentVersion) {
            setUpdateRequest({
                prompt: t('New model configurations are available for {{region}}. Update now?', { region: t('Alibaba Cloud Coding Plan') }),
...
//# sourceMappingURL=useCodingPlanUpdates.js.map