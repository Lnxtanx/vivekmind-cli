...
vi.mock('@vivekmind/core', () => {
    class Storage {
        getProjectTempDir() {
            return path.join('/test/home/', '.vivekmind', 'tmp', 'mocked_hash');
        }
        getHistoryFilePath() {
            return path.join('/test/home/', '.vivekmind', 'tmp', 'mocked_hash', 'shell_history');
        }
    }
    return {
...
const MOCKED_HISTORY_DIR = path.join(MOCKED_HOME_DIR, '.vivekmind', 'tmp', MOCKED_PROJECT_HASH);
