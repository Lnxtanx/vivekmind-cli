...
import { CODING_PLAN_ENV_KEY, getCodingPlanConfig, CodingPlanRegion, AuthType, } from '@vivekmind/core';
...
describe('useCodingPlanUpdates', () => {
    ...
    describe('version comparison', () => {
        it('should not show update prompt when no version is stored', () => {
            ...
            const { result } = renderHook(() => useCodingPlanUpdates(mockSettings, mockConfig, mockAddItem));
        ...
        it('should default to China region when region is not specified', async () => {
            ...
            expect(result.current.codingPlanUpdateRequest?.prompt).toContain('Alibaba Cloud Coding Plan');
        });
    ...
    describe('update execution', () => {
        it('should execute China region update when user confirms', async () => {
            ...
            expect(mockSettings.setValue).toHaveBeenCalledWith(expect.anything(), 'codingPlan.version', chinaConfig.version);
            // Should update region
            expect(mockSettings.setValue).toHaveBeenCalledWith(expect.anything(), 'codingPlan.region', CodingPlanRegion.CHINA);
        ...
        it('should execute Global region update when user confirms', async () => {
            ...
            expect(mockSettings.setValue).toHaveBeenCalledWith(expect.anything(), 'codingPlan.version', globalConfig.version);
            // Should update region
            expect(mockSettings.setValue).toHaveBeenCalledWith(expect.anything(), 'codingPlan.region', CodingPlanRegion.GLOBAL);
            // Should reload and refresh auth
            expect(mockConfig.reloadModelProvidersConfig).toHaveBeenCalled();
            expect(mockConfig.refreshAuth).toHaveBeenCalledWith(AuthType.USE_OPENAI);
        ...
        it('should replace all Coding Plan configs during update (mutually exclusive)', async () => {
            ...
            const modelProvidersCall = setValueCalls.find((call) => call[1].includes('modelProviders'));
            expect(modelProvidersCall).toBeDefined();
            const updatedConfigs = modelProvidersCall[2];
            ...
            expect(updatedConfigs.some((c) => c['baseUrl'] === globalConfig.baseUrl)).toBe(false);
            // Should contain the custom config
            expect(updatedConfigs.some((c) => c['id'] === 'custom-model')).toBe(true);
...
//# sourceMappingURL=useCodingPlanUpdates.test.js.map