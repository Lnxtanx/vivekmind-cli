...
import { createDebugLogger, GitService, logSlashCommand, makeSlashCommandEvent, SlashCommandStatus, ToolConfirmationOutcome, IdeClient, } from '@vivekmind/core';
 ...
 */
export const useSlashCommandProcessor = (config, settings, addItem, clearItems, loadHistory, refreshStatic, toggleVimEnabled, isProcessing, setIsProcessing, isIdleRef, setGeminiMdFileCount, actions, extensionsUpdateState, isConfigInitialized, logger, setSessionName) => {
            ...
            try {
                ...
                if (config) {
                    config.setModelInvocableCommandsProvider(() => commandService.getModelInvocableCommands().map((cmd) => ({
                        name: cmd.name,
                        description: typeof cmd.description === 'string'
                            ? cmd.description
                            : cmd.description,
                    })));
