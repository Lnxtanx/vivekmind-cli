...
import { createDebugLogger, logUserFeedback, UserFeedbackEvent, isNodeError, AuthType, } from '@vivekmind/core';
import { StreamingState, MessageType } from '../types.js';
import { SettingScope, USER_SETTINGS_PATH, } from '../../config/settings.js';
    ...
    useEffect(() => {
        const checkAndShowFeedback = () => {
            if (streamingState === StreamingState.Idle && history.length > 0) {
                ...
                // 1. User is authenticated via VIVEKMIND_OAUTH
                // 2. VivekMind logger is enabled (required for feedback submission)
                ...
                if (config.getAuthType() !== AuthType.VIVEKMIND_OAUTH ||
