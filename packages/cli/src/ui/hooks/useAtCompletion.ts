...
import type { Config, FileSearch } from '@vivekmind/core';
import { FileSearchFactory, escapePath } from '@vivekmind/core';
    ...
    const initialize = async () => {
      try {
        const searcher = FileSearchFactory.create({
          ...
          useVivekMindIgnore:
            config?.getFileFilteringOptions()?.respectVivekMindIgnore ?? true,
          cache: true,
