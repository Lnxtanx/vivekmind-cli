...
describe('useThemeCommand', () => {
    beforeEach(() => {
        vi.restoreAllMocks();
        themeManager.setActiveTheme('VivekMind Dark');
    });
    it('restores previous theme on cancel (Esc)', () => {
        const setValue = vi.fn();
        const settings = {
            merged: { ui: { theme: 'VivekMind Dark' } },
            user: { settings: { ui: {} } },
            workspace: { settings: { ui: {} } },
            setValue,
        };
        const setThemeError = vi.fn();
        const addItem = vi.fn();
        const { result } = renderHook(() => useThemeCommand(settings, setThemeError, addItem, null));
        act(() => {
...
//# sourceMappingURL=useThemeCommand.test.js.map