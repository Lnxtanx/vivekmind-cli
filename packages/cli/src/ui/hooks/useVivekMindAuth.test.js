...
describe('useVivekMindAuth', () => {
    ...
    it('should initialize with default state when not VivekMind auth', () => {
        const { result } = renderHook(() => useVivekMindAuth(AuthType.USE_GEMINI, false));
    ...
    it('should initialize with default state when VivekMind auth but not authenticating', () => {
        const { result } = renderHook(() => useVivekMindAuth(AuthType.VIVEKMIND_OAUTH, false));
        expect(result.current.vivekmindAuthState).toEqual({
            deviceAuth: null,
            authStatus: 'idle',
            authMessage: null,
    ...
    it('should set up event listeners when VivekMind auth and authenticating', () => {
        ...
        expect(mockVivekMindOAuth2Events.on).toHaveBeenCalledWith(VivekMindOAuth2Event.AuthUri, expect.any(Function));
        expect(mockVivekMindOAuth2Events.on).toHaveBeenCalledWith(VivekMindOAuth2Event.AuthProgress, expect.any(Function));
    });
    ...
    it('should handle auth progress event - success', () => {
        ...
        expect(result.current.vivekmindAuthState.authMessage).toBe('Authentication successful!');
    });
    it('should handle auth progress event - error', () => {
        ...
        expect(result.current.vivekmindAuthState.authMessage).toBe('Authentication failed');
    });
    it('should handle auth progress event - polling', () => {
        ...
        expect(result.current.vivekmindAuthState.authStatus).toBe('polling');
        expect(result.current.vivekmindAuthState.authMessage).toBe('Waiting for user authorization...');
    });
    it('should handle auth progress event - rate_limit', () => {
        ...
        expect(result.current.vivekmindAuthState.authMessage).toBe('Too many requests. The server is rate limiting our requests. Please select a different authentication method or try again later.');
    });
    ...
    it('should clean up event listeners when auth type changes', () => {
        const { rerender } = renderHook(({ pendingAuthType, isAuthenticating }) => useVivekMindAuth(pendingAuthType, isAuthenticating), {
            initialProps: {
        ...
        expect(mockVivekMindOAuth2Events.off).toHaveBeenCalledWith(VivekMindOAuth2Event.AuthUri, expect.any(Function));
        expect(mockVivekMindOAuth2Events.off).toHaveBeenCalledWith(VivekMindOAuth2Event.AuthProgress, expect.any(Function));
    });
    it('should clean up event listeners when authentication stops', () => {
        const { rerender } = renderHook(({ isAuthenticating }) => useVivekMindAuth(AuthType.VIVEKMIND_OAUTH, isAuthenticating), { initialProps: { isAuthenticating: true } });
        // Stop authentication
    ...
    it('should clean up event listeners on unmount', () => {
        const { unmount } = renderHook(() => useVivekMindAuth(AuthType.VIVEKMIND_OAUTH, true));
        unmount();
        expect(mockVivekMindOAuth2Events.off).toHaveBeenCalledWith(VivekMindOAuth2Event.AuthUri, expect.any(Function));
        expect(mockVivekMindOAuth2Events.off).toHaveBeenCalledWith(VivekMindOAuth2Event.AuthProgress, expect.any(Function));
    });
    it('should reset state when switching from VivekMind auth to another auth type', () => {
        ...
        const { result, rerender } = renderHook(({ pendingAuthType, isAuthenticating }) => useVivekMindAuth(pendingAuthType, isAuthenticating), {
            initialProps: {
                pendingAuthType: AuthType.VIVEKMIND_OAUTH,
                isAuthenticating: true,
            },
    ...
    it('should reset state when authentication stops', () => {
        ...
        const { result, rerender } = renderHook(({ isAuthenticating }) => useVivekMindAuth(AuthType.VIVEKMIND_OAUTH, isAuthenticating), { initialProps: { isAuthenticating: true } });
        // Simulate device auth
        act(() => {
            handleDeviceAuth(mockDeviceAuth);
        });
    ...
    it('should handle different auth types correctly', () => {
        ...
        const { result: qwenResult } = renderHook(() => useVivekMindAuth(AuthType.VIVEKMIND_OAUTH, true));
        ...
        const { result: geminiResult } = renderHook(() => useVivekMindAuth(AuthType.USE_GEMINI, true));
        expect(geminiResult.current.vivekmindAuthState.authStatus).toBe('idle');
        const { result: oauthResult } = renderHook(() => useVivekMindAuth(AuthType.USE_OPENAI, true));
...
//# sourceMappingURL=useVivekMindAuth.test.js.map