    ...
    describe('Core Hook Behavior', () => {
        describe('State Management', () => {
            it('should initialize with default state', () => {
                const { result } = renderHook(() => useCommandCompletion(useTextBufferForTest(''), testRootDir, [], mockCommandContext, false, mockConfig));
            ...
            it('should reset state when completion mode becomes IDLE', async () => {
                ...
                const { result } = renderHook(() => {
                    ...
                    const completion = useCommandCompletion(textBuffer, testRootDir, [], mockCommandContext, false, mockConfig);
            ...
            it('should reset all state to default values', () => {
                const { result } = renderHook(() => useCommandCompletion(useTextBufferForTest('@files'), testRootDir, [], mockCommandContext, false, mockConfig));
                act(() => {
            ...
            it('should call useAtCompletion with the correct query for an escaped space', async () => {
                ...
                renderHook(() => useCommandCompletion(useTextBufferForTest(text), testRootDir, [], mockCommandContext, false, mockConfig));
                await waitFor(() => {
            ...
            it('should correctly identify the completion context with multiple @ symbols', async () => {
                ...
                renderHook(() => useCommandCompletion(useTextBufferForTest(text, cursorOffset), testRootDir, [], mockCommandContext, false, mockConfig));
                await waitFor(() => {
                    expect(useAtCompletion).toHaveBeenLastCalledWith(expect.objectContaining({
                        enabled: true,
                        pattern: 'file1',
    ...
    describe('argument hint ghost text', () => {
        it('shows argumentHint as inline ghost text for a complete slash command', () => {
            ...
            const { result } = renderHook(() => {
                ...
                const completion = useCommandCompletion(textBuffer, testRootDir, slashCommands, mockCommandContext, false, mockConfig);
                return completion;
            });
