...
import { SlashCommandStatus, ToolConfirmationOutcome, makeFakeConfig, } from '@vivekmind/core';
...
describe('useSlashCommandProcessor', () => {
    ...
    const setupProcessorHook = (builtinCommands = [], fileCommands = [], mcpCommands = [], setIsProcessing = vi.fn()) => {
        ...
        false, // isProcessing
        setIsProcessing, { current: true }, // isIdleRef
    ...
    describe('Action Result Handling', () => {
        ...
        it('should pass interactive execution mode to command actions', async () => {
            ...
            expect(action).toHaveBeenCalledWith(expect.objectContaining({ executionMode: 'interactive' }), '');
        });
    ...
    describe('ui.clear and /btw dialog', () => {
        ...
        it('should abort the in-flight btw request when ui.clear is called', async () => {
            ...
            expect(abortSpy).toHaveBeenCalledTimes(1);
            expect(result.current.commandContext.ui.btwAbortControllerRef.current).toBeNull();
        });
    });
});
