...
import { FileDiscoveryService, StandardFileSystemService, COMMON_IGNORE_PATTERNS,
...
describe('handleAtCommand', () => {
    ...
    describe('vivekmind-ignore filtering', () => {
        it('should skip vivekmind-ignored files in @ commands', async () => {
            await createTestFile(path.join(testRootDir, '.vivekmindignore'), 'build/output.js');
            const vivekMindIgnoredFile = await createTestFile(path.join(testRootDir, 'build', 'output.js'), 'console.log("Hello");');
            ...
            expect(mockOnDebugMessage).toHaveBeenCalledWith(`Path ${vivekMindIgnoredFile} is vivekmind-ignored and will be skipped.`);
    ...
    it('should handle mixed vivekmind-ignored and valid files', async () => {
        await createTestFile(path.join(testRootDir, '.vivekmindignore'), 'dist/bundle.js');
        const validFile = await createTestFile(path.join(testRootDir, 'src', 'main.ts'), '// Main application entry');
        const vivekMindIgnoredFile = await createTestFile(path.join(testRootDir, 'dist', 'bundle.js'), 'console.log("bundle");');
