...
import { fireNotificationHook, NotificationType, } from '@vivekmind/core';
import { sendNotification } from '../../services/notificationService.js';
export const LONG_TASK_NOTIFICATION_THRESHOLD_SECONDS = 20;
const NOTIFICATION_TITLE = 'VivekMind';
    ...
    useEffect(() => {
        if (streamingState === StreamingState.WaitingForConfirmation &&
            ...
            const message = awaitingToolName
                ? `VivekMind needs your permission to use ${awaitingToolName}`
                : 'VivekMind is waiting for your input';
        ...
        if (streamingState === StreamingState.Idle) {
            ...
            if (config && !idleNotificationSentRef.current) {
                ...
                if (hooksEnabled && messageBus) {
                    fireNotificationHook(messageBus, 'VivekMind is waiting for your input', NotificationType.IdlePrompt, 'Waiting for input').catch(() => {
