...
export class DualOutputBridge {
     ...
     */
    emitPermissionRequest(requestId, toolName, toolUseId, input, blockedPath = null) {
        if (!this.active)
            return;
        try {
            this.adapter.emitPermissionRequest(requestId, toolName, toolUseId, input, blockedPath);
        }
        catch (err) {
