...
import { DualOutputBridge, DUAL_OUTPUT_PROTOCOL_VERSION, SUPPORTED_EVENTS, } from './DualOutputBridge.js';
...
describe('DualOutputBridge', () => {
    ...
    describe('--json-fd validation', () => {
        ...
        it('rejects an unopened fd with a clear message', () => {
            ...
            expect(() => new DualOutputBridge(config, { fd: 9999 })).toThrow(/file descriptor is not open/);
        });
    });
    describe('--json-file output', () => {
        ...
        it('session_start carries a capability handshake (version, protocol_version, supported_events)', async () => {
            bridge = new DualOutputBridge(config, { filePath: target }, { version: '1.2.3' });
            await bridge.shutdown();
            const lines = readJsonl(target);
            const start = lines.find((l) => l['type'] === 'system' && l['subtype'] === 'session_start');
        ...
        it('emits session_end on shutdown for a clean termination signal', async () => {
            ...
            await bridge.shutdown();
            const lines = readJsonl(target);
            const end = lines.find((l) => l['type'] === 'system' && l['subtype'] === 'session_end');
        ...
        it('shutdown is idempotent — calling it twice emits session_end only once', async () => {
            ...
            await bridge.shutdown();
            const lines = readJsonl(target);
            const endEvents = lines.filter((l) => l['type'] === 'system' && l['subtype'] === 'session_end');
        ...
        it('emitControlError routes through the adapter as a control_response error', async () => {
            ...
            await bridge.shutdown();
            const lines = readJsonl(target);
            const errorResponse = lines.find((l) => l['type'] === 'control_response' &&
                l['response']?.subtype === 'error');
        ...
        it('reports isConnected=false after shutdown and silently drops further events', async () => {
            ...
            // Should not throw
            expect(() => bridge.emitPermissionRequest('req', 'tool', 'tu', {})).not.toThrow();
            expect(() => bridge.emitControlResponse('req', true)).not.toThrow();
        });
    });
});
//# sourceMappingURL=DualOutputBridge.test.js.map