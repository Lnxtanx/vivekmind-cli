 ...
 */
import { executeToolCall, ToolErrorType, shutdownTelemetry, GeminiEventType, OutputFormat, uiTelemetryService, FatalInputError, ApprovalMode, SendMessageType, } from '@vivekmind/core';
...
describe('runNonInteractive', () => {
    ...
    it('should process input and write text output', async () => {
        ...
        expect(mockGeminiClient.sendMessageStream).toHaveBeenCalledWith([{ text: 'Test input' }], expect.any(AbortSignal), 'prompt-id-1', { type: SendMessageType.UserQuery });
    ...
    it('on EPIPE, destroys stdout and returns normally instead of process.exit', async () => {
        ...
        mockGeminiClient.sendMessageStream.mockImplementation(async function* mockStream() {
    ...
    it('should handle a single tool call and respond', async () => {
        ...
        expect(mockGeminiClient.sendMessageStream).toHaveBeenNthCalledWith(1, [{ text: 'Use a tool' }], expect.any(AbortSignal), 'prompt-id-2', { type: SendMessageType.UserQuery });
        // Verify second call (after tool execution) has type: ToolResult
        expect(mockGeminiClient.sendMessageStream).toHaveBeenNthCalledWith(2, [{ text: 'Tool response' }], expect.any(AbortSignal), 'prompt-id-2', { type: SendMessageType.ToolResult });
    ...
    it('should handle error during tool execution and should send error back to the model', async () => {
        ...
        ], expect.any(AbortSignal), 'prompt-id-3', { type: SendMessageType.ToolResult });
    ...
    it('should preprocess @include commands before sending to the model', async () => {
        ...
        expect(mockGeminiClient.sendMessageStream).toHaveBeenCalledWith(processedParts, expect.any(AbortSignal), 'prompt-id-7', { type: SendMessageType.UserQuery });
    ...
    it('should write JSON output with stats for empty response commands', async () => {
        ...
        expect(mockGeminiClient.sendMessageStream).toHaveBeenCalledWith([{ text: 'Empty response test' }], expect.any(AbortSignal), 'prompt-id-empty', { type: SendMessageType.UserQuery });
        // JSON adapter emits array of messages, last one is result with stats
        const outputCalls = processStdoutSpy.mock.calls.filter((call) => typeof call[0] === 'string');
        expect(outputCalls.length).toBeGreaterThan(0);
        const lastOutput = outputCalls[outputCalls.length - 1][0];
    ...
    it('does not double-wrap or double-format an API error in non-interactive mode', async () => {
        ...
        mockGeminiClient.sendMessageStream.mockReturnValue(createStreamFromEvents([apiErrorEvent]));
        await expect(runNonInteractive(mockConfig, mockSettings, 'Test input', 'prompt-id-double-wrap')).rejects.toBeInstanceOf(AlreadyReportedError);
    ...
    it('should execute a slash command that returns a prompt', async () => {
        ...
        expect(mockGeminiClient.sendMessageStream).toHaveBeenCalledWith([{ text: 'Prompt from command' }], expect.any(AbortSignal), 'prompt-id-slash', { type: SendMessageType.UserQuery });
    ...
    it('should treat an unknown slash command as a regular prompt', async () => {
        ...
        expect(mockGeminiClient.sendMessageStream).toHaveBeenCalledWith([{ text: '/unknowncommand' }], expect.any(AbortSignal), 'prompt-id-unknown', { type: SendMessageType.UserQuery });
    ...
    it('should handle known but unsupported slash commands like /help by returning early', async () => {
        ...
        // Should write error message through adapter to stdout (TEXT mode goes through JsonOutputAdapter)
        expect(processStderrSpy).toHaveBeenCalledWith('The command "/help" is not supported in this mode.\n');
    });
    ...
    it('flushes terminal monitor notifications before the final headless result', async () => {
        ...
        const notificationXml = '<task-notification>\n' +
            ...
            '<result>ready</result>\n' +
            '</task-notification>';
        const cancelledXml = '<task-notification>\n' +
            '<task-id>mon_1</task-id>\n' +
            '<kind>monitor</kind>\n' +
        ...
        mockMonitorRegistry.abortAll.mockImplementation(() => {
            monitorNotificationCallback?.('Monitor "logs" was cancelled.', cancelledXml, {
                monitorId: 'mon_1',
                toolUseId: 'tool_mon_1',
                status: 'cancelled',
                eventCount: 1,
        ...
        await runNonInteractive(mockConfig, mockSettings, 'Watch the logs', 'prompt-monitor');
        expect(mockGeminiClient.sendMessageStream).toHaveBeenCalledTimes(2);
        expect(mockGeminiClient.sendMessageStream).toHaveBeenNthCalledWith(2, [{ text: notificationXml }], expect.any(AbortSignal), 'prompt-monitor', {
        ...
        const envelopes = writes
            ...
            .filter((line) => line.trim().length > 0)
            .map((line) => JSON.parse(line));
        expect(envelopes.some((env) => env.type === 'system' &&
            env.subtype === 'task_notification' &&
            env.data?.task_id === 'mon_1')).toBe(true);
        const cancelledNotificationIndex = envelopes.findIndex((env) => env.type === 'system' &&
            env.subtype === 'task_notification' &&
    ...
    it('does not let late monitor output keep one-shot runs alive', async () => {
        ...
        const firstNotificationXml = '<task-notification>\n' +
            '<task-id>mon_1</task-id>\n' +
            '<kind>monitor</kind>\n' +
            '<status>running</status>\n' +
            '<summary>Monitor emitted event #1.</summary>\n' +
            '<result>ready</result>\n' +
            '</task-notification>';
        const secondNotificationXml = '<task-notification>\n' +
            '<task-id>mon_1</task-id>\n' +
            '<kind>monitor</kind>\n' +
            '<status>running</status>\n' +
        ...
        async function* secondTurnStream() {
            yield { type: GeminiEventType.Content, value: 'Observed.' };
            monitorNotificationCallback?.('Monitor "logs" event #2: still running', secondNotificationXml, {
                monitorId: 'mon_1',
                toolUseId: 'tool_mon_1',
                status: 'running',
                eventCount: 2,
        ...
        await runNonInteractive(mockConfig, mockSettings, 'Watch the logs', 'prompt-monitor-cutover');
        expect(mockGeminiClient.sendMessageStream).toHaveBeenCalledTimes(2);
        const envelopes = writes
            .join('')
            .split('\n')
            .filter((line) => line.trim().length > 0)
            .map((line) => JSON.parse(line));
        const monitorNotifications = envelopes.filter((env) => env.type === 'system' &&
            env.subtype === 'task_notification' &&
            env.data?.task_id === 'mon_1');
        expect(monitorNotifications.filter((env) => env.data?.status === 'running')).toHaveLength(2);
        expect(monitorNotifications.some((env) => env.data?.status === 'cancelled')).toBe(true);
        expect(envelopes.at(-1)).toMatchObject({
            type: 'result',
            is_error: false,
        });
    ...
    it('streams late monitor output to the SDK before one-shot completion', async () => {
        ...
        const runPromise = runNonInteractive(mockConfig, mockSettings, 'Watch the logs', 'prompt-monitor-late-sdk');
        await vi.waitFor(() => {
            const envelopes = writes
                .join('')
                .split('\n')
    ...
    it('should not emit user message when userMessage option is provided (stream-json input binding)', async () => {
        ...
        expect(mockGeminiClient.sendMessageStream).toHaveBeenCalledWith([{ text: 'Message from stream-json input' }], expect.any(AbortSignal), 'prompt-envelope', { type: SendMessageType.UserQuery });
    });
    ...
    it('should handle userMessage with text content blocks in stream-json input mode', async () => {
        ...
        expect(mockGeminiClient.sendMessageStream).toHaveBeenCalledWith([{ text: 'Simple string content' }], expect.any(AbortSignal), 'prompt-string-content', { type: SendMessageType.UserQuery });
        ...
        expect(mockGeminiClient.sendMessageStream).toHaveBeenCalledWith([{ text: 'First part' }, { text: 'Second part' }], expect.any(AbortSignal), 'prompt-blocks-content', { type: SendMessageType.UserQuery });
    });
});
