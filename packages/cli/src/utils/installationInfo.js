 ...
 */
import { createDebugLogger, isGitRepository } from '@vivekmind/core';
    ...
    try {
        ...
        if (process.platform === 'darwin') {
            try {
                ...
                childProcess.execSync('brew list -1 | grep -q "^vivekmind$"', {
                    stdio: 'ignore',
                });
                return {
        ...
        if (realPath.includes('/.pnpm/global')) {
            const updateCommand = 'pnpm add -g vivekmind@latest';
            return {
        ...
        if (realPath.includes('/.yarn/global')) {
            const updateCommand = 'yarn global add vivekmind@latest';
            return {
        ...
        if (realPath.includes('/.bun/bin')) {
            const updateCommand = 'bun add -g vivekmind@latest';
            return {
        ...
        // Assume global npm
        const updateCommand = 'npm install -g vivekmind@latest';
        return {
