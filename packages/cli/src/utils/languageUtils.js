 ...
 * This file handles the creation and maintenance of ~/.vivekmind/output-language.md
...
const LLM_OUTPUT_LANGUAGE_MARKER_PREFIX = 'vivekmind:llm-output-language:';
...
/**
 * Returns the path to the LLM output language rule file (~/.vivekmind/output-language.md).
 */
function getOutputLanguageFilePath() {
    return path.join(Storage.getGlobalVivekMindDir(), LLM_OUTPUT_LANGUAGE_RULE_FILENAME);
}
/**
...
function parseOutputLanguageFromContent(content) {
    // Primary: machine-readable marker (e.g., <!-- vivekmind:llm-output-language: 中文 -->)
 ...
 */
export function writeOutputLanguageFile(language) {
    const filePath = getOutputLanguageFilePath();
 ...
 */
export function updateOutputLanguageFile(settingValue) {
