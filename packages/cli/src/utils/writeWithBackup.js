 ...
 * ```
 */
export async function writeWithBackup(targetPath, content, options = {}) {
 ...
 * @throws Error if any step of the write process fails
 */
export function writeWithBackupSync(targetPath, content, options = {}) {
    ...
    try {
        ...
        if (fs.existsSync(targetPath)) {
            ...
            try {
                ...
                throw new Error(`Failed to backup existing file: ${backupError instanceof Error ? backupError.message : String(backupError)}`);
            }
        }
        ...
        try {
            ...
            const writeFailureMessage = renameError instanceof Error
                ? renameError.message
                : String(renameError);
            if (restoreFailedMessage) {
                throw new Error(`Failed to write file: ${writeFailureMessage}. ` +
            ...
            throw new Error(`Failed to write file: ${writeFailureMessage}. No backup file was available for restoration.`);
        }
    }
    catch (error) {
...
//# sourceMappingURL=writeWithBackup.js.map