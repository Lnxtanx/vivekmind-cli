...
import { initStartupProfiler, profileCheckpoint, finalizeStartupProfile, getStartupReport, resetStartupProfiler, } from './startupProfiler.js';
vi.mock('node:fs');
describe('startupProfiler', () => {
    ...
    beforeEach(() => {
        ...
        vi.restoreAllMocks();
        saveEnv('VIVEKMIND_CODE_PROFILE_STARTUP', 'SANDBOX');
        delete process.env['VIVEKMIND_CODE_PROFILE_STARTUP'];
    ...
    function enableProfiler() {
        process.env['VIVEKMIND_CODE_PROFILE_STARTUP'] = '1';
    ...
    describe('when outside sandbox (SANDBOX not set)', () => {
        it('should not enable profiler even with VIVEKMIND_CODE_PROFILE_STARTUP=1', () => {
            process.env['VIVEKMIND_CODE_PROFILE_STARTUP'] = '1';
            delete process.env['SANDBOX'];
            initStartupProfiler();
            profileCheckpoint('test');
    ...
    describe('when enabled (VIVEKMIND_CODE_PROFILE_STARTUP=1 + SANDBOX)', () => {
        beforeEach(() => {
            enableProfiler();
        });
        ...
        it('should write JSON file on finalize and print path to stderr', () => {
            ...
            expect(fs.mkdirSync).toHaveBeenCalledWith(expect.stringContaining('startup-perf'), { recursive: true });
            ...
            expect(stderrSpy).toHaveBeenCalledWith(expect.stringContaining('Startup profile written to:'));
        });
