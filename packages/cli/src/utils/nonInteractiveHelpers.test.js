...
import { ToolErrorType, MCPServerStatus, getMCPServerStatus, OutputFormat, } from '@vivekmind/core';
import { normalizePartList, extractPartsFromUserMessage, extractUsageFromGeminiClient, computeUsageFromMetrics, buildSystemMessage, createToolProgressHandler, createAgentToolProgressHandler, functionResponsePartsToString, toolResultContent, } from './nonInteractiveHelpers.js';
// Mock dependencies
...
describe('buildSystemMessage', () => {
    ...
    it('should use unknown version when getCliVersion returns null', async () => {
        ...
        const result = await buildSystemMessage(config, 'test-session-id', 'auto');
        expect(result.vivekmind_version).toBe('unknown');
    });
