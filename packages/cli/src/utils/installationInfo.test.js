...
import { isGitRepository } from '@vivekmind/core';
...
describe('getInstallationInfo', () => {
    ...
    it('should detect Homebrew installation via execSync', () => {
        ...
        const info = getInstallationInfo(projectRoot, false);
        expect(mockedExecSync).toHaveBeenCalledWith('brew list -1 | grep -q "^vivekmind$"', { stdio: 'ignore' });
    ...
    it('should detect global pnpm installation', () => {
        const pnpmPath = `/Users/test/.pnpm/global/5/node_modules/.pnpm/some-hash/node_modules/vivekmind/dist/index.js`;
        ...
        expect(info.isGlobal).toBe(true);
        expect(info.updateCommand).toBe('pnpm add -g vivekmind@latest');
    ...
    it('should detect global yarn installation', () => {
        const yarnPath = `/Users/test/.yarn/global/node_modules/vivekmind/dist/index.js`;
        ...
        expect(info.isGlobal).toBe(true);
        expect(info.updateCommand).toBe('yarn global add vivekmind@latest');
        expect(info.updateMessage).toContain('Attempting to automatically update');
        // isAutoUpdateEnabled = false -> "Please run..."
        const infoDisabled = getInstallationInfo(projectRoot, false);
    ...
    it('should detect global bun installation', () => {
        ...
        expect(info.isGlobal).toBe(true);
        expect(info.updateCommand).toBe('bun add -g vivekmind@latest');
        expect(info.updateMessage).toContain('Attempting to automatically update');
        // isAutoUpdateEnabled = false -> "Please run..."
        const infoDisabled = getInstallationInfo(projectRoot, false);
    ...
    it('should default to global npm installation for unrecognized paths', () => {
        ...
        expect(info.packageManager).toBe(PackageManager.NPM);
        expect(info.isGlobal).toBe(true);
        expect(info.updateCommand).toBe('npm install -g vivekmind@latest');
        expect(info.updateMessage).toContain('Attempting to automatically update');
        // isAutoUpdateEnabled = false -> "Please run..."
        const infoDisabled = getInstallationInfo(projectRoot, false);
