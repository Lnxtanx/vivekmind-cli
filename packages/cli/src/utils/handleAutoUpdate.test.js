...
describe('handleAutoUpdate', () => {
    ...
    it('should show manual update message when enableAutoUpdate is false', () => {
        ...
        mockGetInstallationInfo.mockReturnValue({
            updateCommand: 'npm i -g vivekmind@latest',
            updateMessage: 'Please run npm i -g vivekmind@latest to update',
            isGlobal: true,
        ...
        expect(emitSpy).toHaveBeenCalledWith('update-received', {
            message: 'An update is available!\nPlease run npm i -g vivekmind@latest to update',
        });
    ...
    it('should emit "update-failed" when the update process fails', async () => {
        ...
        expect(emitSpy).toHaveBeenCalledWith('update-failed', {
            message: 'Automatic update failed. Please try updating manually. (command: npm i -g vivekmind@2.0.0, stderr: An error occurred)',
        });
    });
    ...
    it('should use the "@nightly" tag for nightly updates', async () => {
        ...
        handleAutoUpdate(mockUpdateInfo, mockSettings, '/root', mockSpawn);
        expect(mockSpawn).toHaveBeenCalledWith(expect.stringMatching(/^(bash|cmd\.exe)$/), expect.arrayContaining([
            expect.stringMatching(/^(-c|\/c)$/),
            'npm i -g vivekmind@nightly',
        ]), {
            stdio: 'pipe',
        });
    });
...
describe('setUpdateHandler', () => {
    ...
    it('should flush deferred notifications when flush is called', () => {
        const isIdleRef = { current: false };
        const { cleanup, flush } = setUpdateHandler(addItem, setUpdateInfo, isIdleRef);
        updateEventEmitter.emit('update-success', {
            message: 'Update successful!',
        });
        expect(addItem).not.toHaveBeenCalled();
    ...
    it('should flush multiple deferred notifications in order', () => {
        ...
        expect(addItem).toHaveBeenNthCalledWith(1, { type: MessageType.INFO, text: 'Info message' }, expect.any(Number));
        expect(addItem).toHaveBeenNthCalledWith(2, {
            type: MessageType.INFO,
            text: 'Update successful! The new version will be used on your next run.',
        }, expect.any(Number));
        cleanup();
    ...
    it('should deliver immediately after transitioning from busy to idle', () => {
        ...
        expect(addItem).toHaveBeenCalledWith({ type: MessageType.INFO, text: 'Immediate msg' }, expect.any(Number));
        // The earlier deferred message should still be in the queue
        flush();
        expect(addItem).toHaveBeenCalledTimes(2);
        expect(addItem).toHaveBeenNthCalledWith(2, { type: MessageType.INFO, text: 'Deferred msg' }, expect.any(Number));
        cleanup();
    });
});
