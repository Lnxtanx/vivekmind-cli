...
import { Logger, uiTelemetryService, createDebugLogger, } from '@vivekmind/core';
...
export const handleSlashCommand = async (rawQuery, abortController, config, settings) => {
    ...
    config.setModelInvocableCommandsProvider(() => commandService.getModelInvocableCommands().map((cmd) => ({
        name: cmd.name,
        description: typeof cmd.description === 'string' ? cmd.description : cmd.description,
    })));
 ...
 */
export const getAvailableCommands = async (config, abortSignal, mode = 'acp') => {
    try {
        const loaders = [
            new BuiltinCommandLoader(config),
            new BundledSkillLoader(config),
