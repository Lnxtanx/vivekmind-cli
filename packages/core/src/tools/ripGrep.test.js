...
describe('RipGrepTool', () => {
    ...
    describe('execute', () => {
        ...
        it('should pass .vivekmindignore to ripgrep when respected', async () => {
            await fs.writeFile(path.join(tempRootDir, '.vivekmindignore'), 'ignored.txt\n');
            runRipgrep.mockResolvedValue({
                stdout: '',
                truncated: false,
                error: undefined,
    ...
    describe('multi-directory workspace', () => {
        it('should search across all workspace directories when no path is specified', async () => {
            ...
            await fs.writeFile(path.join(secondDir, 'extra.txt'), 'hello from second dir');
            ...
            expect(runRipgrep).toHaveBeenCalledWith(expect.arrayContaining([tempRootDir, secondDir]), expect.anything());
        ...
        it('should load .vivekmindignore from each workspace directory', async () => {
            ...
            await fs.writeFile(path.join(tempRootDir, '.vivekmindignore'), 'other-ignored.txt\n');
            const multiDirConfig = {
                ...mockConfig,
                getWorkspaceContext: () => createMockWorkspaceContext(tempRootDir, [secondDir]),
            };
            ...
            const rgArgs = runRipgrep.mock.calls[0][0];
            const ignoreFileArgs = rgArgs.filter((a, i) => i > 0 && rgArgs[i - 1] === '--ignore-file');
        ...
        it('should deduplicate matches from overlapping workspace directories', async () => {
            ...
            const multiDirConfig = {
                ...mockConfig,
                getWorkspaceContext: () => createMockWorkspaceContext(tempRootDir, [subDir]),
            };
            const multiDirGrepTool = new RipGrepTool(multiDirConfig);
    ...
    describe('getDefaultPermission', () => {
        ...
        it('should return allow for subdirectories within workspace', async () => {
            const params = { pattern: 'hello', path: 'sub' };
            const invocation = grepTool.build(params);
            const permission = await invocation.getDefaultPermission();
            expect(permission).toBe('allow');
        });
        it('should return ask for paths outside workspace', async () => {
            const params = { pattern: 'hello', path: '/tmp' };
            const invocation = grepTool.build(params);
            const permission = await invocation.getDefaultPermission();
            expect(permission).toBe('ask');
        });
