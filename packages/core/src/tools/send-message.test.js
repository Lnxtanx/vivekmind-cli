...
describe('SendMessageTool', () => {
    ...
    it('queues a message for a running task', async () => {
        ...
        const result = await tool.validateBuildAndExecute({ task_id: 'agent-1', message: 'do more work' }, new AbortController().signal);
    ...
    it('queues multiple messages in order', async () => {
        ...
        await tool.validateBuildAndExecute({ task_id: 'agent-1', message: 'first' }, new AbortController().signal);
        await tool.validateBuildAndExecute({ task_id: 'agent-1', message: 'second' }, new AbortController().signal);
    ...
    it('returns error for non-existent task', async () => {
        const result = await tool.validateBuildAndExecute({ task_id: 'nope', message: 'hello' }, new AbortController().signal);
    ...
    it('returns error for non-running task', async () => {
        ...
        const result = await tool.validateBuildAndExecute({ task_id: 'agent-1', message: 'hello' }, new AbortController().signal);
    ...
    it('rejects messages for a cancelled task', async () => {
        ...
        const result = await tool.validateBuildAndExecute({ task_id: 'agent-1', message: 'too late' }, new AbortController().signal);
        expect(result.error?.type).toBe(ToolErrorType.SEND_MESSAGE_NOT_RUNNING);
        expect(registry.get('agent-1').pendingMessages).toEqual([]);
    });
    it('resumes a paused task and injects the message as continuation input', async () => {
        ...
        const result = await tool.validateBuildAndExecute({ task_id: 'agent-1', message: 'pick up from the TODO list' }, new AbortController().signal);
        expect(resumeBackgroundAgent).toHaveBeenCalledWith('agent-1', 'pick up from the TODO list');
        expect(result.error).toBeUndefined();
    ...
    it('includes task description in success display', async () => {
        ...
        const result = await tool.validateBuildAndExecute({ task_id: 'agent-1', message: 'focus on login' }, new AbortController().signal);
        expect(result.returnDisplay).toContain('Search for auth code');
    });
});
//# sourceMappingURL=send-message.test.js.map