...
class CronCreateInvocation extends BaseToolInvocation {
    ...
    async execute() {
        ...
        try {
            ...
            const job = scheduler.create(this.params.cron, this.params.prompt, recurring);
            ...
            if (recurring) {
                llmContent =
                    `Scheduled recurring job ${job.id} (${job.cronExpr}). ` +
                        'Session-only (not written to disk, dies when VivekMind exits). ' +
...
export class CronCreateTool extends BaseDeclarativeTool {
    ...
    constructor(config) {
        super(CronCreateTool.Name, ToolDisplayNames.CRON_CREATE, 'Schedule a prompt to be enqueued at a future time. Use for both recurring schedules and one-shot reminders.\n\n' +
            ...
            '## Session-only\n\n' +
            'Jobs live only in this VivekMind session — nothing is written to disk, and the job is gone when VivekMind exits.\n\n' +
            '## Runtime behavior\n\n' +
