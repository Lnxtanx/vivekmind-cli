...
export class SkillTool extends BaseDeclarativeTool {
    ...
    async refreshSkills() {
        try {
            ...
            this.availableSkills = allSkills.filter((s) => !s.disableModelInvocation && this.skillManager.isSkillActive(s));
            ...
            this.pendingConditionalSkillNames = new Set(allSkills
            ...
            const fileBasedSkillNames = new Set(allSkills.filter((s) => !s.disableModelInvocation).map((s) => s.name));
            this.modelInvocableCommands = allCommands.filter((cmd) => !fileBasedSkillNames.has(cmd.name));
...
${skillDescriptions}
    ...
    createInvocation(params) {
        return new SkillToolInvocation(this.config, this.skillManager, params, (name) => this.loadedSkillNames.add(name), this.config.getModelInvocableCommandsExecutor());
    }
...
class SkillToolInvocation extends BaseToolInvocation {
    ...
    onSkillLoaded;
    commandExecutor;
    constructor(config, skillManager, params, onSkillLoaded, commandExecutor = null) {
        super(params);
        this.config = config;
        this.skillManager = skillManager;
