...
import { resolveAndValidatePath, isSubpath, unescapePath, } from '../utils/paths.js';
...
class GlobToolInvocation extends BaseToolInvocation {
     ...
     */
    async globInDirectory(searchDir, pattern, signal) {
        ...
        // FileDiscoveryService uses for .gitignore / .vivekmindignore evaluation).
        ...
        const normalizePathForComparison = (p) => process.platform === 'win32' || process.platform === 'darwin'
            ? p.toLowerCase()
            : p;
        const filteredAbsolutePaths = new Set(filteredPaths.map((p) => normalizePathForComparison(path.resolve(projectRoot, p))));
    ...
    getFileFilteringOptions() {
        ...
        return {
            ...
            respectVivekMindIgnore: options?.respectVivekMindIgnore ??
