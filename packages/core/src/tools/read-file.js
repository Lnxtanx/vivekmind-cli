...
class ReadFileToolInvocation extends BaseToolInvocation {
    ...
    async execute() {
        ...
        const result = await processSingleFileContent(this.params.file_path, this.config, this.params.offset, this.params.limit, this.params.pages);
        if (result.error) {
            return {
        ...
        if (cacheEnabled && stats) {
            const cacheable = typeof result.llmContent === 'string' &&
...
export class ReadFileTool extends BaseDeclarativeTool {
    ...
    constructor(config) {
        super(ReadFileTool.Name, ToolDisplayNames.READ_FILE, `Reads and returns the content of a specified file. If the file is large, the content will be truncated. The tool's response will clearly indicate if truncation has occurred and will provide details on how to read more of the file using the 'offset' and 'limit' parameters. Handles text, images (PNG, JPG, GIF, WEBP, SVG, BMP), PDF files, and Jupyter notebooks (.ipynb). For text files, it can read specific line ranges. For PDF files, use the 'pages' parameter to extract specific page ranges as text (e.g. '1-5'). Max 20 pages per request. This tool can read Jupyter notebooks (.ipynb) and returns structured cell content with outputs.`, Kind.Read, {
            properties: {
                file_path: {
