...
describe('GlobTool', () => {
    ...
    const mockTruncationGlobResults = (prefix, count) => {
        const baseMtimeMs = Date.now();
        const entries = Array.from({ length: count }, (_, index) => {
            const fileNumber = index + 1;
            return {
                fullpath: () => path.join(tempRootDir, `${prefix}${fileNumber}.trunctest`),
    ...
    describe('execute', () => {
        ...
        it('should allow path outside workspace (external path support)', async () => {
            const params = { pattern: '*.txt', path: '/tmp' };
            const invocation = globTool.build(params);
            // External path is now allowed - it should not return a workspace error
            const result = await invocation.execute(abortSignal);
            expect(result.returnDisplay).not.toContain('Path is not within workspace');
        });
    ...
    describe('validateToolParams', () => {
        ...
        it.skipIf(process.platform === 'win32')('should unescape shell-escaped path', async () => {
    ...
    describe('multi-directory workspace', () => {
        it('should search across all workspace directories when no path is specified', async () => {
            ...
            const secondDir = await fs.mkdtemp(path.join(os.tmpdir(), 'glob-tool-second-'));
        ...
        it('should deduplicate entries across overlapping directories', async () => {
            ...
            const multiDirConfig = {
                ...mockConfig,
                getWorkspaceContext: () => createMockWorkspaceContext(tempRootDir, [tempRootDir]),
            };
            const multiDirGlobTool = new GlobTool(multiDirConfig);
            const params = { pattern: '*.txt' };
            const invocation = multiDirGlobTool.build(params);
        ...
        it('should search only the specified path when path is provided (ignoring multi-dir)', async () => {
            ...
            const multiDirGlobTool = new GlobTool(multiDirConfig);
            const params = { pattern: '*.txt', path: 'sub' };
            const invocation = multiDirGlobTool.build(params);
            const result = await invocation.execute(abortSignal);
    ...
    describe('ignore file handling', () => {
        ...
        it('should respect .vivekmindignore files by default', async () => {
            await fs.writeFile(path.join(tempRootDir, '.vivekmindignore'), '*.vivekmindignored.txt');
            await fs.writeFile(path.join(tempRootDir, 'a.vivekmindignored.txt'), 'ignored content');
            await fs.writeFile(path.join(tempRootDir, 'b.notignored.txt'), 'not ignored content');
            // Recreate the tool to pick up the new .vivekmindignore file
            globTool = new GlobTool(mockConfig);
            const params = { pattern: '*.txt' };
            const invocation = globTool.build(params);
            const result = await invocation.execute(abortSignal);
            expect(result.llmContent).toContain('Found 3 file(s)'); // fileA.txt, FileB.TXT, b.notignored.txt
            expect(result.llmContent).not.toContain('a.vivekmindignored.txt');
        });
        it('should respect .gitignore when searching a subdirectory (path option)', async () => {
            ...
            await fs.writeFile(path.join(tempRootDir, 'sub', 'hidden.secret'), 'should be ignored');
            const subDirTool = new GlobTool(mockConfig);
            const params = { pattern: '*', path: 'sub' };
        ...
        it('should respect .vivekmindignore when searching a subdirectory (path option)', async () => {
            await fs.writeFile(path.join(tempRootDir, '.vivekmindignore'), '*.secret');
            await fs.writeFile(path.join(tempRootDir, 'sub', 'visible.txt'), 'ok');
            await fs.writeFile(path.join(tempRootDir, 'sub', 'hidden.secret'), 'should be ignored');
            // Recreate to pick up .vivekmindignore
            const subDirTool = new GlobTool(mockConfig);
            const params = { pattern: '*', path: 'sub' };
            const invocation = subDirTool.build(params);
            const result = await invocation.execute(abortSignal);
