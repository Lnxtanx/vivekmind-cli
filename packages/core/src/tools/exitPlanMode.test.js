...
describe('ExitPlanModeTool', () => {
    ...
    describe('tool execution', () => {
        ...
        it('should set DEFAULT mode on ProceedOnce regardless of pre-plan mode', async () => {
            ...
            mockConfig.getPrePlanMode.mockReturnValue(ApprovalMode.AUTO_EDIT);
            const params = { plan: 'Restore test' };
            const signal = new AbortController().signal;
            const invocation = tool.build(params);
            const confirmation = await invocation.getConfirmationDetails(signal);
            if (confirmation) {
        ...
        it('should restore pre-plan mode on RestorePrevious', async () => {
            mockConfig.getPrePlanMode.mockReturnValue(ApprovalMode.YOLO);
            const params = { plan: 'Restore previous test' };
            const signal = new AbortController().signal;
            const invocation = tool.build(params);
            const confirmation = await invocation.getConfirmationDetails(signal);
            if (confirmation) {
        ...
        it('should fall back to DEFAULT on RestorePrevious when no prePlanMode recorded', async () => {
            ...
            mockConfig.getPrePlanMode.mockReturnValue(ApprovalMode.DEFAULT);
            const params = { plan: 'Fallback test' };
            const signal = new AbortController().signal;
            const invocation = tool.build(params);
            const confirmation = await invocation.getConfirmationDetails(signal);
            if (confirmation) {
    ...
    describe('YOLO mode', () => {
        it('should exit plan mode without onConfirm being called when approval mode is YOLO', async () => {
            ...
            const params = { plan: 'YOLO test plan' };
            const signal = new AbortController().signal;
            const invocation = tool.build(params);
