...
describe('ShellTool', () => {
    ...
    describe('isCommandAllowed', () => {
        it('should allow a command if no restrictions are provided', async () => {
            ...
            mockConfig.getPermissionsDeny.mockReturnValue(undefined);
        ...
        it('should block a command with command substitution using $()', async () => {
            expect((await isCommandAllowed('echo $(rm -rf /)', mockConfig)).allowed).toBe(false);
        });
    });
    describe('build', () => {
        ...
        it('should throw an error for a directory within the user skills directory', async () => {
            expect(() => shellTool.build({
                ...
                is_background: false,
            })).toThrow('Explicitly running shell commands from within the user skills directory is not allowed. Please use absolute paths for command parameter instead.');
        });
    ...
    describe('execute', () => {
        ...
        it('runs background commands as managed pool entries (no & / pgrep wrap)', async () => {
            ...
            expect(mockShellExecutionService).toHaveBeenCalledWith('npm start', '/test/dir', expect.any(Function), expect.any(AbortSignal), false, {}, { streamStdout: true });
            // Entry registered with the spawn pid.
            expect(registry.register).toHaveBeenCalledTimes(1);
            const entry = registry.register.mock.calls[0][0];
        ...
        it('settles a background entry as completed when the process exits cleanly', async () => {
            ...
            expect(registry.complete).toHaveBeenCalledWith(entry.shellId, 0, expect.any(Number));
        ...
        it('settles a background entry as failed when ShellExecutionService reports error', async () => {
            ...
            await new Promise((r) => setImmediate(r));
            expect(registry.fail).toHaveBeenCalledWith(entry.shellId, 'spawn ENOENT', expect.any(Number));
        ...
        it('settles a background entry as failed on non-zero exit code (no error object)', async () => {
            ...
            await new Promise((r) => setImmediate(r));
            expect(registry.fail).toHaveBeenCalledWith(entry.shellId, expect.stringContaining('exited with code 1'), expect.any(Number));
            expect(registry.complete).not.toHaveBeenCalled();
        });
        it('rejects a bare trailing & in managed background mode', async () => {
            expect(() => shellTool.build({
                ...
                is_background: true,
            })).toThrow('Background shell commands must not end with a bare "&". Remove the trailing "&" and rely on is_background: true instead.');
        ...
        it('preserves a trailing && (logical AND would be syntactically broken otherwise)', async () => {
            ...
            await invocation.execute(mockAbortSignal);
            expect(mockShellExecutionService).toHaveBeenCalledWith('npm run dev &&', expect.any(String), expect.any(Function), expect.any(AbortSignal), false, {}, { streamStdout: true });
        });
        it('preserves an escaped trailing \\& (literal &)', async () => {
            ...
            await invocation.execute(mockAbortSignal);
            expect(mockShellExecutionService).toHaveBeenCalledWith('echo foo \\&', expect.any(String), expect.any(Function), expect.any(AbortSignal), false, {}, { streamStdout: true });
        });
        it('preserves quoted trailing ampersands', async () => {
            ...
            await invocation.execute(mockAbortSignal);
            expect(mockShellExecutionService).toHaveBeenCalledWith(`printf '&'`, expect.any(String), expect.any(Function), expect.any(AbortSignal), false, {}, { streamStdout: true });
        });
        it('preserves ampersands inside double-quoted script arguments', async () => {
            ...
            await invocation.execute(mockAbortSignal);
            expect(mockShellExecutionService).toHaveBeenCalledWith(`node -e "console.log('&')"`, expect.any(String), expect.any(Function), expect.any(AbortSignal), false, {}, { streamStdout: true });
        });
        it('preserves ampersands inside command substitutions', async () => {
            ...
            await invocation.execute(mockAbortSignal);
            expect(mockShellExecutionService).toHaveBeenCalledWith(`echo $(printf '&')`, expect.any(String), expect.any(Function), expect.any(AbortSignal), false, {}, { streamStdout: true });
        });
        ...
        it('should not add ampersand when is_background is false', async () => {
            ...
            expect(mockShellExecutionService).toHaveBeenCalledWith('npm test', expect.any(String), expect.any(Function), expect.any(AbortSignal), false, {});
        });
        it('preserves shell wrapper environment and flags during foreground execution', async () => {
            ...
            await promise;
            expect(mockShellExecutionService).toHaveBeenCalledWith(command, expect.any(String), expect.any(Function), expect.any(AbortSignal), false, {});
        });
        it('preserves shell wrapper environment and flags during background execution', async () => {
            ...
            await invocation.execute(mockAbortSignal);
            expect(mockShellExecutionService).toHaveBeenCalledWith(command, expect.any(String), expect.any(Function), expect.any(AbortSignal), false, {}, { streamStdout: true });
        });
        it('should use the provided directory as cwd', async () => {
            ...
            await promise;
            // Foreground commands should not be wrapped with pgrep
            expect(mockShellExecutionService).toHaveBeenCalledWith('ls', '/test/dir/subdir', expect.any(Function), expect.any(AbortSignal), false, {});
        });
        it('should not wrap command on windows', async () => {
            ...
            await promise;
            expect(mockShellExecutionService).toHaveBeenCalledWith('dir', '/test/dir', expect.any(Function), expect.any(AbortSignal), false, {});
        });
        ...
        describe('long-running foreground hint', () => {
            ...
            it('appends the hint AFTER truncation (so it survives `truncateToolOutput`)', async () => {
                ...
                try {
                    ...
                    expect(content).toContain('Tool output was too large and has been truncated.');
                    ...
                    const truncIdx = content.indexOf('Tool output was too large and has been truncated.');
            ...
            it('hint survives the error path (appended to error.message)', async () => {
                ...
                expect(result.error?.message).toContain('PTY initialization failed after 75s');
                expect(result.error?.message).toContain('foreground command ran for 75s');
                ...
                expect(result.error?.message).toMatch(/PTY initialization failed after 75s\n\n---\n/);
            });
        ...
        describe('addCoAuthorToGitCommit', () => {
            ...
            it('should not modify non-git commands', async () => {
                ...
                await promise;
                expect(mockShellExecutionService).toHaveBeenCalledWith(expect.stringContaining('npm install'), expect.any(String), expect.any(Function), expect.any(AbortSignal), false, {});
            });
            it('should not modify git commands without -m flag', async () => {
                ...
                await promise;
                expect(mockShellExecutionService).toHaveBeenCalledWith(expect.stringContaining('git commit'), expect.any(String), expect.any(Function), expect.any(AbortSignal), false, {});
            });
            ...
            it('should not add co-author when disabled in config', async () => {
                ...
                await promise;
                expect(mockShellExecutionService).toHaveBeenCalledWith(expect.stringContaining('git commit -m "Initial commit"'), expect.any(String), expect.any(Function), expect.any(AbortSignal), false, {});
            });
            it('should use custom name and email from config', async () => {
                ...
                await promise;
                expect(mockShellExecutionService).toHaveBeenCalledWith(expect.stringContaining('Co-authored-by: Custom Bot <custom@example.com>'), expect.any(String), expect.any(Function), expect.any(AbortSignal), false, {});
            });
 ...
 spanning multiple lines"`;
    ...
    describe('getDefaultPermission and getConfirmationDetails', () => {
        ...
        it('should exclude read-only sub-commands from confirmation details in compound commands', async () => {
            ...
            expect(details.permissionRules).not.toContainEqual(expect.stringContaining('cd'));
            expect(details.permissionRules).toContainEqual(expect.stringContaining('npm'));
        });
        ...
        it('should exclude already-allowed sub-commands from confirmation details in compound commands', async () => {
            ...
            pm.initialize();
            mockConfig.getPermissionManager.mockReturnValue(pm);
            const invocation = shellTool.build({
        ...
        it('should pass the invocation directory to permission-manager command checks', async () => {
            ...
            expect(pm.isCommandAllowed).toHaveBeenCalledWith('git commit -m "msg"', '/test/dir/subdir');
        });
    ...
    describe('timeout parameter', () => {
        ...
        it('should create combined signal with timeout for foreground execution', async () => {
            ...
            expect(mockShellExecutionService).toHaveBeenCalledWith(expect.any(String), expect.any(String), expect.any(Function), expect.any(AbortSignal), false, {});
        ...
        it('should handle timeout vs user cancellation correctly', async () => {
            ...
            expect(result.llmContent).toContain('Below is the output before it timed out');
        });
...
describe('detectBlockedSleepPattern', () => {
    it('blocks standalone sleep >= 2s', () => {
        ...
        expect(detectBlockedSleepPattern('sleep 2000ms')).toBe('standalone sleep 2000ms');
        expect(detectBlockedSleepPattern('sleep 3m')).toBe('standalone sleep 3m');
    });
    it('blocks sleep followed by another command', () => {
        expect(detectBlockedSleepPattern('sleep 5 && curl http://localhost')).toBe('sleep 5 followed by: curl http://localhost');
        expect(detectBlockedSleepPattern('sleep 3; echo done')).toBe('sleep 3 followed by: echo done');
        expect(detectBlockedSleepPattern('sleep 2.5 || echo done')).toBe('sleep 2.5 followed by: echo done');
        expect(detectBlockedSleepPattern('sleep 2s\necho done')).toBe('sleep 2s followed by: echo done');
    });
    it('allows sleep < 2s', () => {
    ...
    it('allows sleep in pipelines', () => {
        ...
        expect(detectBlockedSleepPattern('sleep 10 | while read line; do echo $line; done')).toBeNull();
    });
    ...
    it('blocks sleep followed by a top-level shell comment', () => {
        ...
        expect(detectBlockedSleepPattern('sleep 5 # wait')).toBe('standalone sleep 5');
        expect(detectBlockedSleepPattern('sleep 5  #wait')).toBe('standalone sleep 5');
        expect(detectBlockedSleepPattern('sleep 2s   # comment')).toBe('standalone sleep 2s');
        expect(detectBlockedSleepPattern('sleep 5 && echo ok # trailing')).toBe('sleep 5 followed by: echo ok');
    });
    it('does not treat in-quoted `#` as a comment', () => {
        ...
        expect(detectBlockedSleepPattern("sleep 5 'arg # not a comment'")).toBeNull();
    });
    it('blocks wrapped foreground sleep when paired with stripShellWrapper', () => {
        ...
        // inside a `-c` script.
        expect(detectBlockedSleepPattern(stripShellWrapper("bash -c 'sleep 5'"))).toBe('standalone sleep 5');
        expect(detectBlockedSleepPattern(stripShellWrapper("sh -c 'sleep 10'"))).toBe('standalone sleep 10');
        expect(detectBlockedSleepPattern(stripShellWrapper("zsh -c 'sleep 2s'"))).toBe('standalone sleep 2s');
        expect(detectBlockedSleepPattern(stripShellWrapper("bash -c 'sleep 5 && curl http://localhost'"))).toBe('sleep 5 followed by: curl http://localhost');
        // A wrapped sleep < 2s is still allowed.
        expect(detectBlockedSleepPattern(stripShellWrapper("bash -c 'sleep 1'"))).toBeNull();
    });
});
