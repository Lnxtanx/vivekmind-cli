...
class LSToolInvocation extends BaseToolInvocation {
    ...
    async execute(_signal) {
        try {
            ...
            const { filteredPaths, gitIgnoredCount, vivekMindIgnoredCount } = fileDiscovery.filterFilesWithReport(relativePaths, {
                respectGitIgnore: this.params.file_filtering_options?.respect_git_ignore ??
                    this.config.getFileFilteringOptions().respectGitIgnore ??
                    DEFAULT_FILE_FILTERING_OPTIONS.respectGitIgnore,
                respectVivekMindIgnore: this.params.file_filtering_options?.respect_vivekmind_ignore ??
        ...
        super(LSTool.Name, ToolDisplayNames.LS, 'Lists the names of files and subdirectories directly within a specified directory path. Can optionally ignore entries matching provided glob patterns.', Kind.Search, {
            properties: {
                ...
                file_filtering_options: {
                    description: 'Optional: Whether to respect ignore patterns from .gitignore or .vivekmindignore',
                    type: 'object',
                    properties: {
                        respect_git_ignore: {
                        ...
                        respect_vivekmind_ignore: {
                            description: 'Optional: Whether to respect .vivekmindignore patterns when listing files. Defaults to true.',
                            type: 'boolean',
                        },
                    },
                },
