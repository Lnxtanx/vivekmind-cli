...
const rootDir = path.resolve(os.tmpdir(), 'vivekmind-test-root');
// --- MOCKS ---
...
describe('WriteFileTool', () => {
    ...
    describe('build', () => {
        ...
        it.skipIf(process.platform === 'win32')('should unescape shell-escaped spaces in file_path', () => {
            ...
            const invocation = tool.build(params);
            expect(invocation).toBeDefined();
            expect(invocation.params.file_path).toBe(path.join(rootDir, 'my file.txt'));
        });
    });
    describe('shouldConfirmExecute', () => {
        ...
        it('should throw if _getCorrectedFileContent returns an error', async () => {
            ...
            const invocation = tool.build(params);
            await expect(invocation.getConfirmationDetails(abortSignal)).rejects.toThrow('Error reading existing file for confirmation');
    ...
    describe('execute', () => {
        ...
        it.skipIf(process.platform === 'win32')('should write to a file with spaces in its name when given an escaped path', async () => {
            // On Windows, unescapePath is a no-op and backslashes are path
