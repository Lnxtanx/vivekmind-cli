...
describe('GrepTool', () => {
    ...
    describe('multi-directory workspace', () => {
        ...
        it('should convert relative paths to absolute when searching multiple directories', async () => {
            const secondDir = await fs.mkdtemp(path.join(os.tmpdir(), 'grep-tool-second-'));
            await fs.writeFile(path.join(secondDir, 'extra.txt'), 'world content in second dir');
            const multiDirConfig = {
                getTargetDir: () => tempRootDir,
                getWorkspaceContext: () => createMockWorkspaceContext(tempRootDir, [secondDir]),
                getFileExclusions: () => ({
            ...
            expect(result.llmContent).toContain(`File: ${path.resolve(secondDir, 'extra.txt')}`);
            // File paths from the first directory should also be absolute
            expect(result.llmContent).toContain(`File: ${path.resolve(tempRootDir, 'fileA.txt')}`);
            await fs.rm(secondDir, { recursive: true, force: true });
        });
