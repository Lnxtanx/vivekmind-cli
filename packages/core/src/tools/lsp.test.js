 ...
 */
const resolvePath = (...segments) => path.join(workspaceRoot, ...segments);
/**
 * Helper to convert file path to URI.
 */
const toUri = (filePath) => pathToFileURL(filePath).toString();
/**
 * Helper to create a mock LspLocation.
 */
const createLocation = (filePath, line, character) => ({
    uri: toUri(filePath),
    range: {
 ...
 */
const createMockConfig = (client, enabled = true) => ({
    getLspClient: () => client,
 ...
 */
const createTool = (client, enabled = true) => new LspTool(createMockConfig(client, enabled));
describe('LspTool', () => {
    describe('validateToolParams', () => {
        ...
        describe('location-based operations', () => {
            ...
            it.each(locationOperations)('requires filePath for %s operation', (operation) => {
                const result = tool.validateToolParams({
                    operation,
                });
                expect(result).toBe(`filePath is required for ${operation}.`);
            });
            it.each(locationOperations)('requires line for %s operation', (operation) => {
                const result = tool.validateToolParams({
                    operation,
                    filePath: 'src/app.ts',
                });
            ...
            it.each(locationOperations)('passes validation with valid params for %s', (operation) => {
                const result = tool.validateToolParams({
                    operation,
                    filePath: 'src/app.ts',
                    line: 10,
        ...
        describe('edge case validation', () => {
            ...
            it.skipIf(process.platform === 'win32')('should unescape shell-escaped filePath', () => {
                const params = {
                    operation: 'goToDefinition',
    ...
    describe('execute', () => {
        describe('LSP disabled or unavailable', () => {
            ...
            it('returns unavailable message when no LSP client', async () => {
                ...
                expect(result.llmContent).toContain('LSP go-to-definition is unavailable');
            });
        });
        describe('goToDefinition operation', () => {
            it('dispatches to definitions and formats results', async () => {
                ...
                client.definitions.mockResolvedValue([definition]);
                const invocation = tool.build({
                    operation: 'goToDefinition',
                    filePath: 'src/app.ts',
                    line: 5,
                ...
                const result = await invocation.execute(abortSignal);
                expect(client.definitions).toHaveBeenCalledWith(expect.objectContaining({
                    uri: toUri(filePath),
        ...
        describe('findReferences operation', () => {
            it('dispatches to references and formats results', async () => {
                ...
                client.references.mockResolvedValue(refs);
                const invocation = tool.build({
                ...
                expect(client.references).toHaveBeenCalledWith(expect.objectContaining({ uri: toUri(filePath) }), undefined, true, 50);
        ...
        describe('hover operation', () => {
            it('dispatches to hover and formats results', async () => {
                ...
                client.hover.mockResolvedValue(hoverResult);
                const invocation = tool.build({
                    operation: 'hover',
                    filePath: 'src/app.ts',
                    line: 10,
        ...
        describe('documentSymbol operation', () => {
            it('dispatches to documentSymbols and formats results', async () => {
                ...
                client.documentSymbols.mockResolvedValue(symbols);
                const invocation = tool.build({
                    operation: 'documentSymbol',
                    filePath: 'src/app.ts',
                });
                ...
                expect(client.documentSymbols).toHaveBeenCalledWith(toUri(filePath), undefined, 50);
        ...
        describe('workspaceSymbol operation', () => {
            it('dispatches to workspaceSymbols and formats results', async () => {
                ...
                client.workspaceSymbols.mockResolvedValue(symbols);
        ...
        describe('goToImplementation operation', () => {
            it('dispatches to implementations and formats results', async () => {
                ...
                client.implementations.mockResolvedValue([impl]);
                const invocation = tool.build({
        ...
        describe('prepareCallHierarchy operation', () => {
            it('dispatches to prepareCallHierarchy and formats results with JSON', async () => {
                ...
                client.prepareCallHierarchy.mockResolvedValue([item]);
                const invocation = tool.build({
        ...
        describe('incomingCalls operation', () => {
            it('dispatches to incomingCalls and formats results', async () => {
                ...
                client.incomingCalls.mockResolvedValue([incomingCall]);
                const invocation = tool.build({
                    operation: 'incomingCalls',
                    callHierarchyItem: targetItem,
                });
                const result = await invocation.execute(abortSignal);
                expect(client.incomingCalls).toHaveBeenCalledWith(targetItem, 'tsserver', 20);
        ...
        describe('outgoingCalls operation', () => {
            it('dispatches to outgoingCalls and formats results', async () => {
                ...
                client.outgoingCalls.mockResolvedValue([outgoingCall]);
                const invocation = tool.build({
                    operation: 'outgoingCalls',
        ...
        describe('error handling', () => {
            it('handles LSP client errors gracefully', async () => {
                const client = createMockClient();
                const tool = createTool(client);
                client.definitions.mockRejectedValue(new Error('Connection refused'));
                const invocation = tool.build({
                    operation: 'goToDefinition',
                    filePath: 'src/app.ts',
                    line: 5,
            ...
            it('handles hover operation errors', async () => {
                const client = createMockClient();
                const tool = createTool(client);
                client.hover.mockRejectedValue(new Error('Server timeout'));
                const invocation = tool.build({
                    operation: 'hover',
                    filePath: 'src/app.ts',
                    line: 5,
            ...
            it('handles call hierarchy errors', async () => {
                const client = createMockClient();
                const tool = createTool(client);
                client.prepareCallHierarchy.mockRejectedValue(new Error('Not supported'));
                const invocation = tool.build({
                    operation: 'prepareCallHierarchy',
                    filePath: 'src/app.ts',
                    line: 5,
        ...
        describe('workspaceSymbol with references', () => {
            it('fetches references for top match when available', async () => {
                ...
                client.workspaceSymbols.mockResolvedValue(symbols);
                client.references.mockResolvedValue(references);
                const invocation = tool.build({
                    operation: 'workspaceSymbol',
                    query: 'TopWidget',
                });
                ...
                expect(client.references).toHaveBeenCalledWith(symbols[0].location, 'tsserver', false, expect.any(Number));
            ...
            it('handles reference lookup failure gracefully', async () => {
                ...
                client.workspaceSymbols.mockResolvedValue(symbols);
                client.references.mockRejectedValue(new Error('References not supported'));
                const invocation = tool.build({
                    operation: 'workspaceSymbol',
                    query: 'Widget',
                });
        ...
        describe('serverName and limit parameter passing', () => {
            it('passes serverName to client methods', async () => {
                ...
                expect(client.definitions).toHaveBeenCalledWith(expect.anything(), 'pylsp', expect.any(Number));
            });
            it('passes custom limit to client methods', async () => {
                ...
                await invocation.execute(abortSignal);
                expect(client.definitions).toHaveBeenCalledWith(expect.anything(), undefined, 5);
            });
        });
    });
    describe('schema compatibility with Claude Code', () => {
        ...
        describe('schema definitions deep validation', () => {
            ...
            it('has LspRange definition with correct structure', () => {
                ...
                expect(rangeDef?.properties?.start?.$ref).toBe('#/definitions/LspPosition');
                expect(rangeDef?.properties?.end?.$ref).toBe('#/definitions/LspPosition');
            ...
            it('callHierarchyItem uses $ref for range fields', () => {
                ...
                expect(schema.properties?.callHierarchyItem?.$ref).toBe('#/definitions/LspCallHierarchyItem');
                // range and selectionRange should use LspRange $ref
                const itemDef = schema.definitions?.LspCallHierarchyItem;
                expect(itemDef?.properties?.range?.$ref).toBe('#/definitions/LspRange');
                expect(itemDef?.properties?.selectionRange?.$ref).toBe('#/definitions/LspRange');
            });
