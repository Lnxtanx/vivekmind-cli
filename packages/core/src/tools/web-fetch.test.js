...
describe('WebFetchTool', () => {
    ...
    describe('execute', () => {
        it('should throw validation error when url parameter is missing', async () => {
            ...
            expect(() => tool.build(params)).toThrow("params must have required property 'url'");
        });
    ...
    describe('format parameter', () => {
        it('should default to auto format when not specified', async () => {
            ...
            expect(fetchSpy).toHaveBeenCalledWith('https://example.com', expect.any(Number), { Accept: 'text/markdown, text/html, text/plain' });
        });
        it('should request only markdown when format is markdown', async () => {
            ...
            const invocation = tool.build(params);
            await invocation.execute(new AbortController().signal);
            expect(fetchSpy).toHaveBeenCalledWith('https://example.com', expect.any(Number), { Accept: 'text/markdown' });
        });
        it('should request only HTML when format is html', async () => {
            ...
            const invocation = tool.build(params);
            await invocation.execute(new AbortController().signal);
            expect(fetchSpy).toHaveBeenCalledWith('https://example.com', expect.any(Number), { Accept: 'text/html' });
        });
        it('should request plain text when format is text', async () => {
            ...
            const invocation = tool.build(params);
            await invocation.execute(new AbortController().signal);
            expect(fetchSpy).toHaveBeenCalledWith('https://example.com', expect.any(Number), { Accept: 'text/plain' });
        });
    ...
    describe('getConfirmationDetails', () => {
        it('should return confirmation details with the correct prompt and urls', async () => {
            ...
            const confirmationDetails = await invocation.getConfirmationDetails(new AbortController().signal);
            expect(confirmationDetails).toEqual({
                type: 'info',
                title: 'Confirm Web Fetch',
                prompt: 'Fetch content from https://example.com and process with: summarize this page',
        ...
        it('should return github urls as-is in confirmation details', async () => {
            ...
            expect(confirmationDetails).toEqual({
                type: 'info',
                title: 'Confirm Web Fetch',
                prompt: 'Fetch content from https://github.com/google/gemini-react/blob/main/README.md and process with: summarize the README',
