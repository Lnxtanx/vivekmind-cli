...
import { FORK_AGENT, FORK_PLACEHOLDER_RESULT, buildForkedMessages, buildChildMessage, isInForkExecution, runInForkContext, } from './fork-subagent.js';
...
import { getAgentJsonlPath, getAgentMetaPath, attachJsonlTranscriptWriter, patchAgentMeta, writeAgentMeta, } from '../../agents/agent-transcript.js';
...
class AgentToolInvocation extends BaseToolInvocation {
    ...
    async createForkSubagent(agentConfig, eventEmitter = this.eventEmitter) {
        ...
        if (rawHistory.length > 0) {
            ...
            if (lastMessage.role === 'model') {
                const forkedMessages = buildForkedMessages(this.params.prompt, lastMessage);
        ...
        if (generationConfig?.systemInstruction) {
            ...
            const parentToolDecls = generationConfig.tools?.flatMap((t) => t.functionDeclarations ?? []) ?? [];
            promptConfig = {
        ...
        const subagent = await AgentHeadless.create(FORK_AGENT.name, agentConfig, promptConfig, {}, {}, toolConfig, eventEmitter);
    ...
    async runSubagentStopHookLoop(subagent, opts) {
        ...
        debugLogger.warn(`[Agent] SubagentStop hook reached maximum iterations (${maxIterations}), forcing stop`);
    }
    /**
        ...
        try {
            ...
            if (isFork) {
                ...
                if (isInForkExecution()) {
                    return {
                        llmContent: 'Error: Cannot create a fork from within an existing fork child. Please execute tasks directly.',
                        returnDisplay: {
                            type: 'task_execution',
                ...
                const loadedConfig = await this.subagentManager.loadSubagent(this.params.subagent_type);
                if (!loadedConfig) {
                    return {
            ...
            if (isFork) {
                ...
                subagent = await this.subagentManager.createAgentHeadless(subagentConfig, agentConfig, { eventEmitter: this.eventEmitter });
                taskPrompt = this.params.prompt;
            }
            ...
            const shouldRunInBackground = this.params.run_in_background === true ||
            ...
            if (shouldRunInBackground) {
                ...
                let bgSubagent;
                let bgInitialMessages;
                let bgTaskPrompt;
                let bgPromptConfig;
                let bgToolConfig;
                if (isFork) {
                    const fork = await this.createForkSubagent(bgConfig, bgEventEmitter);
                    ...
                    bgSubagent = await this.subagentManager.createAgentHeadless(subagentConfig, bgConfig, { eventEmitter: bgEventEmitter });
                    bgTaskPrompt = this.params.prompt;
                }
                const registry = this.config.getBackgroundTaskRegistry();
                const projectDir = this.config.storage.getProjectDir();
                const sessionId = this.config.getSessionId();
                const jsonlPath = getAgentJsonlPath(projectDir, sessionId, hookOpts.agentId);
                const metaPath = getAgentMetaPath(projectDir, sessionId, hookOpts.agentId);
                const projectRoot = this.config.getProjectRoot();
                const { cleanup: cleanupJsonl } = attachJsonlTranscriptWriter(bgEventEmitter, jsonlPath, {
                ...
                const framedBgBody = () => runWithAgentContext({ agentId: hookOpts.agentId }, bgBody);
            ...
            const runFramed = () => runWithAgentContext({ agentId: hookOpts.agentId }, () => this.runSubagentWithHooks(subagent, contextState, hookOpts));
            if (isFork) {
