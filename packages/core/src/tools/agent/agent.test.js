...
import { AgentTool, resolveSubagentApprovalMode, } from './agent.js';
...
describe('AgentTool', () => {
    ...
    describe('AgentToolInvocation', () => {
        ...
        it('should execute subagent successfully', async () => {
            ...
            expect(mockSubagentManager.createAgentHeadless).toHaveBeenCalledWith(mockSubagents[0], expect.any(Object), // config (may be approval-mode override)
            expect.any(Object));
    ...
    describe('SubagentStart hook integration', () => {
        ...
        it('should call fireSubagentStartEvent before execution', async () => {
            ...
            const invocation = agentTool.createInvocation(params);
            await invocation.execute();
            expect(mockHookSystem.fireSubagentStartEvent).toHaveBeenCalledWith(expect.stringContaining('file-search-'), 'file-search', PermissionMode.AutoEdit, undefined);
        });
    ...
    describe('SubagentStop hook integration', () => {
        ...
        it('should call fireSubagentStopEvent after execution', async () => {
            ...
            const invocation = agentTool.createInvocation(params);
            await invocation.execute();
            expect(mockHookSystem.fireSubagentStopEvent).toHaveBeenCalledWith(expect.stringContaining('file-search-'), 'file-search', '/test/transcript', 'Task completed successfully', false, PermissionMode.AutoEdit, undefined);
        });
        it('should re-execute subagent when stop hook returns blocking decision', async () => {
            ...
            expect(mockHookSystem.fireSubagentStopEvent).toHaveBeenNthCalledWith(2, expect.stringContaining('file-search-'), 'file-search', '/test/transcript', 'Task completed successfully', true, PermissionMode.AutoEdit, undefined);
        });
    ...
    describe('Agent-level background: true', () => {
        ...
        it('should run in background when agent definition has background: true', async () => {
            ...
            expect(llmText).toContain(`Use ${ToolNames.SEND_MESSAGE} to continue this agent`);
        ...
        it('forwards the scheduler-provided callId as toolUseId on the registry entry', async () => {
            ...
            const invocation = agentTool.createInvocation(params);
            invocation.setCallId('call-xyz-789');
            await invocation.execute();
            expect(mockRegistry.register).toHaveBeenCalledWith(expect.objectContaining({ toolUseId: 'call-xyz-789' }));
        });
        describe('parentAgentId sidecar', () => {
            let tempProjectDir;
            beforeEach(() => {
                tempProjectDir = fs.mkdtempSync(path.join(os.tmpdir(), 'agent-parent-id-'));
                config['storage'] = {
            ...
            const readSidecar = (agentId) => {
                const metaPath = path.join(tempProjectDir, 'subagents', 'test-session-id', `agent-${agentId}.meta.json`);
            ...
            it('records the launching agent id when launched from a subagent frame', async () => {
                ...
                const invocation = agentTool.createInvocation(params);
                invocation.setCallId('nested-1');
                await runWithAgentContext({ agentId: 'explore-parent-42' }, async () => {
                    await invocation.execute();
                });
        ...
        it('persists fork capability snapshots in the bootstrap transcript', async () => {
            ...
            vi.mocked(config.getGeminiClient).mockReturnValue(geminiClient);
            const attachSpy = vi.spyOn(transcript, 'attachJsonlTranscriptWriter');
            const createSpy = vi
                .spyOn(AgentHeadless, 'create')
                .mockResolvedValue(mockAgent);
            const invocation = agentTool.createInvocation(forkParams);
            await invocation.execute();
            expect(attachSpy).toHaveBeenCalledWith(expect.anything(), expect.any(String), expect.objectContaining({
...
describe('resolveSubagentApprovalMode', () => {
    it('should return yolo when parent is yolo, regardless of agent config', () => {
        expect(resolveSubagentApprovalMode(ApprovalMode.YOLO, 'plan', true)).toBe(PermissionMode.Yolo);
        expect(resolveSubagentApprovalMode(ApprovalMode.YOLO, undefined, false)).toBe(PermissionMode.Yolo);
    });
    it('should return auto-edit when parent is auto-edit, regardless of agent config', () => {
        expect(resolveSubagentApprovalMode(ApprovalMode.AUTO_EDIT, 'plan', true)).toBe(PermissionMode.AutoEdit);
        expect(resolveSubagentApprovalMode(ApprovalMode.AUTO_EDIT, 'default', false)).toBe(PermissionMode.AutoEdit);
    });
    it('should respect agent-declared mode when parent is default and folder is trusted', () => {
        expect(resolveSubagentApprovalMode(ApprovalMode.DEFAULT, 'plan', true)).toBe(PermissionMode.Plan);
        expect(resolveSubagentApprovalMode(ApprovalMode.DEFAULT, 'auto-edit', true)).toBe(PermissionMode.AutoEdit);
        expect(resolveSubagentApprovalMode(ApprovalMode.DEFAULT, 'yolo', true)).toBe(PermissionMode.Yolo);
    });
    it('should block privileged agent-declared modes in untrusted folders', () => {
        expect(resolveSubagentApprovalMode(ApprovalMode.DEFAULT, 'auto-edit', false)).toBe(PermissionMode.Default);
        expect(resolveSubagentApprovalMode(ApprovalMode.DEFAULT, 'yolo', false)).toBe(PermissionMode.Default);
    });
    it('should allow non-privileged agent-declared modes in untrusted folders', () => {
        expect(resolveSubagentApprovalMode(ApprovalMode.DEFAULT, 'plan', false)).toBe(PermissionMode.Plan);
        expect(resolveSubagentApprovalMode(ApprovalMode.DEFAULT, 'default', false)).toBe(PermissionMode.Default);
    });
    it('should default to plan when parent is plan and no agent config', () => {
        expect(resolveSubagentApprovalMode(ApprovalMode.PLAN, undefined, true)).toBe(PermissionMode.Plan);
        expect(resolveSubagentApprovalMode(ApprovalMode.PLAN, undefined, false)).toBe(PermissionMode.Plan);
    });
    it('should allow agent-declared mode to override plan parent', () => {
        expect(resolveSubagentApprovalMode(ApprovalMode.PLAN, 'auto-edit', true)).toBe(PermissionMode.AutoEdit);
    });
    it('should default to auto-edit when parent is default and folder is trusted', () => {
        expect(resolveSubagentApprovalMode(ApprovalMode.DEFAULT, undefined, true)).toBe(PermissionMode.AutoEdit);
    });
    it('should default to parent mode when parent is default and folder is untrusted', () => {
        expect(resolveSubagentApprovalMode(ApprovalMode.DEFAULT, undefined, false)).toBe(PermissionMode.Default);
    });
});
