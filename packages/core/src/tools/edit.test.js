...
describe('EditTool', () => {
    ...
    describe('validateToolParams', () => {
        ...
        it.skipIf(process.platform === 'win32')('should unescape shell-escaped spaces in file_path', () => {
        ...
        it.skipIf(process.platform === 'win32')('should unescape multiple shell-escaped characters in file_path', () => {
            const escapedPath = path.join(rootDir, 'project\\ \\(v2\\)\\ \\&\\ more.txt');
            const params = {
                file_path: escapedPath,
                old_string: 'old',
                new_string: 'new',
            ...
            expect(tool.validateToolParams(params)).toBeNull();
            expect(params.file_path).toBe(path.join(rootDir, 'project (v2) & more.txt'));
        });
        it.skipIf(process.platform === 'win32')('should preserve literal backslashes in file_path', () => {
            ...
            const pathWithBackslash = path.join(rootDir, 'path\\\\with\\\\slashes.txt');
            const params = {
    ...
    describe('getConfirmationDetails', () => {
        ...
        it('should throw if old_string is not found', async () => {
            ...
            const invocation = tool.build(params);
            await expect(invocation.getConfirmationDetails(new AbortController().signal)).rejects.toThrow();
        });
        ...
        it('should rethrow calculateEdit errors when the abort signal is triggered', async () => {
            ...
            await expect(invocation.getConfirmationDetails(abortController.signal)).rejects.toBe(abortError);
    ...
    describe('execute', () => {
        ...
        it('should create new file with BOM when defaultFileEncoding is utf-8-bom', async () => {
            ...
            mockConfig.getDefaultFileEncoding.mockReturnValue('utf-8-bom');
    ...
    describe('Error Scenarios', () => {
        ...
        it.skipIf(process.getuid && process.getuid() === 0)('should return FILE_WRITE_FAILURE on write error', async () => {
            fs.writeFileSync(filePath, 'content', 'utf8');
    ...
    describe('FileReadCache integration', () => {
        it('records a write into the cache so a follow-up Read sees lastWriteAt', async () => {
            ...
            if (after.state === 'fresh') {
                ...
                expect(after.entry.lastWriteAt).toBeGreaterThanOrEqual(after.entry.lastReadAt);
            }
        });
    });
    describe.skipIf(process.platform === 'win32')('escaped paths with spaces (end-to-end)', () => {
