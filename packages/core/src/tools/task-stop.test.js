...
describe('TaskStopTool', () => {
    ...
    it('cancels a running agent', async () => {
        ...
        const result = await tool.validateBuildAndExecute({ task_id: 'agent-1' }, new AbortController().signal);
    ...
    it('returns error for non-existent task', async () => {
        const result = await tool.validateBuildAndExecute({ task_id: 'nope' }, new AbortController().signal);
    ...
    describe('background shell support', () => {
        it('cancels a running background shell', async () => {
            ...
            const result = await tool.validateBuildAndExecute({ task_id: 'bg_a1b2c3d4' }, new AbortController().signal);
            expect(result.error).toBeUndefined();
        ...
        it('returns NOT_RUNNING when the shell already exited', async () => {
            ...
            const result = await tool.validateBuildAndExecute({ task_id: 'bg_done' }, new AbortController().signal);
            expect(result.error?.type).toBe(ToolErrorType.TASK_STOP_NOT_RUNNING);
        ...
        it('prefers an agent over a shell when both have the same id (defensive)', async () => {
            ...
            const result = await tool.validateBuildAndExecute({ task_id: 'shared-id' }, new AbortController().signal);
    ...
    describe('monitor support', () => {
        it('cancels a running monitor', async () => {
            ...
            const result = await tool.validateBuildAndExecute({ task_id: 'mon_123' }, new AbortController().signal);
            expect(result.error).toBeUndefined();
        ...
        it('returns NOT_RUNNING when the monitor already completed', async () => {
            ...
            const result = await tool.validateBuildAndExecute({ task_id: 'mon_done' }, new AbortController().signal);
            expect(result.error?.type).toBe(ToolErrorType.TASK_STOP_NOT_RUNNING);
...
//# sourceMappingURL=task-stop.test.js.map