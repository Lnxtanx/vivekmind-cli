...
describe('ReadFileTool', () => {
    ...
    describe('build', () => {
        ...
        it.skipIf(process.platform === 'win32')('should unescape shell-escaped spaces in file_path', () => {
            ...
            expect(invocation.params.file_path).toBe(path.join(tempRootDir, 'my file.txt'));
        });
    ...
    describe('getDefaultPermission', () => {
        ...
        it('should return allow for paths within the subagent transcripts dir', async () => {
            const params = {
                file_path: path.join(tempRootDir, '.project', 'subagents', 'session-1', 'agent-a.jsonl'),
            };
            const invocation = tool.build(params);
            const permission = await invocation.getDefaultPermission();
            expect(permission).toBe('allow');
    ...
    describe('execute', () => {
        ...
        it.skipIf(process.platform === 'win32')('should read a file with spaces in its name when given an escaped path', async () => {
            ...
            const params = { file_path: escapedPath };
            const invocation = tool.build(params);
            expect(await invocation.execute(abortSignal)).toEqual({
                llmContent: fileContent,
                returnDisplay: '',
        ...
        it('should return error for a file that is too large', async () => {
            ...
            expect(result.error?.message).toContain('File size exceeds the 10MB limit');
        });
        ...
        it('should reject pages range exceeding 20', () => {
            ...
            expect(() => tool.build(params)).toThrow('Pages range exceeds maximum of 20');
        });
        ...
        it('should successfully read files from OS temp directory', async () => {
            const osTempFile = await fsp.mkdtemp(path.join(os.tmpdir(), 'read-file-test-'));
        ...
        describe('with FileReadCache', () => {
            ...
            async function read(params, toolOverride = tool) {
                const invocation = toolOverride.build(params);
            ...
            it('returns the file_unchanged placeholder on a second full Read of an unchanged text file', async () => {
                ...
                expect(second.llmContent).toMatch(/unchanged since last read in this session/);
        ...
        describe('with .vivekmindignore', () => {
            beforeEach(async () => {
                await fsp.writeFile(path.join(tempRootDir, '.vivekmindignore'), ['foo.*', 'ignored/'].join('\n'));
            });
