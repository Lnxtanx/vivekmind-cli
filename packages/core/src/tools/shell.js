...
import { getCommandRoot, getCommandRoots, splitCommands, stripShellWrapper, } from '../utils/shell-utils.js';
import { createDebugLogger } from '../utils/debugLogger.js';
import { isShellCommandReadOnlyAST, extractCommandRules, } from '../utils/shellAstParser.js';
...
export class ShellToolInvocation extends BaseToolInvocation {
    ...
    async execute(signal, updateOutput, shellExecutionConfig, setPidCallback) {
        ...
        const effectiveTimeout = this.params.timeout ?? DEFAULT_FOREGROUND_TIMEOUT_MS;
        ...
        const processedCommand = this.addCoAuthorToGitCommit(this.params.command.trim());
        ...
        }, combinedSignal, this.config.getShouldUseNodePtyShell(), shellExecutionConfig ?? {});
        ...
        if (result.aborted) {
            ...
            const wasTimeout = effectiveTimeout && combinedSignal.aborted && !signal.aborted;
            if (wasTimeout) {
        ...
        const shouldAppendLongRunHint = !result.aborted &&
            result.signal === null &&
        ...
        debugLogger.debug(`long-run hint: elapsed=${Math.round(elapsedMs)}ms threshold=${longRunThreshold}ms ` +
...
export class ShellTool extends BaseDeclarativeTool {
    ...
    validateToolParamValues(params) {
        ...
        if (!params.is_background) {
            const sleepPattern = detectBlockedSleepPattern(stripShellWrapper(params.command));
            if (sleepPattern !== null) {
                return (`Blocked: ${sleepPattern}. ` +
