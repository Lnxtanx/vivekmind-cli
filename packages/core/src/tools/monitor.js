...
import { detectCommandSubstitution, getCommandRoot, getShellConfiguration, hasUnsafeMonitorBackgroundOperator, normalizeMonitorCommand as normalizeMonitorShellCommand, splitCommands, } from '../utils/shell-utils.js';
import { createDebugLogger } from '../utils/debugLogger.js';
import { isSubpaths } from '../utils/paths.js';
import { MAX_CONCURRENT_MONITORS } from '../services/monitorRegistry.js';
import { extractCommandRules, isShellCommandReadOnlyAST, } from '../utils/shellAstParser.js';
...
class MonitorToolInvocation extends BaseToolInvocation {
    ...
    async getDefaultPermission() {
        const command = normalizeMonitorShellCommand(this.params.command).safetyCommand;
    ...
    async execute(_signal) {
        ...
        const cleanup = () => {
            ...
            if (entry.droppedLines > 0) {
                debugLogger.info(`Monitor ${monitorId} dropped ${entry.droppedLines} lines due to throttling`);
            }
        };
        let exitResult = null;
