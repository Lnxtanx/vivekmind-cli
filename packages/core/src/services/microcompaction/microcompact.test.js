...
import { evaluateTimeBasedTrigger, microcompactHistory, MICROCOMPACT_CLEARED_MESSAGE, } from './microcompact.js';
function clearEnv() {
    delete process.env['VIVEKMIND_MC_KEEP_RECENT'];
}
function makeToolCall(name) {
    return {
        role: 'model',
        parts: [{ functionCall: { name, args: {} } }],
    };
}
function makeToolResult(name, output) {
    return {
        role: 'user',
...
function makeModelMessage(text) {
...
describe('evaluateTimeBasedTrigger', () => {
    ...
    it('should return null when gap is under threshold', () => {
        const result = evaluateTimeBasedTrigger(Date.now() - 1 * 60 * 1000, DEFAULT_SETTINGS);
        expect(result).toBeNull();
    });
    it('should fire when gap exceeds threshold', () => {
        const result = evaluateTimeBasedTrigger(Date.now() - 10 * 60 * 1000, DEFAULT_SETTINGS);
...
describe('microcompactHistory', () => {
    ...
    it('should clear old compactable tool results and keep recent', () => {
        ...
        expect(result.history[3].parts[0].functionResponse.response['output']).toBe(MICROCOMPACT_CLEARED_MESSAGE);
        expect(result.history[5].parts[0].functionResponse.response['output']).toBe('recent file content');
    });
    it('should not clear non-compactable tools', () => {
        ...
        expect(result.history[1].parts[0].functionResponse.response['output']).toBe('user answer');
    ...
    it('should handle keepRecent > compactable count (no-op)', () => {
        ...
        expect(result.meta).toBeUndefined();
        expect(result.history[1].parts[0].functionResponse.response['output']).toBe('only result');
    });
    it('should floor keepRecent to 1', () => {
        ...
        expect(result.meta.toolsCleared).toBe(1);
        expect(result.meta.toolsKept).toBe(1);
        expect(result.history[1].parts[0].functionResponse.response['output']).toBe(MICROCOMPACT_CLEARED_MESSAGE);
        expect(result.history[3].parts[0].functionResponse.response['output']).toBe('grep results');
    });
    it('should preserve non-functionResponse parts in cleared Content', () => {
        ...
        expect(result.history[0].parts[1].functionResponse.response['output']).toBe(MICROCOMPACT_CLEARED_MESSAGE);
    });
    it('should preserve functionResponse name after clearing', () => {
        ...
        const result = microcompactHistory(history, twoHoursAgo, DEFAULT_SETTINGS);
        expect(result.history[1].parts[0].functionResponse.name).toBe('read_file');
    });
    it('should count per-part not per-Content for batched tool results', () => {
        ...
        expect(parts[0].functionResponse.response['output']).toBe(MICROCOMPACT_CLEARED_MESSAGE);
        expect(parts[1].functionResponse.response['output']).toBe(MICROCOMPACT_CLEARED_MESSAGE);
    ...
    it('should handle mixed batched and separate tool results', () => {
        ...
        expect(result.history[1].parts[0].functionResponse.response['output']).toBe(MICROCOMPACT_CLEARED_MESSAGE);
        expect(result.history[3].parts[0].functionResponse.response['output']).toBe('batched-read');
        expect(result.history[3].parts[1].functionResponse.response['output']).toBe('batched-grep');
    });
    it('should not clear tool error responses', () => {
        ...
        const result = microcompactHistory(history, twoHoursAgo, DEFAULT_SETTINGS);
        expect(result.history[1].parts[0].functionResponse.response['error']).toBe('File not found: /missing.txt');
        expect(result.history[1].parts[0].functionResponse.response['output']).toBeUndefined();
    });
...
//# sourceMappingURL=microcompact.test.js.map