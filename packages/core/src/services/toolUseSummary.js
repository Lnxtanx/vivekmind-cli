...
export async function generateToolUseSummary(params) {
    ...
    try {
        ...
        const response = await geminiClient.generateContent([{ role: 'user', parts: [{ text: userPrompt }] }], {
            systemInstruction: TOOL_USE_SUMMARY_SYSTEM_PROMPT,
            tools: [],
            maxOutputTokens: 60,
            temperature: 0.3,
        }, signal, model, 'tool_use_summary_generation');
        if (signal.aborted)
            return null;
        ...
        if (signal.aborted)
            return null;
        debugLogger.warn(`Summary generation failed: ${err instanceof Error ? err.message : String(err)}`);
        return null;
    }
}
/**
 ...
 */
export function truncateJson(value, maxLength) {
    try {
 ...
 */
function preTruncate(value, maxLength, depth = 0) {
...
//# sourceMappingURL=toolUseSummary.js.map