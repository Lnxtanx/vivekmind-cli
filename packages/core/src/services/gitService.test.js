...
import { getProjectHash, VIVEKMIND_DIR } from '../utils/paths.js';
...
describe('GitService', () => {
    ...
    describe('setupShadowGitRepository', () => {
        ...
        let gitConfigPath;
        beforeEach(() => {
            repoDir = path.join(homedir, VIVEKMIND_DIR, 'history', hash);
        ...
        it('should initialize git repo when root repo check throws', async () => {
            hoistedMockCheckIsRepo.mockRejectedValueOnce(new Error('fatal: not a git repository'));
            const service = new GitService(projectRoot, storage);
