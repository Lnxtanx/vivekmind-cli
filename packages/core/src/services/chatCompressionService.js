...
export class ChatCompressionService {
    async compress(chat, promptId, force, model, config, hasFailedCompressionAttempt, signal) {
        ...
        const hasOrphanedFuncCall = force &&
        ...
        const compressCharCount = historyToCompress.reduce((sum, c) => sum + JSON.stringify(c).length, 0);
        ...
        if (!isSummaryEmpty) {
            ...
            if (typeof compressionInputTokenCount === 'number' &&
        ...
        if (isSummaryEmpty) {
            ...
            try {
                ...
                await config
                    .getHookSystem()
                    ?.fireSessionStartEvent(SessionStartSource.Compact, model ?? '', permissionMode, undefined, signal);
            }
            catch (err) {
