...
function makeConfig(targetDir, cache, disabled = false) {
    const explicit = {
        ...
        getFileService: () => ({ shouldVivekMindIgnoreFile: () => false }),
...
describe('FileReadCache integration: read after history rewrite', () => {
    ...
    it('returns the file_unchanged placeholder on a follow-up Read after microcompact, exposing why the cache must be cleared on history rewrite', async () => {
        ...
        const r1 = await tool.buildAndExecute({ file_path: filePath }, new AbortController().signal);
        ...
        const fooContentEntries = mcResult.history.filter((c) => c.parts?.some((p) => {
        ...
        const r2 = await tool.buildAndExecute({ file_path: filePath }, new AbortController().signal);
    ...
    it('after cache.clear(), a follow-up Read of the same unchanged file re-emits the real bytes', async () => {
        ...
        cache.clear();
        const r2 = await tool.buildAndExecute({ file_path: filePath }, new AbortController().signal);
        expect(r2.llmContent).toContain('export function hello');
    ...
    it('worst case: when microcompact removes every prior read of a file, the placeholder leaves zero recoverable bytes for the model', async () => {
        ...
        await tool.buildAndExecute({ file_path: filePath }, new AbortController().signal);
        ...
        expect(surviving.some((o) => o.includes('export function hello'))).toBe(false);
        // Now Read foo.ts again — pre-fix, cache returns placeholder.
        const r = await tool.buildAndExecute({ file_path: filePath }, new AbortController().signal);
        expect(r.llmContent).toContain('unchanged since last read in this session');
...
//# sourceMappingURL=fileReadCache.integration.test.js.map