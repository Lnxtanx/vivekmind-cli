...
function makeConfig(opts) {
    const generateJson = vi.fn(async (...args) => {
        const r = opts.generateJsonResult;
        if (!r)
            throw new Error('no generateJsonResult configured');
...
describe('tryGenerateSessionTitle', () => {
    ...
    it('filters tool-call and tool-result turns from the prompt', async () => {
        ...
        const history = [
            { role: 'user', parts: [{ text: 'scan the auth module' }] },
            {
                ...
                parts: [
                    ...
                    { functionCall: { name: 'grep', args: { q: 'auth' } } },
                ],
            },
            {
                role: 'user',
        ...
        const generateJson = vi.fn(async (opts) => {
...
describe('sanitizeTitle', () => {
    ...
    it('strips terminal escape sequences (ANSI, OSC-8, BEL)', () => {
        ...
        expect(sanitizeTitle('\x1b]8;;http://evil\x1b\\click\x1b]8;;\x1b\\')).toBe('click');
...
//# sourceMappingURL=sessionTitle.test.js.map