...
describe('FileReadCache', () => {
    describe('inodeKey', () => {
        it('combines dev and ino into a stable string', () => {
            expect(FileReadCache.inodeKey(makeStats({ dev: 7, ino: 99 }))).toBe('7:99');
        });
    ...
    describe('recordWrite', () => {
        ...
        it('refreshes mtime+size so a follow-up Edit sees fresh', () => {
            ...
            expect(cache.check(makeStats({ mtimeMs: 2000, size: 20 })).state).toBe('fresh');
        });
        it('preserves lastReadAt set by an earlier recordRead', () => {
            ...
            const entry = cache.recordWrite('/x/foo.ts', makeStats({ mtimeMs: 9999 }));
...
//# sourceMappingURL=fileReadCache.test.js.map