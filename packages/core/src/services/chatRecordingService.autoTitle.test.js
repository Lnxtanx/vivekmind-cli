...
function findCustomTitleRecord() {
    return vi
...
describe('ChatRecordingService - auto-title trigger', () => {
    ...
    it('does not overwrite a manual title', async () => {
        ...
        expect(tryGenerateSessionTitleMock).not.toHaveBeenCalled();
        expect(findCustomTitleRecord()).toBeUndefined();
        expect(chatRecordingService.getCurrentCustomTitle()).toBe('chose-this-myself');
    ...
    it('does not trigger when VIVEKMIND_DISABLE_AUTO_TITLE is set', async () => {
        const prev = process.env['VIVEKMIND_DISABLE_AUTO_TITLE'];
        ...
        try {
            ...
            if (prev === undefined)
                delete process.env['VIVEKMIND_DISABLE_AUTO_TITLE'];
            else
                process.env['VIVEKMIND_DISABLE_AUTO_TITLE'] = prev;
        }
    });
    it('still triggers when VIVEKMIND_DISABLE_AUTO_TITLE is falsy ("0")', async () => {
        mockOk('Fix login button');
        const prev = process.env['VIVEKMIND_DISABLE_AUTO_TITLE'];
    ...
    it('does not overwrite a manual title written by another process', async () => {
        ...
        expect(chatRecordingService.getCurrentCustomTitle()).toBe('User chose this');
        expect(chatRecordingService.getCurrentTitleSource()).toBe('manual');
    });
    it('aborts the in-flight generation on finalize and suppresses the title write', async () => {
        ...
        let capturedSignal;
        tryGenerateSessionTitleMock.mockImplementation((_config, signal) => new Promise((_resolve, reject) => {
    ...
    it('respects a late /rename that lands while the LLM call is in flight', async () => {
        ...
        let resolveLlm = () => { };
        tryGenerateSessionTitleMock.mockImplementationOnce(() => new Promise((resolve) => {
            resolveLlm = resolve;
        }));
        chatRecordingService.recordAssistantTurn({
            model: 'qwen-plus',
...
//# sourceMappingURL=chatRecordingService.autoTitle.test.js.map