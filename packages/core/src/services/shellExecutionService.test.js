...
const { Terminal } = pkg;
// Hoisted Mocks
const mockGetSystemEncoding = vi.hoisted(() => vi.fn().mockReturnValue('utf-8'));
...
const mockSerializeTerminalToText = vi.hoisted(() => vi.fn((terminal) => {
...
const mockGetShellConfiguration = vi.hoisted(() => vi.fn().mockReturnValue({
    executable: 'bash',
    argsPrefix: ['-c'],
    shell: 'bash',
}));
...
const EXPECTED_MERGED_WINDOWS_PATH = 'C:\\Windows\\System32;C:\\Shared\\Tools;C:\\Users\\tester\\bin';
...
const expectNormalizedWindowsPathEnv = (env) => {
    expect(env['PATH']).toBe(EXPECTED_MERGED_WINDOWS_PATH);
    expect(env['Path']).toBeUndefined();
};
const waitForDataEventCount = async (onOutputEventMock, expectedCount) => {
    for (let attempt = 0; attempt < 20; attempt++) {
        const dataEvents = onOutputEventMock.mock.calls.filter(([event]) => event.type === 'data');
...
describe('ShellExecutionService', () => {
    ...
    describe('Successful Execution', () => {
        ...
        it('should preserve full raw output when terminal writes are backlogged', async () => {
            ...
            const delayedWrite = vi
                ...
                .mockImplementation(function (data, callback) {
                setTimeout(() => {
            ...
            try {
                const abortController = new AbortController();
                const handle = await ShellExecutionService.execute('fast-output', '/test/dir', onOutputEventMock, abortController.signal, true, shellExecutionConfig);
                const onData = mockPtyProcess.onData.mock.calls[0][0];
                for (let i = 1; i <= 500; i++) {
        ...
        it('should not persist narrow terminal soft wraps as transcript newlines', async () => {
            const { result } = await simulateExecution('narrow-output', (pty) => {
    ...
    describe('pty interaction', () => {
        ...
        it('should write to the pty and trigger a render', async () => {
            ...
            try {
                const abortController = new AbortController();
                const handle = await ShellExecutionService.execute('interactive-app', '/test/dir', onOutputEventMock, abortController.signal, true, shellExecutionConfig);
        ...
        it('should throw unexpected PTY errors from error event', async () => {
            ...
            expect(() => mockPtyProcess.emit('error', unexpectedError)).toThrow('unexpected pty error');
            mockPtyProcess.onExit.mock.calls[0][0]({ exitCode: 0, signal: null });
            await handle.result;
        });
        it('should ignore ioctl EBADF message-only resize race errors', async () => {
            ...
            await simulateExecution('ls -l', (pty) => {
                pty.onData.mock.calls[0][0]('file1.txt\n');
                expect(() => ShellExecutionService.resizePty(pty.pid, 100, 40)).not.toThrow();
                pty.onExit.mock.calls[0][0]({ exitCode: 0, signal: null });
            });
        });
    ...
    describe('Platform-Specific Behavior', () => {
        it('should use cmd.exe on Windows', async () => {
            ...
            expect(mockPtySpawn).toHaveBeenCalledWith('cmd.exe', '/d /s /c dir "foo bar"', expect.any(Object));
            mockGetShellConfiguration.mockReturnValue({
                executable: 'bash',
                argsPrefix: ['-c'],
                shell: 'bash',
        ...
        it('should use PowerShell on Windows with array args and UTF-8 prefix', async () => {
            ...
            await simulateExecution('Test-Path "C:\\Temp\\"', (pty) => pty.onExit.mock.calls[0][0]({ exitCode: 0, signal: null }));
    ...
    describe('AnsiOutput rendering', () => {
        ...
        it('does not re-emit live output when only soft-wrap segmentation changes', async () => {
            ...
            mockSerializeTerminalToObject.mockImplementation((_terminal, _scrollOffset, options) => {
            ...
            await simulateExecution('narrow-output', (pty) => {
        ...
        it('does not re-emit default plain live output when only soft-wrap segmentation changes', async () => {
            const abortController = new AbortController();
            const handle = await ShellExecutionService.execute('narrow-output', '/test/dir', onOutputEventMock, abortController.signal, true, {
                ...shellExecutionConfig,
                terminalWidth: 4,
                terminalHeight: 4,
                showColor: false,
            ...
            const chunk = firstDataEvent.chunk;
...
describe('ShellExecutionService child_process fallback', () => {
    ...
    describe('Successful Execution', () => {
        it('should execute a command and capture stdout and stderr', async () => {
            ...
            expect(mockCpSpawn).toHaveBeenCalledWith('bash', ['-c', 'ls -l'], expect.objectContaining({
                detached: true,
            }));
            expect(result.exitCode).toBe(0);
            expect(result.signal).toBeNull();
    ...
    describe('Platform-Specific Behavior', () => {
        it('should use cmd.exe with windowsVerbatimArguments on Windows', async () => {
            ...
            expect(mockCpSpawn).toHaveBeenCalledWith('cmd.exe', ['/d', '/s', '/c', 'dir "foo bar"'], expect.objectContaining({
                detached: false,
                windowsHide: true,
        ...
        it('should use PowerShell with UTF-8 prefix without windowsVerbatimArguments on Windows', async () => {
            ...
            await simulateExecution('Test-Path "C:\\Temp\\"', (cp) => cp.emit('exit', 0, null));
            // PowerShell commands on Windows are prefixed with UTF-8 output encoding
            expect(mockCpSpawn).toHaveBeenCalledWith('powershell.exe', [
                '-NoProfile',
                '-Command',
                '[Console]::OutputEncoding=[System.Text.Encoding]::UTF8;Test-Path "C:\\Temp\\"',
        ...
        it('should use bash and detached process group on Linux', async () => {
            ...
            expect(mockCpSpawn).toHaveBeenCalledWith('bash', ['-c', 'ls "foo bar"'], expect.objectContaining({
                detached: true,
            }));
        });
    });
