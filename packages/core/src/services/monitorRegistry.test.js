...
import { MAX_RETAINED_TERMINAL_MONITORS, MonitorRegistry, } from './monitorRegistry.js';
...
describe('MonitorRegistry', () => {
    ...
    it('auto-stops when maxEvents is reached', () => {
        ...
        const [, terminalModelText] = callback.mock.calls[3];
    ...
    it('auto-stop is re-entrancy safe: abort-driven flush cannot overshoot maxEvents or double-emit terminal', () => {
        ...
        const terminalCalls = callback.mock.calls.filter((args) => typeof args[1] === 'string' &&
            args[1].includes('Max events reached'));
    ...
    it('strips control characters from the displayText (defense-in-depth, not just XML)', () => {
        ...
        for (let i = 0; i < displayText.length; i++) {
            ...
            const isForbidden = (code < 0x20 && code !== 0x09) || (code >= 0x80 && code <= 0x9f);
    ...
    it('retains only a bounded number of terminal entries', () => {
        ...
        expect(registry.get(`mon-${MAX_RETAINED_TERMINAL_MONITORS + 1}`)).toBeDefined();
    });
    it('reset clears retained entries and running monitor timers', () => {
        ...
        registry.register(createEntry({ monitorId: 'running', abortController: ac }));
        registry.reset();
    ...
    it('rejects registration when max concurrent monitors reached', () => {
        ...
        expect(() => registry.register(createEntry({ monitorId: 'mon-overflow' }))).toThrow('maximum concurrent monitors');
    });
    it('allows registration after completed monitors free up slots', () => {
        ...
        expect(() => registry.register(createEntry({ monitorId: 'mon-new' }))).not.toThrow();
    });
    ...
    describe('setStatusChangeCallback', () => {
        ...
        it('callback failure does not poison the registry', () => {
            ...
            registry.setStatusChangeCallback(cb);
            expect(() => registry.register(createEntry({ monitorId: 'a' }))).not.toThrow();
...
//# sourceMappingURL=monitorRegistry.test.js.map