...
import { SessionStartSource, PreCompactTrigger, PostCompactTrigger, } from '../hooks/types.js';
...
describe('ChatCompressionService', () => {
    ...
    it('should compress if over token threshold', async () => {
        ...
        expect(mockFireSessionStartEvent).toHaveBeenCalledWith(SessionStartSource.Compact, mockModel, 'default', undefined, undefined);
    });
    ...
    it('should return FAILED if usage metadata is missing', async () => {
        ...
        const result = await service.compress(mockChat, mockPromptId, false, mockModel, mockConfig, false);
        expect(result.info.compressionStatus).toBe(CompressionStatus.COMPRESSION_FAILED_TOKEN_COUNT_ERROR);
    ...
    describe('PreCompact hook', () => {
        let mockFirePreCompactEvent;
        let mockFirePostCompactEvent;
        beforeEach(() => {
        ...
        it('should fire PreCompact hook with Manual trigger when force=true', async () => {
            ...
            await service.compress(mockChat, mockPromptId, true, // force = true -> Manual trigger
            mockModel, mockConfig, false);
            expect(mockFirePreCompactEvent).toHaveBeenCalledWith(PreCompactTrigger.Manual, '', undefined);
        });
        it('should fire PreCompact hook with Auto trigger when force=false', async () => {
            ...
            await service.compress(mockChat, mockPromptId, false, // force = false -> Auto trigger
            mockModel, mockConfig, false);
            expect(mockFirePreCompactEvent).toHaveBeenCalledWith(PreCompactTrigger.Auto, '', undefined);
        });
        ...
        it('should handle PreCompact hook errors gracefully', async () => {
            ...
            mockFirePreCompactEvent.mockRejectedValue(new Error('PreCompact hook failed'));
            const mockGenerateContent = vi.fn().mockResolvedValue({
                candidates: [
                    {
                        content: {
    ...
    describe('PostCompact hook', () => {
        ...
        it('should fire PostCompact hook with Manual trigger when force=true', async () => {
            ...
            await service.compress(mockChat, mockPromptId, true, // force = true -> Manual trigger
            mockModel, mockConfig, false);
            expect(mockFirePostCompactEvent).toHaveBeenCalledWith(PostCompactTrigger.Manual, 'Summary', undefined);
        });
        it('should fire PostCompact hook with Auto trigger when force=false', async () => {
            ...
            await service.compress(mockChat, mockPromptId, false, // force = false -> Auto trigger
            mockModel, mockConfig, false);
            expect(mockFirePostCompactEvent).toHaveBeenCalledWith(PostCompactTrigger.Auto, 'Auto Summary', undefined);
        });
        ...
        it('should handle PostCompact hook errors gracefully', async () => {
            ...
            mockFirePostCompactEvent.mockRejectedValue(new Error('PostCompact hook failed'));
            const mockGenerateContent = vi.fn().mockResolvedValue({
                candidates: [
                    {
                        content: {
    ...
    describe('orphaned trailing funcCall handling', () => {
        it('should compress everything when force=true and last message is an orphaned funcCall', async () => {
            ...
            const result = await service.compress(mockChat, mockPromptId, true, // force=true (manual /compress)
            mockModel, mockConfig, false);
        ...
        it('should NOT compress orphaned funcCall when force=false (auto-compress)', async () => {
            ...
            const result = await service.compress(mockChat, mockPromptId, false, // force=false (auto-compress)
            mockModel, mockConfig, false);
