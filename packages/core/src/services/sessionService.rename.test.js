...
describe('SessionService - rename and custom title', () => {
    ...
    describe('renameSession', () => {
        it('should append a custom_title record to the session file', async () => {
            ...
            const result = await sessionService.renameSession(sessionIdA, 'my-feature');
        ...
        it('should return false when session does not exist', async () => {
            vi.mocked(jsonl.readLines).mockResolvedValue([]);
            const result = await sessionService.renameSession('00000000-0000-0000-0000-000000000000', 'test');
    ...
    describe('findSessionsByTitle', () => {
        const now = Date.now();
        function setupSessionFiles(sessions) {
            readdirSyncSpy.mockReturnValue(sessions.map((s) => `${s.id}.jsonl`));
...
//# sourceMappingURL=sessionService.rename.test.js.map