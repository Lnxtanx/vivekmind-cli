...
describe('FileDiscoveryService', () => {
    ...
    describe('shouldGitIgnoreFile & shouldVivekMindIgnoreFile', () => {
        ...
        it('should return true for vivekmind-ignored files', () => {
            const service = new FileDiscoveryService(projectRoot);
            expect(service.shouldVivekMindIgnoreFile(path.join(projectRoot, 'debug.log'))).toBe(true);
        });
        it('should return false for non-vivekmind-ignored files', () => {
            const service = new FileDiscoveryService(projectRoot);
            expect(service.shouldVivekMindIgnoreFile(path.join(projectRoot, 'src/index.ts'))).toBe(false);
        });
    });
    describe('edge cases', () => {
