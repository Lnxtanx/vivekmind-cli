...
export class ChatRecordingService {
    ...
    maybeTriggerAutoTitle() {
        ...
        void (async () => {
            try {
                ...
                if (this.currentTitleSource === 'manual')
                    return;
     ...
     */
    rewindRecording(targetTurnIndex, payload) {
        try {
    ...
    rebuildTurnBoundaries(messages) {
        ...
        let prevUuid = this.config.getResumedSessionData()?.lastCompletedUuid !== undefined
            ? null
            : this.lastRecordUuid;
     ...
     */
    recordCustomTitle(customTitle, titleSource = 'manual') {
        try {
            const record = {
                ...this.createBaseRecord('system'),
                type: 'system',
