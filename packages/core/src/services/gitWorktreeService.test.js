...
describe('GitWorktreeService', () => {
    ...
    it('initializeRepository should initialize a new repo on main', async () => {
        ...
        expect(hoistedMockInit.mock.invocationCallOrder[0]).toBeLessThan(hoistedMockRaw.mock.invocationCallOrder[0]);
        expect(hoistedMockRaw.mock.invocationCallOrder[0]).toBeLessThan(hoistedMockCommit.mock.invocationCallOrder[0]);
    });
    ...
    it('applyWorktreeChanges should apply raw patch via git apply', async () => {
        ...
        const applyCall = hoistedMockRaw.mock.calls.find((call) => Array.isArray(call[0]) && call[0][0] === 'apply');
        ...
        expect(hoistedMockFsRm).toHaveBeenCalledWith(expect.stringContaining('.worktree-apply-'), { force: true });
    });
    ...
    describe('dirty state propagation', () => {
        function makeWorktreeInfo(name, sessionId) {
            return {
                id: `${sessionId}/${name}`,
                name,
        ...
        it('setupWorktrees should apply dirty state snapshot to each worktree', async () => {
            ...
            const stashApplyCalls = hoistedMockRaw.mock.calls.filter((call) => Array.isArray(call[0]) &&
                call[0][0] === 'stash' &&
                call[0][1] === 'apply');
...
//# sourceMappingURL=gitWorktreeService.test.js.map