 ...
 */
function mockOk(title: string, modelUsed = 'vivekmind-turbo'): void {
...
describe('ChatRecordingService - auto-title trigger', () => {
  ...
  it('does not trigger when VIVEKMIND_DISABLE_AUTO_TITLE is set', async () => {
    ...
    try {
      ...
      if (prev === undefined) delete process.env['VIVEKMIND_DISABLE_AUTO_TITLE'];
      else process.env['VIVEKMIND_DISABLE_AUTO_TITLE'] = prev;
    }
  });
