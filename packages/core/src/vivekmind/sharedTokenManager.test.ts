...
import type {
  IVivekMindOAuth2Client,
  VivekMindCredentials,
  TokenRefreshData,
  ErrorData,
...
/**
 * Creates a mock VivekMindOAuth2Client for testing
 */
function createMockVivekMindClient(
  initialCredentials: Partial<VivekMindCredentials> = {},
): IVivekMindOAuth2Client {
  let credentials: VivekMindCredentials = {
  ...
  return {
    setCredentials: vi.fn((creds: VivekMindCredentials) => {
...
function createValidCredentials(
  overrides: Partial<VivekMindCredentials> = {},
): VivekMindCredentials {
  return {
...
describe('SharedTokenManager', () => {
  ...
  describe('getValidCredentials', () => {
    it('should return valid cached credentials without refresh', async () => {
      const mockClient = createMockVivekMindClient();
    ...
    it('should refresh expired credentials', async () => {
      const mockClient = createMockVivekMindClient(createExpiredCredentials());
    ...
    it('should force refresh when forceRefresh is true', async () => {
      const mockClient = createMockVivekMindClient(createValidCredentials());
      const refreshResponse = createSuccessfulRefreshResponse();
  ...
  describe('CredentialsClearRequiredError handling', () => {
    it('should clear memory cache when CredentialsClearRequiredError is thrown during refresh', async () => {
      ...
      const refreshPromise =
        getPrivateProperty<Promise<VivekMindCredentials> | null>(
          tokenManager,
          'refreshPromise',
        );
    ...
    it('should properly clean up timeout when file operation completes before timeout', async () => {
      ...
      ) as (client?: IVivekMindOAuth2Client) => Promise<void>;
