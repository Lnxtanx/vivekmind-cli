...
import { generateCodeChallenge, generateCodeVerifier, generatePKCEPair, isDeviceAuthorizationSuccess, isDeviceTokenPending, isDeviceTokenSuccess, isErrorResponse, vivekmindOAuth2Events, VivekMindOAuth2Event, VivekMindOAuth2Client, } from './vivekmindOAuth2.js';
...
describe('VivekMindOAuth2Client', () => {
    let client;
    let originalFetch;
    beforeEach(() => {
        // Create client instance
        client = new VivekMindOAuth2Client();
        // Mock fetch
    ...
    describe('refreshAccessToken error handling', () => {
        ...
        it('should NOT clear credentials on malformed 200 response (e.g. proxy HTML)', async () => {
            const { CredentialsClearRequiredError } = await import('./vivekmindOAuth2.js');
            const mockResponse = {
                ok: true,
                status: 200,
            ...
            await expect(client.refreshAccessToken()).rejects.not.toBeInstanceOf(CredentialsClearRequiredError);
            await expect(client.refreshAccessToken()).rejects.toThrow('VivekMind OAuth refresh returned invalid JSON:');
        });
        it('should clear credentials and throw CredentialsClearRequiredError on 401 response', async () => {
            ...
            vi.mocked(global.fetch).mockResolvedValue(mockResponse);
            await expect(client.refreshAccessToken()).rejects.toBeInstanceOf(CredentialsClearRequiredError);
            await expect(client.refreshAccessToken()).rejects.toThrow("Refresh token expired or invalid. Please use '/auth' to re-authenticate.");
        });
    });
...
describe('getVivekMindOAuthClient', () => {
    let mockConfig;
    let originalFetch;
    beforeEach(() => {
        mockConfig = {
    ...
    it('should load cached credentials if available', async () => {
        ...
        const client = await import('./vivekmindOAuth2.js').then((module) => module.getVivekMindOAuthClient(mockConfig));
    ...
    it('should handle cached credentials refresh failure', async () => {
        ...
        await expect(import('./vivekmindOAuth2.js').then((module) => module.getVivekMindOAuthClient(mockConfig))).rejects.toThrow('Device authorization flow failed');
        SharedTokenManager.getInstance = originalGetInstance;
    });
    it('should not start device flow when requireCachedCredentials is true', async () => {
        ...
        await expect(import('./vivekmindOAuth2.js').then((module) => module.getVivekMindOAuthClient(mockConfig, {
            requireCachedCredentials: true,
        }))).rejects.toThrow('VivekMind OAuth credentials expired. Please use /auth to re-authenticate with vivekmind-oauth.');
    ...
    it('should include troubleshooting hints when device auth fetch fails', async () => {
        ...
        const emitSpy = vi.spyOn(vivekmindOAuth2Events, 'emit');
        let thrownError;
        try {
            const { getVivekMindOAuthClient } = await import('./vivekmindOAuth2.js');
            await getVivekMindOAuthClient(mockConfig);
        }
        catch (error) {
            thrownError = error;
        }
        ...
        expect(emitSpy).toHaveBeenCalledWith(VivekMindOAuth2Event.AuthProgress, 'error', expect.stringContaining('NODE_EXTRA_CA_CERTS'));
        emitSpy.mockRestore();
        SharedTokenManager.getInstance = originalGetInstance;
    });
});
...
describe('clearVivekMindCredentials', () => {
    it('should successfully clear credentials file', async () => {
        const { promises: fs } = await import('node:fs');
        const { clearVivekMindCredentials } = await import('./vivekmindOAuth2.js');
        vi.mocked(fs.unlink).mockResolvedValue(undefined);
        await expect(clearVivekMindCredentials()).resolves.not.toThrow();
...
describe('VivekMindOAuth2Client - Additional Error Scenarios', () => {
    let client;
    let originalFetch;
    beforeEach(() => {
        client = new VivekMindOAuth2Client();
...
describe('getVivekMindOAuthClient - Enhanced Error Scenarios', () => {
    let mockConfig;
    let originalFetch;
    beforeEach(() => {
        mockConfig = {
    ...
    it('should handle different authentication failure reasons - timeout', async () => {
        ...
        await expect(import('./vivekmindOAuth2.js').then((module) => module.getVivekMindOAuthClient(mockConfig))).rejects.toThrow('Authorization timeout, please restart the process.');
        SharedTokenManager.getInstance = originalGetInstance;
    });
    it('should handle authentication failure reason - rate limit', async () => {
        ...
        await expect(import('./vivekmindOAuth2.js').then((module) => module.getVivekMindOAuthClient(mockConfig))).rejects.toThrow('Too many requests. The server is rate limiting our requests. Please select a different authentication method or try again later.');
        SharedTokenManager.getInstance = originalGetInstance;
    });
...
describe('authWithVivekMindDeviceFlow - Comprehensive Testing', () => {
    let mockConfig;
    let originalFetch;
    beforeEach(() => {
        mockConfig = {
    ...
    it('should handle 401 error during token polling', async () => {
        ...
        await expect(import('./vivekmindOAuth2.js').then((module) => module.getVivekMindOAuthClient(mockConfig))).rejects.toThrow('Device code expired or invalid, please restart the authorization process.');
        SharedTokenManager.getInstance = originalGetInstance;
    });
...
describe('Event Emitter Integration', () => {
    it('should export vivekmindOAuth2Events as EventEmitter', async () => {
        const { vivekmindOAuth2Events } = await import('./vivekmindOAuth2.js');
        expect(vivekmindOAuth2Events).toBeInstanceOf(EventEmitter);
    });
    it('should define correct event enum values', async () => {
        const { VivekMindOAuth2Event } = await import('./vivekmindOAuth2.js');
        expect(VivekMindOAuth2Event.AuthUri).toBe('auth-uri');
        expect(VivekMindOAuth2Event.AuthProgress).toBe('auth-progress');
        expect(VivekMindOAuth2Event.AuthCancel).toBe('auth-cancel');
    });
});
describe('Utility Functions', () => {
    ...
    describe('getVivekMindCachedCredentialPath', () => {
        it('should return correct path to cached credentials', async () => {
            ...
            // Since this is a private function, we test it indirectly through clearVivekMindCredentials
            const { promises: fs } = await import('node:fs');
            const { clearVivekMindCredentials } = await import('./vivekmindOAuth2.js');
            vi.mocked(fs.unlink).mockResolvedValue(undefined);
            await clearVivekMindCredentials();
            expect(fs.unlink).toHaveBeenCalledWith(expectedPath);
        });
    });
});
describe('Credential Caching Functions', () => {
    describe('cacheVivekMindCredentials', () => {
        it('should create directory and write credentials to file', async () => {
            // Mock the internal cacheVivekMindCredentials function by creating client and calling refresh
            const client = new VivekMindOAuth2Client();
            client.setCredentials({
                refresh_token: 'test-refresh',
            });
            const mockResponse = {
...
describe('Enhanced Error Handling and Edge Cases', () => {
    ...
    describe('VivekMindOAuth2Client getAccessToken enhanced scenarios', () => {
    ...
    describe('Enhanced refreshAccessToken scenarios', () => {
        it('should call clearVivekMindCredentials on 400 error', async () => {
            client.setCredentials({
...
describe('SharedTokenManager Integration in VivekMindOAuth2Client', () => {
    let client;
    beforeEach(() => {
        client = new VivekMindOAuth2Client();
    });
    ...
    it('should handle TokenManagerError types correctly in getVivekMindOAuthClient', async () => {
        const mockConfig = {
        ...
        for (const errorInfo of tokenErrors) {
            ...
            global.fetch = vi
                ...
                .mockResolvedValue(mockTokenResponse);
            try {
                await import('./vivekmindOAuth2.js').then((module) => module.getVivekMindOAuthClient(mockConfig));
            }
            catch {
...
//# sourceMappingURL=vivekmindOAuth2.test.js.map