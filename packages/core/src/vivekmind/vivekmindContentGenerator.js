...
/**
 * VivekMind Content Generator that uses VivekMind OAuth tokens with automatic refresh
 */
export class VivekMindContentGenerator extends OpenAIContentGenerator {
    debugLogger = createDebugLogger('VIVEKMIND');
    vivekmindClient;
    sharedManager;
    currentToken;
    constructor(vivekmindClient, contentGeneratorConfig, cliConfig) {
        // Create DashScope provider for VivekMind
        ...
        this.vivekmindClient = vivekmindClient;
    ...
    async getValidToken() {
        try {
            ...
            const credentials = await this.sharedManager.getValidCredentials(this.vivekmindClient);
    ...
    async executeWithCredentialManagement(operation) {
        ...
        try {
            ...
            if (this.isAuthError(error)) {
                ...
                await this.sharedManager.getValidCredentials(this.vivekmindClient, true);
                return await attemptOperation();
            }
            throw error;
        }
...
//# sourceMappingURL=vivekmindContentGenerator.js.map