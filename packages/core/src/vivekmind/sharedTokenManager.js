...
import { isErrorResponse, CredentialsClearRequiredError, } from './vivekmindOAuth2.js';
...
export class SharedTokenManager {
     ...
     */
    async getValidCredentials(vivekmindClient, forceRefresh = false) {
        try {
            ...
            if (!currentRefreshPromise) {
                ...
                currentRefreshPromise = this.performTokenRefresh(vivekmindClient, forceRefresh);
     ...
     */
    async checkAndReloadIfNeeded(vivekmindClient) {
     ...
     */
    async performFileCheck(vivekmindClient, checkTime) {
     ...
     */
    async forceFileCheck(vivekmindClient) {
        try {
            const filePath = this.getCredentialFilePath();
     ...
     */
    async reloadCredentialsFromFile(vivekmindClient) {
        try {
            const filePath = this.getCredentialFilePath();
     ...
     */
    async performTokenRefresh(vivekmindClient, forceRefresh = false) {
    ...
    async saveCredentialsToFile(credentials) {
        ...
        try {
            ...
            await this.withTimeout(fs.writeFile(tempPath, credString, { mode: 0o600 }), 5000, 'File operation');
