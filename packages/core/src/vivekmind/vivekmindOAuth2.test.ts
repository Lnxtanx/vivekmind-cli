...
import {
  ...
  VivekMindOAuth2Client,
  type DeviceAuthorizationResponse,
  type DeviceTokenResponse,
  type ErrorData,
  type VivekMindCredentials,
} from './vivekmindOAuth2.js';
import {
  SharedTokenManager,
  TokenManagerError,
  TokenError,
...
describe('VivekMindOAuth2Client', () => {
  let client: VivekMindOAuth2Client;
    ...
    it('should refresh token if access token is expired', async () => {
      ...
      (
        client as unknown as {
          sharedManager: {
            getValidCredentials: () => Promise<VivekMindCredentials>;
          };
        }
      ).sharedManager = {
  ...
  describe('refreshAccessToken error handling', () => {
    ...
    it('should NOT clear credentials on malformed 200 response (e.g. proxy HTML)', async () => {
      ...
      await expect(client.refreshAccessToken()).rejects.toThrow(
        'VivekMind OAuth refresh returned invalid JSON:',
      );
    });
...
describe('getVivekMindOAuthClient', () => {
  ...
  it('should load cached credentials if available', async () => {
    ...
    const client = await import('./vivekmindOAuth2.js').then((module) =>
      module.getVivekMindOAuthClient(mockConfig),
    );
  ...
  it('should handle cached credentials refresh failure', async () => {
    ...
    await expect(
      import('./vivekmindOAuth2.js').then((module) =>
        module.getVivekMindOAuthClient(mockConfig),
      ),
  ...
  it('should not start device flow when requireCachedCredentials is true', async () => {
    ...
    await expect(
      ...
      'VivekMind OAuth credentials expired. Please use /auth to re-authenticate with vivekmind-oauth.',
    );
...
describe('SharedTokenManager Integration in VivekMindOAuth2Client', () => {
  ...
  it('should handle TokenManagerError types correctly in getVivekMindOAuthClient', async () => {
    ...
    for (const errorInfo of tokenErrors) {
      ...
      try {
        await import('./vivekmindOAuth2.js').then((module) =>
          module.getVivekMindOAuthClient(mockConfig),
        );
      } catch {
