...
import type { IVivekMindOAuth2Client } from './vivekmindOAuth2.js';
import {
...
const VIVEKMIND_LOCK_FILENAME = 'oauth_creds.lock';
...
interface MemoryCache {
  credentials: VivekMindCredentials | null;
  fileModTime: number;
  lastCheck: number;
}

/**
 * Validates that the given data is a valid VivekMindCredentials object
 *
 * @param data - The data to validate
 * @returns The validated credentials object
 * @throws Error if the data is invalid
 */
function validateCredentials(data: unknown): VivekMindCredentials {
  if (!data || typeof data !== 'object') {
    throw new Error('Invalid credentials format');
  }

  const creds = data as Partial<VivekMindCredentials>;
  ...
  return creds as VivekMindCredentials;
}
...
export class SharedTokenManager {
   ...
   */
  private refreshPromise: Promise<VivekMindCredentials> | null = null;
   ...
   *
   * @param vivekmindClient - The OAuth2 client instance
  ...
  async getValidCredentials(
    ...
    try {
      ...
      await this.checkAndReloadIfNeeded(vivekmindClient);
  ...
  private async checkAndReloadIfNeeded(
    ...
    this.checkPromise = this.performFileCheck(vivekmindClient, now);
  ...
  private async performFileCheck(
    vivekmindClient: IVivekMindOAuth2Client | undefined,
    checkTime: number,
  ): Promise<void> {
    ...
    try {
      ...
      if (fileModTime > this.memoryCache.fileModTime) {
        await this.reloadCredentialsFromFile(vivekmindClient);
   ...
   */
  private async forceFileCheck(vivekmindClient?: IVivekMindOAuth2Client): Promise<void> {
    try {
      const filePath = this.getCredentialFilePath();
  ...
  /**
   * Load credentials from the file system into memory cache and sync with vivekmindClient
   */
  private async reloadCredentialsFromFile(
    ...
    try {
      ...
      // Sync with vivekmindClient atomically - rollback on failure
      try {
        if (vivekmindClient) {
          vivekmindClient.setCredentials(credentials);
        }
      } catch (clientError) {
  ...
  private async performTokenRefresh(
    ...
    try {
      ...
      const currentCredentials = vivekmindClient.getCredentials();
      ...
      await this.forceFileCheck(vivekmindClient);
      ...
      if (
        ...
        // No need to call vivekmindClient.setCredentials here as checkAndReloadIfNeeded already did it
        return this.memoryCache.credentials;
      }

      // Perform the actual token refresh
      const response = await vivekmindClient.refreshAccessToken();
  ...
  private async saveCredentialsToFile(
    credentials: VivekMindCredentials,
  ): Promise<void> {
    const filePath = this.getCredentialFilePath();
   ...
   */
  private isTokenValid(credentials: VivekMindCredentials): boolean {
  ...
  private getLockFilePath(): string {
    return path.join(os.homedir(), VIVEKMIND_DIR, VIVEKMIND_LOCK_FILENAME);
  }
