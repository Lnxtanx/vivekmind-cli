...
const debugLogger = createDebugLogger('VIVEKMIND_OAUTH');

// OAuth Endpoints
const VIVEKMIND_OAUTH_BASE_URL = 'https://chat.vivekmind.ai';

const VIVEKMIND_OAUTH_DEVICE_CODE_ENDPOINT = `${VIVEKMIND_OAUTH_BASE_URL}/api/v1/oauth2/device/code`;
const VIVEKMIND_OAUTH_TOKEN_ENDPOINT = `${VIVEKMIND_OAUTH_BASE_URL}/api/v1/oauth2/token`;

// OAuth Client Configuration
const VIVEKMIND_OAUTH_CLIENT_ID = 'f0304373b74a44d2b584a3fb70ca9e56';

const VIVEKMIND_OAUTH_SCOPE = 'openid profile email model.completion';
const VIVEKMIND_OAUTH_GRANT_TYPE = 'urn:ietf:params:oauth:grant-type:device_code';

// File System Configuration
const VIVEKMIND_DIR = '.vivekmind';
const VIVEKMIND_CREDENTIAL_FILENAME = 'oauth_creds.json';
...
/**
 * VivekMind OAuth2 credentials interface
 */
export interface VivekMindCredentials {
  access_token?: string;
...
/**
 * VivekMind OAuth2 client interface
 */
export interface IVivekMindOAuth2Client {
  setCredentials(credentials: VivekMindCredentials): void;
  getCredentials(): VivekMindCredentials;
...
/**
 * VivekMind OAuth2 client implementation
 */
export class VivekMindOAuth2Client implements IVivekMindOAuth2Client {
  private credentials: VivekMindCredentials = {};
  ...
  async requestDeviceAuthorization(options: {
    ...
    const bodyData = {
      client_id: VIVEKMIND_OAUTH_CLIENT_ID,
      scope: options.scope,
      code_challenge: options.code_challenge,
      code_challenge_method: options.code_challenge_method,
    };

    const response = await fetch(VIVEKMIND_OAUTH_DEVICE_CODE_ENDPOINT, {
      method: 'POST',
      headers: {
  ...
  async pollDeviceToken(options: {
    ...
    const bodyData = {
      grant_type: VIVEKMIND_OAUTH_GRANT_TYPE,
      client_id: VIVEKMIND_OAUTH_CLIENT_ID,
      device_code: options.device_code,
      code_verifier: options.code_verifier,
    };

    const response = await fetch(VIVEKMIND_OAUTH_TOKEN_ENDPOINT, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/x-www-form-urlencoded',
        Accept: 'application/json',
  ...
  async refreshAccessToken(): Promise<TokenRefreshResponse> {
    ...
    try {
      ...
      throw new Error(
        `VivekMind OAuth refresh returned invalid JSON: ${responseText || '(empty response body)'}`,
      );
    }
    ...
    const tokens: VivekMindCredentials = {
...
export enum VivekMindOAuth2Event {
  AuthUri = 'auth-uri',
...
/**
 * Global event emitter instance for VivekMindOAuth2 authentication events
 */
export const vivekmindOAuth2Events = new EventEmitter();

export async function getVivekMindOAuthClient(
  config: Config,
  options?: { requireCachedCredentials?: boolean },
): Promise<VivekMindOAuth2Client> {
  ...
  try {
    ...
    if (options?.requireCachedCredentials) {
      throw new Error(
        'VivekMind OAuth credentials expired. Please use /auth to re-authenticate.',
      );
    }
    ...
    const result = await authWithVivekMindDeviceFlow(client, config);
    if (!result.success) {
      ...
      const errorMessage =
        ...
        (() => {
          switch (result.reason) {
            case 'timeout':
              return 'VivekMind OAuth authentication timed out';
            case 'cancelled':
              return 'VivekMind OAuth authentication was cancelled by user';
            case 'rate_limit':
              return 'Too many request for VivekMind OAuth authentication, please try again later.';
            case 'error':
            default:
              return 'VivekMind OAuth authentication failed';
          }
        })();
...
function showFallbackMessage(verificationUriComplete: string): void {
  const title = 'VivekMind OAuth Device Authorization';
...
async function authWithVivekMindDeviceFlow(
  client: VivekMindOAuth2Client,
  config: Config,
): Promise<AuthResult> {
  ...
  vivekmindOAuth2Events.once(VivekMindOAuth2Event.AuthCancel, cancelHandler);
  ...
  const checkCancellation = (): AuthResult | null => {
    ...
    debugLogger.debug('\n' + message);
    vivekmindOAuth2Events.emit(VivekMindOAuth2Event.AuthProgress, 'error', message);
  ...
  const emitAuthProgress = (
    ...
    vivekmindOAuth2Events.emit(VivekMindOAuth2Event.AuthProgress, status, message);
  };
  ...
  try {
    ...
    vivekmindOAuth2Events.emit(VivekMindOAuth2Event.AuthUri, deviceAuth);
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      ...
      try {
        ...
        if (isDeviceTokenSuccess(tokenResponse)) {
          ...
          // Convert to VivekMindCredentials format
          const credentials: VivekMindCredentials = {
          ...
          await cacheVivekMindCredentials(credentials);

          // IMPORTANT:
          // SharedTokenManager maintains an in-memory cache and throttles file checks.
          // If we only write the creds file here, a subsequent `getVivekMindOAuthClient()`
    ...
    vivekmindOAuth2Events.off(VivekMindOAuth2Event.AuthCancel, cancelHandler);
  }
}

async function cacheVivekMindCredentials(credentials: VivekMindCredentials) {
  const filePath = getVivekMindCachedCredentialPath();
  try {
...
/**
 * Clear cached VivekMind credentials from disk
 * This is useful when credentials have expired or need to be reset
 */
export async function clearVivekMindCredentials(): Promise<void> {
  try {
    const filePath = getVivekMindCachedCredentialPath();
    await fs.unlink(filePath);
    debugLogger.debug('Cached VivekMind credentials cleared successfully.');
  } catch (error: unknown) {
    ...
    debugLogger.warn(
      'Warning: Failed to clear cached VivekMind credentials:',
      error,
    );
  } finally {
...
function getVivekMindCachedCredentialPath(): string {
  return path.join(os.homedir(), VIVEKMIND_DIR, VIVEKMIND_CREDENTIAL_FILENAME);
}

export const clearCachedCredentialFile = clearVivekMindCredentials;
