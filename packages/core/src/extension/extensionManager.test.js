...
import { INSTALL_METADATA_FILENAME, EXTENSIONS_CONFIG_FILENAME, } from './variables.js';
import { VIVEKMIND_DIR } from '../config/storage.js';
import { ExtensionManager, SettingScope, validateName, getExtensionId, hashValue, } from './extensionManager.js';
const mockGit = {
    clone: vi.fn(),
    getRemotes: vi.fn(),
    fetch: vi.fn(),
...
function createExtension({ extensionsDir = 'extensions-dir', name = 'my-extension', version = '1.0.0', addContextFile = false, contextFileName = undefined, mcpServers = {}, installMetadata = undefined, } = {}) {
...
describe('extension tests', () => {
    ...
    beforeEach(() => {
        tempHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'vivekmind-test-home-'));
        tempWorkspaceDir = fs.mkdtempSync(path.join(tempHomeDir, 'vivekmind-test-workspace-'));
    ...
    describe('loadExtension', () => {
        ...
        it('should filter trust out of mcp servers', async () => {
            ...
            expect(extensions[0].config.mcpServers?.['test-server']?.trust).toBe(true);
        });
    ...
    describe('enableExtension / disableExtension', () => {
        ...
        it('should disable an extension at the workspace scope', async () => {
            ...
            const manager = createExtensionManager();
            await manager.refreshCache();
            await manager.disableExtension('my-extension', SettingScope.Workspace, tempWorkspaceDir);
        ...
        it('should throw an error if you request system scope', async () => {
            ...
            const manager = createExtensionManager();
            await manager.refreshCache();
            await expect(manager.disableExtension('my-extension', SettingScope.System)).rejects.toThrow('System and SystemDefaults scopes are not supported.');
        });
    ...
    describe('validateExtensionOverrides', () => {
        ...
        it('should log an error for unknown extensions', async () => {
            ...
            await manager.refreshCache();
            const extensions = manager.getLoadedExtensions();
            expect(() => manager.validateExtensionOverrides(extensions)).not.toThrow();
        });
    });
    ...
    describe('extensionManager utility functions', () => {
        describe('validateName', () => {
            ...
            it('should reject names with invalid characters', () => {
                expect(() => validateName('my extension')).toThrow('Invalid extension name');
        ...
        describe('getExtensionId', () => {
            ...
            it('should use hashed source for local install', () => {
                const config = { name: 'test-ext', version: '1.0.0' };
                const metadata = { type: 'local', source: '/path/to/ext' };
            ...
            it('should use source as-is for non-GitHub git URLs (e.g., GitLab)', () => {
                ...
                const id = getExtensionId(config, metadata);
                expect(id).toBe(hashValue('https://gitlab.company.com/team/extension-repo'));
            });
        });
    });
    describe('hooks loading and processing', () => {
        it('should load hooks from vivekmind-extension.json', async () => {
            ...
            fs.writeFileSync(path.join(extensionDir, EXTENSIONS_CONFIG_FILENAME), JSON.stringify(configWithHooks));
            const manager = createExtensionManager();
            await manager.refreshCache();
            const extensions = manager.getLoadedExtensions();
            expect(extensions).toHaveLength(1);
            ...
            expect(extensions[0].hooks['PreToolUse'][0].hooks[0].command).toBe('echo "hello"');
        });
        it('should load hooks from hooks/hooks.json when not in main config', async () => {
            const extensionDir = path.join(userExtensionsDir, 'hooks-from-file-extension');
            fs.mkdirSync(extensionDir, { recursive: true });
            ...
            fs.writeFileSync(path.join(extensionDir, EXTENSIONS_CONFIG_FILENAME), JSON.stringify(configWithoutHooks));
            ...
            expect(extensions[0].hooks['PostToolUse'][0].hooks[0].command).toBe(`echo "installed in ${extensionDir}"`);
        });
        it('should substitute ${CLAUDE_PLUGIN_ROOT} variable in hooks', async () => {
            ...
            expect(extensions[0].hooks).toBeDefined();
            expect(extensions[0].hooks['PreToolUse']).toHaveLength(1);
            expect(extensions[0].hooks['PreToolUse'][0].hooks[0].command).toBe(`${extensionDir}/scripts/setup.sh`);
        });
        it('should load hooks from config.hooks string path', async () => {
            const extensionDir = path.join(userExtensionsDir, 'hooks-from-config-path');
            fs.mkdirSync(extensionDir, { recursive: true });
            ...
            fs.writeFileSync(path.join(customHooksDir, 'hooks.json'), JSON.stringify(hooksJson));
            ...
            fs.writeFileSync(path.join(extensionDir, EXTENSIONS_CONFIG_FILENAME), JSON.stringify(configWithHooksPath));
            const manager = createExtensionManager();
            await manager.refreshCache();
            const extensions = manager.getLoadedExtensions();
            expect(extensions).toHaveLength(1);
            expect(extensions[0].hooks).toBeDefined();
            expect(extensions[0].hooks['PreToolUse']).toHaveLength(1);
            expect(extensions[0].hooks['PreToolUse'][0].hooks[0].command).toBe('echo "custom hooks path"');
        });
        it('should prefer config.hooks string path over hooks/hooks.json', async () => {
            const extensionDir = path.join(userExtensionsDir, 'hooks-prefer-config-path');
            fs.mkdirSync(extensionDir, { recursive: true });
            ...
            fs.mkdirSync(customHooksDir, { recursive: true });
            fs.writeFileSync(path.join(customHooksDir, 'my-hooks.json'), JSON.stringify({
                PreToolUse: [
                    {
            ...
            // Create vivekmind-extension.json with hooks as string path
            fs.writeFileSync(path.join(extensionDir, EXTENSIONS_CONFIG_FILENAME), JSON.stringify({
            ...
            expect(extensions).toHaveLength(1);
            expect(extensions[0].hooks).toBeDefined();
            expect(extensions[0].hooks['PreToolUse'][0].hooks[0].command).toBe('echo "config path"');
        });
        it('should substitute ${CLAUDE_PLUGIN_ROOT} in hooks file from config.hooks string path', async () => {
            const extensionDir = path.join(userExtensionsDir, 'hooks-var-from-config-path');
            fs.mkdirSync(extensionDir, { recursive: true });
