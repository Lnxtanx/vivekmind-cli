...
export async function checkForExtensionUpdate(extension, extensionManager) {
    ...
    if (installMetadata?.type === 'local') {
        ...
        try {
            ...
            debugLogger.error(`Failed to check for update for local extension "${extension.name}". Could not load extension from source path: ${installMetadata.source}. Error: ${getErrorMessage(e)}`);
            return ExtensionUpdateState.NOT_UPDATABLE;
        }
        if (!latestConfig) {
            debugLogger.error(`Failed to check for update for local extension "${extension.name}". Could not load extension from source path: ${installMetadata.source}`);
            return ExtensionUpdateState.NOT_UPDATABLE;
        }
...
export async function downloadFromGitHubRelease(installMetadata, destination) {
    ...
    try {
        ...
        // dir has a vivekmind extension configuration file (or gemini-extension.json
        ...
        if (entries.length === 2) {
            ...
            if (lonelyDir) {
                const hasVivekMindConfig = fs.existsSync(path.join(destination, lonelyDir.name, EXTENSIONS_CONFIG_FILENAME));
                const hasGeminiConfig = fs.existsSync(path.join(destination, lonelyDir.name, 'gemini-extension.json'));
                const hasMarketplaceConfig = fs.existsSync(path.join(destination, lonelyDir.name, '.claude-plugin/marketplace.json'));
                const hasClaudePluginConfig = fs.existsSync(path.join(destination, lonelyDir.name, '.claude-plugin/plugin.json'));
                if (hasVivekMindConfig ||
                    hasGeminiConfig ||
