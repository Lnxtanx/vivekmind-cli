...
import { hydrateString, substituteHookVariables, performVariableReplacement, } from './variables.js';
...
describe('substituteHookVariables', () => {
    it('should substitute ${CLAUDE_PLUGIN_ROOT} with the actual path in hooks', () => {
        ...
        expect(result['PreToolUse'][0].hooks[0].command).toBe('/path/to/plugin/scripts/setup.sh');
    });
    it('should handle multiple hooks with variables', () => {
        ...
        expect(result['PostToolUse'][0].hooks[0].command).toBe('/project/plugins/my-plugin/bin/init.sh');
        expect(result['PostToolUse'][1].hooks[0].command).toBe('chmod +x /project/plugins/my-plugin/bin/executable.sh');
    });
    it('should handle multiple event types with hooks', () => {
        const basePath = '/home/user/.vivekmind/extensions/my-extension';
        const hooks = {
            PreToolUse: [
                {
        ...
        expect(result).toBeDefined();
        expect(result['PreToolUse']).toHaveLength(1);
        expect(result['PreToolUse'][0].hooks[0].command).toBe('/home/user/.vivekmind/extensions/my-extension/scripts/pre-start.sh');
        expect(result['UserPromptSubmit']).toHaveLength(1);
        expect(result['UserPromptSubmit'][0].hooks[0]
            .command).toBe('/home/user/.vivekmind/extensions/my-extension/setup/install.py');
    });
    it('should not modify non-command hooks', () => {
            ...
            SessionStart: [
                {
                    ...
                    hooks: [
                        ...
                        {
                            type: 'non-command', // Non-command type won't be processed
        ...
        expect(result['SessionStart'][0].hooks[0].command).toBe('/path/to/extension/scripts/run.sh');
        expect(result['SessionStart'][0].hooks[1].command).toBe('${CLAUDE_PLUGIN_ROOT}/not-affected'); // Non-command type won't be processed
    });
...
describe('performVariableReplacement', () => {
    ...
    it('should replace .claude with .vivekmind in markdown files', () => {
        const extDir = path.join(testDir, 'ext');
        fs.mkdirSync(extDir, { recursive: true });
        const mdContent = [
            '---',
        ...
        expect(result).toContain('.vivekmind/loop.local.md');
        expect(result).toContain('rm .vivekmind/loop.local.md');
        expect(result).toContain('$HOME/.vivekmind/cache');
        expect(result).toContain('./.vivekmind/local');
    ...
    it('should replace .claude with .vivekmind in shell scripts', () => {
        const extDir = path.join(testDir, 'ext');
        fs.mkdirSync(extDir, { recursive: true });
        const shContent = [
            '#!/bin/bash',
        ...
        expect(result).toContain('~/.vivekmind/cache');
        expect(result).toContain('./.vivekmind/local');
        expect(result).toContain('.vivekmind/config');
        // Note: URLs are also being replaced in current implementation
        expect(result).toContain('https://example.com/.vivekmind/page');
    });
    it('should handle multiple markdown files', () => {
        const extDir = path.join(testDir, 'ext');
        fs.mkdirSync(extDir, { recursive: true });
        fs.mkdirSync(path.join(extDir, 'docs'), { recursive: true });
        fs.writeFileSync(path.join(extDir, 'README.md'), 'Path: `${CLAUDE_PLUGIN_ROOT}/readme`', 'utf-8');
        fs.writeFileSync(path.join(extDir, 'docs', 'guide.md'), 'Path: `${CLAUDE_PLUGIN_ROOT}/docs/guide`', 'utf-8');
        performVariableReplacement(extDir);
        const readme = fs.readFileSync(path.join(extDir, 'README.md'), 'utf-8');
        const guide = fs.readFileSync(path.join(extDir, 'docs', 'guide.md'), 'utf-8');
    ...
    it('should handle multiple shell script files', () => {
        ...
        fs.writeFileSync(path.join(extDir, 'setup.sh'), 'echo "${CLAUDE_PLUGIN_ROOT}/setup"', 'utf-8');
        fs.writeFileSync(path.join(extDir, 'scripts', 'helper.sh'), 'echo "${CLAUDE_PLUGIN_ROOT}/scripts/helper"', 'utf-8');
        performVariableReplacement(extDir);
        const setup = fs.readFileSync(path.join(extDir, 'setup.sh'), 'utf-8');
        const helper = fs.readFileSync(path.join(extDir, 'scripts', 'helper.sh'), 'utf-8');
    ...
    it('should handle directories with no matching files', () => {
        ...
        expect(fs.readFileSync(path.join(extDir, 'file.txt'), 'utf-8')).toBe('content');
    });
