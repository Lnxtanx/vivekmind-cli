...
/**
 * Converter for Claude Code plugins to VivekMind format.
 */
...
/**
 * Converts a Claude agent config to VivekMind subagent format.
 * @param claudeAgent Claude agent configuration
 * @returns Converted agent config compatible with VivekMind SubagentConfig
 */
export function convertClaudeAgentConfig(
  claudeAgent: ClaudeAgentConfig,
): Record<string, unknown> {
  // Base config with required fields
  const vivekmindAgent: Record<string, unknown> = {
  ...
  if (claudeAgent.permissionMode) {
    const claudeToVivekMindMode: Record<string, string> = {
      default: 'default',
      plan: 'plan',
    ...
    const mapped =
      claudeToVivekMindMode[claudeAgent.permissionMode] ??
...
async function convertAgentFiles(agentsDir: string): Promise<void> {
  ...
  for (const file of files) {
    ...
    try {
      ...
      const systemPrompt = (vivekmindAgent['systemPrompt'] as string) || body.trim();
      const newContent = `---
${newYaml}
---
 ...
 */
export function convertClaudeToVivekMindConfig(
...
export async function convertClaudePluginPackage(
  ...
  try {
    ...
    fs.writeFileSync(
      vivekmindConfigPath,
      JSON.stringify(vivekmindConfig, null, 2),
      'utf-8',
    );
