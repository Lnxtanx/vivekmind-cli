...
export const EXTENSIONS_DIRECTORY_NAME = path.join(VIVEKMIND_DIR, 'extensions');
export const EXTENSIONS_CONFIG_FILENAME = 'vivekmind-extension.json';
 ...
 */
export function performVariableReplacement(extensionPath) {
    ...
    try {
        ...
        for (const file of mdFiles) {
            ...
            try {
                ...
                const updatedMdContent = syntaxUpdatedContent.replace(/(\$\{?HOME\}?\/|~\/)?\.claude(\/|$)/g, '$1.vivekmind$2');
    ...
    try {
        ...
        for (const file of scriptFiles) {
            ...
            try {
                ...
                // Only match path references (e.g., ~/.claude/, $HOME/.claude, ./.claude/)
                // Avoid matching URLs, comments, or string literals containing .claude
                const finalScriptContent = adaptedScriptContent.replace(/(\$\{?HOME\}?\/|~\/)?\.claude(\/|$)/g, '$1.vivekmind$2');
                // Only write if content was actually changed
                if (finalScriptContent !== content) {
                    fs.writeFileSync(filePath, finalScriptContent, 'utf8');
                    debugLogger.debug(`Updated transcript format and replaced .claude with .vivekmind in shell script: ${filePath}`);
                }
            }
            catch (error) {
