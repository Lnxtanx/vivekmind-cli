...
describe('ExtensionStorage', () => {
    ...
    it('should return the correct extension directory', () => {
        const expectedDir = path.join(mockHomeDir, '.vivekmind', 'extensions', extensionName);
        expect(storage.getExtensionDir()).toBe(expectedDir);
    });
    it('should return the correct config path', () => {
        const expectedPath = path.join(mockHomeDir, '.vivekmind', 'extensions', extensionName, EXTENSIONS_CONFIG_FILENAME);
        expect(storage.getConfigPath()).toBe(expectedPath);
    });
    it('should return the correct env file path', () => {
        const expectedPath = path.join(mockHomeDir, '.vivekmind', 'extensions', extensionName, EXTENSION_SETTINGS_FILENAME);
    ...
    it('should create a temporary directory', async () => {
        const mockTmpDir = '/tmp/vivekmind-extension-123';
        vi.mocked(fs.promises.mkdtemp).mockResolvedValue(mockTmpDir);
        vi.mocked(os.tmpdir).mockReturnValue('/tmp');
        const result = await ExtensionStorage.createTmpDir();
        expect(fs.promises.mkdtemp).toHaveBeenCalledWith(path.join('/tmp', 'vivekmind-extension'));
