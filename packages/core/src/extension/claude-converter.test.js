...
describe('isClaudePluginConfig', () => {
    it('should identify Claude plugin directory', () => {
        ...
        expect(typeof isClaudePluginConfig(extensionDir, marketplace)).toBe('boolean');
    });
});
describe('convertClaudePluginPackage', () => {
    ...
    it('should only collect specified skills when config provides explicit list', async () => {
        ...
        for (const skill of allSkills) {
            ...
            fs.writeFileSync(path.join(skillDir, 'SKILL.md'), `# ${skill} skill`, 'utf-8');
            fs.writeFileSync(path.join(skillDir, 'index.js'), `module.exports = {};`, 'utf-8');
        }
        ...
        fs.writeFileSync(path.join(marketplaceDir, 'marketplace.json'), JSON.stringify(marketplaceConfig, null, 2), 'utf-8');
        // Execute: Convert the plugin
        const result = await convertClaudePluginPackage(pluginSourceDir, 'document-skills');
    ...
    it('should use all skills from folder when config does not specify skills', async () => {
        ...
        fs.writeFileSync(path.join(marketplaceDir, 'marketplace.json'), JSON.stringify(marketplaceConfig, null, 2), 'utf-8');
        // Execute: Convert the plugin
        const result = await convertClaudePluginPackage(pluginSourceDir, 'default-skills');
    ...
    it('should preserve directory structure when collecting skills', async () => {
        ...
        fs.writeFileSync(path.join(skillsDir, 'nested-skill', 'SKILL.md'), '# Nested Skill', 'utf-8');
        fs.writeFileSync(path.join(nestedSkillDir, 'helper.js'), 'module.exports = {};', 'utf-8');
        ...
        fs.writeFileSync(path.join(marketplaceDir, 'marketplace.json'), JSON.stringify(marketplaceConfig, null, 2), 'utf-8');
        // Execute: Convert the plugin
        const result = await convertClaudePluginPackage(pluginSourceDir, 'nested-plugin');
        ...
        expect(fs.existsSync(path.join(nestedSkillPath, 'subdir', 'helper.js'))).toBe(true);
        // Clean up converted directory
        fs.rmSync(result.convertedDir, { recursive: true, force: true });
    });
    it('should successfully convert agent files with Windows CRLF endings', async () => {
        ...
        fs.writeFileSync(path.join(agentsDir, 'agent.md'), crlfAgentContent, 'utf-8');
        ...
        fs.writeFileSync(path.join(marketplaceDir, 'marketplace.json'), JSON.stringify(marketplaceConfig, null, 2), 'utf-8');
        // Act: Convert
        const result = await convertClaudePluginPackage(pluginSourceDir, 'crlf-agents-plugin');
        // Verify: agent file was properly parsed and converted into .vivekmind/agents folder structure
        ...
        const convertedContent = fs.readFileSync(path.join(convertedAgentsDir, 'agent.md'), 'utf-8');
    ...
    it('should convert hooks from Claude plugin format to VivekMind format with variable substitution', async () => {
        ...
        fs.writeFileSync(path.join(hooksDir, 'hooks.json'), JSON.stringify(hooksJson), 'utf-8');
        // Create marketplace.json
        const marketplaceDir = path.join(pluginSourceDir, '.claude-plugin');
        fs.mkdirSync(marketplaceDir, { recursive: true });
        const marketplaceConfig = {
        ...
        fs.writeFileSync(path.join(marketplaceDir, 'marketplace.json'), JSON.stringify(marketplaceConfig, null, 2), 'utf-8');
        // Execute: Convert the plugin
        const result = await convertClaudePluginPackage(pluginSourceDir, 'hooks-plugin');
        ...
        expect(result.config.hooks['PostToolUse'][0].hooks[0]
            .command).toBe(`${pluginSourceDir}/scripts/post-install.sh`);
        // Clean up converted directory
        fs.rmSync(result.convertedDir, { recursive: true, force: true });
    });
});
describe('performVariableReplacement for Claude extensions', () => {
    ...
    it('should replace .claude with .vivekmind in shell scripts', () => {
      ...
      LOCAL_DIR="./.claude/local"`;
        ...
        expect(result).toContain('$HOME/.vivekmind/config');
...
//# sourceMappingURL=claude-converter.test.js.map