...
export class IdeClient {
    ...
    async connect() {
        if (!this.currentIde) {
            this.setState(IDEConnectionStatus.Disconnected, `IDE integration is not supported in your current environment. To use this feature, run VivekMind in one of these supported IDEs: VS Code or VS Code forks`, false);
            return;
        }
        ...
        const workspacePath = this.connectionConfig?.workspacePath ??
            process.env['VIVEKMIND_CODE_IDE_WORKSPACE_PATH'];
    ...
    getPortFromEnv() {
        const port = process.env['VIVEKMIND_CODE_IDE_SERVER_PORT'];
        if (!port) {
            return undefined;
        }
        return port;
    ...
    getStdioConfigFromEnv() {
        const command = process.env['VIVEKMIND_CODE_IDE_SERVER_STDIO_COMMAND'];
        if (!command) {
            return undefined;
        }
        const argsStr = process.env['VIVEKMIND_CODE_IDE_SERVER_STDIO_ARGS'];
        let args = [];
        if (argsStr) {
            try {
                ...
                if (Array.isArray(parsedArgs)) {
                    ...
                    debugLogger.error('VIVEKMIND_CODE_IDE_SERVER_STDIO_ARGS must be a JSON array string.');
                }
            }
            catch (e) {
                debugLogger.error('Failed to parse VIVEKMIND_CODE_IDE_SERVER_STDIO_ARGS:', e);
            }
        }
        return { command, args };
    }
    async getConnectionConfigFromFile() {
        ...
        return configs.find((config) => config.workspacePath !== undefined &&
    ...
    async getLegacyConnectionConfig(portFromEnv) {
        if (this.ideProcessInfo) {
            try {
                const portFile = path.join(os.tmpdir(), `vivekmind-ide-server-${this.ideProcessInfo.pid}.json`);
        ...
        if (portFromEnv) {
            try {
                const portFile = path.join(os.tmpdir(), `vivekmind-ide-server-${portFromEnv}.json`);
                const portFileContents = await fs.promises.readFile(portFile, 'utf8');
                return JSON.parse(portFileContents);
            }
            catch (_) {
