...
import { IdeClient, IDEConnectionStatus, getIdeServerHost, _resetCachedIdeServerHost, } from './ide-client.js';
...
describe('IdeClient', () => {
    ...
    describe('connect', () => {
        it('should connect using HTTP when port is provided in config file', async () => {
            ...
            expect(fs.promises.readFile).toHaveBeenCalledWith(path.join('/home/test', '.vivekmind', 'ide', '8080.lock'), 'utf8');
        ...
        it('should fall back to host.docker.internal when localhost fails in container', async () => {
            ...
            vi.mocked(fs.promises.readFile).mockRejectedValue(new Error('File not found'));
            vi.mocked(fs.promises.readdir).mockResolvedValue([]);
            vi.mocked(fs.existsSync).mockImplementation((filePath) => filePath === '/.dockerenv');
            dns.lookup.mockImplementation((_hostname, callback) => {
            ...
            expect(StreamableHTTPClientTransport).toHaveBeenNthCalledWith(1, new URL('http://127.0.0.1:9090/mcp'), expect.any(Object));
            // In a container, host.docker.internal is used as fallback.
            expect(StreamableHTTPClientTransport).toHaveBeenNthCalledWith(2, new URL('http://host.docker.internal:9090/mcp'), expect.any(Object));
            expect(ideClient.getConnectionStatus().status).toBe(IDEConnectionStatus.Connected);
            delete process.env['VIVEKMIND_CODE_IDE_SERVER_PORT'];
        });
        it('should try a newer lock-file port when the configured port is stale', async () => {
            ...
            vi.mocked(fs.promises.readdir).mockResolvedValue(['1111.lock', '2222.lock']);
            ...
            vi.mocked(fs.existsSync).mockImplementation((filePath) => String(filePath) === '/test/workspace');
            ...
            const ideClient = await IdeClient.getInstance();
            await ideClient.connect();
            expect(StreamableHTTPClientTransport).toHaveBeenNthCalledWith(1, new URL('http://127.0.0.1:1111/mcp'), expect.objectContaining({
                requestInit: {
                    headers: {
            ...
            expect(StreamableHTTPClientTransport).toHaveBeenNthCalledWith(2, new URL('http://127.0.0.1:2222/mcp'), expect.objectContaining({
                requestInit: {
                    headers: {
    ...
    describe('getConnectionConfigFromFile', () => {
        it('should return config from the env port lock file if it exists', async () => {
            ...
            expect(fs.promises.readFile).toHaveBeenCalledWith(path.join('/home/test', '.vivekmind', 'ide', '1234.lock'), 'utf8');
            delete process.env['VIVEKMIND_CODE_IDE_SERVER_PORT'];
        });
        ...
        it('should read legacy pid config when available', async () => {
            ...
            const result = await ideClient.getConnectionConfigFromFile();
            expect(result).toEqual(config);
            expect(fs.promises.readFile).toHaveBeenCalledWith(path.join('/tmp', 'vivekmind-ide-server-12345.json'), 'utf8');
        });
        it('should fall back to legacy port file when pid file is missing', async () => {
            ...
            expect(fs.promises.readFile).toHaveBeenCalledWith(path.join('/tmp', 'vivekmind-ide-server-12345.json'), 'utf8');
            expect(fs.promises.readFile).toHaveBeenCalledWith(path.join('/tmp', 'vivekmind-ide-server-2222.json'), 'utf8');
            delete process.env['VIVEKMIND_CODE_IDE_SERVER_PORT'];
        });
        ...
        it('should keep a live lock file even when it is older than 7 days', async () => {
            ...
            vi.mocked(fs.promises.readdir).mockResolvedValue(['1000.lock']);
            vi.mocked(fs.promises.stat).mockResolvedValue({ mtimeMs: oldTime });
        ...
        it('should keep incomplete old lock files when there is no stronger stale signal', async () => {
            ...
            vi.mocked(fs.promises.readdir).mockResolvedValue(['1000.lock', '2000.lock']);
            vi.mocked(fs.promises.stat).mockImplementation(async (filePath) => {
                const file = String(filePath);
                return {
        ...
        it('should scan IDE lock directory when env and legacy config are unavailable', async () => {
            ...
            const result = await ideClient.getConnectionConfigFromFile();
            expect(result).toEqual(latestConfig);
            expect(fs.promises.readdir).toHaveBeenCalledWith(path.join('/home/test', '.vivekmind', 'ide'));
        });
...
describe('getIdeServerHost', () => {
    const dnsLookupMock = dns.lookup;
    function mockDnsResolvable(reachable) {
        dnsLookupMock.mockImplementation((_hostname, callback) => {
            if (reachable) {
                callback(null);
            }
            else {
    ...
    it('should return host.docker.internal when in a container and the host is reachable', async () => {
        ...
        expect(dnsLookupMock).toHaveBeenCalledWith('host.docker.internal', expect.any(Function));
    });
    ...
    it('should detect container via /run/.containerenv', async () => {
        vi.mocked(fs.existsSync).mockImplementation((filePath) => filePath === '/run/.containerenv');
        mockDnsResolvable(true);
        const host = await getIdeServerHost();
        expect(host).toBe('host.docker.internal');
    });
