...
describe('managed auto-memory lifecycle integration', () => {
    ...
    beforeEach(async () => {
        ...
        vi.clearAllMocks();
        extractionCount = 0;
        vi.mocked(runAutoMemoryExtractionByAgent).mockImplementation(async (_config, root) => {
            extractionCount += 1;
            const topic = extractionCount > 1 ? 'reference' : 'user';
            const relativePath = topic === 'reference'
            ...
            const description = topic === 'reference'
        ...
        vi.mocked(planManagedAutoMemoryDreamByAgent).mockResolvedValue({
            ...
            filesTouched: [
                getAutoMemoryFilePath(projectRoot, path.join('user', 'terse-responses.md')),
                getAutoMemoryFilePath(projectRoot, path.join('reference', 'latency-dashboard.md')),
            ],
        });
    });
    afterEach(async () => {
    ...
    it('supports a durable memory lifecycle across extraction, recall, and dream', async () => {
        ...
        const projectPath = getAutoMemoryFilePath(projectRoot, path.join('project', 'latency-dashboard.md'));
        ...
        const duplicateUserPath = getAutoMemoryFilePath(projectRoot, path.join('user', 'terse-duplicate.md'));
        await fs.mkdir(path.dirname(duplicateUserPath), { recursive: true });
        await fs.writeFile(duplicateUserPath, [
            '---',
            'type: user',
        ...
        await rebuildManagedAutoMemoryIndex(projectRoot);
        const dreamResult = await runManagedAutoMemoryDream(projectRoot, new Date('2026-04-01T03:00:00.000Z'), mockConfig);
        ...
        const recall = await resolveRelevantAutoMemoryPromptForQuery(projectRoot, 'Check the latency dashboard and use a terse answer.');
...
//# sourceMappingURL=memoryLifecycle.integration.test.js.map