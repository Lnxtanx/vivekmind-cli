...
import { getAutoMemoryConsolidationLockPath, getAutoMemoryExtractCursorPath, getAutoMemoryIndexPath, getAutoMemoryMetadataPath, getAutoMemoryRoot, getAutoMemoryTopicPath, } from './paths.js';
import { createDefaultAutoMemoryIndex, createDefaultAutoMemoryMetadata, ensureAutoMemoryScaffold, readAutoMemoryIndex, } from './store.js';
describe('auto-memory storage scaffold', () => {
    ...
    it('builds stable auto-memory paths under project .vivekmind directory', () => {
        expect(getAutoMemoryRoot(projectRoot)).toBe(path.join(projectRoot, '.vivekmind', 'memory'));
        expect(getAutoMemoryIndexPath(projectRoot)).toBe(path.join(projectRoot, '.vivekmind', 'memory', 'MEMORY.md'));
        expect(getAutoMemoryMetadataPath(projectRoot)).toBe(path.join(projectRoot, '.vivekmind', 'meta.json'));
        expect(getAutoMemoryExtractCursorPath(projectRoot)).toBe(path.join(projectRoot, '.vivekmind', 'extract-cursor.json'));
        expect(getAutoMemoryConsolidationLockPath(projectRoot)).toBe(path.join(projectRoot, '.vivekmind', 'consolidation.lock'));
        expect(getAutoMemoryTopicPath(projectRoot, 'feedback')).toBe(path.join(projectRoot, '.vivekmind', 'memory', 'feedback.md'));
    });
    it('creates a complete managed auto-memory scaffold', async () => {
        ...
        const metadata = JSON.parse(await fs.readFile(getAutoMemoryMetadataPath(projectRoot), 'utf-8'));
        expect(metadata).toEqual(createDefaultAutoMemoryMetadata(now));
        const cursor = JSON.parse(await fs.readFile(getAutoMemoryExtractCursorPath(projectRoot), 'utf-8'));
    ...
    it('is idempotent and preserves existing index content', async () => {
        ...
        await expect(fs.readFile(getAutoMemoryIndexPath(projectRoot), 'utf-8')).resolves.toBe(customIndex);
    });
