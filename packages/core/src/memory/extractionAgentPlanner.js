...
function buildTaskPrompt(memoryRoot, topicSummaries) {
    return [
...
export async function runAutoMemoryExtractionByAgent(config, projectRoot) {
    ...
    const touchedTopics = touchedTopicsFromFilePaths(result.filesTouched, projectRoot);
    return {
        touchedTopics,
        systemMessage: touchedTopics.length > 0
