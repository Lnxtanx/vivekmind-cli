...
describe('auto-memory extraction with agent planner', () => {
    ...
    it('uses the forked-agent execution path when config is provided', async () => {
        ...
        expect(runAutoMemoryExtractionByAgent).toHaveBeenCalledWith(mockConfig, projectRoot);
        const docs = await scanAutoMemoryTopicDocuments(projectRoot);
        expect(docs.find((doc) => doc.type === 'user')?.body).toContain('User prefers terse responses.');
    });
});
//# sourceMappingURL=extractAgent.test.js.map