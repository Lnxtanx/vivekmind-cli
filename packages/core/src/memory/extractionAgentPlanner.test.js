...
describe('runAutoMemoryExtractionByAgent', () => {
    ...
    it('throws when getCacheSafeParams returns null', async () => {
        ...
        await expect(runAutoMemoryExtractionByAgent(mockConfig, '/tmp')).rejects.toThrow('no cache-safe params');
    });
    it('throws when the agent fails to complete', async () => {
        ...
        await expect(runAutoMemoryExtractionByAgent(mockConfig, '/tmp/project')).rejects.toThrow('timeout');
    });
    it('ignores non-memory file paths in filesTouched', async () => {
        ...
        const result = await runAutoMemoryExtractionByAgent(mockConfig, '/tmp');
        expect(result.touchedTopics).toEqual(expect.arrayContaining(['project', 'reference']));
        expect(result.touchedTopics).not.toContain('user');
    });
});
//# sourceMappingURL=extractionAgentPlanner.test.js.map