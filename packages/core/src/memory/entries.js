 ...
 */
export function getAutoMemoryBodyHeading(body) {
    return (body
        .split('\n')
        .map((line) => line.trim())
 ...
 * ```
 */
export function parseAutoMemoryEntries(body) {
    const entries = [];
    let current = null;
    for (const rawLine of body.split('\n')) {
        ...
        if (current) {
            const indentedMatch = rawLine.match(/^[\t ]{2,}(?:[-*][\t ]+)?(Why|How to apply|How_to_apply):[\t ]*(\S.*)$/i);
            if (indentedMatch) {
        ...
        const topLevelMatch = trimmed.match(/^(?:\*\*)?(Why|How to apply|How_to_apply)(?:\*\*)?:[ \t]*(\S.*)$/i);
        if (topLevelMatch) {
...
export function renderAutoMemoryBody(_heading, entries) {
    if (entries.length === 0) {
...
export function mergeAutoMemoryEntry(current, incoming) {
    return {
...
export function buildAutoMemoryEntrySearchText(entry) {
