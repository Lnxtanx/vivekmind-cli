...
export async function resolveRelevantAutoMemoryPromptForQuery(projectRoot, query, options = {}) {
    const t0 = Date.now();
    const docs = filterExcludedAutoMemoryDocuments(await scanAutoMemoryTopicDocuments(projectRoot), options.excludedFilePaths);
    ...
    if (options.config) {
        try {
            const selectedDocs = await selectRelevantAutoMemoryDocumentsByModel(options.config, query, docs, limit, options.recentTools ?? []);
