...
describe('selectRelevantAutoMemoryDocumentsByModel', () => {
    ...
    it('returns documents chosen by the side-query selector', async () => {
        ...
        const selected = await selectRelevantAutoMemoryDocumentsByModel(mockConfig, 'check the latency dashboard', docs, 2);
        expect(selected).toEqual([docs[1]]);
        expect(runSideQuery).toHaveBeenCalledWith(mockConfig, expect.objectContaining({
            purpose: 'auto-memory-recall',
            config: { temperature: 0 },
        }));
    });
    it('returns an empty list for empty query or no docs', async () => {
        await expect(selectRelevantAutoMemoryDocumentsByModel(mockConfig, '   ', docs, 2)).resolves.toEqual([]);
        await expect(selectRelevantAutoMemoryDocumentsByModel(mockConfig, 'hello', [], 2)).resolves.toEqual([]);
    ...
    it('throws when selector returns unknown relative paths', async () => {
        ...
        await expect(selectRelevantAutoMemoryDocumentsByModel(mockConfig, 'check memory', docs, 2)).rejects.toThrow('Recall selector returned unknown relative path');
    });
});
//# sourceMappingURL=relevanceSelector.test.js.map