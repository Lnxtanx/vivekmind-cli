...
async function listIndexedForgetCandidates(projectRoot) {
    ...
    for (const doc of docs) {
        ...
        for (let i = 0; i < entries.length; i++) {
            ...
            candidates.push({
                ...
                id: entries.length === 1 ? doc.relativePath : `${doc.relativePath}:${i}`,
                topic: doc.type,
...
async function selectByModel(candidates, query, config, limit) {
...
export async function forgetManagedAutoMemoryMatches(projectRoot, matches, now = new Date()) {
    if (matches.length === 0) {
        return {
            query: '',
            removedEntries: [],
    ...
    for (const [filePath, fileMatches] of matchesByFile) {
        try {
            ...
            const matchedSummaries = new Set(fileMatches.map((m) => m.summary.toLowerCase()));
            const kept = allEntries.filter((e) => !matchedSummaries.has(e.summary.toLowerCase()));
            if (kept.length === 0) {
                await fs.unlink(filePath);
            }
            else {
...
export async function forgetManagedAutoMemoryEntries(projectRoot, query, options = {}, now = new Date()) {
    ...
    const result = await forgetManagedAutoMemoryMatches(projectRoot, selection.matches, now);
