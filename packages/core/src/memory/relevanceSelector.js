...
export async function selectRelevantAutoMemoryDocumentsByModel(config, query, docs, limit, recentTools = []) {
    ...
    const response = await runSideQuery(config, {
        ...
        validate: (value) => {
            ...
            if (value.selected_memories.some((relativePath) => !validRelativePaths.has(relativePath))) {
