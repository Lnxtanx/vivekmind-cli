...
import { getAutoMemoryMetadataPath, getAutoMemoryConsolidationLockPath, clearAutoMemoryRootCache, } from './paths.js';
// ─── Mocks ────────────────────────────────────────────────────────────────────
vi.mock('./extract.js', () => ({
...
describe('MemoryManager', () => {
    ...
    describe('drain()', () => {
        ...
        it('resolves false when drain times out while a task is in-flight', async () => {
            const mgr = new MemoryManager();
            let resolveExtract;
            vi.mocked(runAutoMemoryExtract).mockReturnValue(new Promise((resolve) => {
    ...
    describe('scheduleExtract()', () => {
        ...
        beforeEach(async () => {
            vi.resetAllMocks();
            process.env['VIVEKMIND_CODE_MEMORY_LOCAL'] = '1';
        ...
        afterEach(async () => {
            delete process.env['VIVEKMIND_CODE_MEMORY_LOCAL'];
            clearAutoMemoryRootCache();
                        ...
                        parts: [
                            {
                                functionCall: {
                                    name: 'write_file',
                                    args: {
                                        file_path: `${projectRoot}/.vivekmind/memory/user/test.md`,
                                    },
                                },
                            },
                        ],
    ...
    describe('scheduleDream()', () => {
        ...
        it('skips when min_hours has not elapsed', async () => {
            ...
            metadata['lastDreamAt'] = new Date('2026-04-01T09:00:00.000Z').toISOString();
        ...
        it('schedules when all conditions are met, releases lock, and records metadata', async () => {
            ...
            await expect(fs.access(getAutoMemoryConsolidationLockPath(projectRoot))).rejects.toThrow();
            // Metadata must be updated
            const meta = JSON.parse(await fs.readFile(getAutoMemoryMetadataPath(projectRoot), 'utf-8'));
