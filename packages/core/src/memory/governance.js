...
async function listGovernanceEntries(projectRoot) {
...
function classifyExpectedTopic(summary) {
...
function maybeConflict(a, b) {
    const pairChecks = [
        [/\balways\b/i, /\bnever\b/i],
        [/\bterse|concise\b/i, /\bdetailed\b/i],
    ];
    return pairChecks.some(([left, right]) => (left.test(a) && right.test(b)) || (left.test(b) && right.test(a)));
}
function buildModelPrompt(entries) {
    return [
...
function buildHeuristicSuggestions(entries) {
    ...
    for (const entry of entries) {
        ...
        // Outdated markers
        if (/\b(today|now|currently|for this task|this session|temporary|temporarily)\b/i.test(entry.summary)) {
            suggestions.push({
                type: 'outdated',
                topic: entry.topic,
                summary: entry.summary,
        ...
        if (!entry.why || !entry.howToApply) {
            suggestions.push({
                ...
                topic: entry.topic,
                summary: entry.summary,
                rationale: 'This durable entry could be upgraded with why/howToApply metadata.',
            });
        }
    }
...
export async function reviewManagedAutoMemoryGovernance(projectRoot, options = {}) {
    ...
    if (options.config) {
        try {
            ...
            const response = await runSideQuery(options.config, {
                ...
                validate: (value) => {
                    ...
                    if (value.suggestions.some((suggestion) => suggestion.relatedEntryId &&
