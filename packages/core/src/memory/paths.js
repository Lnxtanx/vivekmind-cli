...
function findCanonicalGitRoot(startPath) {
    ...
    try {
        ...
        const worktreeGitDir = path.resolve(gitRoot, gitContent.slice('gitdir:'.length).trim());
        const commonDir = path.resolve(worktreeGitDir, fs.readFileSync(path.join(worktreeGitDir, 'commondir'), 'utf-8').trim());
        if (path.resolve(path.dirname(worktreeGitDir)) !==
            path.join(commonDir, 'worktrees')) {
            return gitRoot;
        }
        const backlink = fs.realpathSync(fs.readFileSync(path.join(worktreeGitDir, 'gitdir'), 'utf-8').trim());
...
export function getAutoMemoryRoot(projectRoot) {
    ...
    if (process.env['VIVEKMIND_CODE_MEMORY_LOCAL'] === '1') {
        ...
        result = path.join(getMemoryBaseDir(), 'projects', sanitizeCwd(canonicalRoot), AUTO_MEMORY_DIRNAME);
    }
