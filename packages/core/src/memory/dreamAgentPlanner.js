...
import { AUTO_MEMORY_INDEX_FILENAME, getAutoMemoryRoot, isAutoMemPath, } from './paths.js';
import { ToolNames } from '../tools/tool-names.js';
import { isShellCommandReadOnlyAST } from '../utils/shellAstParser.js';
import { escapeShellArg, getShellConfiguration, stripShellWrapper, } from '../utils/shell-utils.js';
const MAX_TURNS = 8;
const MAX_TIME_MINUTES = 5;
function isScopedTool(toolName) {
    return (toolName === ToolNames.SHELL ||
...
async function evaluateScopedDecision(ctx, projectRoot) {
    switch (ctx.toolName) {
        case ToolNames.SHELL: {
            ...
            const isReadOnly = await isShellCommandReadOnlyAST(stripShellWrapper(ctx.command));
...
function createMemoryScopedAgentConfig(config, projectRoot) {
...
export async function planManagedAutoMemoryDreamByAgent(config, projectRoot) {
