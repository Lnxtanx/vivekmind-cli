...
describe('managed auto-memory dream', () => {
    ...
    it('throws when config is missing', async () => {
        await expect(runManagedAutoMemoryDream(projectRoot)).rejects.toThrow('Managed auto-memory dream requires config');
    });
    it('returns touched topics derived from files touched by the dream agent', async () => {
        ...
        const result = await runManagedAutoMemoryDream(projectRoot, new Date('2026-04-02T00:00:00.000Z'), mockConfig);
        expect(result.touchedTopics).toEqual(expect.arrayContaining(['user', 'reference']));
        expect(result.dedupedEntries).toBe(0);
        expect(result.systemMessage).toContain('Managed auto-memory dream (agent):');
    });
    it('propagates planner failures', async () => {
        vi.mocked(planManagedAutoMemoryDreamByAgent).mockRejectedValue(new Error('agent failed'));
        await expect(runManagedAutoMemoryDream(projectRoot, new Date('2026-04-02T00:00:00.000Z'), mockConfig)).rejects.toThrow('agent failed');
    });
});
