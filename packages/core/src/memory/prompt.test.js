...
import { appendManagedAutoMemoryToUserMemory, buildManagedAutoMemoryPrompt, MAX_MANAGED_AUTO_MEMORY_INDEX_LINES, } from './prompt.js';
describe('managed auto-memory prompt helpers', () => {
    it('builds the memory mechanics prompt even when MEMORY.md is empty', () => {
        const prompt = buildManagedAutoMemoryPrompt('/tmp/project/.vivekmind/memory');
        expect(prompt).toContain('# auto memory');
        expect(prompt).toContain('persistent, file-based memory system');
        expect(prompt).toContain('/tmp/project/.vivekmind/memory');
        expect(prompt).toContain('Your MEMORY.md is currently empty');
    });
    it('embeds the current MEMORY.md index content', () => {
        const prompt = buildManagedAutoMemoryPrompt('/tmp/project/.vivekmind/memory', '- [User Memory](user/terse.md) — User prefers terse responses.');
        expect(prompt).toContain('## /tmp/project/.vivekmind/memory/MEMORY.md');
    ...
    it('returns only managed auto-memory when hierarchical memory is empty', () => {
        const result = appendManagedAutoMemoryToUserMemory('   ', '/tmp/project/.vivekmind/memory', '- [Reference](reference/grafana.md) — Grafana dashboard link.');
        expect(result).toContain('# auto memory');
        expect(result.startsWith('# auto memory')).toBe(true);
    });
    it('truncates oversized managed auto-memory index content', () => {
        const oversizedIndex = Array.from({ length: MAX_MANAGED_AUTO_MEMORY_INDEX_LINES + 50 }, (_, index) => `- [Memory ${index}](memory-${index}.md) — hook ${index}`).join('\n');
        const result = buildManagedAutoMemoryPrompt('/tmp/project/.vivekmind/memory', oversizedIndex);
