 ...
 */
export function registerSkillHooks(sessionHooksManager, sessionId, skill) {
    if (!skill.hooks) {
        ...
        for (const matcher of matchers) {
            ...
            for (const hook of matcher.hooks) {
                ...
                if (hook.type === HookType.Function) {
                    debugLogger.debug('Skipping function hook from skill (not supported in frontmatter)');
                    continue;
                }
                // Register the hook with skillRoot for environment variable
                const hookConfig = prepareHookConfig(hook, skill.skillRoot);
                sessionHooksManager.addSessionHook(sessionId, eventName, matcherPattern, hookConfig, { skillRoot: skill.skillRoot });
                registeredCount++;
                debugLogger.debug(`Registered hook for ${eventName} with matcher '${matcherPattern}' from skill '${skill.name}'`);
            }
        }
    }
    if (registeredCount > 0) {
        debugLogger.info(`Registered ${registeredCount} hooks from skill '${skill.name}'`);
    }
...
function prepareHookConfig(hook, skillRoot) {
    if (hook.type === 'command' && skillRoot) {
        // Add VIVEKMIND_SKILL_ROOT to environment variables
        return {
            ...hook,
            env: {
                ...hook.env,
                VIVEKMIND_SKILL_ROOT: skillRoot,
            },
        };
    }
    return hook;
 ...
 */
export function unregisterSkillHooks(sessionHooksManager, sessionId, skill) {
    if (!skill.hooks) {
        return 0;
    }
    // Note: Current implementation doesn't track hook IDs per skill
    // Session hooks are cleared when session ends
    debugLogger.debug(`Skill hooks for '${skill.name}' will be cleared with session`);
    return 0;
}
//# sourceMappingURL=registerSkillHooks.js.map