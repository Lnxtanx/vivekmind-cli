...
describe('HookRegistry', () => {
    ...
    describe('setHookEnabled', () => {
        it('should disable an enabled hook', async () => {
            ...
            const registry = new HookRegistry(mockConfig);
            await registry.initialize();
            expect(registry.getHooksForEvent(HookEventName.PreToolUse)).toHaveLength(1);
        ...
        it('should update all hooks with matching name', async () => {
            ...
            expect(registry.getHooksForEvent(HookEventName.PreToolUse)).toHaveLength(1);
            expect(registry.getHooksForEvent(HookEventName.PostToolUse)).toHaveLength(1);
            registry.setHookEnabled('same-name', false);
            expect(registry.getHooksForEvent(HookEventName.PreToolUse)).toHaveLength(0);
            expect(registry.getHooksForEvent(HookEventName.PostToolUse)).toHaveLength(0);
        });
    });
    describe('hook validation', () => {
        ...
        it('should skip invalid event names', async () => {
            ...
            await registry.initialize();
            expect(registry.getAllHooks()).toHaveLength(0);
            expect(mockFeedbackEmitter.emitFeedback).toHaveBeenCalledWith('warning', expect.stringContaining('Invalid hook event name'));
        });
    ...
    describe('hook metadata', () => {
        ...
        it('should add source to hook config', async () => {
            ...
            await registry.initialize();
            const hooks = registry.getAllHooks();
            expect(hooks[0].config.source).toBe(HooksConfigSource.User);
        });
    });
