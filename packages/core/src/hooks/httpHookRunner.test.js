...
describe('HttpHookRunner', () => {
    ...
    describe('execute', () => {
        it('should fail for URL not in whitelist', async () => {
            ...
            const result = await httpRunner.execute(config, HookEventName.PreToolUse, input);
        ...
        it('should fail for blocked URL (SSRF - link-local metadata)', async () => {
            ...
            const input = createMockInput();
            const result = await runner.execute(config, HookEventName.PreToolUse, input);
            expect(result.success).toBe(false);
        ...
        it('should handle aborted signal', async () => {
            ...
            const config = createMockConfig();
            const input = createMockInput();
            const result = await httpRunner.execute(config, HookEventName.PreToolUse, input, controller.signal);
            expect(result.success).toBe(false);
...
//# sourceMappingURL=httpHookRunner.test.js.map