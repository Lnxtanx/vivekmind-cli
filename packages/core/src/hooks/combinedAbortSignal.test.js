...
describe('createCombinedAbortSignal', () => {
    ...
    it('should abort when external signal is aborted', () => {
        ...
        const { signal, cleanup } = createCombinedAbortSignal(externalController.signal);
        expect(signal.aborted).toBe(false);
    ...
    it('should work with both external signal and timeout', async () => {
        const externalController = new AbortController();
        const { signal, cleanup } = createCombinedAbortSignal(externalController.signal, { timeoutMs: 200 });
    ...
    it('should timeout before external signal', async () => {
        const externalController = new AbortController();
        const { signal, cleanup } = createCombinedAbortSignal(externalController.signal, { timeoutMs: 50 });
        // Wait for timeout
        await new Promise((resolve) => setTimeout(resolve, 100));
        expect(signal.aborted).toBe(true);
...
//# sourceMappingURL=combinedAbortSignal.test.js.map