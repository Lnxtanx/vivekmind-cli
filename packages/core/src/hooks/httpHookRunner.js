...
/**
 * Maximum output length (10,000 characters as per VivekMind spec)
 */
...
export class HttpHookRunner {
    ...
    async execute(hookConfig, eventName, input, signal) {
        ...
        try {
            ...
            const { signal: combinedSignal, cleanup } = createCombinedAbortSignal(signal, { timeoutMs: timeout });
            try {
                ...
                // Per VivekMind spec: Non-2xx status is a non-blocking error
                ...
                if (fetchError instanceof Error &&
                    ...
                    // Timeout or abort is a non-blocking error per VivekMind spec
                ...
                // Connection failure is a non-blocking error per VivekMind spec
     ...
     * Per VivekMind spec: output is capped at 10,000 characters
     */
    ...
    normalizeOutput(json, eventName) {
        ...
        if ('systemMessage' in json && typeof json['systemMessage'] === 'string') {
            // Apply output length limit per VivekMind spec
