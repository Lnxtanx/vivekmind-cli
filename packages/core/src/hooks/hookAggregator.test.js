...
describe('HookAggregator', () => {
    ...
    describe('aggregateResults', () => {
        ...
        it('should aggregate successful results', () => {
            ...
            const result = aggregator.aggregateResults(results, HookEventName.PreToolUse);
            expect(result.success).toBe(true);
    ...
    describe('mergeWithOrLogic - PreToolUse', () => {
        ...
        it('should concatenate additionalContext', () => {
            ...
            const result = aggregator.aggregateResults(results, HookEventName.PreToolUse);
            expect(result.finalOutput?.hookSpecificOutput?.['additionalContext']).toBe('context 1\ncontext 2');
        });
        it('should preserve other hookSpecificOutput fields', () => {
            ...
            const result = aggregator.aggregateResults(results, HookEventName.PostToolUse);
            expect(result.finalOutput?.hookSpecificOutput?.['additionalContext']).toBe('ctx\nctx2');
        });
    });
    describe('mergePermissionRequestOutputs', () => {
        it('should prioritize deny over allow', () => {
            ...
            const result = aggregator.aggregateResults(results, HookEventName.PermissionRequest);
            // Use accessor to verify - this ensures output is consumable by PermissionRequestHookOutput
            const hookOutput = createHookOutput(HookEventName.PermissionRequest, result.finalOutput ?? {});
    ...
    describe('mergeSimple (default case)', () => {
        it('should use later values for simple fields', () => {
            ...
            const result = aggregator.aggregateResults(results, HookEventName.Notification);
        ...
        it('should concatenate additionalContext from multiple hooks', () => {
            ...
            expect(result.finalOutput?.hookSpecificOutput?.['additionalContext']).toBe('ctx1\nctx2');
            // otherField is overwritten (later value wins since it's not special-cased)
            expect(result.finalOutput?.hookSpecificOutput?.['otherField']).toBeUndefined();
        });
    });
    describe('createSpecificHookOutput', () => {
        it('should create PreToolUseHookOutput for PreToolUse', () => {
            ...
            expect(result.finalOutput).toBeDefined();
            expect(result.finalOutput.continue).toBe(true);
        });
        it('should create StopHookOutput for Stop', () => {
            const output = { stopReason: 'test' };
            const results = [
                {
                    hookConfig: { type: HookType.Command, command: 'echo test' },
                    eventName: HookEventName.Stop,
            ...
            const result = aggregator.aggregateResults(results, HookEventName.Stop);
            expect(result.finalOutput).toBeDefined();
            expect(result.finalOutput.stopReason).toBe('test');
        });
    ...
    describe('edge cases', () => {
        ...
        it('should handle single output', () => {
            const output = { decision: 'allow', reason: 'single' };
            const results = [
                {
                    hookConfig: { type: HookType.Command, command: 'echo test' },
                    eventName: HookEventName.PreToolUse,
        ...
        it('should handle outputs without hookSpecificOutput', () => {
            const outputs = [{ decision: 'allow' }, { reason: 'test' }];
            const results = outputs.map((output) => ({
                hookConfig: { type: HookType.Command, command: 'echo test' },
                eventName: HookEventName.PreToolUse,
                success: true,
    ...
    describe('SubagentStop - mergeWithOrLogic', () => {
        it('should use mergeWithOrLogic for SubagentStop event', () => {
            ...
            const result = aggregator.aggregateResults(results, HookEventName.SubagentStop);
            expect(result.finalOutput?.reason).toBe('first reason\nsecond reason');
        });
        ...
        it('should concatenate additionalContext for SubagentStop', () => {
            ...
            const result = aggregator.aggregateResults(results, HookEventName.SubagentStop);
            expect(result.finalOutput?.hookSpecificOutput?.['additionalContext']).toBe('context from hook 1\ncontext from hook 2');
        });
    ...
    describe('createSpecificHookOutput - SubagentStop', () => {
        ...
        it('should create StopHookOutput with isBlockingDecision for SubagentStop', () => {
            ...
            const hookOutput = createHookOutput(HookEventName.SubagentStop, result.finalOutput ?? {});
            expect(hookOutput.isBlockingDecision()).toBe(true);
            expect(hookOutput.getEffectiveReason()).toBe('Continue working on the task');
        });
    ...
    describe('StopFailure - fire-and-forget special handling', () => {
        it('should always return success true for StopFailure', () => {
            ...
            const result = aggregator.aggregateResults(results, HookEventName.StopFailure);
            expect(result.success).toBe(true);
        });
    ...
    describe('PostCompact - mergeSimple', () => {
        it('should use mergeSimple for PostCompact event', () => {
            ...
            const result = aggregator.aggregateResults(results, HookEventName.PostCompact);
        ...
        it('should handle single output for PostCompact', () => {
            ...
            const result = aggregator.aggregateResults(results, HookEventName.PostCompact);
            expect(result.finalOutput).toBeDefined();
            expect(result.finalOutput?.hookSpecificOutput?.['additionalContext']).toBe('single context');
        });
    });
});
