...
describe('FunctionHookRunner', () => {
    ...
    const createMockConfig = (callback, overrides = {}) => ({
    ...
    describe('execute', () => {
        it('should execute callback successfully', async () => {
            ...
            const result = await functionRunner.execute(config, HookEventName.PreToolUse, input);
        ...
        it('should handle timeout', async () => {
            const mockCallback = vi.fn().mockImplementation(() => new Promise((resolve) => {
        ...
        it('should handle abort signal', async () => {
            ...
            const config = createMockConfig(mockCallback);
            const input = createMockInput();
            const result = await functionRunner.execute(config, HookEventName.PreToolUse, input, { signal: controller.signal });
            expect(result.success).toBe(false);
        ...
        it('should reject invalid callback', async () => {
            const config = createMockConfig('not a function');
            const input = createMockInput();
            const result = await functionRunner.execute(config, HookEventName.PreToolUse, input);
            expect(result.success).toBe(false);
        ...
        it('should handle async callback with context', async () => {
            ...
            const config = createMockConfig(mockCallback);
            const input = createMockInput();
            const result = await functionRunner.execute(config, HookEventName.PreToolUse, input, { messages: [{ role: 'user' }] });
            expect(result.success).toBe(true);
            expect(mockCallback).toHaveBeenCalledTimes(1);
        });
    });
});
//# sourceMappingURL=functionHookRunner.test.js.map