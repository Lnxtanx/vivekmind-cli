...
import { HookEventName, HookType, HooksConfigSource, SessionStartSource, SessionEndReason, PermissionMode, AgentType, PreCompactTrigger, PostCompactTrigger, NotificationType, } from './types.js';
...
describe('HookEventHandler', () => {
    ...
    beforeEach(() => {
        ...
        hookEventHandler = new HookEventHandler(mockConfig, mockHookPlanner, mockHookRunner, mockHookAggregator, mockSessionHooksManager);
    });
    const createMockExecutionPlan = (hookConfigs = [], sequential = false) => ({
        hookConfigs,
        sequential,
        eventName: HookEventName.PreToolUse,
    });
    const createMockAggregatedResult = (success = true, finalOutput) => ({
        success,
        allOutputs: [],
        errors: [],
        totalDuration: 100,
    ...
    describe('fireUserPromptSubmitEvent', () => {
        it('should execute hooks for UserPromptSubmit event', async () => {
            ...
            const result = await hookEventHandler.fireUserPromptSubmitEvent('test prompt');
            expect(mockHookPlanner.createExecutionPlan).toHaveBeenCalledWith(HookEventName.UserPromptSubmit, undefined);
    ...
    describe('fireStopEvent', () => {
        it('should execute hooks for Stop event', async () => {
            ...
            expect(mockHookPlanner.createExecutionPlan).toHaveBeenCalledWith(HookEventName.Stop, undefined);
            expect(result.success).toBe(true);
        });
        ...
        it('should handle missing finalOutput gracefully', async () => {
            ...
            vi.mocked(mockHookPlanner.createExecutionPlan).mockReturnValue(mockPlan);
            vi.mocked(mockHookRunner.executeHooksParallel).mockResolvedValue([]);
            vi.mocked(mockHookAggregator.aggregateResults).mockReturnValue(createMockAggregatedResult(true, undefined));
    ...
    describe('fireSessionStartEvent', () => {
        it('should execute hooks for SessionStart event', async () => {
            ...
            vi.mocked(mockHookRunner.executeHooksParallel).mockResolvedValue([]);
            vi.mocked(mockHookAggregator.aggregateResults).mockReturnValue(mockAggregated);
            const result = await hookEventHandler.fireSessionStartEvent(SessionStartSource.Startup, 'test-model');
            expect(mockHookPlanner.createExecutionPlan).toHaveBeenCalledWith(HookEventName.SessionStart, { trigger: SessionStartSource.Startup });
            expect(result.success).toBe(true);
        });
        it('should include all session start parameters in the hook input', async () => {
            ...
            vi.mocked(mockHookRunner.executeHooksParallel).mockResolvedValue([]);
            vi.mocked(mockHookAggregator.aggregateResults).mockReturnValue(createMockAggregatedResult(true));
            await hookEventHandler.fireSessionStartEvent(SessionStartSource.Resume, 'test-model', PermissionMode.Plan, AgentType.Bash);
            const mockCalls = mockHookRunner.executeHooksParallel.mock
                .calls;
            const input = mockCalls[0][2];
        ...
        it('should use default permission mode when not provided', async () => {
            ...
            vi.mocked(mockHookRunner.executeHooksParallel).mockResolvedValue([]);
            vi.mocked(mockHookAggregator.aggregateResults).mockReturnValue(createMockAggregatedResult(true));
            await hookEventHandler.fireSessionStartEvent(SessionStartSource.Clear, 'test-model');
            const mockCalls = mockHookRunner.executeHooksParallel.mock
                .calls;
            const input = mockCalls[0][2];
        ...
        it('should handle session start event with undefined agent type', async () => {
            ...
            vi.mocked(mockHookRunner.executeHooksParallel).mockResolvedValue([]);
            vi.mocked(mockHookAggregator.aggregateResults).mockReturnValue(createMockAggregatedResult(true));
            await hookEventHandler.fireSessionStartEvent(SessionStartSource.Compact, 'test-model');
            const mockCalls = mockHookRunner.executeHooksParallel.mock
                .calls;
            const input = mockCalls[0][2];
    ...
    describe('fireSessionEndEvent', () => {
        it('should execute hooks for SessionEnd event', async () => {
            ...
            vi.mocked(mockHookRunner.executeHooksParallel).mockResolvedValue([]);
            vi.mocked(mockHookAggregator.aggregateResults).mockReturnValue(mockAggregated);
            const result = await hookEventHandler.fireSessionEndEvent(SessionEndReason.Clear);
            expect(mockHookPlanner.createExecutionPlan).toHaveBeenCalledWith(HookEventName.SessionEnd, { trigger: SessionEndReason.Clear });
            expect(result.success).toBe(true);
        });
        ...
        it('should handle different session end reasons', async () => {
            ...
            for (const reason of testReasons) {
                ...
                const mockCalls = mockHookRunner.executeHooksParallel.mock
                    .calls;
                const input = mockCalls[mockCalls.length - 1][2];
    ...
    describe('error handling', () => {
        ...
        it('should return error result when hook runner throws', async () => {
            ...
            vi.mocked(mockHookPlanner.createExecutionPlan).mockReturnValue(mockPlan);
            vi.mocked(mockHookRunner.executeHooksParallel).mockRejectedValue(new Error('Runner error'));
            const result = await hookEventHandler.fireUserPromptSubmitEvent('test');
            expect(result.success).toBe(false);
            expect(result.errors).toHaveLength(1);
    ...
    describe('firePostToolUseFailureEvent', () => {
        it('should execute hooks for PostToolUseFailure event', async () => {
            ...
            vi.mocked(mockHookRunner.executeHooksParallel).mockResolvedValue([]);
            vi.mocked(mockHookAggregator.aggregateResults).mockReturnValue(mockAggregated);
            const result = await hookEventHandler.firePostToolUseFailureEvent('toolu_test123', 'test-tool', { param: 'value' }, 'An error occurred');
            expect(mockHookPlanner.createExecutionPlan).toHaveBeenCalledWith(HookEventName.PostToolUseFailure, { toolName: 'test-tool' });
            expect(result.success).toBe(true);
        });
        it('should include all parameters in the hook input', async () => {
            ...
            vi.mocked(mockHookRunner.executeHooksParallel).mockResolvedValue([]);
            vi.mocked(mockHookAggregator.aggregateResults).mockReturnValue(createMockAggregatedResult(true));
            await hookEventHandler.firePostToolUseFailureEvent('toolu_test456', 'shell', { command: 'ls' }, 'Command failed', true, PermissionMode.Yolo);
            const mockCalls = mockHookRunner.executeHooksParallel.mock
                .calls;
            const input = mockCalls[0][2];
        ...
        it('should handle default values for optional parameters', async () => {
            ...
            vi.mocked(mockHookRunner.executeHooksParallel).mockResolvedValue([]);
            vi.mocked(mockHookAggregator.aggregateResults).mockReturnValue(createMockAggregatedResult(true));
            await hookEventHandler.firePostToolUseFailureEvent('toolu_test789', 'test-tool', { param: 'value' }, 'An error occurred');
            const mockCalls = mockHookRunner.executeHooksParallel.mock
                .calls;
            const input = mockCalls[0][2];
        ...
        it('should pass tool name as context for matcher filtering', async () => {
            ...
            vi.mocked(mockHookRunner.executeHooksParallel).mockResolvedValue([]);
            vi.mocked(mockHookAggregator.aggregateResults).mockReturnValue(mockAggregated);
            await hookEventHandler.firePostToolUseFailureEvent('toolu_test123', 'special-tool', { param: 'value' }, 'Error occurred');
            expect(mockHookPlanner.createExecutionPlan).toHaveBeenCalledWith(HookEventName.PostToolUseFailure, { toolName: 'special-tool' });
        });
        it('should handle successful execution with final output', async () => {
            ...
            vi.mocked(mockHookRunner.executeHooksParallel).mockResolvedValue([]);
            vi.mocked(mockHookAggregator.aggregateResults).mockReturnValue(mockAggregated);
            const result = await hookEventHandler.firePostToolUseFailureEvent('toolu_test999', 'test-tool', { param: 'value' }, 'Error occurred');
            expect(result.success).toBe(true);
        ...
        it('should handle multiple hooks execution', async () => {
            ...
            vi.mocked(mockHookRunner.executeHooksParallel).mockResolvedValue([]);
            vi.mocked(mockHookAggregator.aggregateResults).mockReturnValue(createMockAggregatedResult(true));
            await hookEventHandler.firePostToolUseFailureEvent('toolu_test111', 'multi-tool', { params: ['a', 'b'] }, 'Multiple errors');
            ...
            ], HookEventName.PostToolUseFailure, expect.any(Object), // input object
        ...
        it('should execute hooks sequentially when plan.sequential is true', async () => {
            ...
            vi.mocked(mockHookRunner.executeHooksSequential).mockResolvedValue([]);
            vi.mocked(mockHookAggregator.aggregateResults).mockReturnValue(createMockAggregatedResult(true));
            await hookEventHandler.firePostToolUseFailureEvent('toolu_sequential', 'seq-tool', { param: 'value' }, 'Sequential error');
            expect(mockHookRunner.executeHooksSequential).toHaveBeenCalled();
            expect(mockHookRunner.executeHooksParallel).not.toHaveBeenCalled();
        });
    });
    describe('firePreToolUseEvent', () => {
        it('should execute hooks for PreToolUse event', async () => {
            ...
            vi.mocked(mockHookRunner.executeHooksParallel).mockResolvedValue([]);
            vi.mocked(mockHookAggregator.aggregateResults).mockReturnValue(mockAggregated);
            const result = await hookEventHandler.firePreToolUseEvent('test-tool', { param: 'value' }, 'toolu_test123', PermissionMode.Default);
            expect(mockHookPlanner.createExecutionPlan).toHaveBeenCalledWith(HookEventName.PreToolUse, { toolName: 'test-tool' });
            expect(result.success).toBe(true);
        });
        it('should include all parameters in the hook input', async () => {
            const mockPlan = createMockExecutionPlan([
            ...
            vi.mocked(mockHookRunner.executeHooksParallel).mockResolvedValue([]);
            vi.mocked(mockHookAggregator.aggregateResults).mockReturnValue(createMockAggregatedResult(true));
            await hookEventHandler.firePreToolUseEvent('shell', { command: 'ls -la' }, 'toolu_abc456', PermissionMode.Plan);
            const mockCalls = mockHookRunner.executeHooksParallel.mock
                .calls;
            const input = mockCalls[0][2];
            expect(input.permission_mode).toBe(PermissionMode.Plan);
        ...
        it('should pass tool name as context for matcher filtering', async () => {
            ...
            vi.mocked(mockHookRunner.executeHooksParallel).mockResolvedValue([]);
            vi.mocked(mockHookAggregator.aggregateResults).mockReturnValue(mockAggregated);
            await hookEventHandler.firePreToolUseEvent('Bash', { command: 'npm test' }, 'toolu_xyz789', PermissionMode.Default);
            expect(mockHookPlanner.createExecutionPlan).toHaveBeenCalledWith(HookEventName.PreToolUse, { toolName: 'Bash' });
        });
        it('should handle permission decision in final output', async () => {
            ...
            vi.mocked(mockHookRunner.executeHooksParallel).mockResolvedValue([]);
            vi.mocked(mockHookAggregator.aggregateResults).mockReturnValue(mockAggregated);
            const result = await hookEventHandler.firePreToolUseEvent('Bash', { command: 'rm -rf /' }, 'toolu_danger', PermissionMode.Default);
            expect(result.success).toBe(true);
        ...
        it('should execute hooks sequentially when plan.sequential is true', async () => {
            ...
            vi.mocked(mockHookRunner.executeHooksSequential).mockResolvedValue([]);
            vi.mocked(mockHookAggregator.aggregateResults).mockReturnValue(createMockAggregatedResult(true));
            await hookEventHandler.firePreToolUseEvent('test-tool', { param: 'value' }, 'toolu_seq', PermissionMode.Default);
            expect(mockHookRunner.executeHooksSequential).toHaveBeenCalled();
            expect(mockHookRunner.executeHooksParallel).not.toHaveBeenCalled();
        });
        it('should handle errors gracefully', async () => {
            ...
            const result = await hookEventHandler.firePreToolUseEvent('test-tool', { param: 'value' }, 'toolu_error', PermissionMode.Default);
            expect(result.success).toBe(false);
            expect(result.errors).toHaveLength(1);
    ...
    describe('firePostToolUseEvent', () => {
        it('should execute hooks for PostToolUse event', async () => {
            ...
            vi.mocked(mockHookRunner.executeHooksParallel).mockResolvedValue([]);
            vi.mocked(mockHookAggregator.aggregateResults).mockReturnValue(mockAggregated);
            const result = await hookEventHandler.firePostToolUseEvent('test-tool', { param: 'value' }, { result: 'success' }, 'toolu_test123', PermissionMode.Default);
            expect(mockHookPlanner.createExecutionPlan).toHaveBeenCalledWith(HookEventName.PostToolUse, { toolName: 'test-tool' });
            expect(result.success).toBe(true);
        });
        it('should include all parameters in the hook input', async () => {
            const mockPlan = createMockExecutionPlan([
            ...
            vi.mocked(mockHookRunner.executeHooksParallel).mockResolvedValue([]);
            vi.mocked(mockHookAggregator.aggregateResults).mockReturnValue(createMockAggregatedResult(true));
            await hookEventHandler.firePostToolUseEvent('shell', { command: 'ls -la' }, { files: ['a.txt', 'b.txt'] }, 'toolu_abc456', PermissionMode.Yolo);
            const mockCalls = mockHookRunner.executeHooksParallel.mock
                .calls;
            const input = mockCalls[0][2];
            expect(input.permission_mode).toBe(PermissionMode.Yolo);
        ...
        it('should pass tool name as context for matcher filtering', async () => {
            ...
            vi.mocked(mockHookRunner.executeHooksParallel).mockResolvedValue([]);
            vi.mocked(mockHookAggregator.aggregateResults).mockReturnValue(mockAggregated);
            await hookEventHandler.firePostToolUseEvent('Write', { file_path: '/test.txt', content: 'hello' }, { success: true }, 'toolu_write123', PermissionMode.Default);
            expect(mockHookPlanner.createExecutionPlan).toHaveBeenCalledWith(HookEventName.PostToolUse, { toolName: 'Write' });
        });
        it('should handle decision block in final output', async () => {
            ...
            vi.mocked(mockHookRunner.executeHooksParallel).mockResolvedValue([]);
            vi.mocked(mockHookAggregator.aggregateResults).mockReturnValue(mockAggregated);
            const result = await hookEventHandler.firePostToolUseEvent('Write', { file_path: '/test.ts', content: 'const x = 1' }, { success: true }, 'toolu_lint', PermissionMode.Default);
            expect(result.success).toBe(true);
        ...
        it('should execute hooks sequentially when plan.sequential is true', async () => {
            ...
            vi.mocked(mockHookRunner.executeHooksSequential).mockResolvedValue([]);
            vi.mocked(mockHookAggregator.aggregateResults).mockReturnValue(createMockAggregatedResult(true));
            await hookEventHandler.firePostToolUseEvent('test-tool', { param: 'value' }, { result: 'ok' }, 'toolu_seq', PermissionMode.Default);
            expect(mockHookRunner.executeHooksSequential).toHaveBeenCalled();
            expect(mockHookRunner.executeHooksParallel).not.toHaveBeenCalled();
        });
        it('should handle errors gracefully', async () => {
            ...
            const result = await hookEventHandler.firePostToolUseEvent('test-tool', { param: 'value' }, { result: 'ok' }, 'toolu_error', PermissionMode.Default);
            expect(result.success).toBe(false);
            expect(result.errors).toHaveLength(1);
    ...
    describe('firePreCompactEvent', () => {
        it('should execute hooks for PreCompact event with manual trigger', async () => {
            ...
            vi.mocked(mockHookRunner.executeHooksParallel).mockResolvedValue([]);
            vi.mocked(mockHookAggregator.aggregateResults).mockReturnValue(mockAggregated);
            const result = await hookEventHandler.firePreCompactEvent(PreCompactTrigger.Manual, 'Keep important code');
            expect(mockHookPlanner.createExecutionPlan).toHaveBeenCalledWith(HookEventName.PreCompact, { trigger: PreCompactTrigger.Manual });
            expect(result.success).toBe(true);
        });
        it('should execute hooks for PreCompact event with auto trigger', async () => {
            ...
            vi.mocked(mockHookRunner.executeHooksParallel).mockResolvedValue([]);
            vi.mocked(mockHookAggregator.aggregateResults).mockReturnValue(mockAggregated);
            const result = await hookEventHandler.firePreCompactEvent(PreCompactTrigger.Auto);
            expect(mockHookPlanner.createExecutionPlan).toHaveBeenCalledWith(HookEventName.PreCompact, { trigger: PreCompactTrigger.Auto });
            expect(result.success).toBe(true);
        });
        it('should include all parameters in the hook input', async () => {
            const mockPlan = createMockExecutionPlan([
            ...
            vi.mocked(mockHookRunner.executeHooksParallel).mockResolvedValue([]);
            vi.mocked(mockHookAggregator.aggregateResults).mockReturnValue(createMockAggregatedResult(true));
            await hookEventHandler.firePreCompactEvent(PreCompactTrigger.Manual, 'Custom instructions for compaction');
            const mockCalls = mockHookRunner.executeHooksParallel.mock
                .calls;
            const input = mockCalls[0][2];
            expect(input.trigger).toBe(PreCompactTrigger.Manual);
            expect(input.custom_instructions).toBe('Custom instructions for compaction');
        });
        ...
        it('should handle both trigger types correctly', async () => {
            ...
            // Test Manual trigger
            await hookEventHandler.firePreCompactEvent(PreCompactTrigger.Manual);
            let mockCalls = mockHookRunner.executeHooksParallel.mock.calls;
            let input = mockCalls[mockCalls.length - 1][2];
            expect(input.trigger).toBe(PreCompactTrigger.Manual);
            // Test Auto trigger
            await hookEventHandler.firePreCompactEvent(PreCompactTrigger.Auto);
            mockCalls = mockHookRunner.executeHooksParallel.mock.calls;
            input = mockCalls[mockCalls.length - 1][2];
            expect(input.trigger).toBe(PreCompactTrigger.Auto);
        });
    });
    describe('fireNotificationEvent', () => {
        it('should execute hooks for Notification event', async () => {
            ...
            vi.mocked(mockHookRunner.executeHooksParallel).mockResolvedValue([]);
            vi.mocked(mockHookAggregator.aggregateResults).mockReturnValue(mockAggregated);
            const result = await hookEventHandler.fireNotificationEvent('Test notification message', NotificationType.PermissionPrompt, 'Permission needed');
            expect(mockHookPlanner.createExecutionPlan).toHaveBeenCalledWith(HookEventName.Notification, { notificationType: 'permission_prompt' });
            expect(result.success).toBe(true);
        });
        it('should include all parameters in the hook input', async () => {
            const mockPlan = createMockExecutionPlan([
            ...
            vi.mocked(mockHookRunner.executeHooksParallel).mockResolvedValue([]);
            vi.mocked(mockHookAggregator.aggregateResults).mockReturnValue(createMockAggregatedResult(true));
            await hookEventHandler.fireNotificationEvent('VivekMind needs your permission to use Bash', NotificationType.PermissionPrompt, 'Permission needed');
            const mockCalls = mockHookRunner.executeHooksParallel.mock
                .calls;
            const input = mockCalls[0][2];
            expect(input.message).toBe('VivekMind needs your permission to use Bash');
        ...
        it('should pass notification_type as context for matcher filtering', async () => {
            ...
            vi.mocked(mockHookRunner.executeHooksParallel).mockResolvedValue([]);
            vi.mocked(mockHookAggregator.aggregateResults).mockReturnValue(mockAggregated);
            await hookEventHandler.fireNotificationEvent('VivekMind is waiting for your input', NotificationType.IdlePrompt, 'Waiting for input');
            expect(mockHookPlanner.createExecutionPlan).toHaveBeenCalledWith(HookEventName.Notification, { notificationType: 'idle_prompt' });
        });
        it('should handle notification without title', async () => {
            ...
            vi.mocked(mockHookRunner.executeHooksParallel).mockResolvedValue([]);
            vi.mocked(mockHookAggregator.aggregateResults).mockReturnValue(createMockAggregatedResult(true));
            await hookEventHandler.fireNotificationEvent('Authentication successful', NotificationType.AuthSuccess);
            const mockCalls = mockHookRunner.executeHooksParallel.mock
                .calls;
            const input = mockCalls[0][2];
        ...
        it('should handle auth_success notification type', async () => {
            ...
            vi.mocked(mockHookRunner.executeHooksParallel).mockResolvedValue([]);
            vi.mocked(mockHookAggregator.aggregateResults).mockReturnValue(mockAggregated);
            const result = await hookEventHandler.fireNotificationEvent('Authentication successful', NotificationType.AuthSuccess);
            expect(mockHookPlanner.createExecutionPlan).toHaveBeenCalledWith(HookEventName.Notification, { notificationType: 'auth_success' });
            expect(result.success).toBe(true);
        });
        it('should handle elicitation_dialog notification type', async () => {
            ...
            vi.mocked(mockHookRunner.executeHooksParallel).mockResolvedValue([]);
            vi.mocked(mockHookAggregator.aggregateResults).mockReturnValue(mockAggregated);
            const result = await hookEventHandler.fireNotificationEvent('Dialog shown to user', NotificationType.ElicitationDialog, 'Dialog');
            expect(mockHookPlanner.createExecutionPlan).toHaveBeenCalledWith(HookEventName.Notification, { notificationType: 'elicitation_dialog' });
            expect(result.success).toBe(true);
        });
        it('should execute hooks sequentially when plan.sequential is true', async () => {
            const mockPlan = createMockExecutionPlan([
            ...
            vi.mocked(mockHookRunner.executeHooksSequential).mockResolvedValue([]);
            vi.mocked(mockHookAggregator.aggregateResults).mockReturnValue(createMockAggregatedResult(true));
            await hookEventHandler.fireNotificationEvent('Test notification', NotificationType.PermissionPrompt);
            expect(mockHookRunner.executeHooksSequential).toHaveBeenCalled();
            expect(mockHookRunner.executeHooksParallel).not.toHaveBeenCalled();
        });
        it('should handle errors gracefully', async () => {
            ...
            const result = await hookEventHandler.fireNotificationEvent('Test notification', NotificationType.PermissionPrompt);
            expect(result.success).toBe(false);
            expect(result.errors).toHaveLength(1);
        ...
        it('should handle all notification types correctly', async () => {
            ...
            await hookEventHandler.fireNotificationEvent('Permission needed', NotificationType.PermissionPrompt);
            let mockCalls = mockHookRunner.executeHooksParallel.mock.calls;
            let input = mockCalls[mockCalls.length - 1][2];
            expect(input.notification_type).toBe('permission_prompt');
            // Test idle_prompt
            await hookEventHandler.fireNotificationEvent('Waiting for input', NotificationType.IdlePrompt);
            mockCalls = mockHookRunner.executeHooksParallel.mock.calls;
            input = mockCalls[mockCalls.length - 1][2];
            ...
            await hookEventHandler.fireNotificationEvent('Dialog shown', NotificationType.ElicitationDialog);
            mockCalls = mockHookRunner.executeHooksParallel.mock.calls;
            input = mockCalls[mockCalls.length - 1][2];
    ...
    describe('firePermissionRequestEvent', () => {
        it('should execute hooks for PermissionRequest event', async () => {
            ...
            vi.mocked(mockHookRunner.executeHooksParallel).mockResolvedValue([]);
            vi.mocked(mockHookAggregator.aggregateResults).mockReturnValue(mockAggregated);
            const result = await hookEventHandler.firePermissionRequestEvent('Bash', { command: 'ls -la' }, PermissionMode.Default);
            expect(mockHookPlanner.createExecutionPlan).toHaveBeenCalledWith(HookEventName.PermissionRequest, { toolName: 'Bash' });
            expect(result.success).toBe(true);
        });
        it('should include all parameters in the hook input', async () => {
            const mockPlan = createMockExecutionPlan([
            ...
            vi.mocked(mockHookRunner.executeHooksParallel).mockResolvedValue([]);
            vi.mocked(mockHookAggregator.aggregateResults).mockReturnValue(createMockAggregatedResult(true));
            await hookEventHandler.firePermissionRequestEvent('Write', { file_path: '/test.txt', content: 'hello' }, PermissionMode.Yolo);
            const mockCalls = mockHookRunner.executeHooksParallel.mock
                .calls;
            const input = mockCalls[0][2];
            expect(input.permission_mode).toBe(PermissionMode.Yolo);
        ...
        it('should include permission_suggestions when provided', async () => {
            ...
            vi.mocked(mockHookRunner.executeHooksParallel).mockResolvedValue([]);
            vi.mocked(mockHookAggregator.aggregateResults).mockReturnValue(createMockAggregatedResult(true));
            await hookEventHandler.firePermissionRequestEvent('Bash', { command: 'npm test' }, PermissionMode.Default, suggestions);
            const mockCalls = mockHookRunner.executeHooksParallel.mock
                .calls;
            const input = mockCalls[0][2];
        ...
        it('should pass tool name as context for matcher filtering', async () => {
            ...
            vi.mocked(mockHookRunner.executeHooksParallel).mockResolvedValue([]);
            vi.mocked(mockHookAggregator.aggregateResults).mockReturnValue(mockAggregated);
            await hookEventHandler.firePermissionRequestEvent('ReadFile', { file_path: '/test.txt' }, PermissionMode.Plan);
            expect(mockHookPlanner.createExecutionPlan).toHaveBeenCalledWith(HookEventName.PermissionRequest, { toolName: 'ReadFile' });
        });
        it('should handle decision block in final output', async () => {
            const mockPlan = createMockExecutionPlan([
                {
            ...
            vi.mocked(mockHookRunner.executeHooksParallel).mockResolvedValue([]);
            vi.mocked(mockHookAggregator.aggregateResults).mockReturnValue(mockAggregated);
            const result = await hookEventHandler.firePermissionRequestEvent('Bash', { command: 'rm -rf /' }, PermissionMode.Default);
            expect(result.success).toBe(true);
            expect(result.finalOutput?.decision).toBe('block');
        ...
        it('should handle allow decision with updatedInput', async () => {
            ...
            vi.mocked(mockHookRunner.executeHooksParallel).mockResolvedValue([]);
            vi.mocked(mockHookAggregator.aggregateResults).mockReturnValue(mockAggregated);
            const result = await hookEventHandler.firePermissionRequestEvent('Bash', { command: 'npm install' }, PermissionMode.Default);
            expect(result.success).toBe(true);
            expect(result.finalOutput?.hookSpecificOutput).toEqual({
                hookEventName: 'PermissionRequest',
                decision: {
        ...
        it('should execute hooks sequentially when plan.sequential is true', async () => {
            ...
            vi.mocked(mockHookRunner.executeHooksSequential).mockResolvedValue([]);
            vi.mocked(mockHookAggregator.aggregateResults).mockReturnValue(createMockAggregatedResult(true));
            await hookEventHandler.firePermissionRequestEvent('Bash', { command: 'ls' }, PermissionMode.Default);
            expect(mockHookRunner.executeHooksSequential).toHaveBeenCalled();
            expect(mockHookRunner.executeHooksParallel).not.toHaveBeenCalled();
        });
        it('should handle errors gracefully', async () => {
            ...
            const result = await hookEventHandler.firePermissionRequestEvent('Bash', { command: 'test' }, PermissionMode.Default);
            expect(result.success).toBe(false);
            expect(result.errors).toHaveLength(1);
        ...
        it('should handle all permission modes correctly', async () => {
            ...
            vi.mocked(mockHookAggregator.aggregateResults).mockReturnValue(createMockAggregatedResult(true));
            // Test Default mode
            await hookEventHandler.firePermissionRequestEvent('Bash', { command: 'test' }, PermissionMode.Default);
            let mockCalls = mockHookRunner.executeHooksParallel.mock.calls;
            let input = mockCalls[mockCalls.length - 1][2];
            expect(input.permission_mode).toBe(PermissionMode.Default);
            // Test Plan mode
            await hookEventHandler.firePermissionRequestEvent('Bash', { command: 'test' }, PermissionMode.Plan);
            mockCalls = mockHookRunner.executeHooksParallel.mock.calls;
            input = mockCalls[mockCalls.length - 1][2];
            expect(input.permission_mode).toBe(PermissionMode.Plan);
            // Test Yolo mode
            await hookEventHandler.firePermissionRequestEvent('Bash', { command: 'test' }, PermissionMode.Yolo);
            mockCalls = mockHookRunner.executeHooksParallel.mock.calls;
            input = mockCalls[mockCalls.length - 1][2];
            expect(input.permission_mode).toBe(PermissionMode.Yolo);
        });
    ...
    describe('fireSubagentStartEvent', () => {
        it('should execute hooks for SubagentStart event', async () => {
            ...
            vi.mocked(mockHookRunner.executeHooksParallel).mockResolvedValue([]);
            vi.mocked(mockHookAggregator.aggregateResults).mockReturnValue(mockAggregated);
            const result = await hookEventHandler.fireSubagentStartEvent('agent-123', 'code-reviewer', PermissionMode.Default);
            expect(mockHookPlanner.createExecutionPlan).toHaveBeenCalledWith(HookEventName.SubagentStart, { agentType: 'code-reviewer' });
            expect(result.success).toBe(true);
        });
        it('should include all parameters in the hook input', async () => {
            const mockPlan = createMockExecutionPlan([
        ...
        it('should pass agentType as context for matcher filtering', async () => {
            ...
            vi.mocked(mockHookRunner.executeHooksParallel).mockResolvedValue([]);
            vi.mocked(mockHookAggregator.aggregateResults).mockReturnValue(mockAggregated);
            await hookEventHandler.fireSubagentStartEvent('agent-789', AgentType.Bash, PermissionMode.Default);
            expect(mockHookPlanner.createExecutionPlan).toHaveBeenCalledWith(HookEventName.SubagentStart, { agentType: String(AgentType.Bash) });
        });
        it('should handle additional context in final output', async () => {
            ...
            vi.mocked(mockHookRunner.executeHooksParallel).mockResolvedValue([]);
            vi.mocked(mockHookAggregator.aggregateResults).mockReturnValue(mockAggregated);
            const result = await hookEventHandler.fireSubagentStartEvent('agent-111', 'code-reviewer', PermissionMode.Default);
            expect(result.success).toBe(true);
            expect(result.finalOutput?.hookSpecificOutput).toEqual({
                hookEventName: 'SubagentStart',
                additionalContext: 'Injected context for subagent',
        ...
        it('should execute hooks sequentially when plan.sequential is true', async () => {
            ...
            vi.mocked(mockHookRunner.executeHooksSequential).mockResolvedValue([]);
            vi.mocked(mockHookAggregator.aggregateResults).mockReturnValue(createMockAggregatedResult(true));
            await hookEventHandler.fireSubagentStartEvent('agent-seq', 'code-reviewer', PermissionMode.Default);
            expect(mockHookRunner.executeHooksSequential).toHaveBeenCalled();
            expect(mockHookRunner.executeHooksParallel).not.toHaveBeenCalled();
        });
        it('should handle errors gracefully', async () => {
            ...
            const result = await hookEventHandler.fireSubagentStartEvent('agent-err', 'code-reviewer', PermissionMode.Default);
            expect(result.success).toBe(false);
            expect(result.errors).toHaveLength(1);
    ...
    describe('fireSubagentStopEvent', () => {
        it('should execute hooks for SubagentStop event', async () => {
            ...
            vi.mocked(mockHookRunner.executeHooksParallel).mockResolvedValue([]);
            vi.mocked(mockHookAggregator.aggregateResults).mockReturnValue(mockAggregated);
            const result = await hookEventHandler.fireSubagentStopEvent('agent-123', 'code-reviewer', '/path/to/transcript.jsonl', 'Final output from subagent', false, PermissionMode.Default);
            expect(mockHookPlanner.createExecutionPlan).toHaveBeenCalledWith(HookEventName.SubagentStop, { agentType: 'code-reviewer' });
            expect(result.success).toBe(true);
        });
        it('should include all parameters in the hook input', async () => {
            const mockPlan = createMockExecutionPlan([
        ...
        it('should pass agentType as context for matcher filtering', async () => {
            ...
            vi.mocked(mockHookRunner.executeHooksParallel).mockResolvedValue([]);
            vi.mocked(mockHookAggregator.aggregateResults).mockReturnValue(mockAggregated);
            await hookEventHandler.fireSubagentStopEvent('agent-789', 'custom-agent', '/path/transcript.jsonl', 'output', false, PermissionMode.Default);
            expect(mockHookPlanner.createExecutionPlan).toHaveBeenCalledWith(HookEventName.SubagentStop, { agentType: 'custom-agent' });
        });
        it('should handle block decision in final output', async () => {
            ...
            vi.mocked(mockHookRunner.executeHooksParallel).mockResolvedValue([]);
            vi.mocked(mockHookAggregator.aggregateResults).mockReturnValue(mockAggregated);
            const result = await hookEventHandler.fireSubagentStopEvent('agent-block', 'code-reviewer', '/path/transcript.jsonl', 'short', false, PermissionMode.Default);
            expect(result.success).toBe(true);
            expect(result.finalOutput?.decision).toBe('block');
            expect(result.finalOutput?.reason).toBe('Output too short, continue working');
        });
        it('should execute hooks sequentially when plan.sequential is true', async () => {
            const mockPlan = createMockExecutionPlan([
                {
            ...
            vi.mocked(mockHookRunner.executeHooksSequential).mockResolvedValue([]);
            vi.mocked(mockHookAggregator.aggregateResults).mockReturnValue(createMockAggregatedResult(true));
            await hookEventHandler.fireSubagentStopEvent('agent-seq', 'code-reviewer', '/path/transcript.jsonl', 'output', false, PermissionMode.Default);
            expect(mockHookRunner.executeHooksSequential).toHaveBeenCalled();
            expect(mockHookRunner.executeHooksParallel).not.toHaveBeenCalled();
        });
        it('should handle errors gracefully', async () => {
            ...
            const result = await hookEventHandler.fireSubagentStopEvent('agent-err', 'code-reviewer', '/path/transcript.jsonl', 'output', false, PermissionMode.Default);
            expect(result.success).toBe(false);
            expect(result.errors).toHaveLength(1);
        ...
        it('should handle stop_hook_active flag correctly', async () => {
            ...
            await hookEventHandler.fireSubagentStopEvent('agent-1', 'code-reviewer', '/path/transcript.jsonl', 'output', false, PermissionMode.Default);
            let mockCalls = mockHookRunner.executeHooksParallel.mock.calls;
            let input = mockCalls[mockCalls.length - 1][2];
            expect(input.stop_hook_active).toBe(false);
            // Test with stop_hook_active = true
            await hookEventHandler.fireSubagentStopEvent('agent-2', 'code-reviewer', '/path/transcript.jsonl', 'output', true, PermissionMode.Default);
            mockCalls = mockHookRunner.executeHooksParallel.mock.calls;
            input = mockCalls[mockCalls.length - 1][2];
            expect(input.stop_hook_active).toBe(true);
        });
    ...
    describe('fireStopFailureEvent', () => {
        it('should execute hooks for StopFailure event', async () => {
            ...
            vi.mocked(mockHookRunner.executeHooksParallel).mockResolvedValue([]);
            vi.mocked(mockHookAggregator.aggregateResults).mockReturnValue(mockAggregated);
            const result = await hookEventHandler.fireStopFailureEvent('rate_limit', '429 Too Many Requests', 'API Error: Rate limit reached');
            expect(mockHookPlanner.createExecutionPlan).toHaveBeenCalledWith(HookEventName.StopFailure, { error: 'rate_limit' });
            expect(result.success).toBe(true);
        });
        it('should include all parameters in the hook input', async () => {
            const mockPlan = createMockExecutionPlan([
            ...
            vi.mocked(mockHookRunner.executeHooksParallel).mockResolvedValue([]);
            vi.mocked(mockHookAggregator.aggregateResults).mockReturnValue(createMockAggregatedResult(true));
            await hookEventHandler.fireStopFailureEvent('authentication_failed', '401 Unauthorized', 'Please check your API key');
            const mockCalls = mockHookRunner.executeHooksParallel.mock
                .calls;
            const input = mockCalls[0][2];
        ...
        it('should pass error type as context for matcher filtering', async () => {
            ...
            expect(mockHookPlanner.createExecutionPlan).toHaveBeenCalledWith(HookEventName.StopFailure, { error: 'server_error' });
        });
    ...
    describe('firePostCompactEvent', () => {
        it('should execute hooks for PostCompact event with manual trigger', async () => {
            ...
            vi.mocked(mockHookRunner.executeHooksParallel).mockResolvedValue([]);
            vi.mocked(mockHookAggregator.aggregateResults).mockReturnValue(mockAggregated);
            const result = await hookEventHandler.firePostCompactEvent(PostCompactTrigger.Manual, 'Summary of compacted conversation');
            expect(mockHookPlanner.createExecutionPlan).toHaveBeenCalledWith(HookEventName.PostCompact, { trigger: PostCompactTrigger.Manual });
            expect(result.success).toBe(true);
        });
        it('should execute hooks for PostCompact event with auto trigger', async () => {
            ...
            vi.mocked(mockHookRunner.executeHooksParallel).mockResolvedValue([]);
            vi.mocked(mockHookAggregator.aggregateResults).mockReturnValue(mockAggregated);
            const result = await hookEventHandler.firePostCompactEvent(PostCompactTrigger.Auto, 'Auto-generated summary');
            expect(mockHookPlanner.createExecutionPlan).toHaveBeenCalledWith(HookEventName.PostCompact, { trigger: PostCompactTrigger.Auto });
            expect(result.success).toBe(true);
        });
        it('should include all parameters in the hook input', async () => {
            const mockPlan = createMockExecutionPlan([
            ...
            await hookEventHandler.firePostCompactEvent(PostCompactTrigger.Manual, summary);
            const mockCalls = mockHookRunner.executeHooksParallel.mock
                .calls;
            const input = mockCalls[0][2];
        ...
        it('should pass trigger as context for matcher filtering', async () => {
            ...
            vi.mocked(mockHookRunner.executeHooksParallel).mockResolvedValue([]);
            vi.mocked(mockHookAggregator.aggregateResults).mockReturnValue(mockAggregated);
            await hookEventHandler.firePostCompactEvent(PostCompactTrigger.Auto, 'summary');
            expect(mockHookPlanner.createExecutionPlan).toHaveBeenCalledWith(HookEventName.PostCompact, { trigger: PostCompactTrigger.Auto });
        });
        it('should execute hooks sequentially when plan.sequential is true', async () => {
            const mockPlan = createMockExecutionPlan([
        ...
        it('should handle errors gracefully', async () => {
            ...
            const result = await hookEventHandler.firePostCompactEvent(PostCompactTrigger.Auto, 'summary');
            expect(result.success).toBe(false);
            expect(result.errors).toHaveLength(1);
        ...
        it('should handle both trigger types correctly', async () => {
            ...
            vi.mocked(mockHookAggregator.aggregateResults).mockReturnValue(createMockAggregatedResult(true));
            // Test manual trigger
            await hookEventHandler.firePostCompactEvent(PostCompactTrigger.Manual, 'manual summary');
            let mockCalls = mockHookRunner.executeHooksParallel.mock.calls;
            let input = mockCalls[mockCalls.length - 1][2];
            expect(input.trigger).toBe(PostCompactTrigger.Manual);
            // Test auto trigger
            await hookEventHandler.firePostCompactEvent(PostCompactTrigger.Auto, 'auto summary');
            mockCalls = mockHookRunner.executeHooksParallel.mock.calls;
            input = mockCalls[mockCalls.length - 1][2];
    ...
    describe('telemetry', () => {
        const createMockHookExecutionResult = (success, hookConfig, duration = 100, output, error) => ({
            hookConfig,
            eventName: HookEventName.PreToolUse,
            success,
            output,
        ...
        it('should log hook call with correct event name', async () => {
            ...
            vi.mocked(mockHookAggregator.aggregateResults).mockReturnValue(createMockAggregatedResult(true));
            await hookEventHandler.firePreToolUseEvent('read_file', { path: '/test' }, 'tool-123', PermissionMode.Default);
            expect(logHookCall).toHaveBeenCalledWith(mockConfig, expect.objectContaining({
        ...
        it('should log hook call with failure status', async () => {
            ...
            const mockPlan = createMockExecutionPlan([hookConfig]);
            vi.mocked(mockHookPlanner.createExecutionPlan).mockReturnValue(mockPlan);
            const result = createMockHookExecutionResult(false, hookConfig, 100, undefined, new Error('Hook failed'));
            vi.mocked(mockHookRunner.executeHooksParallel).mockResolvedValue([
                result,
            ]);
            vi.mocked(mockHookAggregator.aggregateResults).mockReturnValue(createMockAggregatedResult(false));
            await hookEventHandler.fireUserPromptSubmitEvent('test');
            expect(logHookCall).toHaveBeenCalledWith(mockConfig, expect.objectContaining({
                success: false,
                error: 'Hook failed',
        ...
        it('should log telemetry for different event types', async () => {
            ...
            vi.mocked(mockHookAggregator.aggregateResults).mockReturnValue(createMockAggregatedResult(true));
            // Test SessionStart
            await hookEventHandler.fireSessionStartEvent(SessionStartSource.Startup, 'test-model');
            expect(logHookCall).toHaveBeenCalledWith(mockConfig, expect.objectContaining({
    ...
    describe('MessagesProvider integration', () => {
        it('should accept messagesProvider in constructor', () => {
            ...
            const handler = new HookEventHandler(mockConfig, mockHookPlanner, mockHookRunner, mockHookAggregator, mockSessionHooksManager, messagesProvider);
        ...
        it('should pass messages to function hooks via context', async () => {
            ...
            vi.mocked(mockHookPlanner.createExecutionPlan).mockReturnValue(mockPlan);
            vi.mocked(mockHookRunner.executeHooksParallel).mockResolvedValue([]);
            await hookEventHandler.firePreToolUseEvent('Bash', { command: 'ls' }, 'toolu_test', PermissionMode.Default);
            // Verify context was passed with messages
            expect(mockHookRunner.executeHooksParallel).toHaveBeenCalledWith(expect.any(Array), HookEventName.PreToolUse, expect.any(Object), expect.any(Function), expect.any(Function), undefined, expect.objectContaining({
                messages,
        ...
        it('should pass toolUseID from input to context', async () => {
            ...
            vi.mocked(mockHookPlanner.createExecutionPlan).mockReturnValue(mockPlan);
            vi.mocked(mockHookRunner.executeHooksParallel).mockResolvedValue([]);
            await hookEventHandler.firePostToolUseEvent('Write', { file_path: '/test.txt' }, { content: 'test' }, 'toolu_12345', PermissionMode.Default);
            expect(mockHookRunner.executeHooksParallel).toHaveBeenCalledWith(expect.any(Array), HookEventName.PostToolUse, expect.any(Object), expect.any(Function), expect.any(Function), undefined, expect.objectContaining({
