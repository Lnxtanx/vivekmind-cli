...
export class AsyncHookRegistry {
    ...
    constructor(options = {}) {
        ...
        if (options.enableAutoTimeoutCheck) {
            const interval = options.timeoutCheckInterval ?? DEFAULT_TIMEOUT_CHECK_INTERVAL;
     ...
     */
    getRunningCount() {
        return Array.from(this.pendingHooks.values()).filter((hook) => hook.status === 'running').length;
    }
    /**
    ...
    register(hook) {
        ...
        if (!this.canAcceptMore()) {
            debugLogger.warn(`Async hook registration rejected: concurrency limit reached (${this.maxConcurrentHooks})`);
            return null;
        }
        ...
        debugLogger.debug(`Registered async hook: ${hookId} (${hook.hookName}) for event ${hook.hookEvent} [${this.getRunningCount()}/${this.maxConcurrentHooks}]`);
        return hookId;
    }
    /**
     * Update hook output (stdout/stderr)
     */
    updateOutput(hookId, stdout, stderr) {
     ...
     */
    getPendingHooksForSession(sessionId) {
        return Array.from(this.pendingHooks.values()).filter((hook) => hook.sessionId === sessionId);
    }
    /**
     ...
     */
    hasPendingOutput() {
        return (this.completedOutputs.length > 0 || this.completedContexts.length > 0);
    }
    /**
    ...
    clearSession(sessionId) {
        for (const [hookId, hook] of this.pendingHooks.entries()) {
            if (hook.sessionId === sessionId) {
                this.pendingHooks.delete(hookId);
                debugLogger.debug(`Cleared async hook on session end: ${hookId} (${hook.hookName})`);
            }
        }
    }
    /**
        ...
        if (hook.stdout) {
            try {
                ...
                if (parsed.hookSpecificOutput?.additionalContext &&
                    ...
                    this.completedContexts.push(parsed.hookSpecificOutput.additionalContext);
                }
            }
            catch {
