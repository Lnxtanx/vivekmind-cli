...
describe('HookRunner', () => {
    ...
    describe('executeHook', () => {
        it('should return error when hook command is missing', async () => {
            ...
            const result = await hookRunner.executeHook(hookConfig, HookEventName.PreToolUse, input);
        ...
        it('should ignore stdout on exit code 2 and use stderr only', async () => {
            ...
            const mockProcess = createMockProcess(2, 'stdout should be ignored', 'stderr error message');
            mockSpawn.mockImplementation(() => mockProcess);
            const hookConfig = {
                type: HookType.Command,
        ...
        it('should not parse JSON on exit code 2', async () => {
            ...
            const mockProcess = createMockProcess(2, '{"decision":"allow"}', 'blocking error');
            mockSpawn.mockImplementation(() => mockProcess);
            const hookConfig = {
                type: HookType.Command,
    ...
    describe('executeHooksParallel', () => {
        it('should execute multiple hooks in parallel', async () => {
            ...
            const input = createMockInput();
            const results = await hookRunner.executeHooksParallel(hookConfigs, HookEventName.PreToolUse, input);
        ...
        it('should call onHookStart and onHookEnd callbacks', async () => {
            ...
            await hookRunner.executeHooksParallel(hookConfigs, HookEventName.PreToolUse, input, onHookStart, onHookEnd);
    ...
    describe('executeHooksSequential', () => {
        it('should execute hooks sequentially', async () => {
            ...
            const input = createMockInput();
            const results = await hookRunner.executeHooksSequential(hookConfigs, HookEventName.PreToolUse, input);
            expect(results).toHaveLength(2);
            expect(results[0].success).toBe(true);
            expect(results[1].success).toBe(true);
        });
        it('should call onHookStart and onHookEnd callbacks', async () => {
            ...
            const onHookStart = vi.fn();
            const onHookEnd = vi.fn();
            await hookRunner.executeHooksSequential(hookConfigs, HookEventName.PreToolUse, input, onHookStart, onHookEnd);
            expect(onHookStart).toHaveBeenCalledTimes(1);
            expect(onHookEnd).toHaveBeenCalledTimes(1);
        });
    });
