...
import { DEFAULT_LSP_COMMAND_CHECK_TIMEOUT_MS, DEFAULT_LSP_MAX_RESTARTS, DEFAULT_LSP_SOCKET_MAX_RETRY_DELAY_MS, DEFAULT_LSP_SOCKET_RETRY_DELAY_MS, DEFAULT_LSP_STARTUP_TIMEOUT_MS, DEFAULT_LSP_WARMUP_DELAY_MS, } from './constants.js';
...
export class LspServerManager {
    ...
    async doStartServer(name, handle) {
        ...
        if (!trusted) {
            debugLogger.info(`Workspace trust check failed, not starting LSP server ${name}`);
            handle.status = 'FAILED';
            return;
        }
    ...
    async shutdownConnection(handle) {
        ...
        try {
            ...
            if (typeof handle.config.shutdownTimeout === 'number') {
                await Promise.race([
                    shutdownPromise,
                    new Promise((resolve) => setTimeout(resolve, handle.config.shutdownTimeout)),
                ]);
            }
            else {
    ...
    async connectSocketWithRetry(socket, timeoutMs) {
        ...
        while (true) {
            ...
            try {
                return await LspConnectionFactory.createSocketConnection(socket, remaining);
            }
            catch (error) {
                attempt += 1;
     ...
     * Create LSP connection
     */
    async createLspConnection(config) {
        ...
        if (config.transport === 'stdio') {
            ...
            try {
                const lspConnection = await this.connectSocketWithRetry(config.socket, startupTimeout);
                return {
                    connection: lspConnection.connection,
                    process,
                    shutdown: async () => {
