...
const results = [];
function record(method, language, passed, detail) {
...
function loc(filePath, line, char) {
    return {
...
function createService(workspaceRoot) {
    ...
    const workspaceContext = {
        ...
        readFile: async () => '{}',
        resolvePath: (p) => path.resolve(workspaceRoot, p),
    };
    const fileDiscovery = {
        discoverFiles: async () => [],
        shouldIgnoreFile: () => false,
    };
    return new NativeLspService(config, workspaceContext, new EventEmitter(), fileDiscovery, {}, { workspaceRoot, requireTrustedWorkspace: false });
}
/* ------------------------------------------------------------------ */
/*  Per-language test runner                                          */
/* ------------------------------------------------------------------ */
async function testLanguage(cfg) {
    const { langName, workspaceRoot, positions, indexWaitMs, serverLimitedMethods, } = cfg;
    const isServerLimited = (method) => serverLimitedMethods?.has(method) ?? false;
    console.log(bold(`\n=============== ${langName} ===============`));
    ...
    try {
        ...
        /* ---------- 1. hover ---------- */
        try {
            const hoverLoc = loc(positions.hover.file, positions.hover.line, positions.hover.char);
            const hover = await service.hover(hoverLoc);
            if (hover?.contents) {
                record('hover', langName, true, `"${hover.contents.substring(0, 100)}"`);
            }
            else {
        ...
        /* ---------- 2. definitions ---------- */
        try {
            const defLoc = loc(positions.definitions.file, positions.definitions.line, positions.definitions.char);
            const defs = await service.definitions(defLoc);
            record('definitions', langName, defs.length > 0, defs.length > 0 ? `${defs.length} def(s)` : 'Empty result');
        }
        catch (e) {
        ...
        /* ---------- 3. references ---------- */
        try {
            const refLoc = loc(positions.references.file, positions.references.line, positions.references.char);
            const refs = await service.references(refLoc, undefined, true);
            record('references', langName, refs.length > 0, refs.length > 0 ? `${refs.length} ref(s)` : 'Empty result');
        }
        catch (e) {
        ...
        try {
            ...
            if (symbols.length > 0) {
                const names = symbols
                    ...
                    .map((s) => s.name)
                    .join(', ');
                record('documentSymbols', langName, true, `${symbols.length} symbol(s): ${names}`);
            }
            else {
        ...
        try {
            ...
            if (wsSymbols.length > 0) {
                const names = wsSymbols
                    ...
                    .map((s) => s.name)
                    .join(', ');
                record('workspaceSymbols', langName, true, `${wsSymbols.length} symbol(s): ${names}`);
            }
            else {
        ...
        /* ---------- 6. implementations ---------- */
        try {
            const implLoc = loc(positions.implementations.file, positions.implementations.line, positions.implementations.char);
            const impls = await service.implementations(implLoc);
            record('implementations', langName, impls.length > 0, impls.length > 0 ? `${impls.length} impl(s)` : 'Empty result');
        }
        catch (e) {
        ...
        try {
            const callLoc = loc(positions.callHierarchy.file, positions.callHierarchy.line, positions.callHierarchy.char);
            const callItems = await service.prepareCallHierarchy(callLoc);
            if (callItems.length > 0) {
                record('prepareCallHierarchy', langName, true, `${callItems.length} item(s): ${callItems[0].name}`);
                try {
                    const incoming = await service.incomingCalls(callItems[0]);
                    record('incomingCalls', langName, incoming.length > 0, incoming.length > 0
                ...
                try {
                    ...
                    if (outgoing.length > 0) {
                        record('outgoingCalls', langName, true, `${outgoing.length} callee(s)`);
                    }
                    else if (isServerLimited('outgoingCalls')) {
                        record('outgoingCalls', langName, true, 'Empty (server does not implement this method)');
                    }
                    else {
                        record('outgoingCalls', langName, false, 'Empty (no callees found)');
                    }
                }
                catch (e) {
        ...
        try {
            ...
            const actions = await service.codeActions(caUri, { start: { line: 0, character: 0 }, end: { line: 0, character: 10 } }, { diagnostics: [], triggerKind: 'invoked' });
        ...
        try {
            ...
            record('workspaceDiagnostics', langName, true, `${wsDiags.length} file(s) with diagnostics`);
        }
        catch (e) {
...
async function main() {
    ...
    console.log('Verifying all 12 LSP methods with real servers (TS / C++ / Java)\n');
    ...
    console.log(`Total: ${results.length} | ${green(`Passed: ${passed}`)} | ${red(`Failed: ${failed}`)}`);
    console.log(bold('\nPer Language:'));
    for (const lang of ['TypeScript', 'C++', 'Java']) {
        const lr = results.filter((r) => r.language === lang);
        const lp = lr.filter((r) => r.passed).length;
        const icon = lp === lr.length ? green('ALL PASS') : yellow(`${lp}/${lr.length}`);
...
main();
//# sourceMappingURL=lsp-e2e-test.js.map