...
describe('GeminiChat', async () => {
    ...
    beforeEach(() => {
        ...
        chat = new GeminiChat(mockConfig, config, [], undefined, uiTelemetryService);
    });
    afterEach(() => {
        vi.restoreAllMocks();
        vi.resetAllMocks();
    ...
    describe('sendMessageStream', () => {
        ...
        it('should succeed when there is finish reason and only thought content (reasoning models)', async () => {
            ...
            vi.mocked(mockContentGenerator.generateContentStream).mockResolvedValue(thoughtOnlyStream);
            const stream = await chat.sendMessageStream('test-model', { message: 'test' }, 'prompt-id-thought-only');
        ...
        it('should succeed for thought-only content when finish reason arrives in a later chunk', async () => {
            ...
            vi.mocked(mockContentGenerator.generateContentStream).mockResolvedValue(streamWithDelayedFinishReason);
            const stream = await chat.sendMessageStream('test-model', { message: 'test' }, 'prompt-id-thought-delayed-finish');
            await expect((async () => {
                for await (const _ of stream) {
                    // consume stream
                }
        ...
        it('should succeed for thought-only responses with finish reason followed by usage-only chunk', async () => {
            ...
            vi.mocked(mockContentGenerator.generateContentStream).mockResolvedValue(thoughtThenUsageOnlyStream);
            const stream = await chat.sendMessageStream('test-model', { message: 'test' }, 'prompt-id-thought-usage-tail');
            await expect((async () => {
                for await (const _ of stream) {
                    // consume stream
                }
        ...
        it('should not update global telemetry when no telemetryService is provided (subagent isolation)', async () => {
            ...
            vi.mocked(mockContentGenerator.generateContentStream).mockResolvedValue(response);
            const stream = await subagentChat.sendMessageStream('test-model', { message: 'subagent task' }, 'prompt-id-subagent');
            for await (const _ of stream) {
                // consume stream
            }
        ...
        it('should retry immediately when skipDelay is called during rate-limit wait', async () => {
            ...
            try {
                ...
                vi.mocked(mockContentGenerator.generateContentStream)
                    ...
                    .mockResolvedValueOnce(successStream);
                const stream = await chat.sendMessageStream('test-model', { message: 'test' }, 'prompt-id-skip-delay');
                const iterator = stream[Symbol.asyncIterator]();
        ...
        it('should exit retry loop when aborted during rate-limit delay', async () => {
            ...
            try {
                ...
                vi.mocked(mockContentGenerator.generateContentStream)
                    ...
                    .mockResolvedValueOnce(failingStreamGenerator());
                const stream = await chat.sendMessageStream('test-model', { message: 'test', config: { abortSignal: abortController.signal } }, 'prompt-id-abort-delay');
                const iterator = stream[Symbol.asyncIterator]();
                ...
                const stream2 = await chat.sendMessageStream('test-model', { message: 'follow-up' }, 'prompt-id-after-abort');
                const events = [];
    ...
    describe('output token recovery', () => {
        function makeChunk(parts, finishReason) {
            return {
                candidates: [
                    {
        ...
        it('should enter recovery loop when escalated response is also truncated', async () => {
            ...
            let callIndex = 0;
            vi.mocked(mockContentGenerator.generateContentStream).mockImplementation(async () => streams[callIndex++]);
            const stream = await chat.sendMessageStream('gemini-3-pro', { message: 'write a long essay' }, 'prompt-recovery');
            const events = [];
            for await (const event of stream) {
                events.push(event);
            }
            ...
            expect(retries[0].isContinuation).toBe(undefined);
            expect(retries[1].isContinuation).toBe(true);
        ...
        it('should skip recovery when truncated turn has a functionCall', async () => {
            ...
            let callIndex = 0;
            vi.mocked(mockContentGenerator.generateContentStream).mockImplementation(async () => streams[callIndex++]);
            const stream = await chat.sendMessageStream('gemini-3-pro', { message: 'write a file' }, 'prompt-recovery-skip');
            const events = [];
            for await (const event of stream) {
                events.push(event);
            }
            // Only the escalation RETRY should fire; no continuation RETRY.
            const continuations = events.filter((e) => e.type === StreamEventType.RETRY &&
            ...
            expect(lastEntry.parts?.some((p) => 'functionCall' in p && p.functionCall)).toBe(true);
        });
        it('should cap recovery attempts at MAX_OUTPUT_RECOVERY_ATTEMPTS (3)', async () => {
            ...
            vi.mocked(mockContentGenerator.generateContentStream).mockImplementation(async () => makeStream([makeChunk([{ text: 'x' }], 'MAX_TOKENS')]));
            const stream = await chat.sendMessageStream('gemini-3-pro', { message: 'infinite loop test' }, 'prompt-recovery-cap');
            // Consume
            for await (const _ of stream) {
                /* consume */
            }
            // 1 initial + 1 escalation + 3 recovery = 5 total.
            expect(mockContentGenerator.generateContentStream).toHaveBeenCalledTimes(5);
        });
        it('should pop dangling recovery message and emit STOP chunk when recovery throws', async () => {
            ...
            let callIndex = 0;
            vi.mocked(mockContentGenerator.generateContentStream).mockImplementation(async () => streams[callIndex++]);
            const stream = await chat.sendMessageStream('gemini-3-pro', { message: 'recovery fails' }, 'prompt-recovery-fail');
            const events = [];
            for await (const event of stream) {
                events.push(event);
            }
            // The last chunk should be the synthetic STOP chunk from the catch.
            const chunkEvents = events.filter((e) => e.type === StreamEventType.CHUNK);
            const lastChunk = chunkEvents[chunkEvents.length - 1];
            expect(lastChunk.value.candidates?.[0]
        ...
        it('should stop recovery mid-loop when a later iteration emits functionCall', async () => {
            ...
            let callIndex = 0;
            vi.mocked(mockContentGenerator.generateContentStream).mockImplementation(async () => streams[callIndex++]);
            const stream = await chat.sendMessageStream('gemini-3-pro', { message: 'mixed recovery' }, 'prompt-recovery-mixed');
            for await (const _ of stream) {
                /* consume */
            }
        ...
        it('should coalesce successful recovery iterations into the preceding model turn', async () => {
            ...
            let callIndex = 0;
            vi.mocked(mockContentGenerator.generateContentStream).mockImplementation(async () => streams[callIndex++]);
            const stream = await chat.sendMessageStream('gemini-3-pro', { message: 'essay' }, 'prompt-recovery-coalesce');
            for await (const _ of stream) {
                /* consume */
            }
            const history = chat.getHistory();
