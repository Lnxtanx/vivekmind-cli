...
import { generateToolUseId, firePreToolUseHook, firePostToolUseHook, firePostToolUseFailureHook, fireNotificationHook, appendAdditionalContext, firePermissionRequestHook, } from './toolHookTriggers.js';
...
describe('toolHookTriggers', () => {
    ...
    describe('firePreToolUseHook', () => {
        it('should return shouldProceed: true when no messageBus is provided', async () => {
            const result = await firePreToolUseHook(undefined, 'test-tool', {}, 'test-id', 'auto');
        ...
        it('should return shouldProceed: true when hook execution fails', async () => {
            ...
            const result = await firePreToolUseHook(mockMessageBus, 'test-tool', {}, 'test-id', 'auto');
            expect(result).toEqual({ shouldProceed: true });
        });
    ...
    describe('firePostToolUseHook', () => {
        it('should return shouldStop: false when no messageBus is provided', async () => {
            const result = await firePostToolUseHook(undefined, 'test-tool', {}, {}, 'test-id', 'auto');
        ...
        it('should return shouldStop: false when hook execution fails', async () => {
            ...
            const result = await firePostToolUseHook(mockMessageBus, 'test-tool', {}, {}, 'test-id', 'auto');
            expect(result).toEqual({ shouldStop: false });
        });
    ...
    describe('firePostToolUseFailureHook', () => {
        it('should return empty object when no messageBus is provided', async () => {
            const result = await firePostToolUseFailureHook(undefined, 'test-id', 'test-tool', {}, 'error message');
            expect(result).toEqual({});
        });
        it('should return empty object when hook execution fails', async () => {
            ...
            const result = await firePostToolUseFailureHook(mockMessageBus, 'test-id', 'test-tool', {}, 'error message');
            expect(result).toEqual({});
        });
    ...
    describe('appendAdditionalContext', () => {
        ...
        it('should append context to string content', () => {
            const result = appendAdditionalContext('original content', 'additional context');
            expect(result).toBe('original content\n\nadditional context');
        });
        it('should append context as text part to PartListUnion array', () => {
            const originalContent = [{ text: 'original' }];
            const result = appendAdditionalContext(originalContent, 'additional context');
            expect(result).toEqual([
                { text: 'original' },
    ...
    describe('fireNotificationHook', () => {
        it('should return empty object when no messageBus is provided', async () => {
            const result = await fireNotificationHook(undefined, 'Test notification', NotificationType.PermissionPrompt, 'Test Title');
            expect(result).toEqual({});
        });
        it('should return empty object when hook execution fails', async () => {
            const mockMessageBus = createMockMessageBus();
            ...
            const result = await fireNotificationHook(mockMessageBus, 'Test notification', NotificationType.PermissionPrompt);
            expect(result).toEqual({});
        });
        it('should return empty object when hook output is empty', async () => {
            const mockMessageBus = createMockMessageBus();
            ...
            const result = await fireNotificationHook(mockMessageBus, 'Test notification', NotificationType.IdlePrompt);
            expect(result).toEqual({});
        });
        it('should return additional context when available', async () => {
            const mockOutput = {
            ...
            const result = await fireNotificationHook(mockMessageBus, 'Test notification', NotificationType.AuthSuccess);
            expect(result).toEqual({
                additionalContext: 'Additional context from notification hook',
            });
        });
        it('should send correct parameters to MessageBus for permission_prompt', async () => {
            ...
            await fireNotificationHook(mockMessageBus, 'VivekMind needs your permission to use Bash', NotificationType.PermissionPrompt, 'Permission needed');
            expect(mockMessageBus.request).toHaveBeenCalledWith({
                type: MessageBusType.HOOK_EXECUTION_REQUEST,
                eventName: 'Notification',
                input: {
                    message: 'VivekMind needs your permission to use Bash',
        ...
        it('should send correct parameters to MessageBus for idle_prompt', async () => {
            ...
            await fireNotificationHook(mockMessageBus, 'VivekMind is waiting for your input', NotificationType.IdlePrompt, 'Waiting for input');
            expect(mockMessageBus.request).toHaveBeenCalledWith({
                type: MessageBusType.HOOK_EXECUTION_REQUEST,
                eventName: 'Notification',
                input: {
                    message: 'VivekMind is waiting for your input',
        ...
        it('should send correct parameters to MessageBus for auth_success', async () => {
            ...
            await fireNotificationHook(mockMessageBus, 'Authentication successful', NotificationType.AuthSuccess);
            expect(mockMessageBus.request).toHaveBeenCalledWith({
                type: MessageBusType.HOOK_EXECUTION_REQUEST,
                eventName: 'Notification',
                input: {
        ...
        it('should send correct parameters to MessageBus for elicitation_dialog', async () => {
            ...
            await fireNotificationHook(mockMessageBus, 'Dialog shown to user', NotificationType.ElicitationDialog, 'Dialog');
            expect(mockMessageBus.request).toHaveBeenCalledWith({
                type: MessageBusType.HOOK_EXECUTION_REQUEST,
                eventName: 'Notification',
                input: {
        ...
        it('should handle notification without title', async () => {
            ...
            await fireNotificationHook(mockMessageBus, 'Test notification without title', NotificationType.IdlePrompt);
            expect(mockMessageBus.request).toHaveBeenCalledWith({
                type: MessageBusType.HOOK_EXECUTION_REQUEST,
                eventName: 'Notification',
                input: {
    ...
    describe('firePermissionRequestHook', () => {
        it('should return hasDecision: false when no messageBus is provided', async () => {
            const result = await firePermissionRequestHook(undefined, 'test-tool', {}, 'auto');
        ...
        it('should return hasDecision: false when hook execution fails', async () => {
            ...
            const result = await firePermissionRequestHook(mockMessageBus, 'test-tool', {}, 'auto');
            expect(result).toEqual({ hasDecision: false });
        });
        ...
        it('should return hasDecision: true with allow decision when tool is allowed', async () => {
            ...
            const result = await firePermissionRequestHook(mockMessageBus, 'run_shell_command', { command: 'ls' }, 'auto');
            expect(result).toEqual({
                hasDecision: true,
                shouldAllow: true,
                updatedInput: { command: 'ls -la' },
        ...
        it('should return hasDecision: true with deny decision when tool is denied', async () => {
            ...
            const result = await firePermissionRequestHook(mockMessageBus, 'run_shell_command', { command: 'rm -rf /' }, 'auto');
            expect(result).toEqual({
                hasDecision: true,
                shouldAllow: false,
        ...
        it('should send correct parameters to MessageBus', async () => {
            ...
            await firePermissionRequestHook(mockMessageBus, 'run_shell_command', { command: 'ls' }, 'auto', [
                {
                    type: 'always_allow',
        ...
        it('should handle permission_suggestions being undefined', async () => {
            ...
            await firePermissionRequestHook(mockMessageBus, 'run_shell_command', { command: 'ls' }, 'auto', undefined);
            expect(mockMessageBus.request).toHaveBeenCalledWith({
                type: MessageBusType.HOOK_EXECUTION_REQUEST,
                eventName: 'PermissionRequest',
                input: {
        ...
        it('should handle different permission modes', async () => {
            ...
            const result1 = await firePermissionRequestHook(mockMessageBus, 'test-tool', {}, 'plan');
            expect(result1.hasDecision).toBe(true);
            const result2 = await firePermissionRequestHook(mockMessageBus, 'test-tool', {}, 'yolo');
            expect(result2.hasDecision).toBe(true);
        });
    });
});
//# sourceMappingURL=toolHookTriggers.test.js.map