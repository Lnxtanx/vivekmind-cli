...
export class BedrockContentConverter {
     ...
     */
    convertGeminiRequestToConverse(request) {
        const messages = [];
     ...
     */
    async convertGeminiToolsToConverse(geminiTools) {
        const tools = [];
     ...
     */
    convertConverseResponseToGemini(response, model) {
        ...
        const finishReason = this.mapBedrockStopReasonToGemini(response.stopReason);
     ...
     */
    buildGeminiStreamChunk(part, model, finishReason, usageMetadata) {
     ...
     */
    mapBedrockStopReasonToGemini(reason) {
        if (!reason)
            return undefined;
        const mapping = {
    ...
    processContent(content, messages) {
        ...
        for (const part of parts) {
            ...
            if (part.functionResponse && role === 'user') {
                const toolResultBlocks = this.createToolResultBlocks(part.functionResponse);
                contentBlocks.push({
                    toolResult: {
    ...
    createToolResultBlocks(response) {
        const blocks = [];
