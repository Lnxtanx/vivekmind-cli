...
describe('OpenAIContentConverter', () => {
    ...
    function withStreamParser(toolCallParser = new StreamingToolCallParser()) {
        return {
            ...requestContext,
            toolCallParser,
        };
    }
    function withTaggedThinkingOptions() {
        return {
            ...requestContext,
            responseParsingOptions: { taggedThinkingTags: true },
        };
    }
    function withTaggedThinkingStreamParser() {
        return {
            ...withStreamParser(),
            responseParsingOptions: { taggedThinkingTags: true },
    ...
    describe('stream-local parser state', () => {
        ...
        it('demuxes interleaved chunks from two concurrent streams correctly (#3516)', () => {
            ...
            const resultA = converter.convertOpenAIChunkToGemini(finisher('A-finish'), streamA);
            const resultB = converter.convertOpenAIChunkToGemini(finisher('B-finish'), streamB);
            const fnA = resultA.candidates?.[0]?.content?.parts?.find((p) => p.functionCall)?.functionCall;
            const fnB = resultB.candidates?.[0]?.content?.parts?.find((p) => p.functionCall)?.functionCall;
    ...
    describe('convertGeminiRequestToOpenAI', () => {
        ...
        it('should extract raw output from function response objects', () => {
            ...
            const contentArray = toolMessage?.content;
        ...
        it('should convert function responses with inlineData to tool message with embedded image_url', () => {
            ...
            expect(contentArray[1].image_url?.url).toBe('data:image/png;base64,base64encodedimagedata');
        ...
        it('should split tool-result media into a follow-up user message when splitToolMedia is enabled (issue #3616)', () => {
            ...
            const messages = converter.convertGeminiRequestToOpenAI(request, strictContext);
            ...
            expect(imagePart?.image_url?.url).toBe('data:image/png;base64,base64encodedimagedata');
        });
        it('should keep all tool messages contiguous and merge split media into a single follow-up user message for parallel tool calls (issue #3616)', () => {
            ...
            const isSpecCompliantToolContent = (content) => {
                if (typeof content === 'string')
                    return true;
                if (!Array.isArray(content))
                    return false;
                return content.every((p) => p.type === 'text');
            };
            expect(isSpecCompliantToolContent(messages[assistantIdx + 1].content)).toBe(true);
            expect(isSpecCompliantToolContent(messages[assistantIdx + 2].content)).toBe(true);
            ...
            expect(imageParts[0].image_url?.url).toBe('data:image/png;base64,shotbase64');
        });
        ...
        it('should fall back to a placeholder string when the tool response is media-only (issue #3616)', () => {
            ...
            const toolMessage = messages.find((m) => m.role === 'tool');
            expect(toolMessage).toBeDefined();
            expect(toolMessage?.content).toBe('[media attached in following user message]');
            const userMessage = messages.find((m) => m.role === 'user');
            const userContent = userMessage?.content;
        ...
        it('should preserve prior embedded-media behavior when splitToolMedia is false (default) on parallel tool calls (issue #3616)', () => {
            ...
            const firstToolContent = toolMessages[0].content;
        ...
        it('should convert PDF inlineData to tool message with embedded input_file', () => {
            ...
            expect(contentArray[1].file?.file_data).toBe('data:application/pdf;base64,base64pdfdata');
            // No separate user message should be created
            const userMessage = messages.find((message) => message.role === 'user');
            expect(userMessage).toBeUndefined();
        });
        it('should convert audio parts to tool message with embedded input_audio', () => {
            ...
            expect(contentArray[1].input_audio?.data).toBe('data:audio/wav;base64,audiobase64data');
        ...
        it('should convert image fileData URL to tool message with embedded image_url', () => {
                                    ...
                                    parts: [
                                        {
                                            fileData: {
                                                mimeType: 'image/jpeg',
                                                fileUri: 'https://upload.wikimedia.org/wikipedia/commons/a/a7/Camponotus_flavomarginatus_ant.jpg',
                                                displayName: 'ant.jpg',
                                            },
                                        },
                                    ],
            ...
            expect(contentArray[0].text).toBe('Image content');
            expect(contentArray[1].type).toBe('image_url');
            expect(contentArray[1].image_url?.url).toBe('https://upload.wikimedia.org/wikipedia/commons/a/a7/Camponotus_flavomarginatus_ant.jpg');
        });
        it('should convert PDF fileData URL to tool message with embedded file', () => {
                                    ...
                                    parts: [
                                        {
                                            fileData: {
                                                mimeType: 'application/pdf',
                                                fileUri: 'https://assets.anthropic.com/m/1cd9d098ac3e6467/original/Claude-3-Model-Card-October-Addendum.pdf',
                                                displayName: 'document.pdf',
                                            },
                                        },
                                    ],
            ...
            expect(contentArray[1].type).toBe('file');
            expect(contentArray[1].file?.filename).toBe('document.pdf');
            expect(contentArray[1].file?.file_data).toBe('https://assets.anthropic.com/m/1cd9d098ac3e6467/original/Claude-3-Model-Card-October-Addendum.pdf');
        });
        it('should convert video inlineData to tool message with embedded video_url', () => {
            ...
            expect(contentArray[1].video_url?.url).toBe('data:video/mp4;base64,videobase64data');
            // No separate user message should be created
            const userMessage = messages.find((message) => message.role === 'user');
            expect(userMessage).toBeUndefined();
        });
        it('should convert video fileData URL to tool message with embedded video_url', () => {
            ...
            expect(contentArray[0].text).toBe('Video content');
            expect(contentArray[1].type).toBe('video_url');
            expect(contentArray[1].video_url?.url).toBe('https://www.youtube.com/watch?v=dQw4w9WgXcQ');
        });
    ...
    describe('MCP multi-part tool results (issue #1520)', () => {
        ...
        it('should include text and image in tool message when function response has media parts', () => {
                        ...
                        parts: [
                            {
                                functionResponse: {
                                    ...
                                    name: 'figma_get_screenshot',
                                    response: {
                                        output: "[Tool 'figma' provided the following image data with mime-type: image/png]",
                                    },
                                    parts: [
                                        {
                                            inlineData: {
            ...
            const toolMessage = messages.find((m) => m.role === 'tool');
            expect(toolMessage).toBeDefined();
            expect(toolMessage.tool_call_id).toBe('call_mcp_2');
            // Tool message should contain both text and image
            const toolContent = toolMessage?.content;
            expect(Array.isArray(toolContent)).toBe(true);
            const contentArray = toolContent;
            expect(contentArray).toHaveLength(2);
            expect(contentArray[0].type).toBe('text');
...
describe('MCP tool result end-to-end through OpenAI converter (issue #1520)', () => {
    ...
    it('should preserve MCP multi-text content in tool message (not leak to user message)', () => {
        ...
        const responseParts = convertToFunctionResponse(toolName, callId, mcpTransformedParts);
...
describe('Truncated tool call detection in streaming', () => {
    ...
    function createStreamingRequestContext(model = 'test-model') {
        return {
            model,
            modalities: {},
            startTime: 0,
     ...
     */
    function feedToolCallChunks(conv, toolCallChunks, finishReason) {
    ...
    it('should override finishReason to MAX_TOKENS when tool call JSON is truncated and provider reports "stop"', () => {
        ...
        const result = feedToolCallChunks(converter, [
            {
                index: 0,
                id: 'call_1',
                name: 'write_file',
...
describe('modality filtering', () => {
    ...
    function makeRequestContext(model, modalities) {
        return {
            model,
            modalities,
            startTime: 0,
    ...
    function getUserContentParts(messages) {
    ...
    it('replaces image with placeholder when image modality is disabled', () => {
        ...
        const messages = conv.convertGeminiRequestToOpenAI(request, makeRequestContext('deepseek-chat', {}));
    ...
    it('keeps image when image modality is enabled', () => {
        ...
        const messages = conv.convertGeminiRequestToOpenAI(request, makeRequestContext('gpt-4o', { image: true }));
        const parts = getUserContentParts(messages);
        expect(parts).toHaveLength(1);
    ...
    it('replaces PDF with placeholder when pdf modality is disabled', () => {
        ...
        const messages = conv.convertGeminiRequestToOpenAI(request, makeRequestContext('test-model', { image: true }));
        const parts = getUserContentParts(messages);
        expect(parts).toHaveLength(1);
        expect(parts[0].type).toBe('text');
    ...
    it('keeps PDF when pdf modality is enabled', () => {
        ...
        const messages = conv.convertGeminiRequestToOpenAI(request, makeRequestContext('claude-sonnet', { image: true, pdf: true }));
        const parts = getUserContentParts(messages);
        expect(parts).toHaveLength(1);
    ...
    it('replaces video with placeholder when video modality is disabled', () => {
        ...
        const messages = conv.convertGeminiRequestToOpenAI(request, makeRequestContext('test-model', {}));
        const parts = getUserContentParts(messages);
        expect(parts).toHaveLength(1);
        expect(parts[0].type).toBe('text');
    ...
    it('defaults to text-only when no modalities are specified', () => {
        ...
        const messages = conv.convertGeminiRequestToOpenAI(request, makeRequestContext('unknown-model', {}));
        const parts = getUserContentParts(messages);
        expect(parts).toHaveLength(1);
        expect(parts[0].type).toBe('text');
        expect(parts[0].text).toContain('image file');
