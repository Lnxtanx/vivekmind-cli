 ...
 */
async function* interleavingStream(chunks) {
...
function openerChunk(id, name, firstArgs) {
    return {
        id: `${id}-opener`,
...
function continuationChunk(argsFragment) {
    return {
        id: 'cont',
        object: 'chat.completion.chunk',
        created: 1,
...
describe('ContentGenerationPipeline — concurrent streams (issue #3516)', () => {
    function buildPipeline(createStreamImpl) {
        const mockClient = {
            chat: {
                completions: {
    ...
    it('two concurrent streams keep their tool-call buffers isolated', async () => {
        ...
        const streamQueue = [];
        streamQueue.push(() => interleavingStream([
        ...
        const { pipeline } = buildPipeline(() => {
            ...
            if (!next)
                throw new Error('unexpected extra stream request');
            return next();
        });
        const request = {
            model: 'test-model',
        ...
        while (true) {
            ...
            if (!aNext.done)
                collectedA.push(aNext.value);
            if (!bNext.done)
                collectedB.push(bNext.value);
            if (aNext.done && bNext.done)
                break;
        }
        const extractFunctionCall = (responses) => {
...
//# sourceMappingURL=pipeline.concurrent.test.js.map