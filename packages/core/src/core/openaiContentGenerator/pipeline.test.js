...
describe('ContentGenerationPipeline', () => {
    ...
    describe('execute', () => {
        it('should successfully execute non-streaming request', async () => {
            ...
            expect(mockConverter.convertGeminiRequestToOpenAI).toHaveBeenCalledWith(request, expect.objectContaining({
                model: 'test-model',
                modalities: {},
            }));
            ...
            expect(mockConverter.convertOpenAIResponseToGemini).toHaveBeenCalledWith(mockOpenAIResponse, expect.objectContaining({
                model: 'test-model',
                modalities: {},
            }));
        });
        ...
        it('should handle tools in request', async () => {
            ...
            expect(mockConverter.convertGeminiToolsToOpenAI).toHaveBeenCalledWith(request.config.tools, 'auto');
            expect(mockClient.chat.completions.create).toHaveBeenCalledWith(expect.objectContaining({
                tools: mockTools,
            }), expect.objectContaining({
                signal: undefined,
        ...
        it('should skip empty tools array in request', async () => {
            ...
            const apiCall = mockClient.chat.completions.create.mock
                .calls[0][0];
        ...
        it('should override enable_thinking when thinkingConfig disables it', async () => {
            ...
            mockProvider.buildRequest.mockImplementation((req) => ({
                ...req,
    ...
    describe('executeStream', () => {
        it('should successfully execute streaming request', async () => {
            ...
            const [, firstChunkContext] = mockConverter.convertOpenAIChunkToGemini.mock.calls[0];
            const [, secondChunkContext] = mockConverter.convertOpenAIChunkToGemini.mock.calls[1];
            expect(firstChunkContext).toEqual(expect.objectContaining({
                model: 'test-model',
                modalities: {},
                toolCallParser: expect.any(StreamingToolCallParser),
            }));
            expect(secondChunkContext.toolCallParser).toBe(firstChunkContext.toolCallParser);
            expect(mockClient.chat.completions.create).toHaveBeenCalledWith(expect.objectContaining({
                stream: true,
        ...
        it('should not duplicate function calls when trailing chunks arrive after finish+usage merge', async () => {
            ...
            expect(mergedResult.candidates?.[0]?.finishReason).toBe(FinishReason.STOP);
            expect(mergedResult.candidates?.[0]?.content?.parts?.[0]?.functionCall?.name).toBe('todoWrite');
            ...
            for (const result of results) {
                ...
                totalFunctionCalls += parts.filter((p) => p.functionCall).length;
            }
    ...
    describe('buildRequest', () => {
        ...
        it('should pass arbitrary samplingParams keys through verbatim (e.g. max_completion_tokens for GPT-5)', async () => {
            ...
            const call = mockClient.chat.completions.create.mock
                .calls[0][0];
