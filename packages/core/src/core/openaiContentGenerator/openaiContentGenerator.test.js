...
describe('OpenAIContentGenerator (Refactored)', () => {
    ...
    describe('shouldSuppressErrorLogging', () => {
        ...
        beforeEach(() => {
            ...
            testGenerator = new TestGenerator(contentGeneratorConfig, mockConfig, mockProvider);
        });
        ...
        it('should return true for AbortError when signal is also aborted (user cancellation)', () => {
            ...
            const result = testGenerator.testShouldSuppressErrorLogging(abortError, request);
        ...
        it('should return false for non-AbortError even when signal is aborted', () => {
            ...
            const result = testGenerator.testShouldSuppressErrorLogging(new Error('Network error'), request);
            expect(result).toBe(false);
        });
        ...
        it('should allow subclasses to override error suppression behavior', async () => {
            ...
            const customGenerator = new CustomTestGenerator(contentGeneratorConfig, mockConfig, mockProvider);
            const request = {
                contents: [{ role: 'user', parts: [{ text: 'Hello' }] }],
                model: 'gpt-4',
            };
            const result = customGenerator.testShouldSuppressErrorLogging(new Error('Test error'), request);
            expect(result).toBe(true);
        });
    });
});
