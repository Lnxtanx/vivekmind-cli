...
describe('DeepSeekOpenAICompatibleProvider', () => {
    ...
    describe('buildRequest', () => {
        ...
        it('does NOT translate reasoning_effort on a non-DeepSeek hostname (model-name fallback only)', () => {
            ...
            const selfHostedProvider = new DeepSeekOpenAICompatibleProvider(selfHostedConfig, mockCliConfig);
            const originalRequest = {
                model: 'deepseek-v4-pro',
                messages: [
                    {
            ...
            const result = selfHostedProvider.buildRequest(originalRequest, userPromptId);
            const r = result;
    ...
    describe('isDeepSeekHostname', () => {
        it('matches api.deepseek.com baseUrls', () => {
            expect(DeepSeekOpenAICompatibleProvider.isDeepSeekHostname({
                baseUrl: 'https://api.deepseek.com/v1',
            })).toBe(true);
        });
