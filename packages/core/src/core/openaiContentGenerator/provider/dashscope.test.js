...
describe('DashScopeOpenAICompatibleProvider', () => {
    ...
    beforeEach(() => {
        vi.clearAllMocks();
        const mockedBuildRuntimeFetchOptions = buildRuntimeFetchOptions;
        ...
        provider = new DashScopeOpenAICompatibleProvider(mockContentGeneratorConfig, mockCliConfig);
    });
    ...
    describe('isDashScopeProvider', () => {
        it('should return true for VIVEKMIND_OAUTH auth type', () => {
            const config = {
                authType: AuthType.VIVEKMIND_OAUTH,
    ...
    describe('buildMetadata', () => {
        ...
        it('should handle missing session ID', () => {
            ...
            delete mockCliConfig['getSessionId'];
            const userPromptId = 'test-prompt-id';
            const metadata = provider.buildMetadata(userPromptId);
            expect(metadata).toEqual({
                metadata: {
    ...
    describe('buildRequest', () => {
        ...
        it('should add cache control to system, last history message, and last tool definition for streaming requests', () => {
            ...
            const result = provider.buildRequest(requestWithToolMessage, 'test-prompt-id');
            expect(result.messages).toHaveLength(4);
            // System message should have cache control
            const systemMessage = result.messages[0];
            expect(systemMessage.content).toEqual([
        ...
        it('should not add cache control to tool messages when request.tools is undefined', () => {
            const requestWithoutConfiguredTools = {
                ...baseRequest,
                messages: [
                    { role: 'system', content: 'You are a helpful assistant.' },
                    {
            ...
            const result = provider.buildRequest(requestWithoutConfiguredTools, 'test-prompt-id');
        ...
        it('should handle messages with array content for streaming requests', () => {
            ...
            const result = provider.buildRequest(requestWithArrayContent, 'test-prompt-id');
        ...
        it('should handle messages without content for streaming requests', () => {
            ...
            const result = provider.buildRequest(requestWithoutContent, 'test-prompt-id');
        ...
        it('should add cache control to last text item in mixed content for streaming requests', () => {
            ...
            const result = provider.buildRequest(requestWithMixedContent, 'test-prompt-id');
        ...
        it('should add cache control to last item even if not text for streaming requests', () => {
            ...
            const result = provider.buildRequest(requestWithNonTextLast, 'test-prompt-id');
            const content = result.messages[0]
                .content;
            expect(content).toHaveLength(2);
    ...
    describe('cache control edge cases', () => {
        ...
        it('should handle empty content array for streaming requests', () => {
            ...
            const result = provider.buildRequest(emptyContentRequest, 'test-prompt-id');
            const content = result.messages[0]
                .content;
    ...
    describe('output token limits', () => {
        ...
        it('should set high resolution flag for the coder-model model', () => {
            ...
            expect(result
                .vl_high_resolution_images).toBe(true);
        });
...
//# sourceMappingURL=dashscope.test.js.map