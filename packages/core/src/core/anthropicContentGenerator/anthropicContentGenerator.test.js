...
describe('AnthropicContentGenerator', () => {
    ...
    describe('per-request anthropic-beta header', () => {
        ...
        async function callOnce(config, requestConfig) {
            const { AnthropicContentGenerator } = await importGenerator();
        ...
        it('merges user-supplied customHeaders[anthropic-beta] with computed flags (no overwrite)', async () => {
            ...
            expect(beta.split(',')).toEqual(expect.arrayContaining([
                'experimental-x',
                'experimental-y',
        ...
        it('honors customHeaders[anthropic-beta] under mixed-case keys (Anthropic-Beta / ANTHROPIC-BETA)', async () => {
            ...
            expect(headersUpper['anthropic-beta']).toContain('interleaved-thinking-2025-05-14');
            ...
            expect(headersTitle['anthropic-beta']).toContain('interleaved-thinking-2025-05-14');
        });
        ...
        it('omits beta header when per-request thinkingConfig.includeThoughts=false', async () => {
            ...
            const headers = await callOnce({ ...baseConfig, reasoning: { effort: 'medium' } }, { thinkingConfig: { includeThoughts: false } });
            expect(headers['anthropic-beta']).toBeUndefined();
        });
        it('keeps customHeaders + User-Agent in defaultHeaders while sending computed anthropic-beta per-request', async () => {
            ...
            const [, options] = anthropicState.lastCreateArgs;
            const reqHeaders = (options
                ?.headers || {});
            expect(reqHeaders['User-Agent']).toBeUndefined();
            expect(reqHeaders['X-Custom']).toBeUndefined();
            expect(reqHeaders['anthropic-beta']).toContain('interleaved-thinking-2025-05-14');
        });
        it('also sends the computed beta header on streaming requests', async () => {
            ...
            const generator = new AnthropicContentGenerator({ ...baseConfig, reasoning: { effort: 'medium' } }, mockConfig);
    ...
    describe('generateContent', () => {
        ...
        it("still clamps effort: 'max' when model name says 'deepseek' but hostname is api.anthropic.com", async () => {
            ...
            const [anthropicRequest] = anthropicState.lastCreateArgs;
            expect(anthropicRequest).toEqual(expect.objectContaining({ output_config: { effort: 'high' } }));
        });
        ...
        describe('output token limits', () => {
            it('caps configured samplingParams.max_tokens to model output limit', async () => {
                ...
                const [anthropicRequest] = anthropicState.lastCreateArgs;
                expect(anthropicRequest).toEqual(expect.objectContaining({ max_tokens: 65536 }));
            });
            ...
            it('uses conservative default when max_tokens is not explicitly configured', async () => {
                ...
                const [anthropicRequest] = anthropicState.lastCreateArgs;
                expect(anthropicRequest).toEqual(expect.objectContaining({ max_tokens: 8000 }));
            });
            it('respects configured max_tokens for unknown models', async () => {
                ...
                const [anthropicRequest] = anthropicState.lastCreateArgs;
                expect(anthropicRequest).toEqual(expect.objectContaining({ max_tokens: 100_000 }));
            });
    ...
    describe('DeepSeek anthropic-compatible provider', () => {
        ...
        it('reflects runtime model changes (no stale provider cache)', async () => {
            ...
            let [req] = anthropicState.lastCreateArgs;
            ...
            [req] = anthropicState.lastCreateArgs;
        ...
        it('does not inject when request sets thinkingConfig.includeThoughts=false', async () => {
            ...
            const messages = anthropicRequest.messages;
            expect(anthropicRequest).toEqual(expect.not.objectContaining({ thinking: expect.anything() }));
            expect(anthropicRequest).toEqual(expect.not.objectContaining({ output_config: expect.anything() }));
            expect(messages[1]).toEqual(toolOnlyAssistant);
        });
    });
