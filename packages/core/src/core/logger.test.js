...
describe('Logger', () => {
    ...
    describe('initialize', () => {
        ...
        it('should load existing logs and set correct messageId for the current session', async () => {
            ...
            await fs.writeFile(testLogFilePath, JSON.stringify(existingLogs, null, 2));
    ...
    describe('saveCheckpoint', () => {
        ...
        it.each([
            ...
            const taggedFilePath = path.join(testGeminiDir, `checkpoint-${encodedTag}.json`);
    ...
    describe('loadCheckpoint', () => {
        ...
        beforeEach(async () => {
            await fs.writeFile(testCheckpointFilePath, JSON.stringify(conversation, null, 2));
        });
        it.each([
            {
                tag: 'test-tag',
    ...
    describe('deleteCheckpoint', () => {
        ...
        let taggedFilePath;
        beforeEach(async () => {
            taggedFilePath = path.join(testGeminiDir, `checkpoint-${encodedTag}.json`);
        ...
        it('should delete both new and old checkpoint files if they exist', async () => {
            ...
            const oldStylePath = path.join(testGeminiDir, `checkpoint-${oldTag}.json`);
