...
import { evaluatePermissionFlow, needsConfirmation, isPlanModeBlocked, isAutoEditApproved, } from './permissionFlow.js';
...
export function extractToolFilePaths(toolName, toolInput) {
    ...
    const canonical = ToolNamesMigration[toolName] ?? toolName;
...
export class CoreToolScheduler {
    ...
    setArgsInternal(targetCallId, args) {
        this.toolCalls = this.toolCalls.map((call) => {
            ...
            const invocationOrError = this.buildInvocation(call.tool, args, targetCallId);
    ...
    buildInvocation(tool, args, callId) {
        try {
        ...
        try {
            ...
            for (const reqInfo of requestsToProcess) {
                ...
                if (pm && !(await pm.isToolEnabled(reqInfo.name))) {
                    ...
                    const ruleInfo = matchingRule
                        ...
                        : '';
                    const permissionErrorMessage = `VivekMind requires permission to use "${reqInfo.name}", but that permission was declined.${ruleInfo}`;
                    newToolCalls.push({
                        status: 'error',
                        request: reqInfo,
                ...
                if (!pm) {
                    ...
                    if (excludeTools && excludeTools.length > 0) {
                        ...
                        if (excludedMatch) {
                            const permissionErrorMessage = `VivekMind requires permission to use ${excludedMatch}, but that permission was declined.`;
                            newToolCalls.push({
                                status: 'error',
                                request: reqInfo,
                                response: createErrorResponse(reqInfo, new Error(permissionErrorMessage), ToolErrorType.EXECUTION_DENIED),
                ...
                const invocationOrError = this.buildInvocation(toolInstance, reqInfo.args, reqInfo.callId);
                if (invocationOrError instanceof Error) {
            ...
            for (const toolCall of newToolCalls) {
                ...
                try {
                    ...
                    const flowResult = await evaluatePermissionFlow(this.config, invocation, reqInfo.name, toolParams);
                    ...
                    if (finalPermission === 'deny') {
                        ...
                        this.setStatusInternal(reqInfo.callId, 'error', createErrorResponse(reqInfo, new Error(denyMessage ?? `Tool "${reqInfo.name}" is denied.`), ToolErrorType.EXECUTION_DENIED));
                        continue;
                    }
                    ...
                    if (!needsConfirmation(finalPermission, approvalMode, reqInfo.name)) {
                        ...
                        if (isNonInteractiveDeny) {
                            const errorMessage = `VivekMind requires permission to use "${reqInfo.name}", but that permission was declined (non-interactive mode cannot prompt for confirmation).`;
                        ...
                        if (hooksEnabled && messageBus) {
                            fireNotificationHook(messageBus, `VivekMind needs your permission to use ${reqInfo.name}`, NotificationType.PermissionPrompt, 'Permission needed').catch((error) => {
    ...
    async executeSingleToolCall(toolCall, signal) {
        ...
        try {
            ...
            if (toolResult.error === undefined) {
                ...
                // activation pipeline.
                const candidatePaths = extractToolFilePaths(toolName, toolInput).map((p) => unescapePath(p));
                if (candidatePaths.length > 0) {
                    ...
                    if (activatedSkills && activatedSkills.length > 0) {
                        ...
                        if (hasSkillTool) {
                            ...
                            reminderBlocks.push(`The following skill(s) are now available via the Skill tool based on the file you just accessed: ${names}. Use them if relevant to the task.`);
                        }
                    }
                    if (reminderBlocks.length > 0) {
                        ...
                        content = appendAdditionalContext(content, `<system-reminder>\n${body}\n</system-reminder>`);
                    }
                }
    ...
    async autoApproveCompatiblePendingTools(signal, triggeringCallId) {
        ...
        for (const pendingTool of pendingTools) {
            try {
                ...
                const flowResult = await evaluatePermissionFlow(this.config, pendingTool.invocation, pendingTool.request.name, toolParams);
