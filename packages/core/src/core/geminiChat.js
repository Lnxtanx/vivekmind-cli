 ...
 */
const OUTPUT_RECOVERY_MESSAGE = 'Output token limit hit. Resume directly — no apology, no recap of what ' +
        ...
        return (async function* () {
            try {
                    ...
                    try {
                        ...
                        if (isContentError) {
                            if (attempt < INVALID_CONTENT_RETRY_OPTIONS.maxAttempts - 1) {
                                ...
                                await delay(INVALID_CONTENT_RETRY_OPTIONS.initialDelayMs * (attempt + 1), params.config?.abortSignal).promise;
                                continue;
                            }
                        }
                        break;
                ...
                if (lastError === null &&
                    ...
                    const escalatedLimit = Math.max(ESCALATED_MAX_TOKENS, tokenLimit(model, 'output'));
                    debugLogger.info(`Output truncated at capped default. Escalating to ${escalatedLimit} tokens.`);
                    ...
                    let escalatedFinishReason;
                    const escalatedStream = await self.makeApiCallAndProcessStream(model, requestContents, escalatedParams, prompt_id);
                    for await (const chunk of escalatedStream) {
                        const fr = chunk.candidates?.[0]?.finishReason;
                        if (fr)
                            escalatedFinishReason = fr;
                        yield { type: StreamEventType.CHUNK, value: chunk };
                    }
                    ...
                    while (escalatedFinishReason === FinishReason.MAX_TOKENS &&
                        ...
                        recoveryCount++;
                        debugLogger.info(`Output still truncated after escalation. ` +
                        ...
                        try {
                            const recoveryStream = await self.makeApiCallAndProcessStream(model, recoveryContents, escalatedParams, prompt_id);
