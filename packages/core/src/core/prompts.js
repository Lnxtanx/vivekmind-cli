...
export function getCoreSystemPrompt(userMemory, model, appendInstruction) {
...
`.trim();
    ...
    const writeSystemMdResolution = resolvePathFromEnv(process.env['VIVEKMIND_WRITE_SYSTEM_MD']);
...
function getToolCallExamples(model) {
    ...
    if (toolCallStyle) {
        switch (toolCallStyle.toLowerCase()) {
            ...
            default:
                debugLogger.warn(`Unknown VIVEKMIND_CODE_TOOL_CALL_STYLE value: ${toolCallStyle}. Using model-based detection.`);
                break;
        }
    }
