...
import { getArenaSystemReminder, getCoreSystemPrompt, getCustomSystemPrompt, getPlanModeSystemReminder, getSubagentSystemReminder, } from './prompts.js';
...
export class GeminiClient {
    ...
    stripOrphanedUserEntriesFromHistory() {
        ...
        debugLogger.debug('[FILE_READ_CACHE] clear after stripOrphanedUserEntriesFromHistory');
    ...
    async *sendMessageStream(request, signal, prompt_id, options, turns = MAX_TURNS) {
        ...
        if (messageType === SendMessageType.UserQuery ||
            ...
            const mcResult = microcompactHistory(this.getChat().getHistory(), this.lastApiCompletionTimestamp, this.config.getClearContextOnIdle());
            if (mcResult.meta) {
                ...
                debugLogger.debug(`[TIME-BASED MC] gap ${m.gapMinutes}min > ${m.thresholdMinutes}min, ` +
        ...
        // waiting for a response. The VivekMind API requires that a functionResponse
        ...
        for await (const event of resultStream) {
            ...
            if (event.type === GeminiEventType.Error) {
                if (arenaAgentClient) {
                    const errorMsg = event.value instanceof Error
                        ? event.value.message
                        : 'Unknown error';
        ...
        if (hooksEnabled &&
            ...
            if (stopOutput?.isBlockingDecision() ||
                ...
                return yield* this.sendMessageStream(continueRequest, signal, prompt_id, {
        ...
        if (!turn.pendingToolCalls.length && signal && !signal.aborted) {
            ...
            try {
                ...
                const cachedHistory = fullHistory.length > maxHistoryForCache
    ...
    async generateContent(contents, generationConfig, abortSignal, model, promptIdOverride) {
        ...
        try {
            ...
            const result = await retryWithBackoff(apiCall, {
                ...
                signal: abortSignal,
                heartbeatFn: (info) => {
                    process.stderr.write(`[vivekmind] Waiting for API capacity... attempt ${info.attempt}, retry in ${Math.ceil(info.remainingMs / 1000)}s\n`);
                },
            });
            return result;
        }
    ...
    async tryCompressChat(prompt_id, force = false, signal) {
        ...
        const { newHistory, info } = await compressionService.compress(this.getChat(), prompt_id, force, this.config.getModel(), this.config, this.hasFailedCompressionAttempt, signal);
