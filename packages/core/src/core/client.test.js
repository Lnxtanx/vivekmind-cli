...
import { DEFAULT_VIVEKMIND_FLASH_MODEL } from '../config/models.js';
...
vi.mock('../telemetry/index.js', async (importOriginal) => {
    ...
    return {
        ...
        // but we can spy on VivekMindLogger if needed
    };
});
...
describe('Gemini Client (client.ts)', () => {
    ...
    describe('history mutation invalidates FileReadCache', () => {
         ...
         */
        function mockChatWithLengths(before, after) {
            return {
                getHistoryLength: vi
                    .fn()
        ...
        it('retry strips orphaned trailing user entries and clears the cache', async () => {
            ...
            const stream = client.sendMessageStream([{ text: 'retry' }], new AbortController().signal, 'prompt-retry-1', { type: SendMessageType.Retry });
     ...
     */
    function mockFileReadCacheClear() {
    ...
    describe('thinking block idle cleanup and latch', () => {
        ...
        it('should update lastApiCompletionTimestamp after API call', async () => {
            ...
            const gen = client.sendMessageStream([{ text: 'Hello' }], new AbortController().signal, 'prompt-4', { type: SendMessageType.UserQuery });
            for await (const _ of gen) {
                /* drain */
            }
            expect(client['lastApiCompletionTimestamp']).toBeGreaterThanOrEqual(before);
        });
    ...
    describe('microcompaction FileReadCache invalidation', () => {
        function makeReadFileResponses(count) {
            const out = [];
        ...
        it('clears the cache after microcompaction strips old read_file results', async () => {
            ...
            const stream = client.sendMessageStream([{ text: 'hi' }], new AbortController().signal, 'prompt-mc-clear-1', { type: SendMessageType.UserQuery });
            for await (const _ of stream) {
                /* drain */
            }
        ...
        it('does not clear the cache when the idle gap is below the threshold', async () => {
            ...
            const stream = client.sendMessageStream([{ text: 'hi' }], new AbortController().signal, 'prompt-mc-clear-2', { type: SendMessageType.UserQuery });
            for await (const _ of stream) {
                /* drain */
            }
            expect(cacheClear).not.toHaveBeenCalled();
...
\`\`\``;
        ...
        it('should prepend relevant managed auto-memory prompt when recall returns content', async () => {
            ...
            client['chat'] = mockChat;
            const stream = client.sendMessageStream([{ text: 'Please answer tersely' }], new AbortController().signal, 'prompt-id-memory');
            for await (const _ of stream) {
                // consume stream
            }
            expect(mockMemoryManager.recall).toHaveBeenCalledWith('/test/project/root', 'Please answer tersely', expect.objectContaining({
                config: mockConfig,
                excludedFilePaths: expect.any(Set),
            }));
            expect(mockTurnRunFn).toHaveBeenCalledWith('test-model', expect.arrayContaining([
                '## Relevant memory\n\nUser prefers terse responses.',
        ...
        it('should track surfaced managed memory paths across user queries', async () => {
            ...
            client['chat'] = mockChat;
            const first = client.sendMessageStream([{ text: 'Please answer tersely' }], new AbortController().signal, 'prompt-id-memory-1');
            for await (const _ of first) {
                // consume stream
            }
            const second = client.sendMessageStream([{ text: 'Keep it short again' }], new AbortController().signal, 'prompt-id-memory-2');
            for await (const _ of second) {
                // consume stream
            }
            expect(mockMemoryManager.recall).toHaveBeenNthCalledWith(2, '/test/project/root', 'Keep it short again', expect.objectContaining({
        ...
        it('should run managed auto-memory extraction after a completed user query', async () => {
            ...
            client['chat'] = mockChat;
            const events = await fromAsync(client.sendMessageStream([{ text: 'Please answer tersely' }], new AbortController().signal, 'prompt-id-extract'));
...
Other open files:
  ...
  - /path/to/recent/file2.ts`;
        ...
        it('should respect MAX_TURNS limit even when turns parameter is set to a large value', async () => {
            ...
            const stream = client.sendMessageStream([{ text: 'Start conversation' }], signal, 'prompt-id-3', { type: SendMessageType.UserQuery }, Number.MAX_SAFE_INTEGER);
            // Count how many stream events we get
            let eventCount = 0;
        ...
        describe('retry sendMessageType', () => {
            it('should call stripOrphanedUserEntriesFromHistory before executing', async () => {
                ...
                const stream = client.sendMessageStream([{ text: 'second message' }], new AbortController().signal, 'prompt-retry', { type: SendMessageType.Retry });
                for await (const _ of stream) {
                    /* consume */
                }
                // Assert: the cleanup method was called
                expect(mockChat.stripOrphanedUserEntriesFromHistory).toHaveBeenCalledOnce();
            });
            it('should not increment sessionTurnCount for retry', async () => {
                ...
                const stream = client.sendMessageStream([{ text: 'retry' }], new AbortController().signal, 'prompt-retry-3', { type: SendMessageType.Retry });
                for await (const _ of stream) {
                    /* consume */
                }
        ...
        describe('hooks fast-path optimization', () => {
            ...
            it('should skip messageBus.request for UserPromptSubmit when hasHooksForEvent returns false', async () => {
                ...
                vi.mocked(mockConfig.getMessageBus).mockReturnValue(mockMessageBus);
                vi.mocked(mockConfig.hasHooksForEvent).mockReturnValue(false);
                const stream = client.sendMessageStream([{ text: 'Hi' }], new AbortController().signal, 'prompt-hooks-1');
                for await (const _ of stream) {
                    // consume stream
                }
            ...
            it('should skip messageBus.request for Stop when hasHooksForEvent returns false', async () => {
                ...
                vi.mocked(mockConfig.getMessageBus).mockReturnValue(mockMessageBus);
                vi.mocked(mockConfig.hasHooksForEvent).mockReturnValue(false);
                const stream = client.sendMessageStream([{ text: 'Hi' }], new AbortController().signal, 'prompt-hooks-2');
                for await (const _ of stream) {
                    // consume stream
                }
            ...
            it('should not skip hooks when hasHooksForEvent returns true', async () => {
                ...
                vi.mocked(mockConfig.getDisableAllHooks).mockReturnValue(false);
                vi.mocked(mockConfig.getMessageBus).mockReturnValue(mockMessageBus);
                vi.mocked(mockConfig.hasHooksForEvent).mockImplementation((event) => event === 'UserPromptSubmit');
                const stream = client.sendMessageStream([{ text: 'Hi' }], new AbortController().signal, 'prompt-hooks-3');
                for await (const _ of stream) {
                    // consume stream
                }
    ...
    describe('generateContent', () => {
        it('should call generateContent with the correct parameters', async () => {
            ...
            await client.generateContent(contents, generationConfig, abortSignal, DEFAULT_VIVEKMIND_FLASH_MODEL);
            expect(mockContentGenerator.generateContent).toHaveBeenCalledWith(expect.objectContaining({
                model: DEFAULT_VIVEKMIND_FLASH_MODEL,
        ...
        it('should use current model from config for content generation', async () => {
            ...
            await client.generateContent(contents, {}, new AbortController().signal, DEFAULT_VIVEKMIND_FLASH_MODEL);
        ...
        it('should prefer the current prompt id context for stateless requests', async () => {
            ...
            await promptIdContext.run('btw-prompt-id', async () => {
                await client.generateContent(contents, {}, abortSignal, DEFAULT_VIVEKMIND_FLASH_MODEL);
            });
            expect(mockContentGenerator.generateContent).toHaveBeenCalledWith(expect.objectContaining({
                model: DEFAULT_VIVEKMIND_FLASH_MODEL,
                contents,
        ...
        it('should prefer an explicit prompt id override over the current context', async () => {
            ...
            await promptIdContext.run('context-prompt-id', async () => {
                await client.generateContent(contents, {}, abortSignal, DEFAULT_VIVEKMIND_FLASH_MODEL, 'override-prompt-id');
            });
            expect(mockContentGenerator.generateContent).toHaveBeenCalledWith(expect.objectContaining({
                model: DEFAULT_VIVEKMIND_FLASH_MODEL,
                contents,
        ...
        it('should use config system prompt override when provided', async () => {
            const contents = [{ role: 'user', parts: [{ text: 'hello' }] }];
            const abortSignal = new AbortController().signal;
            vi.spyOn(client['config'], 'getSystemPrompt').mockReturnValue('Override prompt');
            vi.spyOn(client['config'], 'getUserMemory').mockReturnValue('Saved memory');
            vi.mocked(getCustomSystemPrompt).mockReturnValueOnce('Override prompt with memory');
            await client.generateContent(contents, {}, abortSignal, DEFAULT_VIVEKMIND_FLASH_MODEL);
            expect(getCustomSystemPrompt).toHaveBeenCalledWith('Override prompt', 'Saved memory', undefined);
            expect(mockContentGenerator.generateContent).toHaveBeenCalledWith(expect.objectContaining({
                config: expect.objectContaining({
        ...
        it('should append config appendSystemPrompt to the core system prompt', async () => {
            ...
            vi.spyOn(client['config'], 'getAppendSystemPrompt').mockReturnValue('Be extra concise.');
            await client.generateContent(contents, {}, abortSignal, DEFAULT_VIVEKMIND_FLASH_MODEL);
            expect(getCoreSystemPrompt).toHaveBeenCalledWith('', 'test-model', 'Be extra concise.');
        });
        it('should append config appendSystemPrompt after a config system prompt override', async () => {
            const contents = [{ role: 'user', parts: [{ text: 'hello' }] }];
            const abortSignal = new AbortController().signal;
            vi.spyOn(client['config'], 'getSystemPrompt').mockReturnValue('Override prompt');
            vi.spyOn(client['config'], 'getAppendSystemPrompt').mockReturnValue('Focus on findings only.');
            vi.spyOn(client['config'], 'getUserMemory').mockReturnValue('Saved memory');
            vi.mocked(getCustomSystemPrompt).mockReturnValueOnce('Override prompt with memory and append');
            await client.generateContent(contents, {}, abortSignal, DEFAULT_VIVEKMIND_FLASH_MODEL);
            expect(getCustomSystemPrompt).toHaveBeenCalledWith('Override prompt', 'Saved memory', 'Focus on findings only.');
            expect(mockContentGenerator.generateContent).toHaveBeenCalledWith(expect.objectContaining({
                config: expect.objectContaining({
