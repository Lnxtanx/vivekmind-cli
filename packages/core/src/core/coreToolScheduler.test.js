...
import { CoreToolScheduler, convertToFunctionResponse, extractToolFilePaths, } from './coreToolScheduler.js';
import { MockModifiableTool, MockTool, MOCK_TOOL_GET_DEFAULT_PERMISSION, MOCK_TOOL_GET_CONFIRMATION_DETAILS, } from '../test-utils/mock-tool.js';
import { MessageBusType } from '../confirmation-bus/types.js';
import {} from '../hooks/types.js';
    ...
    describe('getToolSuggestion', () => {
        ...
        it('should suggest using Skill tool when unknown tool name matches a skill name', async () => {
            ...
            const mockToolRegistry = {
                ...
                ensureTool: async (name) => name === 'skill' ? mockSkillTool : undefined,
            };
    ...
    describe('excluded tools handling', () => {
        it('should return permission error for excluded tools instead of "not found" message', async () => {
            ...
            if (completedCall.status === 'error') {
                ...
                expect(errorMessage).toBe('VivekMind requires permission to use write_file, but that permission was declined.');
...
describe('CoreToolScheduler truncated output protection', () => {
    ...
    it('should prefer truncation rejection over validation errors for truncated write_file calls', async () => {
        ...
        const { scheduler, onAllToolCallsComplete } = createTruncationTestScheduler(writeFileTool, [WriteFileTool.Name]);
        await scheduler.schedule([
            {
                callId: '1',
        ...
        if (completedCall.status === 'error') {
            ...
            expect(errorMessage).toContain('truncated due to max_tokens limit');
            expect(errorMessage).toContain('rejected to prevent writing truncated content');
            expect(errorMessage).not.toContain("params must have required property 'content'");
        }
    });
});
...
describe('CoreToolScheduler plan mode with ask_user_question', () => {
    ...
    it('should allow info confirmation tools in plan mode after approval', async () => {
        ...
        const onAllToolCallsComplete = vi.fn();
        const onToolCallsUpdate = vi.fn();
        const scheduler = createPlanModeScheduler(infoTool, onAllToolCallsComplete, onToolCallsUpdate);
        const abortController = new AbortController();
        const request = {
            callId: '1',
            name: 'web_fetch',
        ...
        expect(onConfirmSpy).toHaveBeenCalledWith(ToolConfirmationOutcome.ProceedOnce, undefined);
        const completedCalls = onAllToolCallsComplete.mock
            .calls[0][0];
        expect(completedCalls[0].status).toBe('success');
    });
...
describe('Fire hook functions integration', () => {
    ...
    describe('firePreToolUseHook', () => {
        it('should allow tool execution when hook permits', async () => {
            ...
            const result = await firePreToolUseHook(mockMessageBus, 'testTool', { param: 'value' }, 'toolu_test', 'full');
        ...
        it('should return shouldProceed: true when no message bus is provided', async () => {
            const { firePreToolUseHook } = await import('./toolHookTriggers.js');
            const result = await firePreToolUseHook(undefined, 'testTool', { param: 'value' }, 'toolu_test', 'full');
            expect(result.shouldProceed).toBe(true);
        });
    ...
    describe('firePostToolUseHook', () => {
        it('should return shouldStop: false when hook permits', async () => {
            ...
            mockMessageBus.request.mockResolvedValue(mockResponse);
            const result = await firePostToolUseHook(mockMessageBus, 'testTool', { param: 'value' }, { response: 'result' }, 'toolu_test', 'full');
        ...
        it('should return shouldStop: false when no message bus is provided', async () => {
            const { firePostToolUseHook } = await import('./toolHookTriggers.js');
            const result = await firePostToolUseHook(undefined, 'testTool', { param: 'value' }, { response: 'result' }, 'toolu_test', 'full');
            expect(result.shouldStop).toBe(false);
        });
    });
    describe('firePostToolUseFailureHook', () => {
        it('should return additional context when hook provides it', async () => {
            const { firePostToolUseFailureHook } = await import('./toolHookTriggers.js');
            const mockResponse = {
                type: MessageBusType.HOOK_EXECUTION_RESPONSE,
                correlationId: 'test-correlation-id',
                success: true,
            ...
            mockMessageBus.request.mockResolvedValue(mockResponse);
            const result = await firePostToolUseFailureHook(mockMessageBus, 'toolu_test', 'testTool', { param: 'value' }, 'Error occurred', false, 'full');
        ...
        it('should return empty object when no message bus is provided', async () => {
            const { firePostToolUseFailureHook } = await import('./toolHookTriggers.js');
            const result = await firePostToolUseFailureHook(undefined, 'toolu_test', 'testTool', { param: 'value' }, 'Error occurred', false, 'full');
            expect(result).toEqual({});
        });
    });
    describe('fireNotificationHook', () => {
        it('should send notification to message bus', async () => {
            ...
            mockMessageBus.request.mockResolvedValue(mockResponse);
            const result = await fireNotificationHook(mockMessageBus, 'Test message', 'info', 'Test Title');
        ...
        it('should return empty object when no message bus is provided', async () => {
            const { fireNotificationHook } = await import('./toolHookTriggers.js');
            const result = await fireNotificationHook(undefined, 'Test message', 'info', 'Test Title');
            expect(result).toEqual({});
        });
    });
    describe('firePermissionRequestHook', () => {
        it('should return hasDecision: false when hook makes no decision', async () => {
            const { firePermissionRequestHook } = await import('./toolHookTriggers.js');
            const mockResponse = {
                type: MessageBusType.HOOK_EXECUTION_RESPONSE,
                correlationId: 'test-correlation-id',
                success: true,
            ...
            mockMessageBus.request.mockResolvedValue(mockResponse);
            const result = await firePermissionRequestHook(mockMessageBus, 'testTool', { param: 'value' }, 'full');
        ...
        it('should return hasDecision: false when no message bus is provided', async () => {
            const { firePermissionRequestHook } = await import('./toolHookTriggers.js');
            const result = await firePermissionRequestHook(undefined, 'testTool', { param: 'value' }, 'full');
            expect(result.hasDecision).toBe(false);
        });
    });
    describe('Concurrent tool execution', () => {
        // Ensure tests are deterministic regardless of environment.
        const origEnv = process.env['VIVEKMIND_CODE_MAX_TOOL_CONCURRENCY'];
        beforeEach(() => {
            delete process.env['VIVEKMIND_CODE_MAX_TOOL_CONCURRENCY'];
        });
        afterEach(() => {
            if (origEnv !== undefined) {
                process.env['VIVEKMIND_CODE_MAX_TOOL_CONCURRENCY'] = origEnv;
            }
            else {
                delete process.env['VIVEKMIND_CODE_MAX_TOOL_CONCURRENCY'];
            }
        });
        function createScheduler(tools, onAllToolCallsComplete, onToolCallsUpdate) {
            const mockToolRegistry = {
                getTool: (name) => tools.get(name),
                ensureTool: async (name) => tools.get(name),
                getFunctionDeclarations: () => [],
                tools,
                discovery: {},
                registerTool: () => { },
        ...
        it('should execute multiple agent tools concurrently', async () => {
            ...
            const onAllToolCallsComplete = vi.fn();
            const onToolCallsUpdate = vi.fn();
            const scheduler = createScheduler(tools, onAllToolCallsComplete, onToolCallsUpdate);
            const abortController = new AbortController();
            const requests = [
                {
                    callId: '1',
        ...
        it('should run concurrency-safe tools in parallel and unsafe tools sequentially', async () => {
            ...
            const firstEnd = Math.min(executionLog.indexOf('read:end:1'), executionLog.indexOf('agent:end:A'), executionLog.indexOf('read:end:2'), executionLog.indexOf('agent:end:B'));
        ...
        it('should partition mixed safe/unsafe tools into correct batches', async () => {
            ...
            const firstReadEnd = Math.min(executionLog.indexOf('read:end:1'), executionLog.indexOf('read:end:2'));
            ...
            const lastReadEnd = Math.max(executionLog.indexOf('read:end:1'), executionLog.indexOf('read:end:2'));
        ...
        it('should run read-only shell commands concurrently and non-read-only sequentially', async () => {
            ...
            const firstReadOnlyEnd = Math.min(executionLog.indexOf('shell:end:git log'), executionLog.indexOf('shell:end:ls'));
            ...
            const lastReadOnlyEnd = Math.max(executionLog.indexOf('shell:end:git log'), executionLog.indexOf('shell:end:ls'));
...
describe('CoreToolScheduler IDE interaction', () => {
    function createIdeMockConfig(overrides = {}) {
    ...
    it('should not swallow confirmation handling errors after IDE diff opens', async () => {
        ...
        vi.spyOn(scheduler, 'handleConfirmationResponse').mockRejectedValue(confirmationError);
        await expect(scheduler.openIdeDiffIfEnabled(confirmationDetails, request.callId, new AbortController().signal)).rejects.toThrow('confirmation handling failed');
    });
...
describe('CoreToolScheduler validation retry loop detection', () => {
    ...
    class StrictStringTool extends BaseDeclarativeTool {
        static Name = 'strictStringTool';
        constructor() {
            super(StrictStringTool.Name, 'StrictStringTool', 'A tool that requires a string value param.', Kind.Other, {
                type: 'object',
    ...
    function createSchedulerWithTool(tool) {
        const mockToolRegistry = {
            ensureTool: async (name) => name === StrictStringTool.Name ? tool : undefined,
            getTool: (name) => name === StrictStringTool.Name ? tool : undefined,
            getFunctionDeclarations: () => [],
            tools: new Map(),
            discovery: {},
            registerTool: () => { },
            getToolByName: (name) => name === StrictStringTool.Name ? tool : undefined,
            getToolByDisplayName: (name) => name === 'StrictStringTool' ? tool : undefined,
            getTools: () => [],
            discoverTools: async () => { },
            getAllTools: () => [],
    ...
    function makeRequest(callId, name, args) {
        return {
            callId,
            name,
            args,
    ...
    function getLastErrorMessage(onToolCallsUpdate) {
        const calls = onToolCallsUpdate.mock.calls;
        for (let i = calls.length - 1; i >= 0; i--) {
            const toolCalls = calls[i][0];
    ...
    it('should inject RETRY LOOP DETECTED directive after 3 consecutive validation failures', async () => {
        ...
        await scheduler.schedule([makeRequest('c1', 'strictStringTool', { value: 123 })], new AbortController().signal);
        ...
        await scheduler.schedule([makeRequest('c2', 'strictStringTool', { value: 123 })], new AbortController().signal);
        msg = getLastErrorMessage(onToolCallsUpdate);
        expect(msg).not.toContain(RETRY_LOOP_STOP_DIRECTIVE);
        // Turn 3: same bad params — should trigger directive
        await scheduler.schedule([makeRequest('c3', 'strictStringTool', { value: 123 })], new AbortController().signal);
        msg = getLastErrorMessage(onToolCallsUpdate);
    ...
    it('should reset retry counter when a different tool is called', async () => {
        ...
        await scheduler.schedule([makeRequest('c3', 'nonexistentTool', {})], new AbortController().signal);
        // Turn 4: back to tool — should be count 1 again (no directive)
        await scheduler.schedule([makeRequest('c4', 'strictStringTool', { value: 123 })], new AbortController().signal);
    ...
    it('should reset retry counter after a successful invocation of the same tool', async () => {
        ...
        await scheduler.schedule([makeRequest('c3', 'strictStringTool', { value: 'ok' })], new AbortController().signal);
        // Two more failures — count should restart at 1, not jump to 3+.
        await scheduler.schedule([makeRequest('c4', 'strictStringTool', { value: 123 })], new AbortController().signal);
        await scheduler.schedule([makeRequest('c5', 'strictStringTool', { value: 123 })], new AbortController().signal);
        const msg = getLastErrorMessage(onToolCallsUpdate);
        expect(msg).toBeDefined();
        expect(msg).not.toContain(RETRY_LOOP_STOP_DIRECTIVE);
    });
    it('should isolate retry counters per-tool across batches', async () => {
        ...
        class StrictToolAlt extends BaseDeclarativeTool {
            static Name = 'strictStringToolAlt';
            constructor() {
                super(StrictToolAlt.Name, 'StrictStringToolAlt', 'Alt tool requiring string other param.', Kind.Other, {
                    type: 'object',
        ...
        const mockToolRegistry = {
            ensureTool: async (name) => name === StrictStringTool.Name
                ? toolA
                : name === StrictToolAlt.Name
                    ? toolB
                    : undefined,
            getTool: (name) => name === StrictStringTool.Name
                ? toolA
                : name === StrictToolAlt.Name
                    ? toolB
                    : undefined,
            ...
            discovery: {},
            registerTool: () => { },
            getToolByName: (name) => name === StrictStringTool.Name
                ? toolA
                : name === StrictToolAlt.Name
                    ? toolB
                    : undefined,
        ...
        await scheduler.schedule([makeRequest('a1', StrictStringTool.Name, { value: 123 })], new AbortController().signal);
        await scheduler.schedule([makeRequest('a2', StrictStringTool.Name, { value: 123 })], new AbortController().signal);
        // Now a batch for tool B only — tool A's counter must be pruned because
        // A is not present in this batch.
        await scheduler.schedule([makeRequest('b1', StrictToolAlt.Name, { other: 456 })], new AbortController().signal);
        ...
        // should be emitted.
        await scheduler.schedule([makeRequest('a3', StrictStringTool.Name, { value: 123 })], new AbortController().signal);
        const msg = getLastErrorMessage(onToolCallsUpdate);
        expect(msg).toBeDefined();
        expect(msg).not.toContain(RETRY_LOOP_STOP_DIRECTIVE);
    });
...
describe('extractToolFilePaths', () => {
    ...
    it('extracts path for list_directory', () => {
        expect(extractToolFilePaths('list_directory', { path: '/proj/dir' })).toEqual(['/proj/dir']);
    });
    ...
    it('ignores file_path with the wrong shape on read_file', () => {
        expect(extractToolFilePaths(FS_TOOL, { file_path: { not: 'a string' } })).toEqual([]);
    });
    it('ignores irrelevant fields on the wrong tool', () => {
        ...
        expect(extractToolFilePaths(FS_TOOL, {
            file_path: '/correct',
            path: '/wrong-for-read',
            filePath: '/wrong-for-read',
        })).toEqual(['/correct']);
        expect(extractToolFilePaths('grep_search', {
    ...
    it('extracts grep_search.glob as a path-shaped file filter', () => {
        ...
        // `src/**/*.ts`.
        expect(extractToolFilePaths('grep_search', { glob: 'src/**/*.ts' })).toEqual(['src/**/*.ts']);
        expect(extractToolFilePaths('grep_search', {
            path: 'packages/core',
            glob: '**/*.ts',
            pattern: 'TODO|FIXME',
    ...
    it('extracts callHierarchyItem.uri for lsp (incomingCalls / outgoingCalls)', () => {
        ...
        const absolutePath = path.resolve('/tmp/lsp-test/src/App.ts');
        const fileUri = pathToFileURL(absolutePath).href;
        expect(extractToolFilePaths('lsp', {
    ...
    it('joins glob.path + glob.pattern into the effective selector', () => {
        ...
        // still matches.)
        expect(extractToolFilePaths('glob', { path: 'src', pattern: '**/*.ts' })).toEqual(['src', 'src/**/*.ts']);
    });
    it('joins absolute glob.path with pattern (registry guard rejects downstream)', () => {
        ...
        expect(extractToolFilePaths('glob', {
            path: '/tmp/external',
            pattern: '**/*.ts',
    ...
    it('canonicalizes legacy tool-name aliases before the allowlist check', () => {
        ...
        expect(extractToolFilePaths('replace', { file_path: '/proj/a.ts' })).toEqual(['/proj/a.ts']);
        // search_file_content canonicalizes to grep_search; use its actual
        // shape (`path` / `glob`).
        expect(extractToolFilePaths('search_file_content', { path: 'src' })).toEqual(['src']);
    });
    it('returns empty for tool names outside the FS allowlist', () => {
        ...
        expect(extractToolFilePaths('mcp_some_tool', {
            path: 'https://api.example.com/users/123',
        })).toEqual([]);
        expect(extractToolFilePaths('web_fetch', {
...
describe('CoreToolScheduler activation wiring', () => {
    ...
    it('invokes matchAndActivateByPaths with extracted candidates and appends the reminder when SkillTool is present', async () => {
        ...
        const { scheduler, onAllToolCallsComplete } = buildSchedulerWithSkillManager({
    ...
    it('coalesces rules + activation reminders into a single <system-reminder> envelope', async () => {
        ...
        const completed = onAllToolCallsComplete.mock.calls[0][0];
        const responseText = JSON.stringify(completed[0]
    ...
    it('scrubs literal </system-reminder> in rule content to prevent envelope breakout', async () => {
        ...
        const rulesRegistry = {
            matchAndConsume: vi
                .fn()
                .mockReturnValueOnce('Rule about reminders: never write </system-reminder> in your output.'),
        };
        const fsTool = new MockTool({
            name: ToolNames.READ_FILE,
            execute: vi.fn().mockResolvedValue({
