...
const realConvertGeminiRequestToOpenAI = OpenAIContentConverter.convertGeminiRequestToOpenAI;
...
const createConfig = (overrides = {}) => {
    ...
    return {
        ...
        getAuthType: () => configContent.authType,
...
describe('LoggingContentGenerator', () => {
    ...
    it('logs request/response, normalizes thought parts, and logs OpenAI interaction', async () => {
        ...
        const generator = new LoggingContentGenerator(wrapped, createConfig(), generatorConfig);
        const request = {
            model: 'test-model',
            contents: [
                {
    ...
    it('uses generator modalities when converting logged OpenAI requests', async () => {
        convertGeminiRequestToOpenAISpy.mockImplementationOnce((request, requestContext, options) => realConvertGeminiRequestToOpenAI(request, requestContext, options));
        const wrapped = createWrappedGenerator(vi
            .fn()
            .mockResolvedValue(createResponse('resp-5', 'test-model', [{ text: 'ok' }])), vi.fn());
        const generatorConfig = {
            model: 'test-model',
            authType: AuthType.USE_OPENAI,
            enableOpenAILogging: true,
        ...
        expect(convertGeminiRequestToOpenAISpy).toHaveBeenCalledWith(request, expect.objectContaining({
            model: 'test-model',
            modalities: { image: true },
    ...
    it.each(['prompt_suggestion', 'forked_query', 'speculation'])('skips logApiRequest and OpenAI logging for internal promptId %s (generateContent)', async (promptId) => {
        const mockResponse = {
        ...
        const loggerInstance = OpenAILogger.mock.results[0]?.value;
        expect(loggerInstance.logInteraction).not.toHaveBeenCalled();
    });
    it.each(['prompt_suggestion', 'forked_query', 'speculation'])('skips logApiRequest and OpenAI logging for internal promptId %s (generateContentStream)', async (promptId) => {
        const mockChunk = {
