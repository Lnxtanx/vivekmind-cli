...
describe('SubagentValidator', () => {
    ...
    describe('validateName', () => {
        ...
        it('should reject names starting with special characters', () => {
            ...
            for (const name of invalidNames) {
                const result = validator.validateName(name);
                expect(result.isValid).toBe(false);
                expect(result.errors).toContain('Name cannot start with a hyphen or underscore');
            }
        });
        it('should reject names ending with special characters', () => {
            ...
            for (const name of invalidNames) {
                const result = validator.validateName(name);
                expect(result.isValid).toBe(false);
                expect(result.errors).toContain('Name cannot end with a hyphen or underscore');
            }
        });
        it('should reject reserved names', () => {
            ...
            for (const name of reservedNames) {
                const result = validator.validateName(name);
                expect(result.isValid).toBe(false);
                expect(result.errors).toContain(`"${name}" is a reserved name and cannot be used`);
            }
        });
        it('should warn about naming conventions', () => {
            const result = validator.validateName('TestAgent');
            expect(result.isValid).toBe(true);
            expect(result.warnings).toContain('Consider using lowercase names for consistency');
        });
        it('should not warn about case for non-Latin names', () => {
            const result = validator.validateName('项目管理');
            expect(result.isValid).toBe(true);
            expect(result.warnings).not.toContain('Consider using lowercase names for consistency');
        });
        it('should warn about mixed separators', () => {
            const result = validator.validateName('test-agent_helper');
            expect(result.isValid).toBe(true);
            expect(result.warnings).toContain('Consider using either hyphens or underscores consistently, not both');
        });
    });
    describe('validateSystemPrompt', () => {
        ...
        it('should reject empty prompts', () => {
            ...
            for (const prompt of invalidPrompts) {
                const result = validator.validateSystemPrompt(prompt);
                expect(result.isValid).toBe(false);
                expect(result.errors).toContain('System prompt is required and cannot be empty');
            }
        });
        ...
        it('should warn about long prompts', () => {
            ...
            expect(result.isValid).toBe(true);
            expect(result.warnings).toContain('System prompt is quite long (>10,000 characters), consider shortening');
        });
    });
    describe('validateTools', () => {
        ...
        it('should warn about empty arrays', () => {
            ...
            expect(result.isValid).toBe(true);
            expect(result.warnings).toContain('Empty tools array - subagent will inherit all available tools');
        });
    ...
    describe('validateModel', () => {
        ...
        it('should reject missing model IDs after valid authType prefixes', () => {
            ...
            expect(result.isValid).toBe(false);
            expect(result.errors).toContain('Model selector must include a model ID after the authType');
        });
        it('should warn when inherit is explicit', () => {
            const result = validator.validateModel('inherit');
            expect(result.isValid).toBe(true);
            expect(result.warnings).toContain('Explicit "inherit" is optional because omitting the model uses the main conversation model');
        });
    });
    describe('validateRunConfig', () => {
        ...
        it('should warn about very long execution times', () => {
            ...
            expect(result.isValid).toBe(true);
            expect(result.warnings).toContain('Very long execution time (>60 minutes) may cause resource issues');
        });
        ...
        it('should warn about high turn limits', () => {
            ...
            expect(result.isValid).toBe(true);
            expect(result.warnings).toContain('Very high turn limit (>100) may cause long execution times');
        });
    });
    describe('validateConfig', () => {
        ...
        it('should reject non-string entries in disallowedTools', () => {
            const result = validator.validateConfig({
                ...validConfig,
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                disallowedTools: [123, 'write_file'],
            });
            expect(result.isValid).toBe(false);
            expect(result.errors).toContain('Tool name must be a string, got: number');
        });
    ...
    describe('validateOrThrow', () => {
        ...
        it('should throw SubagentError for invalid configurations', () => {
            ...
            expect(() => validator.validateOrThrow(invalidConfig)).toThrow(SubagentError);
            expect(() => validator.validateOrThrow(invalidConfig)).toThrow(/Validation failed/);
        });
        it('should include subagent name in error', () => {
            ...
            try {
                ...
                expect(error.subagentName).toBe('custom-name');
            }
        });
    });
});
