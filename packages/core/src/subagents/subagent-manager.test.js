...
describe('SubagentManager', () => {
        ...
        mockParseYaml.mockImplementation((yamlString) => {
            ...
            if (yamlString.includes('background:')) {
                ...
                return {
                    ...
                    description: yamlString.match(/description:\s*(.+)/)?.[1] ?? 'A test subagent',
                    background: bgValue,
                };
            }
    ...
    const validConfig = {
        ...
        level: 'project',
        filePath: '/test/project/.vivekmind/agents/test-agent.md',
    };
    const validMarkdown = `---
name: test-agent
description: A test subagent
...
`;
    describe('parseSubagentContent', () => {
        ...
        it('should parse valid markdown content with CRLF line endings', () => {
            ...
            const config = manager.parseSubagentContent(markdownWithCRLF, validConfig.filePath, 'project');
            expect(config.name).toBe('test-agent');
            expect(config.description).toBe('A test subagent');
...
You are a helpful assistant.
`;
            const config = manager.parseSubagentContent(markdownWithDisallowed, validConfig.filePath, 'project');
...
You are a helpful assistant.
`;
            const config = manager.parseSubagentContent(markdownWithScalar, validConfig.filePath, 'project');
...
`;
            ...
            const config = manager.parseSubagentContent(markdownWithLegacyModel, validConfig.filePath, 'project');
...
`;
        ...
        it('should determine level from file path', () => {
            const projectPath = '/test/project/.vivekmind/agents/test-agent.md';
            const userPath = '/home/user/.vivekmind/agents/test-agent.md';
...
`;
        ...
        it('should not warn when filename matches subagent name', () => {
            ...
            const matchingPath = '/test/project/.vivekmind/agents/test-agent.md';
...
You are a monitor.
`;
            const config = manager.parseSubagentContent(markdownWithBackground, validConfig.filePath, 'project');
...
You are a monitor.
`;
            const config = manager.parseSubagentContent(markdownWithBgString, validConfig.filePath, 'project');
            expect(config.background).toBe(true);
        });
...
You are an agent.
`;
            const config = manager.parseSubagentContent(markdownWithBgFalse, validConfig.filePath, 'project');
    ...
    describe('serializeSubagent', () => {
        ...
        it('should roundtrip disallowedTools through serialize and parse', () => {
            ...
            expect(serialized).toContain('- write_file');
            expect(serialized).toContain('- mcp__slack');
            const parsed = manager.parseSubagentContent(serialized, validConfig.filePath, 'project');
    ...
    describe('loadSubagent', () => {
        it('should load subagent from project level first', async () => {
            ...
            expect(config.name).toBe('test-agent');
            expect(fs.readdir).toHaveBeenCalledWith(path.normalize('/test/project/.vivekmind/agents'));
            expect(fs.readFile).toHaveBeenCalledWith(path.normalize('/test/project/.vivekmind/agents/test-agent.md'), 'utf8');
        });
        it('should fall back to user level if project level fails', async () => {
            ...
            expect(config).toBeDefined();
            expect(config.name).toBe('test-agent');
            expect(fs.readdir).toHaveBeenCalledWith(path.normalize('/home/user/.vivekmind/agents'));
            expect(fs.readFile).toHaveBeenCalledWith(path.normalize('/home/user/.vivekmind/agents/test-agent.md'), 'utf8');
        });
...
You are another assistant.`;
            ...
            expect(config.filePath).toBe(path.normalize('/test/project/.vivekmind/agents/wrong-filename.md'));
...
You are a helpful assistant.`;
            ...
            expect(config.filePath).toBe(path.normalize('/home/user/.vivekmind/agents/user-agent.md'));
...
You are a helpful assistant.`;
            ...
            expect(config.filePath).toBe(path.normalize('/test/project/.vivekmind/agents/misnamed-file.md'));
        });
    });
    ...
    describe('deleteSubagent', () => {
        it('should delete subagent from specified level', async () => {
            ...
            expect(fs.unlink).toHaveBeenCalledWith(path.normalize('/test/project/.vivekmind/agents/test-agent.md'));
        });
        it('should delete from both levels if no level specified', async () => {
            ...
            expect(fs.unlink).toHaveBeenCalledWith(path.normalize('/test/project/.vivekmind/agents/test-agent.md'));
            expect(fs.unlink).toHaveBeenCalledWith(path.normalize('/home/user/.vivekmind/agents/test-agent.md'));
        });
        it('should throw error if subagent not found', async () => {
            vi.mocked(fs.readdir).mockRejectedValue(new Error('Directory not found'));
...
You are a helpful assistant.`;
            ...
            expect(fs.unlink).toHaveBeenCalledWith(path.normalize('/test/project/.vivekmind/agents/wrong-name.md'));
        });
...
Target content`,
            ...
            expect(fs.unlink).toHaveBeenCalledWith(path.normalize('/test/project/.vivekmind/agents/target-file.md'));
        });
    });
    ...
    describe('Runtime Configuration Methods', () => {
        describe('convertToRuntimeConfig', () => {
            ...
            it('should include tool configuration when tools are specified', async () => {
                ...
                const runtimeConfig = await manager.convertToRuntimeConfig(configWithTools);
            ...
            it('should transform display names to tool names in tool configuration', async () => {
                ...
                const runtimeConfig = await manager.convertToRuntimeConfig(configWithDisplayNames);
                expect(runtimeConfig.toolConfig).toBeDefined();
                expect(runtimeConfig.toolConfig.tools).toEqual([
            ...
            it('should set modelConfig.model from model selector and merge run configurations', async () => {
                ...
                const runtimeConfig = await manager.convertToRuntimeConfig(configWithCustom);
            ...
            it('should accept cross-provider model selectors', async () => {
                const configWithCrossProvider = {
                    ...validConfig,
                    model: 'openai:gpt-4',
                };
                const runtimeConfig = await manager.convertToRuntimeConfig(configWithCrossProvider);
        ...
        describe('createAgentHeadless model override', () => {
            ...
            it('should create a new ContentGenerator for bare model IDs', async () => {
                ...
                expect(mockCreateContentGenerator).toHaveBeenCalledWith(expect.objectContaining({ model: 'custom-model' }), expect.anything());
            });
            it('should create a new ContentGenerator for cross-provider selectors', async () => {
                ...
                await manager.createAgentHeadless(config, mockConfig);
                expect(mockCreateContentGenerator).toHaveBeenCalledWith(expect.objectContaining({
                    model: 'claude-sonnet',
                    authType: 'anthropic',
                }), expect.anything());
            });
