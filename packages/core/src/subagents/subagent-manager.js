...
export class SubagentManager {
     ...
     */
    async deleteSubagent(name, level, extensionName) {
    ...
    async refreshCache() {
        ...
        const levels = ['project', 'user', 'builtin', 'extension'];
    ...
    serializeSubagent(config) {
        ...
        if (config.approvalMode &&
            APPROVAL_MODES.includes(config.approvalMode)) {
    ...
    async createAgentHeadless(config, runtimeContext, options) {
        try {
            ...
            const toolConfig = options?.toolConfigOverride ?? runtimeConfig.toolConfig;
            ...
            const agentContext = await this.maybeOverrideContentGenerator(config, runtimeContext);
            return await AgentHeadless.create(config.name, agentContext, promptConfig, modelConfig, runConfig, toolConfig, options?.eventEmitter, options?.hooks);
        }
        catch (error) {
     ...
     */
    async maybeOverrideContentGenerator(config, base) {
        const selection = parseSubagentModelSelection(config.model);
        if (selection.inherits) {
            return base;
        }
        const authType = selection.authType ?? base.getContentGeneratorConfig().authType;
        const authOverrides = {
            authType: authType,
        };
        const agentGeneratorConfig = buildAgentContentGeneratorConfig(base, selection.modelId, authOverrides);
        const agentGenerator = await createContentGenerator(agentGeneratorConfig, base);
        ...
        debugLogger.info(`Created per-agent ContentGenerator for subagent "${config.name}": authType=${authType}, model=${agentGeneratorConfig.model}`);
        return override;
    }
    /**
     ...
     */
    async convertToRuntimeConfig(config) {
        const promptConfig = {
    ...
    getSubagentPath(name, level) {
        ...
        const baseDir = level === 'project'
            ? path.join(this.config.getProjectRoot(), VIVEKMIND_CONFIG_DIR, AGENT_CONFIG_DIR)
            : path.join(os.homedir(), VIVEKMIND_CONFIG_DIR, AGENT_CONFIG_DIR);
    ...
    async listSubagentsAtLevel(level) {
        ...
        baseDir = path.join(baseDir, VIVEKMIND_CONFIG_DIR, AGENT_CONFIG_DIR);
        try {
...
function parseSubagentContent(content, filePath, level, validator) {
    try {
        ...
        if (approvalModeRaw !== undefined &&
            ...
            throw new Error(`Invalid "approvalMode" value: expected a string, got ${typeof approvalModeRaw}. Valid values: ${APPROVAL_MODES.join(', ')}`);
        }
        const approvalMode = typeof approvalModeRaw === 'string' && approvalModeRaw !== ''
            ? approvalModeRaw
            : undefined;
        if (approvalMode !== undefined &&
            !APPROVAL_MODES.includes(approvalMode)) {
            throw new Error(`Invalid "approvalMode" value "${approvalMode}". Valid values: ${APPROVAL_MODES.join(', ')}`);
        }
