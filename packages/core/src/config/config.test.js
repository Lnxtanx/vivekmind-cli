...
import { DEFAULT_TELEMETRY_TARGET, DEFAULT_OTLP_ENDPOINT, VivekMindLogger, isTelemetrySdkInitialized, shutdownTelemetry, } from '../telemetry/index.js';
...
describe('Server Config (config.ts)', () => {
    ...
    beforeEach(() => {
        ...
        vi.spyOn(VivekMindLogger.prototype, 'logStartSessionEvent').mockImplementation(async () => undefined);
    ...
    describe('FileReadCache isolation', () => {
        it('returns a distinct cache for child Configs created via Object.create', () => {
            ...
            parentCache.recordRead('/tmp/parent.ts', {
                dev: 1,
                ino: 100,
                mtimeMs: 1_000_000,
                size: 42,
    ...
    describe('startNewSession', () => {
        it('clears the FileReadCache so a new session does not inherit prior reads', () => {
            ...
            cache.recordRead('/tmp/whatever.ts', {
                dev: 1,
                ino: 100,
                mtimeMs: 1_000_000,
                size: 42,
    ...
    describe('initialize', () => {
        ...
        it('should skip implicit startup discovery in bare mode', async () => {
            ...
            expect(ToolRegistry.prototype.registerFactory.mock.calls.map((call) => call[0])).toEqual([ToolNames.READ_FILE, ToolNames.EDIT, ToolNames.SHELL]);
        });
    });
    describe('refreshAuth', () => {
        ...
        it('should fire auth_success notification hook when hooks are enabled', async () => {
            ...
            config.setMessageBus(mockMessageBus);
            const authType = AuthType.USE_GEMINI;
            const mockContentConfig = {
                apiKey: 'test-key',
                model: 'qwen3-coder-plus',
            ...
            expect(fireNotificationHook).toHaveBeenCalledWith(mockMessageBus, `Successfully authenticated with ${authType}`, 'auth_success', 'Authentication successful');
        });
    ...
    describe('model switching optimization (VIVEKMIND_OAUTH)', () => {
        it('should switch vivekmind-oauth model in-place without refreshing auth when safe', async () => {
            const config = new Config(baseParams);
            const mockContentConfig = {
            ...
            // Establish initial vivekmind-oauth content generator config/content generator.
            ...
            await config.switchModel(AuthType.VIVEKMIND_OAUTH, 'coder-model');
    ...
    it('refreshHierarchicalMemory should append managed auto-memory index when present', async () => {
        ...
        vi.mocked(readAutoMemoryIndex).mockResolvedValue('# Managed Auto-Memory Index\n\n- [Project Memory](project.md)');
    ...
    describe('Usage Statistics', () => {
        ...
        it('logs the session start event', async () => {
            ...
            expect(VivekMindLogger.prototype.logStartSessionEvent).toHaveBeenCalledOnce();
        });
    });
    ...
    describe('Per-Signal OTLP Endpoint Configuration', () => {
        it('should return per-signal endpoints when provided', () => {
            ...
            const config = new Config(params);
            expect(config.getTelemetryOtlpTracesEndpoint()).toBe('http://traces:4318/v1/traces');
            expect(config.getTelemetryOtlpLogsEndpoint()).toBe('http://logs:4318/v1/logs');
            expect(config.getTelemetryOtlpMetricsEndpoint()).toBe('http://metrics:4318/v1/metrics');
        });
    ...
    describe('createToolRegistry', () => {
        it('should ignore coreTools overrides in bare mode', async () => {
            ...
            await config.initialize();
            const registerToolMock = (await vi.importMock('../tools/tool-registry')).ToolRegistry.prototype.registerFactory;
            expect(config.getCoreTools()).toEqual([
                ToolNames.READ_FILE,
                ToolNames.EDIT,
                ToolNames.SHELL,
            ]);
            expect(registerToolMock.mock.calls.map((call) => call[0])).toEqual([ToolNames.READ_FILE, ToolNames.EDIT, ToolNames.SHELL]);
        });
        it('should register a tool if coreTools contains an argument-specific pattern', async () => {
            ...
            const wasShellToolRegistered = registerToolMock.mock.calls.some((call) => call[0] === ToolNames.SHELL);
            expect(wasShellToolRegistered).toBe(true);
            // Check that registerTool was NOT called for ReadFileTool
            const wasReadFileToolRegistered = registerToolMock.mock.calls.some((call) => call[0] === ToolNames.READ_FILE);
        ...
        it('should register a tool if coreTools contains a legacy tool name alias', async () => {
            ...
            await config.initialize();
            const registerToolMock = (await vi.importMock('../tools/tool-registry')).ToolRegistry.prototype.registerFactory;
            const wasGrepToolRegistered = registerToolMock.mock.calls.some((call) => call[0] === ToolNames.GREP);
...
describe('setApprovalMode with folder trust', () => {
    ...
    describe('plan file persistence', () => {
        it('should save plan to disk', () => {
            ...
            expect(fs.mkdirSync).toHaveBeenCalledWith(expect.stringContaining('plans'), { recursive: true });
            expect(fs.writeFileSync).toHaveBeenCalledWith(expect.stringContaining('.md'), '# My Plan\n1. Step one\n2. Step two', 'utf-8');
        });
        it('should load plan from disk', () => {
            const config = new Config(baseParams);
            fs.readFileSync.mockReturnValue('# Saved Plan');
    ...
    describe('registerCoreTools', () => {
        ...
        it('should register grep tool when useRipgrep is true and it is available', async () => {
            ...
            await config.initialize();
            const calls = ToolRegistry.prototype.registerFactory.mock.calls;
            const grepRegistrations = calls.filter((call) => call[0] === ToolNames.GREP);
...
describe('Model Switching and Config Updates', () => {
    ...
    it('should trigger full refresh when switching to non-vivekmind-oauth provider', async () => {
        const config = new Config(baseParams);
