...
import { parseRule, parseRules, matchesRule, matchesCommandPattern, matchesPathPattern, matchesDomainPattern, resolveToolName, resolvePathPattern, getSpecifierKind, toolMatchesRuleToolName, splitCompoundCommand, buildPermissionRules, getRuleDisplayName, buildHumanReadableRuleLabel, } from './rule-parser.js';
...
describe('toolMatchesRuleToolName', () => {
    ...
    it('does not cross categories', async () => {
        ...
        expect(toolMatchesRuleToolName('read_file', 'run_shell_command')).toBe(false);
    });
});
// ─── parseRule ───────────────────────────────────────────────────────────────
describe('parseRule', () => {
...
describe('matchesCommandPattern', () => {
    ...
    describe('prefix matching without glob', () => {
        ...
        it('prefix + space', async () => {
            ...
            expect(matchesCommandPattern('git commit', 'git commit -m "test"')).toBe(true);
        });
    ...
    describe('wildcard at tail', () => {
        ...
        it('matches commands with leading env var assignments', async () => {
            expect(matchesCommandPattern('python3 *', 'PYTHONPATH=/tmp/lib python3 -c "print(1)"')).toBe(true);
        });
        it('matches commands containing embedded newlines (dotAll)', async () => {
            expect(matchesCommandPattern('python3 *', 'python3 -c "\nimport sys\nprint(sys.version)\n"')).toBe(true);
        });
    ...
    describe('wildcard at head', () => {
        it('matches any command ending with pattern', async () => {
            ...
            expect(matchesCommandPattern('* --help *', 'npm --help install')).toBe(true);
        });
    ...
    describe('wildcard in middle', () => {
        it('matches middle segments', async () => {
            expect(matchesCommandPattern('git * main', 'git checkout main')).toBe(true);
            expect(matchesCommandPattern('git * main', 'git merge main')).toBe(true);
        });
        it('does not match different suffix', async () => {
            expect(matchesCommandPattern('git * main', 'git checkout dev')).toBe(false);
        });
    });
    ...
    describe('simple command matching (no operators)', () => {
        it('matches when no operators are present', async () => {
            expect(matchesCommandPattern('git *', 'git commit -m "hello world"')).toBe(true);
        });
    ...
    describe('exact command specifier', () => {
        it('Bash(npm run build) matches exact command', async () => {
            expect(matchesCommandPattern('npm run build', 'npm run build')).toBe(true);
        });
        it('Bash(npm run build) also matches with trailing args (prefix)', async () => {
            expect(matchesCommandPattern('npm run build', 'npm run build --verbose')).toBe(true);
        });
        it('Bash(npm run build) does not match different command', async () => {
            expect(matchesCommandPattern('npm run build', 'npm run test')).toBe(false);
        });
    });
});
...
describe('resolvePathPattern', () => {
    ...
    it('// prefix → absolute from filesystem root', async () => {
        expect(resolvePathPattern('//Users/alice/secrets/**', projectRoot, cwd)).toBe('/Users/alice/secrets/**');
    });
    ...
    it('/ prefix → relative to project root (NOT absolute)', async () => {
        expect(resolvePathPattern('/src/**/*.ts', projectRoot, cwd)).toBe('/project/src/**/*.ts');
    });
    it('./ prefix → relative to cwd', async () => {
        expect(resolvePathPattern('./secrets/**', projectRoot, cwd)).toBe('/project/subdir/secrets/**');
    });
    it('no prefix → relative to cwd', async () => {
        expect(resolvePathPattern('*.env', projectRoot, cwd)).toBe('/project/subdir/*.env');
    });
    it('/Users/alice/file is relative to project root, NOT absolute', async () => {
        // This is a gotcha from the Claude Code docs
        expect(resolvePathPattern('/Users/alice/file', projectRoot, cwd)).toBe('/project/Users/alice/file');
    });
});
// ─── matchesPathPattern ──────────────────────────────────────────────────────
describe('matchesPathPattern', () => {
    ...
    it('matches dotfiles (e.g. .env)', async () => {
        expect(matchesPathPattern('.env', '/project/.env', projectRoot, cwd)).toBe(true);
        expect(matchesPathPattern('*.env', '/project/.env', projectRoot, cwd)).toBe(true);
    });
    it('** matches recursively across directories', async () => {
        expect(matchesPathPattern('./secrets/**', '/project/secrets/deep/nested/file.txt', projectRoot, cwd)).toBe(true);
    });
    it('* matches single directory only', async () => {
        expect(matchesPathPattern('/src/*.ts', '/project/src/index.ts', projectRoot, cwd)).toBe(true);
        expect(matchesPathPattern('/src/*.ts', '/project/src/nested/index.ts', projectRoot, cwd)).toBe(false);
    });
    it('/docs/** matches under project root docs', async () => {
        expect(matchesPathPattern('/docs/**', '/project/docs/readme.md', projectRoot, cwd)).toBe(true);
        expect(matchesPathPattern('/docs/**', '/project/src/docs/readme.md', projectRoot, cwd)).toBe(false);
    });
    it('//tmp/scratch.txt matches absolute path', async () => {
        expect(matchesPathPattern('//tmp/scratch.txt', '/tmp/scratch.txt', projectRoot, cwd)).toBe(true);
    });
    it('does not match unrelated paths', async () => {
        expect(matchesPathPattern('./secrets/**', '/project/public/index.html', projectRoot, cwd)).toBe(false);
    });
});
// ─── matchesDomainPattern ────────────────────────────────────────────────────
describe('matchesDomainPattern', () => {
    it('matches exact domain', async () => {
        expect(matchesDomainPattern('domain:example.com', 'example.com')).toBe(true);
    });
    it('matches subdomain', async () => {
        expect(matchesDomainPattern('domain:example.com', 'sub.example.com')).toBe(true);
        expect(matchesDomainPattern('domain:example.com', 'deep.sub.example.com')).toBe(true);
    });
    it('does not match different domain', async () => {
        expect(matchesDomainPattern('domain:example.com', 'notexample.com')).toBe(false);
    });
...
describe('matchesRule', () => {
    ...
    it('Monitor rule does not match run_shell_command', async () => {
        const rule = parseRule('Monitor(tail -f *)');
        expect(matchesRule(rule, 'run_shell_command', 'tail -f /var/log/app.log')).toBe(false);
    });
    ...
    it('Read with path specifier requires filePath', async () => {
        ...
        expect(matchesRule(rule, 'read_file')).toBe(false);
        // With filePath
        expect(matchesRule(rule, 'read_file', undefined, '/project/.env', undefined, pathCtx)).toBe(true);
        expect(matchesRule(rule, 'read_file', undefined, '/project/other.txt', undefined, pathCtx)).toBe(false);
    });
    it('Edit path specifier matches write_file too', async () => {
        const rule = parseRule('Edit(/src/**/*.ts)');
        const pathCtx = { projectRoot: '/project', cwd: '/project' };
        expect(matchesRule(rule, 'write_file', undefined, '/project/src/index.ts', undefined, pathCtx)).toBe(true);
        expect(matchesRule(rule, 'write_file', undefined, '/project/docs/readme.md', undefined, pathCtx)).toBe(false);
    });
    // WebFetch domain matching
    it('WebFetch domain specifier', async () => {
        const rule = parseRule('WebFetch(domain:example.com)');
        expect(matchesRule(rule, 'web_fetch', undefined, undefined, 'example.com')).toBe(true);
        expect(matchesRule(rule, 'web_fetch', undefined, undefined, 'sub.example.com')).toBe(true);
        expect(matchesRule(rule, 'web_fetch', undefined, undefined, 'other.com')).toBe(false);
        // No domain → no match
    ...
    it('Agent literal specifier', async () => {
        ...
        expect(matchesRule(rule, 'task', undefined, undefined, undefined, undefined, 'Explore')).toBe(true);
        expect(matchesRule(rule, 'task', undefined, undefined, undefined, undefined, 'Plan')).toBe(false);
...
describe('PermissionManager', () => {
    ...
    describe('basic rule evaluation', () => {
        ...
        it('deny takes precedence over ask and allow', async () => {
            ...
            pm2.initialize();
            expect(await pm2.evaluate({ toolName: 'run_shell_command' })).toBe('deny');
        });
    ...
    describe('compound command evaluation', () => {
        ...
        it('isCommandAllowed also handles compound commands', async () => {
            ...
            pm.initialize();
            expect(await pm.isCommandAllowed('safe-cmd a && one-cmd b')).toBe('allow');
            // 'unknown-cmd' is not readonly, resolves to 'ask'
            expect(await pm.isCommandAllowed('safe-cmd a && unknown-cmd')).toBe('ask');
            expect(await pm.isCommandAllowed('safe-cmd a && evil-cmd b')).toBe('deny');
        });
    });
    describe('file path evaluation', () => {
        ...
        it('denies reading a denied file', async () => {
            expect(await pm.evaluate({ toolName: 'read_file', filePath: '/project/.env' })).toBe('deny');
        });
    ...
    describe('WebFetch domain evaluation', () => {
        ...
        it('allows fetch to allowed domain', async () => {
            expect(await pm.evaluate({ toolName: 'web_fetch', domain: 'github.com' })).toBe('allow');
        });
        it('allows fetch to subdomain of allowed domain', async () => {
            expect(await pm.evaluate({ toolName: 'web_fetch', domain: 'api.github.com' })).toBe('allow');
        });
        it('denies fetch to denied domain', async () => {
            expect(await pm.evaluate({ toolName: 'web_fetch', domain: 'evil.com' })).toBe('deny');
        });
        it('returns default for unmatched domain', async () => {
            expect(await pm.evaluate({ toolName: 'web_fetch', domain: 'example.com' })).toBe('default');
        });
    });
    describe('isToolEnabled', () => {
        it('returns false for deny-ruled tools', async () => {
            pm = new PermissionManager(makeConfig({ permissionsDeny: ['ShellTool'] }));
            pm.initialize();
            expect(await pm.isToolEnabled('run_shell_command')).toBe(false);
        });
        it('returns true for tools with only specifier deny rules', async () => {
            pm = new PermissionManager(makeConfig({ permissionsDeny: ['Bash(rm *)'] }));
            pm.initialize();
        ...
        it('excludeTools passed via permissionsDeny disables the tool', async () => {
            pm = new PermissionManager(makeConfig({ permissionsDeny: ['run_shell_command'] }));
            pm.initialize();
            expect(await pm.isToolEnabled('run_shell_command')).toBe(false);
        });
        it('coreTools allowlist: listed tool is enabled', async () => {
            pm = new PermissionManager(makeConfig({ coreTools: ['read_file', 'Bash'] }));
            pm.initialize();
        ...
        it('permissionsAllow alone does NOT restrict unlisted tools (not a whitelist)', async () => {
            ...
            pm = new PermissionManager(makeConfig({ permissionsAllow: ['read_file'] }));
            pm.initialize();
            expect(await pm.isToolEnabled('read_file')).toBe(true);
        ...
        it('MCP tools bypass coreTools allowlist check', async () => {
            ...
            expect(await pm.isToolEnabled('mcp__markitdown__convert_to_markdown')).toBe(true);
        ...
        it('Non-core tools still respect deny rules', async () => {
            ...
            expect(await pm.isToolEnabled('mcp__markitdown__convert_to_markdown')).toBe(false);
    ...
    describe('allowedTools via permissionsAllow', () => {
        it('allow rule auto-approves matching tools/commands', async () => {
            pm = new PermissionManager(makeConfig({ permissionsAllow: ['ReadFileTool', 'Bash(git *)'] }));
            pm.initialize();
            expect(await pm.evaluate({ toolName: 'read_file' })).toBe('allow');
            expect(await pm.evaluate({
                toolName: 'run_shell_command',
    ...
    describe('listRules', () => {
        it('returns all rules with type and scope', async () => {
            ...
            const sessionAllow = rules.find((r) => r.scope === 'session' && r.type === 'allow');
    ...
    describe('hasMatchingAskRule', () => {
        it('returns false when shell ask comes only from default permission fallback', async () => {
            pm = new PermissionManager(makeConfig({ permissionsAllow: ['Bash(git add *)'] }));
            pm.initialize();
            expect(pm.hasMatchingAskRule({
                toolName: 'run_shell_command',
                command: 'git add file && git commit -m "msg"',
            })).toBe(false);
        });
        it('returns true when an explicit ask rule matches a shell sub-command', async () => {
            pm = new PermissionManager(makeConfig({ permissionsAsk: ['Bash(git commit *)'] }));
            pm.initialize();
            expect(pm.hasMatchingAskRule({
                toolName: 'run_shell_command',
                command: 'git add file && git commit -m "msg"',
...
describe('buildPermissionRules', () => {
    ...
    describe('generated rules round-trip through parseRule and matchesRule', () => {
        it('Read rule for external file covers the containing directory', async () => {
            ...
            expect(matchesRule(parsed, 'read_file', undefined, '/Users/alice/.secrets', undefined, { projectRoot: '/some/project', cwd: '/some/project' })).toBe(true);
            // Should also match other files in the same directory
            expect(matchesRule(parsed, 'read_file', undefined, '/Users/alice/.other', undefined, { projectRoot: '/some/project', cwd: '/some/project' })).toBe(true);
            // Should NOT match files in a different directory
            expect(matchesRule(parsed, 'read_file', undefined, '/Users/bob/.secrets', undefined, { projectRoot: '/some/project', cwd: '/some/project' })).toBe(false);
        });
        it('Read rule also matches other read-family tools on the same path', async () => {
            ...
            expect(matchesRule(parsed, 'grep_search', undefined, '/external/dir/file.txt', undefined, { projectRoot: '/p', cwd: '/p' })).toBe(true);
            // Should also match read_file (Read meta-category)
            expect(matchesRule(parsed, 'read_file', undefined, '/external/dir/other.ts', undefined, { projectRoot: '/p', cwd: '/p' })).toBe(true);
        });
    });
...
describe('buildHumanReadableRuleLabel', () => {
    ...
    it('converts Read with absolute path specifier', () => {
        const label = buildHumanReadableRuleLabel(['Read(//Users/mochi/.vivekmind/**)']);
        expect(label).toBe('read files in /Users/mochi/.vivekmind/');
    });
...
describe('PermissionManager.findMatchingDenyRule', () => {
    it('returns the raw deny rule string when context matches', () => {
        const pm = new PermissionManager(makeConfig({ permissionsDeny: ['Bash(rm *)'] }));
        pm.initialize();
    ...
    it('returns undefined for non-denied tool', () => {
        const pm = new PermissionManager(makeConfig({ permissionsDeny: ['ShellTool'] }));
        pm.initialize();
...
//# sourceMappingURL=permission-manager.test.js.map