...
function sorted(ops) {
    return [...ops].sort((a, b) => `${a.virtualTool}:${a.filePath ?? ''}:${a.domain ?? ''}`.localeCompare(`${b.virtualTool}:${b.filePath ?? ''}:${b.domain ?? ''}`));
}
describe('extractShellOperations', () => {
    ...
    it('grep: -e flag shifts all positionals to paths', () => {
        const ops = extractShellOperations('grep -e password /etc/passwd /etc/shadow', CWD);
        expect(sorted(ops)).toEqual([
            { virtualTool: 'read_file', filePath: '/etc/passwd' },
            { virtualTool: 'read_file', filePath: '/etc/shadow' },
        ]);
    ...
    it('redirect 2>/dev/null: ignored (no op)', () => {
        ...
        expect(ops).not.toContainEqual(expect.objectContaining({ filePath: '/dev/null' }));
        expect(ops).toContainEqual({
            virtualTool: 'read_file',
            filePath: '/etc/passwd',
        });
    ...
    it('curl: extracts domain', () => {
        const ops = extractShellOperations('curl https://api.example.com/data', CWD);
        expect(ops).toEqual([
            { virtualTool: 'web_fetch', domain: 'api.example.com' },
        ]);
    });
    it('curl: -o flag value not treated as URL', () => {
        const ops = extractShellOperations('curl -o /tmp/out.json https://api.example.com', CWD);
        expect(ops).toEqual([
            { virtualTool: 'web_fetch', domain: 'api.example.com' },
        ]);
    });
    it('wget: extracts domain', () => {
        const ops = extractShellOperations('wget https://example.com/file.tar.gz', CWD);
        expect(ops).toEqual([{ virtualTool: 'web_fetch', domain: 'example.com' }]);
    });
    it('wget: -O flag value not treated as URL', () => {
        const ops = extractShellOperations('wget -O /tmp/file.gz https://example.com/f.gz', CWD);
        expect(ops).toEqual([{ virtualTool: 'web_fetch', domain: 'example.com' }]);
    });
    ...
    it('timeout cmd: transparent wrapper', () => {
        const ops = extractShellOperations('timeout 30 wget https://example.com', CWD);
        expect(ops).toEqual([{ virtualTool: 'web_fetch', domain: 'example.com' }]);
    });
    ...
    it('grep pattern file > out: read + write', () => {
        const ops = extractShellOperations('grep secret /etc/config > /tmp/out', CWD);
        expect(sorted(ops)).toEqual([
...
//# sourceMappingURL=shell-semantics.test.js.map