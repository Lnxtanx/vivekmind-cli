...
import { parsePDFPageRange, isPdftotextAvailable, getPDFPageCount, extractPDFText, resetPdftotextCache, } from './pdf.js';
 ...
 */
function mockMaxBufferExceeded(partialStdout) {
    mockExecFile.mockImplementationOnce((_cmd, _args, _opts, cb) => {
        const callback = cb;
...
describe('pdf utilities', () => {
    ...
    describe('parsePDFPageRange', () => {
        ...
        it('should reject page numbers past the safe precision limit', () => {
            ...
            expect(parsePDFPageRange('999999999999999998-999999999999999999')).toBeNull();
    ...
    describe('extractPDFText', () => {
        ...
        it('should quote the filename with -- so hyphen-prefixed paths are not parsed as options', async () => {
            ...
            const extractionArgs = mockExecFile.mock.calls[1][1];
        ...
        it('should surface a dedicated error on timeout', async () => {
            ...
            mockExecFile.mockImplementationOnce((_cmd, _args, _opts, cb) => {
                ...
                const err = new Error('Command failed: pdftotext');
                err.killed = true;
                err.signal = 'SIGTERM';
                callback(err, '', '');
                return {};
