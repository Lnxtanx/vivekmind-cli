...
function encoderForMimeType(image, mimeType) {
    switch (mimeType) {
        case 'image/jpeg':
 ...
 */
export async function preprocessImage(filePath, maxBytes) {
    ...
    const resizeDimension = maxDimension > 0 ? Math.max(1, Math.floor(maxDimension * 0.75)) : undefined;
    const resized = encoderForMimeType(resizeDimension
        ? image.resize({
    ...
    const outputPath = path.join(tempDir, `${path.basename(filePath, path.extname(filePath))}-processed${extensionForMimeType(mimeType)}`);
