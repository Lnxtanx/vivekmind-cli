...
import { setGeminiMdFilename, DEFAULT_CONTEXT_FILENAME, } from '../memory/const.js';
...
describe('loadServerHierarchicalMemory', () => {
    ...
    describe('when untrusted', () => {
        it('does not load context files from untrusted workspaces', async () => {
            ...
            const { fileCount } = await loadServerHierarchicalMemory(cwd, [], new FileDiscoveryService(projectRoot), [], false);
        ...
        it('loads context from outside the untrusted workspace', async () => {
            ...
            const filepath = path.join(homedir, VIVEKMIND_DIR, DEFAULT_CONTEXT_FILENAME);
            await createTestFile(filepath, 'default context content'); // In user home dir (outside untrusted space).
            const { fileCount, memoryContent } = await loadServerHierarchicalMemory(cwd, [], new FileDiscoveryService(projectRoot), [], false);
    ...
    it('should return empty memory and count if no context files are found', async () => {
        const result = await loadServerHierarchicalMemory(cwd, [], new FileDiscoveryService(projectRoot), [], DEFAULT_FOLDER_TRUST);
    ...
    it('should skip implicit global, project, and rule discovery in explicit-only mode', async () => {
        await createTestFile(path.join(homedir, VIVEKMIND_DIR, DEFAULT_CONTEXT_FILENAME), 'global context');
        await createTestFile(path.join(projectRoot, DEFAULT_CONTEXT_FILENAME), 'project context');
        await createTestFile(path.join(cwd, DEFAULT_CONTEXT_FILENAME), 'cwd context');
        await createTestFile(path.join(projectRoot, VIVEKMIND_DIR, 'rules', 'baseline.md'), 'project rule');
        const result = await loadServerHierarchicalMemory(cwd, [], new FileDiscoveryService(projectRoot), [], DEFAULT_FOLDER_TRUST, 'tree', [], { explicitOnly: true });
        expect(result).toEqual({
            memoryContent: '',
            fileCount: 0,
            ruleCount: 0,
    ...
    it('should still load context from explicit include directories in explicit-only mode', async () => {
        ...
        const explicitContextFile = await createTestFile(path.join(extraDir, DEFAULT_CONTEXT_FILENAME), 'explicit context');
        await createTestFile(path.join(homedir, VIVEKMIND_DIR, DEFAULT_CONTEXT_FILENAME), 'global context');
        await createTestFile(path.join(projectRoot, DEFAULT_CONTEXT_FILENAME), 'project context');
        await createTestFile(path.join(projectRoot, VIVEKMIND_DIR, 'rules', 'baseline.md'), 'project rule');
        const result = await loadServerHierarchicalMemory(cwd, [extraDir], new FileDiscoveryService(projectRoot), [], DEFAULT_FOLDER_TRUST, 'tree', [], { explicitOnly: true });
        expect(result).toEqual({
    ...
    it('should load only the global context file if present and others are not (default filename)', async () => {
        const defaultContextFile = await createTestFile(path.join(homedir, VIVEKMIND_DIR, DEFAULT_CONTEXT_FILENAME), 'default context content');
        const result = await loadServerHierarchicalMemory(cwd, [], new FileDiscoveryService(projectRoot), [], DEFAULT_FOLDER_TRUST);
        expect(result).toEqual({
    ...
    it('should load only the global custom context file if present and filename is changed', async () => {
        ...
        const customContextFile = await createTestFile(path.join(homedir, VIVEKMIND_DIR, customFilename), 'custom context content');
        const result = await loadServerHierarchicalMemory(cwd, [], new FileDiscoveryService(projectRoot), [], DEFAULT_FOLDER_TRUST);
        expect(result).toEqual({
    ...
    it('should load and correctly order global and upward context files', async () => {
        ...
        await createTestFile(path.join(cwd, 'sub', DEFAULT_CONTEXT_FILENAME), 'Subdir memory');
        const result = await loadServerHierarchicalMemory(cwd, [], new FileDiscoveryService(projectRoot), [], DEFAULT_FOLDER_TRUST);
    ...
    it('should load extension context file paths', async () => {
        const extensionFilePath = await createTestFile(path.join(testRootDir, 'extensions/ext1/VIVEKMIND.md'), 'Extension memory content');
        const result = await loadServerHierarchicalMemory(cwd, [], new FileDiscoveryService(projectRoot), [extensionFilePath], DEFAULT_FOLDER_TRUST);
        expect(result).toEqual({
    ...
    it('should load memory from included directories', async () => {
        ...
        const result = await loadServerHierarchicalMemory(cwd, [includedDir], new FileDiscoveryService(projectRoot), [], DEFAULT_FOLDER_TRUST);
        expect(result).toEqual({
    ...
    it('should handle multiple directories and files in parallel correctly', async () => {
        ...
        const result = await loadServerHierarchicalMemory(cwd, createdFiles.map((f) => path.dirname(f)), new FileDiscoveryService(projectRoot), [], DEFAULT_FOLDER_TRUST);
    ...
    it('should preserve order and prevent duplicates when processing multiple directories', async () => {
        ...
        new FileDiscoveryService(projectRoot), [], DEFAULT_FOLDER_TRUST);
