 ...
 * `~/.vivekmind/tmp/<hash>/chats/` (by another local user or an extension with
 ...
 */
export function readLastJsonStringFieldSync(filePath, key, lineContains) {
    let fd;
    try {
 ...
 */
export function readLastJsonStringFieldsSync(filePath, primaryKey, otherKeys, lineContains) {
    const emptyResult = {};
    ...
    try {
        ...
        while (readOffset < scanLimit) {
            ...
            const complete = chunk.slice(0, lastNewline + 1);
            carry = chunk.slice(lastNewline + 1);
            const hit = extractLastJsonStringFields(complete, primaryKey, otherKeys, lineContains);
            if (hit[primaryKey] !== undefined)
                latest = hit;
        }
        if (carry) {
            const hit = extractLastJsonStringFields(carry, primaryKey, otherKeys, lineContains);
            if (hit[primaryKey] !== undefined)
                latest = hit;
        }
