...
import { buildRuntimeFetchOptions, getOrCreateSharedDispatcher, resetDispatcherCache, } from './runtimeFetchOptions.js';
vi.mock('undici', () => {
    class MockAgent {
        options;
        constructor(options) {
...
describe('buildRuntimeFetchOptions (node runtime)', () => {
    ...
    it('disables undici timeouts for Agent in OpenAI options', () => {
        ...
        const dispatcher = result.fetchOptions?.dispatcher;
...
describe('getOrCreateSharedDispatcher', () => {
    ...
    it('shares the same dispatcher with buildRuntimeFetchOptions', () => {
        ...
        const result = buildRuntimeFetchOptions('openai');
        const sdkDispatcher = result.fetchOptions?.dispatcher;
        expect(sdkDispatcher).toBe(shared);
    });
});
//# sourceMappingURL=runtimeFetchOptions.test.js.map