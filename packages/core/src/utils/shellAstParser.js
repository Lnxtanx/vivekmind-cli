...
function collectDescendants(node, types) {
    const result = [];
    const stack = [node];
...
function containsCommandSubstitutionAST(node) {
    return (collectDescendants(node, new Set(['command_substitution', 'process_substitution'])).length > 0);
}
/** Check if a redirected_statement contains a write-redirection. */
function hasWriteRedirection(node) {
 ...
 */
function evaluateCommandReadOnly(commandNode) {
...
function evaluateGitReadOnly(args) {
...
function evaluateFindReadOnly(args) {
...
function evaluateSedReadOnly(args) {
    for (const arg of args) {
        if (BLOCKED_SED_PREFIXES.some((p) => arg.startsWith(p)) ||
            arg === '--in-place') {
            return false;
        }
    }
...
function evaluateAwkReadOnly(args) {
    const scriptContent = args.join(' ');
