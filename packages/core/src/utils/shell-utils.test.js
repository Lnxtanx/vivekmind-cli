...
import { checkArgumentSafety, checkCommandPermissions, escapeShellArg, getCommandRoots, getShellConfiguration, hasNonFinalTopLevelBackgroundOperator, hasUnsafeMonitorBackgroundOperator, isCommandAllowed, isCommandNeedsPermission, normalizeMonitorCommand, stripTrailingBackgroundAmp, stripShellWrapper, } from './shell-utils.js';
...
vi.mock('shell-quote', async () => {
    const actual = await vi.importActual('shell-quote');
    return {
        ...actual,
        quote: mockQuote,
    };
...
describe('isCommandAllowed', () => {
    ...
    describe('command substitution', () => {
        ...
        it('should block command substitution using `>(...)`', async () => {
            const result = await isCommandAllowed('echo "Log message" > >(tee log.txt)', config);
            expect(result.allowed).toBe(false);
            expect(result.reason).toContain('Command substitution');
        });
...
describe('checkCommandPermissions', () => {
    ...
    describe('in "Default Deny" mode (with sessionAllowlist)', () => {
        it('should allow a command on the sessionAllowlist', async () => {
            const result = await checkCommandPermissions('ls -l', config, new Set(['ls -l']));
            expect(result.allAllowed).toBe(true);
        });
        it('should block a command not on the sessionAllowlist or global allowlist', async () => {
            const result = await checkCommandPermissions('rm -rf /', config, new Set(['ls -l']));
        ...
        it('should allow a command on the global allowlist even if not on the session allowlist', async () => {
            ...
            const result = await checkCommandPermissions('git status', config, new Set(['ls -l']));
            expect(result.allAllowed).toBe(true);
        });
        it('should allow a chained command if parts are on different allowlists', async () => {
            config.getCoreTools = () => ['ShellTool(git status)'];
            const result = await checkCommandPermissions('git status && git commit', config, new Set(['git commit']));
            expect(result.allAllowed).toBe(true);
        });
        it('should block a command on the sessionAllowlist if it is also globally blocked', async () => {
            config.getPermissionsDeny = () => ['run_shell_command(rm)'];
            const result = await checkCommandPermissions('rm -rf /', config, new Set(['rm -rf /']));
            expect(result.allAllowed).toBe(false);
        ...
        it('should block a chained command if one part is not on any allowlist', async () => {
            ...
            const result = await checkCommandPermissions('echo "hello" && rm -rf /', config, new Set(['echo']));
            expect(result.allAllowed).toBe(false);
            expect(result.disallowedCommands).toEqual(['rm -rf /']);
        });
    });
...
describe('getCommandRoots', () => {
    ...
    it('should treat escaped newlines in chained commands as line continuations', async () => {
        const result = getCommandRoots('cd project && \\\ngit add file.php && \\\ngit commit -m "feat"');
    ...
    it('should skip leading env var assignments', async () => {
        expect(getCommandRoots('PYTHONPATH=/Users/jinjing/.vivekmind/skills/scripts python3 -c "print(1)"')).toEqual(['python3']);
    });
...
describe('stripShellWrapper', () => {
    ...
    it('should strip env-prefixed wrapper', async () => {
        ...
        expect(stripShellWrapper("A=1 B=2 /bin/bash -c 'sleep 5'")).toEqual('sleep 5');
    });
    ...
    it('should consume shell options with separate operands before -c', async () => {
        expect(stripShellWrapper("bash -o pipefail -c 'sleep 5'")).toEqual('sleep 5');
...
describe('stripTrailingBackgroundAmp', () => {
    it('strips a single bare trailing ampersand', () => {
        expect(stripTrailingBackgroundAmp('tail -f app.log &')).toBe('tail -f app.log');
        expect(stripTrailingBackgroundAmp('tail -f app.log &   ')).toBe('tail -f app.log');
    });
    ...
    it('does not strip non-trailing background operators', () => {
        expect(stripTrailingBackgroundAmp('sleep 5 & echo done')).toBe('sleep 5 & echo done');
    });
});
describe('hasNonFinalTopLevelBackgroundOperator', () => {
    it('detects top-level background operators followed by more syntax', () => {
        expect(hasNonFinalTopLevelBackgroundOperator('tail -f app.log & echo ok')).toBe(true);
        expect(hasNonFinalTopLevelBackgroundOperator('tail -f app.log & # watch')).toBe(true);
    });
    it('ignores final, logical, escaped, quoted, and redirection ampersands', () => {
        expect(hasNonFinalTopLevelBackgroundOperator('tail -f app.log &')).toBe(false);
        expect(hasNonFinalTopLevelBackgroundOperator('echo hi && echo ok')).toBe(false);
        expect(hasNonFinalTopLevelBackgroundOperator('echo foo \\& echo ok')).toBe(false);
        expect(hasNonFinalTopLevelBackgroundOperator(`printf '&' && echo ok`)).toBe(false);
...
describe('hasUnsafeMonitorBackgroundOperator', () => {
    it('detects unsafe backgrounding inside shell wrapper scripts and suffixes', () => {
        expect(hasUnsafeMonitorBackgroundOperator("bash -c 'tail -f app.log & echo ready'")).toBe(true);
        expect(hasUnsafeMonitorBackgroundOperator("bash -c 'tail -f app.log' & echo ready")).toBe(true);
    });
    it('allows final trailing ampersands that normalization strips', () => {
        ...
        expect(hasUnsafeMonitorBackgroundOperator("bash -c 'tail -f app.log &'")).toBe(false);
    });
});
describe('normalizeMonitorCommand', () => {
    it('unwraps quoted env-prefixed shell wrappers for analysis', () => {
        expect(normalizeMonitorCommand(`FOO="bar baz" /bin/bash -c 'echo $(cat secret.txt)'`)).toEqual({
    ...
    it('preserves wrapper flags while stripping trailing ampersands', () => {
        expect(normalizeMonitorCommand(`/bin/bash --noprofile -c 'tail -f /tmp/app.log &'`)).toEqual({
    ...
    it('unwraps shell wrappers with option operands for safety analysis', () => {
        expect(normalizeMonitorCommand(`/bin/bash -o pipefail -c 'echo $(cat secret.txt)'`)).toEqual({
            analysisCommand: 'echo $(cat secret.txt)',
    ...
    it('analyzes only the script word after -c while preserving later argv', () => {
        expect(normalizeMonitorCommand(`/bin/bash -c 'echo $(cat secret.txt)' ignored`)).toEqual({
            analysisCommand: 'echo $(cat secret.txt)',
    ...
    it('strips trailing ampersands from the -c script without dropping later argv', () => {
        expect(normalizeMonitorCommand(`/bin/bash -c 'tail -f /tmp/app.log &' ignored`)).toEqual({
            analysisCommand: 'tail -f /tmp/app.log',
    ...
    it('keeps substitutions in wrapper argv suffix in the safety command', () => {
        expect(normalizeMonitorCommand(`/bin/bash -c 'echo ok' $(cat secret.txt)`)).toEqual({
    ...
    it('handles escaped whitespace in env-prefixed wrappers', () => {
        expect(normalizeMonitorCommand(String.raw `FOO=bar\ baz /bin/bash --noprofile -c 'tail -f /tmp/app.log &'`)).toEqual({
            analysisCommand: 'tail -f /tmp/app.log',
    ...
    it('falls back to the original command when no wrapper is detected', () => {
        expect(normalizeMonitorCommand(`FOO="bar baz" tail -f /tmp/app.log`)).toEqual({
    ...
    it('keeps env-prefix substitutions in the safety command', () => {
        expect(normalizeMonitorCommand(`FOO=$(cat secret.txt) /bin/bash -c 'echo ok'`)).toEqual({
            analysisCommand: 'echo ok',
    ...
    describe('on Windows', () => {
        ...
        describe('Git Bash / MSYS2 / MinTTY detection', () => {
            it('should return bash configuration when MSYSTEM starts with MINGW', () => {
                ...
                expect(config.executable.endsWith('bash.exe') ||
                    config.executable === 'bash').toBe(true);
                expect(config.argsPrefix).toEqual(['-c']);
                expect(config.shell).toBe('bash');
            });
