...
import { parseRuleFile, loadRules, ConditionalRulesRegistry, } from './rulesDiscovery.js';
import { VIVEKMIND_DIR, unescapePath } from './paths.js';
...
describe('rulesDiscovery', () => {
    ...
    let projectRoot;
    let homedir;
    async function createTestFile(fullPath, content) {
        await fsPromises.mkdir(path.dirname(fullPath), { recursive: true });
        await fsPromises.writeFile(fullPath, content);
        return fullPath;
    }
    beforeEach(async () => {
        testRootDir = await fsPromises.mkdtemp(path.join(os.tmpdir(), 'rules-discovery-test-'));
        vi.resetAllMocks();
...
`;
    ...
    describe('loadRules', () => {
        ...
        it('loads baseline rules into content', async () => {
            const rulesDir = path.join(projectRoot, VIVEKMIND_DIR, 'rules');
            await createTestFile(path.join(rulesDir, 'general.md'), `---
description: General
---
Always write tests.`);
            const result = await loadRules(projectRoot, true);
        ...
        it('puts conditional rules in conditionalRules, not in content', async () => {
            const rulesDir = path.join(projectRoot, VIVEKMIND_DIR, 'rules');
            await createTestFile(path.join(rulesDir, 'fe.md'), `---
paths:
  - "src/**/*.tsx"
---
Use hooks.`);
        ...
        it('splits baseline and conditional correctly', async () => {
            const rulesDir = path.join(projectRoot, VIVEKMIND_DIR, 'rules');
            await createTestFile(path.join(rulesDir, '01-general.md'), 'Write clean code.');
            await createTestFile(path.join(rulesDir, '02-py.md'), `---\npaths:\n  - "**/*.py"\n---\nUse type hints.`);
            await createTestFile(path.join(rulesDir, '03-ts.md'), `---\npaths:\n  - "**/*.ts"\n---\nUse strict.`);
            const result = await loadRules(projectRoot, true);
            expect(result.ruleCount).toBe(1);
        ...
        it('recursively scans subdirectories', async () => {
            const rulesDir = path.join(projectRoot, VIVEKMIND_DIR, 'rules');
            await createTestFile(path.join(rulesDir, 'frontend', 'react.md'), 'Use hooks.');
            await createTestFile(path.join(rulesDir, 'backend', 'api.md'), 'Validate inputs.');
        ...
        it('skips project rules when folder is untrusted', async () => {
            await createTestFile(path.join(projectRoot, VIVEKMIND_DIR, 'rules', 'r.md'), 'Untrusted.');
            const result = await loadRules(projectRoot, false);
            expect(result.ruleCount).toBe(0);
        });
        it('loads global rules even when folder is untrusted', async () => {
            await createTestFile(path.join(homedir, VIVEKMIND_DIR, 'rules', 'g.md'), 'Global.');
            const result = await loadRules(projectRoot, false);
            expect(result.ruleCount).toBe(1);
            expect(result.content).toContain('Global.');
        });
        it('does not duplicate rules when projectRoot equals homedir', async () => {
            await createTestFile(path.join(homedir, VIVEKMIND_DIR, 'rules', 's.md'), 'Shared.');
        ...
        it('excludes rules matching exclude patterns', async () => {
            ...
            const skipped = await createTestFile(path.join(rulesDir, 'skip.md'), 'Skip.');
        ...
        it('excludes rules in subdirectories by glob', async () => {
            const rulesDir = path.join(projectRoot, VIVEKMIND_DIR, 'rules');
            await createTestFile(path.join(rulesDir, 'other-team', 'r.md'), 'Their rule.');
        ...
        it('formats rules with source markers', async () => {
            await createTestFile(path.join(projectRoot, VIVEKMIND_DIR, 'rules', 'test.md'), 'Content.');
            const result = await loadRules(projectRoot, true);
            expect(result.content).toContain(`--- Rule from: ${VIVEKMIND_DIR}/rules/test.md ---`);
        });
    });
    // ─────────────────────────────────────────────────────────────────────────
    ...
    describe('ConditionalRulesRegistry', () => {
        const rule = (fp, pats, body) => ({
            filePath: fp,
            paths: pats,
            content: body,
        });
        it('matches a file and returns formatted content', () => {
            const reg = new ConditionalRulesRegistry([rule('/r/fe.md', ['src/**/*.tsx'], 'Use hooks.')], '/project');
        ...
        it('tracks totalCount and injectedCount', () => {
            const reg = new ConditionalRulesRegistry([rule('/r/a.md', ['**/*.ts'], 'A'), rule('/r/b.md', ['**/*.py'], 'B')], '/project');
        ...
        it('does not match files outside the project root', () => {
            const reg = new ConditionalRulesRegistry([rule('/r/ts.md', ['**/*.ts'], 'Strict.')], '/project');
        ...
        it('rejects the exact `..` relative path (parent of projectRoot)', () => {
            ...
            const reg = new ConditionalRulesRegistry([rule('/r/dot.md', ['..'], 'Parent rule.')], '/project');
        ...
        it('resolves relative paths against projectRoot', () => {
            const reg = new ConditionalRulesRegistry([rule('/r/ts.md', ['src/**/*.ts'], 'Strict.')], '/project');
        ...
        it('activates on dotfiles when glob covers them (dot: true semantics)', () => {
            ...
            const reg = new ConditionalRulesRegistry([rule('/r/yml.md', ['**/*.yml'], 'YAML rule.')], '/project');
            expect(reg.matchAndConsume('/project/.github/workflows/ci.yml')).toContain('YAML rule.');
        });
        it('rejects Windows cross-drive paths (shared cross-drive guard)', () => {
            ...
            const reg = new ConditionalRulesRegistry([rule('/r/broad.md', ['**/*.ts'], 'Broad rule.')], '/project');
            expect(reg.matchAndConsume('/totally/other/place/file.ts')).toBeUndefined();
        });
        it.skipIf(process.platform === 'win32')('should match shell-escaped file paths after unescaping', () => {
            // On Windows, unescapePath is a no-op (backslash is a path
            // separator, not a shell escape character).
            const reg = new ConditionalRulesRegistry([rule('/r/ts.md', ['src/**/*.tsx'], 'Use hooks.')], '/project');
            ...
            const result = reg.matchAndConsume(path.resolve('/project', normalizedPath));
            expect(result).toContain('Use hooks.');
        });
    });
});
//# sourceMappingURL=rulesDiscovery.test.js.map