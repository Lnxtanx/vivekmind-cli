...
describe('subagentGenerator', () => {
    ...
    it('should throw error for empty user description', async () => {
        await expect(subagentGenerator('', mockConfig, abortSignal)).rejects.toThrow('User description cannot be empty');
        await expect(subagentGenerator('   ', mockConfig, abortSignal)).rejects.toThrow('User description cannot be empty');
    ...
    it('should successfully generate content with valid LLM response', async () => {
        ...
        mockClient.generateJson.mockResolvedValue(mockApiResponse);
        const result = await subagentGenerator(userDescription, mockConfig, abortSignal);
        ...
        const generateJsonCall = mockClient.generateJson.mock.calls[0];
    ...
    it('should throw error when LLM response is missing required fields', async () => {
        ...
        mockClient.generateJson.mockResolvedValue(incompleteResponse);
        await expect(subagentGenerator(userDescription, mockConfig, abortSignal)).rejects.toThrow('Invalid response from LLM: missing required fields');
        expect(mockClient.generateJson).toHaveBeenCalledTimes(1);
    });
    it('should throw error when LLM response has empty fields', async () => {
        ...
        mockClient.generateJson.mockResolvedValue(emptyFieldsResponse);
        await expect(subagentGenerator(userDescription, mockConfig, abortSignal)).rejects.toThrow('Invalid response from LLM: missing required fields');
    });
    it('should throw error when generateJson throws an error', async () => {
        ...
        await expect(subagentGenerator(userDescription, mockConfig, abortSignal)).rejects.toThrow('API Error');
    });
    ...
    it('should throw error for undefined response from generateJson', async () => {
        ...
        mockClient.generateJson.mockResolvedValue(undefined);
        await expect(subagentGenerator(userDescription, mockConfig, abortSignal)).rejects.toThrow('Invalid response from LLM: missing required fields');
    });
});
