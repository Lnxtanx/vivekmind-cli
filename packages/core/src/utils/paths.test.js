...
import { escapePath, resolvePath, validatePath, resolveAndValidatePath, unescapePath, isSubpath, shortenPath, tildeifyPath, getProjectHash, _resetValidatePathCacheForTest, } from './paths.js';
...
describe('unescapePath', () => {
    ...
    it.skipIf(!isWindows)('should be a no-op on Windows', () => {
        expect(unescapePath('C:\\Users\\my file.txt')).toBe('C:\\Users\\my file.txt');
        expect(unescapePath('C:\\(v2)\\file.txt')).toBe('C:\\(v2)\\file.txt');
        expect(unescapePath('path\\to\\file\\ name.txt')).toBe('path\\to\\file\\ name.txt');
    });
...
describe('validatePath', () => {
    ...
    it('does not cache ENOENT — recreating the path between calls succeeds', () => {
        ...
        expect(() => validatePath(config, ephemeralDir)).toThrowError(/Path does not exist:/);
...
describe('shortenPath', () => {
    ...
    it('handles paths with single segment after root', () => {
        const result = shortenPath('/verylongfilenamethatshouldbetruncated.txt', 20);
        expect(result).toContain('...');
    ...
    it('example from documentation: /path/to/a/very/long/file.txt', () => {
        ...
        expect(result).toMatch(new RegExp(`^${sepForRegex}path${sepForRegex}\\.\\.\\..+file\\.txt$`));
        expect(result.length).toBeLessThanOrEqual(35);
    });
});
