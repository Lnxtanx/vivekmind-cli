...
import { getEnvironmentContext, getDirectoryContextString, getInitialChatHistory, stripStartupContext, } from './environmentContext.js';
...
describe('stripStartupContext', () => {
    it('should strip the env context + model ack from the start of history', () => {
        const history = [
            { role: 'user', parts: [{ text: 'This is the VivekMind...' }] },
            {
                role: 'model',
                parts: [{ text: 'Got it. Thanks for the context!' }],
            },
    ...
    it('should return history unchanged when it has fewer than 2 entries', () => {
        ...
        expect(stripStartupContext([{ role: 'user', parts: [{ text: 'Hello' }] }])).toEqual([{ role: 'user', parts: [{ text: 'Hello' }] }]);
    });
    it('should round-trip with getInitialChatHistory', async () => {
        ...
        const withStartup = await getInitialChatHistory(mockConfig, conversation);
