...
import { createDebugLogger, isDebugLoggingDegraded, resetDebugLoggingState, setDebugLogSession, } from './debugLogger.js';
...
describe('debugLogger', () => {
    ...
    const previousDebugLogFileEnv = process.env['VIVEKMIND_DEBUG_LOG_FILE'];
    beforeEach(() => {
        process.env['VIVEKMIND_DEBUG_LOG_FILE'] = '1';
    ...
    afterEach(() => {
        ...
        if (previousDebugLogFileEnv === undefined) {
            delete process.env['VIVEKMIND_DEBUG_LOG_FILE'];
        }
        else {
            process.env['VIVEKMIND_DEBUG_LOG_FILE'] = previousDebugLogFileEnv;
        }
    });
    describe('createDebugLogger', () => {
        ...
        it('writes debug log with correct format', async () => {
            ...
            expect(fs.appendFile).toHaveBeenCalledWith(Storage.getDebugLogPath('test-session-123'), '2026-01-24T10:30:00.000Z [DEBUG] Hello world\n', 'utf8');
        });
        it('writes log with tag when provided', async () => {
            const logger = createDebugLogger('STARTUP');
            logger.info('Server started');
            await vi.runAllTimersAsync();
            expect(fs.appendFile).toHaveBeenCalledWith(Storage.getDebugLogPath('test-session-123'), '2026-01-24T10:30:00.000Z [INFO] [STARTUP] Server started\n', 'utf8');
        });
        ...
        it('formats multiple arguments', async () => {
            ...
            await vi.runAllTimersAsync();
            expect(fs.appendFile).toHaveBeenCalledWith(expect.any(String), expect.stringContaining('Count: 42 items'), 'utf8');
        });
    ...
    describe('isDebugLoggingDegraded', () => {
        ...
        it('stays true after failure even if subsequent writes succeed', async () => {
            vi.mocked(fs.appendFile).mockRejectedValueOnce(new Error('Temporary error'));
            const logger = createDebugLogger();
    ...
    describe('latest debug log symlink', () => {
        ...
        it('creates a symlink to the current session log file', async () => {
            ...
            expect(fs.symlink).toHaveBeenCalledWith('test-session-123.txt', expectedLatestPath);
        });
        ...
        it('does not create symlink when debug logging is disabled', async () => {
            process.env['VIVEKMIND_DEBUG_LOG_FILE'] = '0';
            vi.clearAllMocks();
            resetDebugLoggingState();
            setDebugLogSession(mockSession);
            await vi.runAllTimersAsync();
