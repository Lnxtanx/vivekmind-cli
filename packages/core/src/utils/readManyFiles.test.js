...
function contentToString(parts) {
...
describe('readManyFiles', () => {
    ...
    const createMockConfig = (rootDir) => ({
    ...
    async function createTestDir(...pathSegments) {
        const absolutePath = path.join(tempRootDir, ...pathSegments);
        await fs.mkdir(absolutePath, { recursive: true });
        return absolutePath;
    }
    beforeEach(async () => {
        tempRootDir = nodeFs.realpathSync(await fs.mkdtemp(path.join(os.tmpdir(), 'read-many-files-test-')));
    });
    afterEach(async () => {
    ...
    describe('file reading', () => {
        ...
        it('should return message when no files found', async () => {
            ...
            expect(contentToString(result.contentParts)).toContain('No files matching the criteria were found');
        });
    });
    ...
    describe('edge cases', () => {
        ...
        it('should allow directories outside project root', async () => {
            ...
            const outsideDir = await fs.mkdtemp(path.join(os.tmpdir(), 'outside-workspace-'));
    ...
    describe('per-file error surfacing', () => {
        it('should surface processSingleFileContent errors instead of silently skipping the file', async () => {
            ...
            expect(content).not.toContain('No files matching the criteria were found');
            expect(result.files).toHaveLength(1);
            expect(result.files[0].filePath).toBe(absolutePath);
...
//# sourceMappingURL=readManyFiles.test.js.map