...
import { retryWithBackoff, isTransientCapacityError, isUnattendedMode, } from './retry.js';
...
describe('retryWithBackoff', () => {
    ...
    it('should default to 7 maxAttempts if no options are provided', async () => {
        ...
        // eslint-disable-next-line vitest/valid-expect
        const assertionPromise = expect(promise).rejects.toThrow('Simulated error attempt 7');
        await vi.runAllTimersAsync();
        await assertionPromise;
    ...
    describe('VivekMind OAuth 429 error handling', () => {
        it('should retry for VivekMind OAuth 429 errors that are throttling-related', async () => {
        ...
        it('should throw immediately for VivekMind OAuth with insufficient_quota message', async () => {
            const errorWithInsufficientQuota = Object.assign(new Error('Free allocated quota exceeded.'), { status: 429, code: 'insufficient_quota' });
            ...
            await expect(promise).rejects.toThrow(/VivekMind OAuth free tier has been discontinued/);
            // Should be called only once (no retries)
            expect(fn).toHaveBeenCalledTimes(1);
        });
        it('should throw immediately for VivekMind OAuth with free allocated quota exceeded message', async () => {
            const errorWithQuotaExceeded = Object.assign(new Error('Free allocated quota exceeded.'), { status: 429, code: 'insufficient_quota' });
        ...
        it('should retry for VivekMind OAuth with throttling message', async () => {
        ...
        it('should retry for VivekMind OAuth with throttling error', async () => {
        ...
        it('should throw immediately for VivekMind OAuth with quota message', async () => {
            const errorWithQuota = Object.assign(new Error('Free allocated quota exceeded.'), { status: 429, code: 'insufficient_quota' });
        ...
        it('should retry normal errors for VivekMind OAuth (not quota-related)', async () => {
...
describe('isUnattendedMode', () => {
    ...
    beforeEach(() => {
        ...
        delete process.env['VIVEKMIND_CODE_UNATTENDED_RETRY'];
    });
    afterAll(() => {
        process.env = originalEnv;
    });
    it('should return true when VIVEKMIND_CODE_UNATTENDED_RETRY=1', () => {
        process.env['VIVEKMIND_CODE_UNATTENDED_RETRY'] = '1';
        expect(isUnattendedMode()).toBe(true);
    });
    it('should return true when VIVEKMIND_CODE_UNATTENDED_RETRY=true', () => {
        process.env['VIVEKMIND_CODE_UNATTENDED_RETRY'] = 'true';
        expect(isUnattendedMode()).toBe(true);
    });
    ...
    it('should return false for non-matching values', () => {
        process.env['VIVEKMIND_CODE_UNATTENDED_RETRY'] = '0';
        expect(isUnattendedMode()).toBe(false);
        process.env['VIVEKMIND_CODE_UNATTENDED_RETRY'] = 'false';
        expect(isUnattendedMode()).toBe(false);
        process.env['VIVEKMIND_CODE_UNATTENDED_RETRY'] = '';
        expect(isUnattendedMode()).toBe(false);
    });
    it('should use strict matching consistent with parseBooleanEnvFlag', () => {
        ...
        process.env['VIVEKMIND_CODE_UNATTENDED_RETRY'] = 'yes';
        expect(isUnattendedMode()).toBe(false);
    });
});
describe('retryWithBackoff - persistent mode', () => {
    ...
    it('should NOT retry indefinitely for 500 errors in persistent mode', async () => {
        ...
        // eslint-disable-next-line vitest/valid-expect
        const assertionPromise = expect(promise).rejects.toThrow('Internal Server Error');
        await vi.runAllTimersAsync();
        await assertionPromise;
    ...
    it('should abort persistent retry when signal is aborted', async () => {
        ...
        // eslint-disable-next-line vitest/valid-expect
        const assertionPromise = expect(promise).rejects.toThrow('Retry aborted by signal');
        await vi.runAllTimersAsync();
        await assertionPromise;
    });
...
describe('retryWithBackoff - Retry-After handling in persistent mode', () => {
    ...
    function make429WithRetryAfter(seconds) {
...
describe('getErrorStatus', () => {
    ...
    it('should prefer status over statusCode over response.status over error.code', () => {
        ...
        expect(getErrorStatus({ response: { status: 502 }, error: { code: 503 } })).toBe(502);
    });
    ...
    it('should return undefined for non-numeric status values', () => {
        ...
        expect(getErrorStatus({ error: { code: 'invalid_api_key' } })).toBeUndefined();
    });
    ...
    it('should prefer numeric status fields over HTTP_STATUS/NNN in message', () => {
        const error = new Error(':HTTP_STATUS/500');
        error.status = 429;
        expect(getErrorStatus(error)).toBe(429);
    });
