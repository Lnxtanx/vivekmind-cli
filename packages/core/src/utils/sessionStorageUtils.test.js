...
import { extractJsonStringField, extractLastJsonStringField, extractLastJsonStringFields, LITE_READ_BUF_SIZE, readLastJsonStringFieldSync, readLastJsonStringFieldsSync, unescapeJsonString, } from './sessionStorageUtils.js';
describe('sessionStorageUtils', () => {
    ...
    describe('extractLastJsonStringField', () => {
        ...
        it('should handle mixed spacing styles', () => {
            ...
            expect(extractLastJsonStringField(text, 'customTitle')).toBe('with-space');
        });
        ...
        it('should filter by lineContains when provided', () => {
            ...
            expect(extractLastJsonStringField(text, 'customTitle', 'custom_title')).toBe('real-title');
        });
        it('should ignore matches on lines without lineContains marker', () => {
            ...
            expect(extractLastJsonStringField(text, 'customTitle', 'custom_title')).toBe('legit');
        });
        it('should return undefined when lineContains excludes all matches', () => {
            const text = '{"customTitle":"no-subtype-here"}';
            expect(extractLastJsonStringField(text, 'customTitle', 'custom_title')).toBeUndefined();
        });
        it('should not confuse different field names', () => {
            ...
            expect(extractLastJsonStringField(text, 'otherField')).toBe('other-value');
        });
        it('should handle many occurrences', () => {
            const lines = Array.from({ length: 10 }, (_, i) => `{"customTitle":"title-${i}"}`).join('\n');
    ...
    describe('readLastJsonStringFieldSync', () => {
        ...
        it('returns undefined for a missing file', () => {
            ...
            expect(readLastJsonStringFieldSync(p, 'customTitle', 'custom_title')).toBeUndefined();
        });
        ...
        it('returns the only match for a small file', () => {
            const p = writeFile('small.jsonl', '{"type":"user"}\n{"subtype":"custom_title","customTitle":"only"}\n');
            expect(readLastJsonStringFieldSync(p, 'customTitle', 'custom_title')).toBe('only');
        });
        it('returns the last match when the tail contains the field', () => {
            const p = writeFile('tail-hit.jsonl', [
                '{"subtype":"custom_title","customTitle":"old"}',
                '{"subtype":"custom_title","customTitle":"new"}',
                '',
            ].join('\n'));
            expect(readLastJsonStringFieldSync(p, 'customTitle', 'custom_title')).toBe('new');
        });
        it('falls back to full-file scan when tail has no match (Phase 2)', () => {
            ...
            const titleLine = '{"subtype":"custom_title","customTitle":"buried-in-middle"}';
            ...
            expect(readLastJsonStringFieldSync(p, 'customTitle', 'custom_title')).toBe('buried-in-middle');
        });
        it('returns the last occurrence even when multiple land in the full-scan region', () => {
            ...
            expect(readLastJsonStringFieldSync(p, 'customTitle', 'custom_title')).toBe('second-rename');
        });
        it('respects the lineContains filter when scanning', () => {
            const p = writeFile('filter.jsonl', [
                '{"type":"user","customTitle":"spoofed-in-user-content"}',
                '{"subtype":"custom_title","customTitle":"legit"}',
                '',
            ].join('\n'));
            expect(readLastJsonStringFieldSync(p, 'customTitle', 'custom_title')).toBe('legit');
        });
        ...
        it('handles a final line without a trailing newline', () => {
            const p = writeFile('no-trailing-newline.jsonl', '{"type":"user"}\n{"subtype":"custom_title","customTitle":"last"}');
            expect(readLastJsonStringFieldSync(p, 'customTitle', 'custom_title')).toBe('last');
        });
    });
    describe('extractLastJsonStringFields', () => {
        it('returns undefined for every key when primary is absent', () => {
            const hit = extractLastJsonStringFields('{"type":"user","message":"hi"}', 'customTitle', ['titleSource'], 'custom_title');
        ...
        it('extracts secondary field from the same line as the primary', () => {
            const text = '{"subtype":"custom_title","customTitle":"A","titleSource":"auto"}\n';
            const hit = extractLastJsonStringFields(text, 'customTitle', ['titleSource'], 'custom_title');
        ...
        it('handles escaped quotes inside the primary value', () => {
            const text = '{"subtype":"custom_title","customTitle":"He said \\"hi\\"","titleSource":"manual"}\n';
            const hit = extractLastJsonStringFields(text, 'customTitle', ['titleSource'], 'custom_title');
            expect(hit).toEqual({
    ...
    describe('readLastJsonStringFieldsSync', () => {
        ...
        it('returns all-undefined for a missing file', () => {
            ...
            expect(readLastJsonStringFieldsSync(p, 'customTitle', ['titleSource'], 'custom_title')).toEqual({ customTitle: undefined, titleSource: undefined });
        });
        it('returns the atomic pair when tail contains the match', () => {
            const p = writeFile('tail.jsonl', '{"subtype":"custom_title","customTitle":"A","titleSource":"auto"}\n');
            expect(readLastJsonStringFieldsSync(p, 'customTitle', ['titleSource'], 'custom_title')).toEqual({ customTitle: 'A', titleSource: 'auto' });
        });
        it('falls through to full-file scan when tail has no match and finds the pair', () => {
            ...
            expect(readLastJsonStringFieldsSync(p, 'customTitle', ['titleSource'], 'custom_title')).toEqual({ customTitle: 'X', titleSource: 'auto' });
        });
        it('handles records straddling a Phase-2 chunk boundary', () => {
            ...
            const p = writeFile('straddle.jsonl', userHeader +
                ...
                padB +
                userFooter);
            expect(readLastJsonStringFieldsSync(p, 'customTitle', ['titleSource'], 'custom_title')).toEqual({ customTitle: 'spanner', titleSource: 'manual' });
        });
        it('does not let a truncated trailing partial record win', () => {
            const p = writeFile('truncated.jsonl', '{"subtype":"custom_title","customTitle":"A","titleSource":"auto"}\n' +
                '{"subtype":"custom_title","customTitle":"B');
            expect(readLastJsonStringFieldsSync(p, 'customTitle', ['titleSource'], 'custom_title')).toEqual({ customTitle: 'A', titleSource: 'auto' });
        });
    });
});
//# sourceMappingURL=sessionStorageUtils.test.js.map