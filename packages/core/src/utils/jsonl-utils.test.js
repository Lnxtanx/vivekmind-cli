...
import { _recoverObjectsFromLine, _resetEnsuredDirsCacheForTest, read, readLines, } from './jsonl-utils.js';
let tmpRoot;
beforeAll(() => {
...
describe('_recoverObjectsFromLine', () => {
    ...
    it('recovers objects around an unbalanced fragment', () => {
        ...
        expect(_recoverObjectsFromLine('{"a":1}{"oops":}{"b":2}')).toEqual([{ a: 1 }, { b: 2 }]);
    });
...
describe('read() / readLines() with malformed lines', () => {
    ...
    it('readLines respects the limit when objects come from recovery', async () => {
        ...
        expect((await readLines(file, 3)).map((r) => r.i)).toEqual([
            1, 2, 3,
        ]);
    });
    it('readLines recovers when the malformed line is within the first N', async () => {
        const file = tmpFile('{"i":1}{"i":2}\n{"i":3}\n');
        expect((await readLines(file, 5)).map((r) => r.i)).toEqual([
            1, 2, 3,
        ]);
    });
    it('skips blank lines', async () => {
        const file = tmpFile('{"a":1}\n\n{"a":2}\n');
        expect(await read(file)).toEqual([{ a: 1 }, { a: 2 }]);
    });
});
//# sourceMappingURL=jsonl-utils.test.js.map