...
import { iconvDecode, iconvEncodingExists, isUtf8CompatibleEncoding, } from './iconvHelper.js';
 ...
 */
export async function readFileWithEncodingInfo(filePath) {
    ...
    if (detected && !isUtf8CompatibleEncoding(detected)) {
        try {
            ...
            debugLogger.warn(`Failed to decode file ${filePath} as ${detected}: ${e instanceof Error ? e.message : String(e)}`);
        }
    }
 ...
 */
export async function processSingleFileContent(filePath, config, offset, limit, pages) {
    ...
    try {
        ...
        const willExtractPdfText = fileType === 'pdf' && (pages !== undefined || !modalities.pdf);
        ...
        switch (fileType) {
            ...
            case 'pdf': {
                ...
                const pdfResult = await extractPDFText(filePath, pageRange ?? undefined);
