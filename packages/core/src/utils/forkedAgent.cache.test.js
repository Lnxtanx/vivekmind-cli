...
import { saveCacheSafeParams, getCacheSafeParams, clearCacheSafeParams, runForkedAgent, } from './forkedAgent.js';
...
describe('CacheSafeParams', () => {
    ...
    describe('saveCacheSafeParams / getCacheSafeParams', () => {
        ...
        it('deep clones generationConfig', () => {
            ...
            config.tools[0].functionDeclarations.push({ name: 'tool2' });
            const params = getCacheSafeParams();
            const savedTools = params.generationConfig.tools;
    ...
    describe('version detection', () => {
        ...
        it('increments version when tools change', () => {
            saveCacheSafeParams({ tools: [{ functionDeclarations: [{ name: 'a' }] }] }, [], 'model');
            const v1 = getCacheSafeParams().version;
            saveCacheSafeParams({ tools: [{ functionDeclarations: [{ name: 'a' }, { name: 'b' }] }] }, [], 'model');
            const v2 = getCacheSafeParams().version;
            expect(v2).toBeGreaterThan(v1);
        });
        it('does not increment version when only history changes', () => {
            ...
            saveCacheSafeParams(config, [], 'model');
            const v1 = getCacheSafeParams().version;
            saveCacheSafeParams(config, [{ role: 'user', parts: [{ text: 'hi' }] }], 'model');
            const v2 = getCacheSafeParams().version;
            expect(v2).toBe(v1);
        });
    });
...
describe('runForkedAgent (cache path)', () => {
    ...
    it('passes tools: [] in per-request config so the model cannot produce function calls', async () => {
        ...
        }, [{ role: 'user', parts: [{ text: 'hello' }] }], 'test-model');
        // Track what sendMessageStream receives
        let capturedParams = null;
        const mockSendMessageStream = vi.fn((_model, params, _promptId) => {
        ...
        const chatGenerationConfig = ctorArgs[1];
        ...
        const sendParams = capturedParams;
        ...
        expect(mockSendMessageStream).toHaveBeenCalledWith('test-model', expect.objectContaining({
...
//# sourceMappingURL=forkedAgent.cache.test.js.map