...
export async function retryWithBackoff(fn, options) {
    ...
    const cleanOptions = options
        ...
        : {};
    const { maxAttempts, initialDelayMs, maxDelayMs, authType, shouldRetryOnError, shouldRetryOnContent, persistentMode, persistentMaxBackoffMs, persistentCapMs, heartbeatIntervalMs, heartbeatFn, signal, } = {
    ...
    while (attempt < maxAttempts) {
        ...
        try {
            ...
            const callerAllowsRetry = shouldRetryOnError(error);
            ...
            if (shouldPersist) {
                ...
                if (retryAfterMs > 0) {
                    ...
                    delayMs = Math.min(initialDelayMs * Math.pow(2, persistentAttempt - 1), maxBackoff);
                ...
                debugLogger.warn(`[Persistent] Attempt ${reportedAttempt} failed with status ${errorStatus ?? 'unknown'}. ` +
                    `Retrying in ${Math.ceil(delayMs / 1000)}s...`, error);
