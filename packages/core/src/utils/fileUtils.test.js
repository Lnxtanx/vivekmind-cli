...
import { isWithinRoot, isBinaryFile, detectFileType, processSingleFileContent, detectBOM, readFileWithEncoding, readFileWithEncodingInfo, detectFileEncoding, fileExists, } from './fileUtils.js';
...
vi.mock('node:child_process', async (importOriginal) => {
    ...
    return {
        ...actual,
        execFile: vi.fn((_command, _args, _optionsOrCallback, _callback) => {
...
describe('fileUtils', () => {
    ...
    describe('BOM detection and encoding', () => {
        ...
        describe('readFileWithEncodingInfo', () => {
            ...
            it('should return bom: true and encoding utf-8 for UTF-8 BOM file', async () => {
                ...
                await fsPromises.writeFile(filePath, Buffer.concat([utf8Bom, Buffer.from('Hello', 'utf8')]));
                const result = await readFileWithEncodingInfo(filePath);
                expect(result.content).toBe('Hello');
                expect(result.encoding).toBe('utf-8');
    ...
    describe('processSingleFileContent', () => {
        ...
        it('should process an image file', async () => {
            ...
            expect(result.llmContent
                .inlineData.displayName).toBe('image.png');
        ...
        it('should reject image files when model does not support image', async () => {
            ...
            const result = await processSingleFileContent(testImageFilePath, mockConfigNoImage);
        ...
        it('should fall back to pdftotext when model does not support PDF', async () => {
            ...
            const result = await processSingleFileContent(testPdfFilePath, mockConfigNoPdf);
            expect(typeof result.llmContent).toBe('string');
        ...
        it('should skip the 10MB size gate when extracting PDF text by pages', async () => {
            ...
            try {
                ...
                const result = await processSingleFileContent(testPdfFilePath, mockConfigNoPdf, undefined, undefined, '1-5');
        ...
        it('should still reject oversized PDFs when routing to the native base64 path', async () => {
            ...
            try {
                ...
                const result = await processSingleFileContent(testPdfFilePath, mockConfigWithPdf);
  ...
  `;
        ...
        it('should return an error if the file size exceeds 10MB', async () => {
            ...
            try {
                ...
                expect(result.returnDisplay).toContain('File size exceeds the 10MB limit');
