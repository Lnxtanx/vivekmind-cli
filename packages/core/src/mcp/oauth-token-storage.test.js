...
import { VIVEKMIND_DIR } from '../utils/paths.js';
// Mock debugLogger
    ...
    describe('with encrypted flag false', () => {
        ...
        describe('getAllCredentials', () => {
            ...
            it('should load tokens from file successfully', async () => {
                ...
                expect(fs.readFile).toHaveBeenCalledWith(path.join('/mock/home', VIVEKMIND_DIR, 'mcp-oauth-tokens.json'), 'utf-8');
            });
            it('should handle corrupted token file gracefully', async () => {
                vi.mocked(fs.readFile).mockResolvedValue('invalid json');
                const tokens = await tokenStorage.getAllCredentials();
                expect(tokens.size).toBe(0);
                expect(mockDebugLogger.error).toHaveBeenCalledWith(expect.stringContaining('Failed to load MCP OAuth tokens'));
            });
        ...
        describe('saveToken', () => {
            it('should save token with restricted permissions', async () => {
                ...
                expect(fs.mkdir).toHaveBeenCalledWith(path.join('/mock/home', VIVEKMIND_DIR), { recursive: true });
                expect(fs.writeFile).toHaveBeenCalledWith(path.join('/mock/home', VIVEKMIND_DIR, 'mcp-oauth-tokens.json'), expect.stringContaining('test-server'), { mode: 0o600 });
            });
            ...
            it('should handle write errors gracefully', async () => {
                ...
                expect(mockDebugLogger.error).toHaveBeenCalledWith(expect.stringContaining('Failed to save MCP OAuth token'));
            });
        });
        ...
        describe('deleteCredentials', () => {
            ...
            it('should remove token file when no tokens remain', async () => {
                ...
                expect(fs.unlink).toHaveBeenCalledWith(path.join('/mock/home', VIVEKMIND_DIR, 'mcp-oauth-tokens.json'));
            ...
            it('should handle file operation errors gracefully', async () => {
                ...
                await tokenStorage.deleteCredentials('test-server');
                expect(mockDebugLogger.error).toHaveBeenCalledWith(expect.stringContaining('Failed to remove MCP OAuth token'));
            });
        });
        ...
        describe('clearAll', () => {
            ...
            it('should handle other file errors gracefully', async () => {
                vi.mocked(fs.unlink).mockRejectedValue(new Error('Permission denied'));
                await tokenStorage.clearAll();
                expect(mockDebugLogger.error).toHaveBeenCalledWith(expect.stringContaining('Failed to clear MCP OAuth tokens'));
            });
        });
    });
