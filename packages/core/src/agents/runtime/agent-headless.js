...
export class AgentHeadless {
    ...
    static async create(name, runtimeContext, promptConfig, modelConfig, runConfig, toolConfig, eventEmitter, hooks) {
        const core = new AgentCore(name, runtimeContext, promptConfig, modelConfig, runConfig, toolConfig, eventEmitter, hooks);
     ...
     */
    async execute(context, externalSignal) {
        ...
        const initialMessages = initialMessagesOverride && initialMessagesOverride.length > 0
        ...
        try {
            ...
            const completionEvent = new SubagentExecutionEvent(this.core.name, this.terminateMode === AgentTerminateMode.GOAL ? 'completed' : 'failed', {
