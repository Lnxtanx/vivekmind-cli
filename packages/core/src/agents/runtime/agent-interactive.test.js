...
function createMockCore(overrides = {}) {
    ...
    const core = {
        ...
        runReasoningLoop: vi.fn().mockResolvedValue(overrides.loopResult ?? {
            text: 'Done',
            terminateMode: null,
            turnsUsed: 1,
        }),
    ...
    emitter.on(AgentEventType.ROUND_TEXT, (event) => {
        if (event.thoughtText) {
            core.pushMessage('assistant', event.thoughtText, { thought: true });
        }
        if (event.text) {
            core.pushMessage('assistant', event.text);
        }
    });
    emitter.on(AgentEventType.TOOL_CALL, (event) => {
        core.pushMessage('tool_call', `Tool call: ${event.name}`, {
            metadata: {
                callId: event.callId,
                toolName: event.name,
                args: event.args,
    ...
    emitter.on(AgentEventType.TOOL_RESULT, (event) => {
        ...
        core.pushMessage('tool_result', summary, {
            metadata: {
                callId: event.callId,
                toolName: event.name,
...
describe('AgentInteractive', () => {
    ...
    it('should survive round errors and recover', async () => {
        const { core } = createMockCore();
        let callCount = 0;
        core.runReasoningLoop.mockImplementation(() => {
            callCount++;
            if (callCount === 1) {
        ...
        const errorMsg = messages.find((m) => m.role === 'info' &&
    ...
    it('should cancel current round without killing the agent', async () => {
        const { core } = createMockCore();
        let resolveLoop;
        core.runReasoningLoop.mockImplementation(() => new Promise((resolve) => {
            resolveLoop = () => resolve({ text: '', terminateMode: 'cancelled', turnsUsed: 0 });
        }));
        const config = createConfig();
        const agent = new AgentInteractive(config, core);
        await agent.start(context);
    ...
    it('should not cross-contaminate text across messages', async () => {
        ...
        const corrupted = assistantMessages.find((m) => m.content.includes('response-1') && m.content.includes('response-2'));
    ...
    it('should capture thinking text as assistant messages with thought=true', async () => {
        ...
        const messages = agent.getMessages();
        const thoughtMsg = messages.find((m) => m.role === 'assistant' && m.thought === true);
    ...
    it('should place text before tool_call to preserve temporal ordering', async () => {
        ...
        const textIdx = nonUser.findIndex((m) => m.role === 'assistant' && m.content === 'Let me check');
...
//# sourceMappingURL=agent-interactive.test.js.map