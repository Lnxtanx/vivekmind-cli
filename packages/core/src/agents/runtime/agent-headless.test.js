...
import { createContentGenerator, createContentGeneratorConfig, resolveContentGeneratorConfigWithSources, AuthType, } from '../../core/contentGenerator.js';
...
vi.mock('../../core/contentGenerator.js', async (importOriginal) => {
    ...
    const { DEFAULT_VIVEKMIND_MODEL } = await import('../../config/models.js');
    return {
        ...actual,
    ...
    describe('AgentHeadless', () => {
        ...
        describe('create (Tool Validation)', () => {
            ...
            it('should not block creation when a tool may require confirmation', async () => {
                ...
                const scope = await AgentHeadless.create('test-agent', config, promptConfig, defaultModelConfig, defaultRunConfig, toolConfig);
                expect(scope).toBeInstanceOf(AgentHeadless);
            });
        ...
        describe('execute - Initialization and Prompting', () => {
            ...
            it('should append userMemory to the system prompt when available', async () => {
                const { config } = await createMockConfig();
                const userMemoryContent = '# Output language preference: English\nRespond in English.';
                ...
                await scope.execute(context);
                const generationConfig = getGenerationConfigFromMock();
                expect(generationConfig.systemInstruction).toContain('You are a test agent.');
                expect(generationConfig.systemInstruction).toContain('Important Rules:');
                expect(generationConfig.systemInstruction).toContain('# Output language preference: English');
                expect(generationConfig.systemInstruction).toContain('Respond in English.');
            });
            it('should not append userMemory separator when userMemory is empty', async () => {
                ...
                await scope.execute(context);
                const generationConfig = getGenerationConfigFromMock();
                const sysPrompt = generationConfig.systemInstruction;
            ...
            it('should throw an error if template variables are missing', async () => {
                ...
                await expect(scope.execute(context)).rejects.toThrow('Missing context values for the following keys: missing');
            ...
            it('should validate that systemPrompt and renderedSystemPrompt are mutually exclusive', async () => {
                ...
                const context = new ContextState();
                const agent = await AgentHeadless.create('TestAgent', config, promptConfig, defaultModelConfig, defaultRunConfig);
                await expect(agent.execute(context)).rejects.toThrow('PromptConfig cannot have both `systemPrompt` and `renderedSystemPrompt` defined.');
        ...
        describe('execute - Termination and Recovery', () => {
            ...
            it('should terminate with MAX_TURNS if the limit is reached', async () => {
                ...
                const scope = await AgentHeadless.create('test-agent', config, promptConfig, defaultModelConfig, runConfig);
                await scope.execute(new ContextState());
            ...
            it.skip('should terminate with ERROR if the model call throws', async () => {
                ...
                const scope = await AgentHeadless.create('test-agent', config, promptConfig, defaultModelConfig, defaultRunConfig);
                await expect(scope.execute(new ContextState())).rejects.toThrow('API Failure');
                expect(scope.getTerminateMode()).toBe(AgentTerminateMode.ERROR);
            });
        });
        describe('execute - Streaming and Thought Handling', () => {
            ...
            const createMockStreamWithParts = (parts) => vi.fn().mockImplementation(async () => (async function* () {
                yield {
                    type: 'chunk',
                    value: {
                        candidates: [
            ...
            it('should emit STREAM_TEXT events with thought flag', async () => {
                ...
                const events = [];
                eventEmitter.on(AgentEventType.STREAM_TEXT, (...args) => {
                    events.push(args[0]);
                });
                const scope = await AgentHeadless.create('test-agent', config, promptConfig, defaultModelConfig, defaultRunConfig, undefined, eventEmitter);
                await scope.execute(new ContextState());
        ...
        describe('execute - Tool Restriction Enforcement (Issue #1121)', () => {
            ...
            it('should NOT execute tools that are not in the allowed tools list', async () => {
                ...
                const toolConfig = { tools: ['read_file'] };
                ...
                const eventEmitter = new AgentEventEmitter();
                eventEmitter.on(AgentEventType.TOOL_CALL, (event) => {
                    toolCallEvents.push(event);
                });
                eventEmitter.on(AgentEventType.TOOL_RESULT, (event) => {
                    toolResultEvents.push(event);
                });
                const scope = await AgentHeadless.create('test-agent', config, promptConfig, defaultModelConfig, defaultRunConfig, toolConfig, eventEmitter);
                await scope.execute(new ContextState());
            ...
            it('should mark truncated subagent write_file calls as output-truncated errors', async () => {
                ...
                const toolConfig = { tools: [WriteFileTool.Name] };
                const toolResultEvents = [];
                const eventEmitter = new AgentEventEmitter();
                eventEmitter.on(AgentEventType.TOOL_RESULT, (event) => {
                    toolResultEvents.push(event);
                ...
                const scope = await AgentHeadless.create('test-agent', config, promptConfig, defaultModelConfig, defaultRunConfig, toolConfig, eventEmitter);
                await scope.execute(new ContextState());
                const writeResult = toolResultEvents.find((event) => event.name === WriteFileTool.Name);
                expect(writeResult).toBeDefined();
                expect(writeResult.success).toBe(false);
                expect(writeResult.error).toContain('truncated due to max_tokens limit');
                expect(writeResult.error).toContain('rejected to prevent writing truncated content');
                expect(writeResult.error).not.toContain("params must have required property 'content'");
            });
            it('should NOT reject write_file when truncated attempt is followed by successful retry', async () => {
                ...
                expect(writeResult.error).not.toContain('truncated due to max_tokens limit');
                expect(writeResult.error).not.toContain('rejected to prevent writing truncated content');
            });
        });
    });
});
//# sourceMappingURL=agent-headless.test.js.map