...
export class AgentCore {
    ...
    async createChat(context, options) {
        ...
        const hasInitialMessages = !!this.promptConfig.initialMessages &&
    ...
    async prepareTools() {
        ...
        if (this.toolConfig) {
            ...
            if (hasWildcard ||
                ...
                toolsList.push(...toolRegistry.getFunctionDeclarationsFiltered(asStrings.filter((name) => !excludedFromSubagents.has(name))));
            }
    ...
    async runReasoningLoop(chat, initialMessages, toolsList, abortController, options) {
        ...
        return subagentNameContext.run(this.name, () => this._runReasoningLoopInner(chat, initialMessages, toolsList, abortController, options));
    }
    async _runReasoningLoopInner(chat, initialMessages, toolsList, abortController, options) {
        ...
        while (true) {
            ...
            const responseStream = await chat.sendMessageStream(this.modelConfig.model ||
                ...
                DEFAULT_VIVEKMIND_MODEL, messageParams, promptId);
            ...
            if (functionCalls.length > 0) {
                currentMessages = await this.processFunctionCalls(functionCalls, roundAbortController, promptId, turnCounter, toolsList, currentResponseId, wasOutputTruncated);
                ...
                if (externalMsgs.length > 0) {
                    ...
                    last.parts.push(...externalMsgs.map((text) => ({
     ...
     */
    async processFunctionCalls(functionCalls, abortController, promptId, currentRound, toolsList, responseId, wasOutputTruncated = false) {
...
Important Rules:
 ...
 - When the task is complete, return the final result as a normal model response (not a tool call) and stop.`;
        ...
        // Append user memory (VIVEKMIND.md + output-language.md) to ensure subagent respects project conventions
