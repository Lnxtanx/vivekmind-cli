...
import { getAgentJsonlPath, getAgentMetaPath, writeAgentMeta, } from './agent-transcript.js';
...
import { FORK_SUBAGENT_TYPE, buildChildMessage, } from '../tools/agent/fork-subagent.js';
describe('BackgroundAgentResumeService', () => {
    ...
    function createService() {
        const subagentManager = {
            loadSubagent: vi.fn(async (name) => name === 'researcher'
                ? {
                    name: 'researcher',
                    color: 'cyan',
                }
    ...
    it('loads only interrupted running background agents as paused entries', async () => {
        ...
        const runningMetaPath = getAgentMetaPath(tempDir, sessionId, runningAgentId);
        const completedMetaPath = getAgentMetaPath(tempDir, sessionId, completedAgentId);
        ...
        fs.writeFileSync(getAgentJsonlPath(tempDir, sessionId, runningAgentId), [
            JSON.stringify({
                uuid: 'u1',
                parentUuid: null,
                sessionId,
        ...
        fs.writeFileSync(getAgentJsonlPath(tempDir, sessionId, completedAgentId), '', 'utf8');
    ...
    it('keeps interrupted fork tasks visible as paused entries', async () => {
        ...
        fs.writeFileSync(getAgentJsonlPath(tempDir, sessionId, agentId), JSON.stringify({
            uuid: 'u1',
            parentUuid: null,
            sessionId,
            timestamp: '2026-04-20T00:00:00.000Z',
    ...
    it('fires SubagentStart hooks when resuming and injects hook context', async () => {
        ...
        const execute = vi.fn(async (_context) => undefined);
        ...
        expect(hookSystem.fireSubagentStartEvent).toHaveBeenCalledWith(agentId, 'researcher', expect.anything(), expect.any(AbortSignal));
    ...
    it('passes the sidechain transcript path to SubagentStop hooks on resume', async () => {
        ...
        expect(resumed).toBeDefined();
        await vi.waitFor(() => {
            expect(hookSystem.fireSubagentStopEvent).toHaveBeenCalledWith(agentId, 'researcher', outputFile, 'done', false, expect.anything(), expect.any(AbortSignal));
        });
    });
    it('downgrades persisted privileged approval modes when folder trust is revoked', async () => {
        ...
        service.config.isTrustedFolder = () => false;
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        service.config.getApprovalMode = () => 'default';
    ...
    it('coalesces concurrent resume calls into a single running agent', async () => {
        ...
        const provider = subagent.setExternalMessageProvider.mock.calls[0]?.[0];
    ...
    it('resumes fork agents from transcript bootstrap instead of current parent config', async () => {
        ...
        const contextArg = executeCall?.[0];
        expect(contextArg).toBeDefined();
        if (!contextArg) {
            throw new Error('Expected resume execute context');
        }
    ...
    it('keeps legacy fork tasks paused when transcript bootstrap is missing', async () => {
        ...
        expect(registry.get(agentId)?.resumeBlockedReason).toContain('bootstrap transcript is missing');
    ...
    it('keeps fork tasks paused when bootstrap capabilities are missing', async () => {
        ...
        expect(resumed).toBeUndefined();
        expect(registry.get(agentId)?.status).toBe('paused');
        expect(registry.get(agentId)?.resumeBlockedReason).toContain('runtime constraints are missing');
        expect(createSpy).not.toHaveBeenCalled();
        createSpy.mockRestore();
    });
    ...
    it('preserves pending trailing user text in history and sends continuation as the new turn', async () => {
        ...
        const execute = vi.fn(async (context) => {
            const override = context.get('initial_messages_override');
        ...
        expect(subagentManager.createAgentHeadless).toHaveBeenCalledWith(expect.anything(), expect.anything(), expect.objectContaining({
...
//# sourceMappingURL=background-agent-resume.test.js.map