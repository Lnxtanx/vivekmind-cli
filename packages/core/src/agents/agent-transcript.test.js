...
import { getSubagentSessionDir, getAgentJsonlPath, getAgentMetaPath, attachJsonlTranscriptWriter, readAgentMeta, readLastTranscriptRecordUuidSync, writeAgentMeta, } from './agent-transcript.js';
...
describe('agent-transcript', () => {
    describe('path helpers', () => {
        it('places the session dir under projectDir/subagents/<sessionId>', () => {
            expect(getSubagentSessionDir('/proj', 'sess-1')).toBe(path.join('/proj', 'subagents', 'sess-1'));
        });
        it('returns .jsonl path for the canonical transcript', () => {
            expect(getAgentJsonlPath('/proj', 'sess-1', 'agent-1')).toBe(path.join('/proj', 'subagents', 'sess-1', 'agent-agent-1.jsonl'));
        });
        it('returns .meta.json path for the sidecar', () => {
            expect(getAgentMetaPath('/proj', 'sess-1', 'agent-1')).toBe(path.join('/proj', 'subagents', 'sess-1', 'agent-agent-1.meta.json'));
        });
        it('sanitizes agentId to prevent path traversal', () => {
            const result = getAgentJsonlPath('/proj', 'sess-1', '../../../etc/passwd');
            expect(result).not.toContain('..');
            expect(result).toContain(path.join('/proj', 'subagents', 'sess-1') + path.sep);
        ...
        it('sanitizes sessionId to prevent path traversal', () => {
            ...
            expect(result).not.toContain('..');
            expect(result.startsWith(path.join('/proj', 'subagents') + path.sep)).toBe(true);
        });
        it('preserves alphanumerics, underscores, and hyphens in agentId', () => {
            expect(getAgentJsonlPath('/proj', 'sess', 'agent_1-abc')).toBe(path.join('/proj', 'subagents', 'sess', 'agent-agent_1-abc.jsonl'));
        });
    });
    describe('writeAgentMeta', () => {
        ...
        it('writes a JSON sidecar with the expected fields', () => {
            const metaPath = path.join(tempDir, 'subagents', 's1', 'agent-a.meta.json');
            const meta = {
                agentId: 'a',
                agentType: 'explore',
        ...
        it('creates parent directories that do not yet exist', () => {
            const metaPath = path.join(tempDir, 'a', 'deeply', 'nested', 'agent.meta.json');
    ...
    describe('attachJsonlTranscriptWriter (canonical)', () => {
        ...
        function makeWriter(jsonlPath, extra = {}) {
        ...
        it('writes TOOL_CALL events as assistant records with functionCall parts', () => {
            ...
            const records = readJsonl(jsonlPath);
            expect(records).toHaveLength(1);
            const part = records[0].message?.parts?.[0];
...
//# sourceMappingURL=agent-transcript.test.js.map