...
rename to src/new.ts`);
    ...
    it('includes mode-only diffs without textual line changes', () => {
        const summary = summarizeUnifiedDiff(`diff --git a/scripts/run.sh b/scripts/run.sh
old mode 100644
new mode 100755`);
        expect(summary).toEqual({
...
describe('buildFallbackApproachSummary', () => {
    it('summarizes changed files and tool usage', () => {
        ...
        expect(buildFallbackApproachSummary(result)).toBe('Changed 1 file with 3 tool calls (+2/-1).');
    });
    it('reports no changes when the diff is empty', () => {
        ...
        expect(buildFallbackApproachSummary(result)).toBe('No code changes detected.');
    });
});
//# sourceMappingURL=diff-summary.test.js.map