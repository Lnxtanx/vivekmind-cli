...
export class ArenaManager {
    ...
    buildAgentSpawnConfig(agent) {
        ...
        const spawnConfig = {
            ...
            inProcess: {
                ...
                runtimeConfig: {
                    promptConfig: {
                        systemPrompt: getCoreSystemPrompt(this.config.getUserMemory(), model.modelId),
                    },
     ...
     * `~/.vivekmind/worktrees/<sessionId>/`.  The directory contains:
    ...
    getAgentSummaryGenerator(agentId) {
        if (this.backend?.type !== DISPLAY_MODE.IN_PROCESS) {
            return undefined;
        }
        return this.backend.getAgentContentGenerator(agentId);
    }
    async generateAgentApproachSummary(summaryInput) {
        ...
        const timeout = setTimeout(() => abortController.abort(), ARENA_SUMMARY_TIMEOUT_MS);
        try {
            ...
            return (parseApproachSummaryResponse(getResponseText(response) ?? '')?.trim() ||
