...
describe('ArenaAgentClient', () => {
    ...
    beforeEach(async () => {
        ...
        vi.spyOn(uiTelemetryService, 'getMetrics').mockReturnValue(createMockMetrics());
    });
    afterEach(async () => {
        vi.restoreAllMocks();
        try {
    ...
    describe('updateStatus()', () => {
        it('should write per-agent status file with stats from telemetry', async () => {
            ...
            await reporter.init();
            vi.mocked(uiTelemetryService.getMetrics).mockReturnValue(createMockMetrics({
                totalRequests: 3,
                totalTokens: 1500,
                promptTokens: 1000,
            ...
            const statusPath = path.join(tempDir, 'agents', `${safeAgentId(agentId)}.json`);
    ...
    describe('checkControlSignal()', () => {
        ...
        it('should read and delete control file', async () => {
            ...
            const controlPath = path.join(tempDir, 'control', `${safeAgentId(agentId)}.json`);
    ...
    describe('stats aggregation and wall-clock durationMs', () => {
        it('should aggregate multi-model stats and use wall-clock durationMs', async () => {
            ...
            await reporter.init();
            await reporter.updateStatus();
            const statusPath = path.join(tempDir, 'agents', `${safeAgentId('model-a')}.json`);
            const content = JSON.parse(await fs.readFile(statusPath, 'utf-8'));
...
//# sourceMappingURL=ArenaAgentClient.test.js.map