 ...
 */
export function buildFallbackApproachSummary(result) {
    if (!isSuccessStatus(result.status)) {
        const suffix = result.error ? `: ${result.error}` : '';
        return `Did not produce an applicable result${suffix}.`;
    }
    const diffSummary = result.diffSummary ?? summarizeUnifiedDiff(result.diff ?? '');
...
export function formatLineStats(additions, deletions) {
