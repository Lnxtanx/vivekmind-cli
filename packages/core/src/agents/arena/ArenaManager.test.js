...
vi.mock('../../services/gitWorktreeService.js', () => {
    ...
    MockClass['getBaseDir'] = () => path.join(os.tmpdir(), 'arena-mock');
    MockClass['getSessionDir'] = (sessionId) => path.join(os.tmpdir(), 'arena-mock', sessionId);
    MockClass['getWorktreesDir'] = (sessionId) => path.join(os.tmpdir(), 'arena-mock', sessionId, 'worktrees');
    return { GitWorktreeService: MockClass };
});
// Mock the Config class
const createMockConfig = (workingDir, arenaSettings = {}) => ({
...
describe('ArenaManager', () => {
    ...
    beforeEach(async () => {
        ...
        hoistedMockSetupWorktrees.mockImplementation(async ({ sessionId, sourceRepoPath, worktreeNames, }) => {
            ...
            return {
                ...
                sessionId,
                worktrees,
                worktreesByName: Object.fromEntries(worktrees.map((worktree) => [worktree.name, worktree])),
                errors: [],
            };
        });
    ...
    describe('constructor', () => {
        it('should create an ArenaManager instance', () => {
            const manager = new ArenaManager(mockConfig);
    ...
    describe('start validation', () => {
        ...
        it('should reject start with more than max models', async () => {
            ...
            await expect(manager.start({
                models,
                task: 'Test task',
            })).rejects.toThrow(`Arena supports a maximum of ${ARENA_MAX_AGENTS} models`);
        });
    ...
    describe('backend initialization', () => {
        it('should emit SESSION_UPDATE with type warning when backend detection returns warning', async () => {
            ...
            expect(hoistedMockDetectBackend).toHaveBeenCalledWith(undefined, expect.anything());
        ...
        it('should emit SESSION_ERROR and mark FAILED when backend init fails', async () => {
            ...
            await expect(manager.start(createValidStartOptions())).rejects.toThrow('init failed');
...
+extra`);
            ...
            await waitForCondition(() => mockBackend.spawnAgent.mock.calls.length >= 2);
            const agentsDir = path.join(os.tmpdir(), 'arena-mock', 'testsess', 'agents');
            await fs.mkdir(agentsDir, { recursive: true });
            for (const modelId of ['model-1', 'model-2']) {
                await fs.writeFile(path.join(agentsDir, `${modelId}.json`), JSON.stringify({
                    agentId: modelId,
            ...
            expect(result.agents[0]?.approachSummary).toBe('Changed 1 file with 0 tool calls (+2/-1).');
        });
        it('uses each in-process agent generator for semantic approach summaries', async () => {
            ...
            mockBackend.getAgent.mockImplementation((agentId) => agentInteractives.get(agentId));
            mockBackend.getAgentContentGenerator.mockImplementation((agentId) => agentId === 'model-1'
                ? { generateContent: model1GenerateContent }
                : { generateContent: model2GenerateContent });
            mockBackend.spawnAgent.mockImplementation(async (config) => {
                agentInteractives.set(config.agentId, createMockInteractive(config.agentId));
            });
            ...
            expect(result.agents[0]?.approachSummary).toBe('Model 1 used a strategy pattern.');
            expect(result.agents[1]?.approachSummary).toBe('Model 2 made inline edits.');
        });
...
function createMockInteractive(agentId) {
    const emitter = {
        on: vi.fn(),
        off: vi.fn(),
    };
...
//# sourceMappingURL=ArenaManager.test.js.map