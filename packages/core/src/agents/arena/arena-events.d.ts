 ...
 */
import type { ArenaModelConfig, ArenaAgentResult, ArenaSessionResult } from './types.js';
...
export type ArenaEvent = 'session_start' | 'session_update' | 'session_complete' | 'session_error' | 'agent_start' | 'agent_status_change' | 'agent_complete' | 'agent_error';
/**
 ...
 */
export declare class ArenaEventEmitter {
    private ee;
    on<E extends keyof ArenaEventMap>(event: E, listener: (payload: ArenaEventMap[E]) => void): void;
    off<E extends keyof ArenaEventMap>(event: E, listener: (payload: ArenaEventMap[E]) => void): void;
    emit<E extends keyof ArenaEventMap>(event: E, payload: ArenaEventMap[E]): void;
    once<E extends keyof ArenaEventMap>(event: E, listener: (payload: ArenaEventMap[E]) => void): void;
