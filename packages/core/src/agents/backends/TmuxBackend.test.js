 ...
 */
async function spawnWithTimers(backend, config) {
...
describe('TmuxBackend', () => {
    ...
    it('spawns first agent outside tmux by respawning the initial pane', async () => {
        ...
        expect(hoistedTmuxRespawnPane).toHaveBeenCalledWith('%0', expect.any(String), expect.any(String));
    ...
    it('spawns first agent inside tmux by splitting from main pane', async () => {
        ...
        expect(hoistedTmuxSplitWindow).toHaveBeenCalledWith('%0', expect.objectContaining({ horizontal: true, percent: 70 }));
    ...
    it('forwardInput sends literal keys to active agent pane', async () => {
        ...
        expect(result).toBe(true);
        expect(hoistedTmuxSendKeys).toHaveBeenCalledWith('%0', 'hello', { literal: true }, expect.any(String));
    });
...
//# sourceMappingURL=TmuxBackend.test.js.map