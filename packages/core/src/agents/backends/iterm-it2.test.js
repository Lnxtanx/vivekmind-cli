...
import { isIt2Available, ensureIt2Installed, verifyITerm, itermSplitPane, itermRunCommand, itermFocusSession, itermSendText, itermCloseSession, } from './iterm-it2.js';
describe('iterm-it2', () => {
    beforeEach(() => {
        vi.clearAllMocks();
    });
    ...
    describe('ensureIt2Installed', () => {
        ...
        it('installs via uv when uv is available', async () => {
            ...
            await ensureIt2Installed();
            expect(hoistedExecCommand).toHaveBeenCalledWith('uv', ['tool', 'install', 'it2'], expect.any(Object));
        });
        it('falls back to pipx when uv is unavailable', async () => {
            ...
            await ensureIt2Installed();
            expect(hoistedExecCommand).toHaveBeenCalledWith('pipx', ['install', 'it2'], expect.any(Object));
        });
        it('falls back to pip when uv and pipx are unavailable', async () => {
            ...
            hoistedIsCommandAvailable.mockReturnValueOnce({ available: true }); // recheck
            await ensureIt2Installed();
            expect(hoistedExecCommand).toHaveBeenCalledWith('pip', ['install', '--user', 'it2'], expect.any(Object));
        });
        it('throws if no installer succeeds', async () => {
            hoistedIsCommandAvailable.mockReturnValue({ available: false });
            await expect(ensureIt2Installed()).rejects.toThrow('it2 is not installed');
        });
    });
    // ─── verifyITerm ──────────────────────────────────────────────
    ...
    describe('itermSplitPane', () => {
        it('splits vertically without session ID', async () => {
            ...
            expect(hoistedExecCommand).toHaveBeenCalledWith('it2', ['session', 'split', '-v'], expect.any(Object));
        });
        it('passes -s flag when session ID is provided', async () => {
            ...
            expect(hoistedExecCommand).toHaveBeenCalledWith('it2', ['session', 'split', '-v', '-s', 'sess-123'], expect.any(Object));
        });
    ...
    describe('itermRunCommand', () => {
        it('calls it2 session run with correct args', async () => {
            ...
            expect(hoistedExecCommand).toHaveBeenCalledWith('it2', ['session', 'run', '-s', 'sess-1', 'ls -la'], expect.any(Object));
        });
    });
    // ─── itermFocusSession ────────────────────────────────────────
    describe('itermFocusSession', () => {
        it('calls it2 session focus with correct args', async () => {
            ...
            expect(hoistedExecCommand).toHaveBeenCalledWith('it2', ['session', 'focus', 'sess-1'], expect.any(Object));
        });
    });
    // ─── itermSendText ─────────────────────────────────────────────
    describe('itermSendText', () => {
        it('calls it2 session send with correct args', async () => {
            ...
            expect(hoistedExecCommand).toHaveBeenCalledWith('it2', ['session', 'send', '-s', 'sess-1', 'hello world'], expect.any(Object));
        });
    });
    // ─── itermCloseSession ────────────────────────────────────────
    describe('itermCloseSession', () => {
        it('calls it2 session close with correct args', async () => {
            ...
            expect(hoistedExecCommand).toHaveBeenCalledWith('it2', ['session', 'close', '-s', 'sess-1'], expect.any(Object));
        });
    });
});
//# sourceMappingURL=iterm-it2.test.js.map