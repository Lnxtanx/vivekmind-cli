...
describe('ITermBackend', () => {
    ...
    it('throws if spawnAgent is called before init', async () => {
        await expect(backend.spawnAgent(makeConfig('a1'))).rejects.toThrow('not initialized');
    });
    ...
    it('init creates exit marker directory', async () => {
        await backend.init();
        expect(hoistedFsMkdir).toHaveBeenCalledWith(expect.stringContaining('agent-iterm-exit-'), { recursive: true });
    });
    ...
    it('spawns first agent using ITERM_SESSION_ID when set', async () => {
        ...
        expect(hoistedItermRunCommand).toHaveBeenCalledWith('sess-new-1', expect.any(String));
    ...
    it('rejects duplicate agent IDs', async () => {
        ...
        await expect(backend.spawnAgent(makeConfig('dup'))).rejects.toThrow('already exists');
    });
    ...
    it('rejects invalid environment variable names', async () => {
        await backend.init();
        await expect(backend.spawnAgent(makeConfig('bad-env', { env: { 'FOO BAR': 'baz' } }))).rejects.toThrow('Invalid environment variable name');
    });
    it('rejects env key starting with a digit', async () => {
        await backend.init();
        await expect(backend.spawnAgent(makeConfig('bad-env', { env: { '1VAR': 'baz' } }))).rejects.toThrow('Invalid environment variable name');
    });
    it('accepts valid environment variable names', async () => {
        await backend.init();
        await expect(backend.spawnAgent(makeConfig('good-env', {
    ...
    it('builds command with atomic exit marker write', async () => {
        ...
        const cmdArg = hoistedItermRunCommand.mock.calls[0][1];
    ...
    it('cleanup closes sessions and removes exit marker directory', async () => {
        ...
        await backend.cleanup();
        expect(hoistedItermCloseSession).toHaveBeenCalledWith('sess-1');
        expect(hoistedFsRm).toHaveBeenCalledWith(expect.stringContaining('agent-iterm-exit-'), { recursive: true, force: true });
        expect(backend.getActiveAgentId()).toBeNull();
    });
...
//# sourceMappingURL=ITermBackend.test.js.map