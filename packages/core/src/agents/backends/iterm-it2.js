...
function parseCreatedPaneId(output) {
...
export function isIt2Available() {
    return isCommandAvailable('it2').available;
}
async function tryInstallIt2(command, args) {
...
export async function ensureIt2Installed() {
    if (isIt2Available())
        return;
    const installers = [
...
export async function verifyITerm() {
    ...
    if (combined.includes('api') ||
        ...
        throw new Error('iTerm2 Python API not enabled. Enable it in iTerm2 → Settings → General → Magic → Enable Python API, then restart iTerm2.');
    }
    throw new Error(`it2 session list failed: ${result.stderr.trim() || result.stdout.trim()}`);
}
// ─── Public API ─────────────────────────────────────────────────
...
export async function itermRunCommand(sessionId, command) {
    await it2(['session', 'run', '-s', sessionId, command]);
}
export async function itermFocusSession(sessionId) {
    await it2(['session', 'focus', sessionId]);
}
export async function itermSendText(sessionId, text) {
    await it2(['session', 'send', '-s', sessionId, text]);
}
export async function itermCloseSession(sessionId) {
    await it2(['session', 'close', '-s', sessionId]);
}
//# sourceMappingURL=iterm-it2.js.map