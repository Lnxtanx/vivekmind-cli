...
export class TmuxBackend {
    ...
    async init() {
        ...
        if (this.insideTmux) {
            ...
            debugLogger.info(`Initialized inside tmux: pane ${this.mainPaneId}, window ${this.windowTarget}`);
        }
        else {
    ...
    async spawnAgentAsync(config, cmd) {
        const { agentId } = config;
        const options = this.resolveTmuxOptions(config);
        debugLogger.info(`[spawnAgentAsync] Starting spawn for agent "${agentId}", mainPane="${this.mainPaneId}", currentPanesCount=${this.panes.size}`);
        try {
            let paneId = '';
            if (this.insideTmux) {
    ...
    async cleanup() {
        ...
        if (!this.insideTmux && this.sessionName && this.serverName) {
            try {
                ...
                debugLogger.info(`Killed external tmux session "${this.sessionName}" on server "${this.serverName}"`);
            }
            catch (_error) {
    ...
    getAgentSnapshot(agentId, _scrollOffset = 0) {
    ...
    async ensureExternalSession(config, options) {
    ...
    async spawnInsideTmux(cmd, options) {
        if (!this.windowTarget) {
        ...
        if (paneCount === 1) {
            debugLogger.info(`[spawnInsideTmux] First agent — split -h -l ${options.firstSplitPercent}% from ${this.mainPaneId}`);
        ...
        debugLogger.info(`[spawnInsideTmux] Split from middle pane ${splitTarget} (${paneCount} panes, ${horizontal ? 'horizontal' : 'vertical'})`);
        return await tmuxSplitWindow(splitTarget, {
            horizontal,
            command: cmd,
        });
    }
    async spawnOutsideTmux(config, cmd, options) {
        ...
        debugLogger.info(`[spawnOutsideTmux] Split from middle pane ${splitTarget} (${panes.length} panes, ${horizontal ? 'horizontal' : 'vertical'})`);
    ...
    shouldSplitHorizontally(paneCount) {
        return paneCount % 2 === 1;
    }
    async applyPaneDecorations(paneId, options, serverName) {
        if (!this.windowTarget)
            return;
        if (options.paneBorderStatus) {
            await tmuxSetOption(this.windowTarget, 'pane-border-status', options.paneBorderStatus, serverName);
        }
        if (options.paneBorderFormat) {
            await tmuxSetOption(this.windowTarget, 'pane-border-format', options.paneBorderFormat, serverName);
        }
        if (options.paneBorderStyle) {
            await tmuxSetOption(this.windowTarget, 'pane-border-style', options.paneBorderStyle, serverName);
            await tmuxSelectPaneStyle(paneId, options.paneBorderStyle, serverName);
        }
        if (options.paneActiveBorderStyle) {
            await tmuxSetOption(this.windowTarget, 'pane-active-border-style', options.paneActiveBorderStyle, serverName);
        }
        await tmuxSelectPaneTitle(paneId, options.paneTitle, serverName);
    }
    async applyInsideLayout(options) {
    ...
    async applyExternalLayout(serverName) {
        if (!this.windowTarget)
            return;
    ...
    buildShellCommand(config) {
        ...
        debugLogger.info(`[buildShellCommand] agentId=${config.agentId}, command=${config.command}, args=${JSON.stringify(config.args)}, cwd=${config.cwd}`);
    ...
    async pollPaneStatus() {
        ...
        try {
            ...
            debugLogger.info(`[pollPaneStatus] Failed to list panes for window "${this.windowTarget}": ${err}`);
            return;
        }
        ...
        if (this.panes.size > 0) {
            debugLogger.info(`[pollPaneStatus] paneCount=${paneInfos.length}, agentPanes=${JSON.stringify(Array.from(this.panes.values()).map((p) => {
        ...
        for (const agent of this.panes.values()) {
            ...
            if (!info) {
                ...
                agent.status = 'exited';
                agent.exitCode = 1;
                debugLogger.info(`[pollPaneStatus] Agent "${agent.agentId}" pane ${agent.paneId} not found in tmux list — marking as exited`);
            ...
            if (info.dead) {
                ...
                debugLogger.info(`[pollPaneStatus] Agent "${agent.agentId}" (pane ${agent.paneId}) detected as DEAD with exit code ${info.deadStatus}`);
