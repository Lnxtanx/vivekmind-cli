...
vi.mock('../runtime/agent-core.js', () => ({
    AgentCore: vi.fn().mockImplementation(() => {
        ...
        return {
            ...
            setPendingApproval: (callId, details) => pendingApprovals.set(callId, details),
            deletePendingApproval: (callId) => pendingApprovals.delete(callId),
...
function createSpawnConfig(agentId) {
    return {
        agentId,
        command: 'node',
        args: [],
...
describe('InProcessBackend', () => {
    ...
    it('should throw when spawning without inProcess config', async () => {
        ...
        await expect(backend.spawnAgent(config)).rejects.toThrow('InProcessBackend requires inProcess config');
    });
    ...
    it('stopAgent disposes the per-agent tool registry and clears the Map entry', async () => {
        ...
        await backend.init();
        await backend.spawnAgent(createSpawnConfig('agent-1'));
        const registries = backend.agentRegistries;
    ...
    it('cleanup disposes all remaining registries (covers the in-flight shutdown path)', async () => {
        ...
        const localBackend = new InProcessBackend(config);
        ...
        const registries = localBackend.agentRegistries;
    ...
    it('should fire exit callback when agent completes', async () => {
        ...
        await vi.waitFor(() => {
            expect(exitCallback).toHaveBeenCalledWith('agent-1', expect.any(Number), null);
        });
    });
    ...
    it('should give each agent its own cwd even when sharing a backend', async () => {
        ...
        const ctx1 = calls.at(-2)[1];
        const ctx2 = calls.at(-1)[1];
    ...
    it('should throw when spawning a duplicate agent ID', async () => {
        await backend.init();
        await backend.spawnAgent(createSpawnConfig('agent-1'));
        await expect(backend.spawnAgent(createSpawnConfig('agent-1'))).rejects.toThrow('Agent "agent-1" already exists.');
    });
    it('should fire exit callback with code 1 when start() throws', async () => {
        ...
        await expect(backend.spawnAgent(createSpawnConfig('agent-fail'))).resolves.toBeUndefined();
    ...
    describe('auth isolation', () => {
        it('should create per-agent ContentGenerator when authOverrides is provided', async () => {
            ...
            await backend.spawnAgent(config);
            const mockCreate = createContentGenerator;
        ...
        it('should expose inherited ContentGenerator without authOverrides', async () => {
            ...
            expect(backend.getAgentContentGenerator('agent-1')).toBe(mockContentGenerator);
        });
...
//# sourceMappingURL=InProcessBackend.test.js.map