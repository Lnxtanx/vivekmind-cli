...
// ─── Helpers ────────────────────────────────────────────────────
async function tmuxResult(args, serverName) {
...
async function tmux(args, serverName) {
    const result = await tmuxResult(args, serverName);
    if (result.code !== 0) {
        throw new Error(`tmux ${args[0]} failed (exit ${result.code}): ${result.stderr.trim() || result.stdout.trim()}`);
    }
...
export async function verifyTmux() {
    ...
    if (!isVersionAtLeast(version, MIN_TMUX_VERSION)) {
        throw new Error(`tmux version ${MIN_TMUX_VERSION}+ required for split-pane mode (found: ${version}).`);
    }
}
/**
 * Get the current tmux session name (when running inside tmux).
 */
export async function tmuxCurrentSession() {
 ...
 */
export async function tmuxCurrentPaneId() {
 ...
 */
export async function tmuxCurrentWindowTarget() {
 ...
 */
export async function tmuxHasSession(name, serverName) {
 ...
 */
export async function tmuxListWindows(sessionName, serverName) {
    const output = await tmux(['list-windows', '-t', sessionName, '-F', '#{window_name} #{window_id}'], serverName);
    const windows = [];
 ...
 */
export async function tmuxHasWindow(sessionName, windowName, serverName) {
 ...
 */
export async function tmuxNewSession(name, opts, serverName) {
 ...
 */
export async function tmuxNewWindow(targetSession, windowName, serverName) {
 ...
 */
export async function tmuxSplitWindow(target, opts, serverName) {
 ...
 */
export async function tmuxSendKeys(paneId, keys, opts, serverName) {
 ...
 */
export async function tmuxSelectPane(paneId, serverName) {
    await tmux(['select-pane', '-t', paneId], serverName);
}
/**
 * Set a pane title.
 */
export async function tmuxSelectPaneTitle(paneId, title, serverName) {
    await tmux(['select-pane', '-t', paneId, '-T', title], serverName);
}
/**
 * Set a pane border style via select-pane -P.
 */
export async function tmuxSelectPaneStyle(paneId, style, serverName) {
 ...
 */
export async function tmuxSelectLayout(target, layout, serverName) {
 ...
 */
export async function tmuxCapturePaneContent(paneId, serverName) {
 ...
 */
export async function tmuxListPanes(target, serverName) {
    const output = await tmux([
        'list-panes',
        '-t',
        target,
 ...
 */
export async function tmuxSetOption(target, option, value, serverName) {
 ...
 */
export async function tmuxResizePane(paneId, opts, serverName) {
 ...
 * Kill a tmux session.
 */
export async function tmuxKillSession(name, serverName) {
    await tmux(['kill-session', '-t', name], serverName);
}
/**
 * Kill a tmux window.
 */
export async function tmuxKillWindow(target, serverName) {
    await tmux(['kill-window', '-t', target], serverName);
}
/**
 * Get the first pane ID of a target window.
 */
export async function tmuxGetFirstPaneId(target, serverName) {
...
//# sourceMappingURL=tmux-commands.js.map