...
import { buildAgentContentGeneratorConfig, resolveCredentialField, } from './content-generator-config.js';
function createMockConfig(parentConfig, resolvedModel) {
    return {
...
describe('buildAgentContentGeneratorConfig', () => {
    ...
    describe('with registry-resolved model', () => {
        ...
        it('should apply registry generation config over cleared parent config', () => {
            ...
            const result = buildAgentContentGeneratorConfig(config, 'registry-model-id', { authType: 'anthropic' });
        ...
        it('should prefer explicit auth overrides over registry values', () => {
            const config = createMockConfig(parentConfig, resolvedModel);
            const result = buildAgentContentGeneratorConfig(config, 'registry-model-id', {
                authType: 'anthropic',
                apiKey: 'explicit-key',
                baseUrl: 'https://explicit.example.com',
            });
...
describe('resolveCredentialField', () => {
    ...
    it('should prefer explicit value', () => {
        expect(resolveCredentialField('explicit', 'inherited', 'openai', 'apiKey')).toBe('explicit');
    });
    it('should fall back to inherited value', () => {
        expect(resolveCredentialField(undefined, 'inherited', 'openai', 'apiKey')).toBe('inherited');
    });
    it('should fall back to env var', () => {
        vi.stubEnv('OPENAI_API_KEY', 'env-key');
        expect(resolveCredentialField(undefined, undefined, 'openai', 'apiKey')).toBe('env-key');
    });
    it('should return undefined when nothing matches', () => {
        expect(resolveCredentialField(undefined, undefined, 'unknown', 'apiKey')).toBeUndefined();
    });
});
//# sourceMappingURL=content-generator-config.test.js.map