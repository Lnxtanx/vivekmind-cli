...
describe('ModelRegistry', () => {
  describe('initialization', () => {
    ...
    it('should initialize with empty config', () => {
      const registry = new ModelRegistry();
      expect(registry.getModelsForAuthType(AuthType.VIVEKMIND_OAUTH).length).toBe(
        VIVEKMIND_OAUTH_MODELS.length,
      );
  ...
  describe('reloadModels', () => {
    ...
    it('should preserve hard-coded vivekmind-oauth models after reload', () => {
      ...
      expect(
        registry.getModel(AuthType.VIVEKMIND_OAUTH, 'coder-model'),
      ).toBeDefined();
    });
