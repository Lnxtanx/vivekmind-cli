...
import { ModelRegistry, VIVEKMIND_OAUTH_MODELS } from './modelRegistry.js';
...
describe('ModelRegistry', () => {
    describe('initialization', () => {
        ...
        it('should initialize with empty config', () => {
            const registry = new ModelRegistry();
            expect(registry.getModelsForAuthType(AuthType.VIVEKMIND_OAUTH).length).toBe(VIVEKMIND_OAUTH_MODELS.length);
        ...
        it('should ignore vivekmind-oauth models in config (hard-coded)', () => {
            const modelProvidersConfig = {
                'vivekmind-oauth': [
                    {
                        id: 'custom-qwen',
    ...
    describe('hasModel', () => {
        ...
        it('should return false for non-existent model', () => {
            expect(registry.hasModel(AuthType.USE_OPENAI, 'non-existent')).toBe(false);
        });
    ...
    describe('getDefaultModelForAuthType', () => {
        it('should return coder-model for vivekmind-oauth', () => {
            const registry = new ModelRegistry();
            const defaultModel = registry.getDefaultModelForAuthType(AuthType.VIVEKMIND_OAUTH);
        ...
        it('should return first model for other authTypes', () => {
            ...
            const defaultModel = registry.getDefaultModelForAuthType(AuthType.USE_OPENAI);
    ...
    describe('default base URLs', () => {
        it('should apply default dashscope URL for vivekmind-oauth', () => {
            const registry = new ModelRegistry();
            const model = registry.getModel(AuthType.VIVEKMIND_OAUTH, 'coder-model');
            expect(model?.baseUrl).toBe('DYNAMIC_VIVEKMIND_OAUTH_BASE_URL');
        });
    ...
    describe('duplicate model id handling', () => {
        ...
        it('should handle multiple duplicate ids in same authType', () => {
            ...
            expect(registry.getModel(AuthType.USE_OPENAI, 'model-a')?.name).toBe('Model A First');
            expect(registry.getModel(AuthType.USE_OPENAI, 'model-b')?.name).toBe('Model B First');
            expect(registry.getModel(AuthType.USE_OPENAI, 'model-c')?.name).toBe('Model C');
        });
        it('should treat same id in different authTypes as different models', () => {
            ...
            const openaiModel = registry.getModel(AuthType.USE_OPENAI, 'shared-model');
            const geminiModel = registry.getModel(AuthType.USE_GEMINI, 'shared-model');
    ...
    describe('reloadModels', () => {
        ...
        it('should preserve hard-coded vivekmind-oauth models after reload', () => {
            const registry = new ModelRegistry({
                openai: [{ id: 'gpt-4', name: 'GPT-4' }],
            });
            expect(registry.getModelsForAuthType(AuthType.VIVEKMIND_OAUTH).length).toBe(VIVEKMIND_OAUTH_MODELS.length);
            ...
            expect(registry.getModelsForAuthType(AuthType.VIVEKMIND_OAUTH).length).toBe(VIVEKMIND_OAUTH_MODELS.length);
            expect(registry.getModel(AuthType.VIVEKMIND_OAUTH, 'coder-model')).toBeDefined();
        });
        ...
        it('should handle reload with multiple authTypes', () => {
            ...
            expect(registry.getModel(AuthType.USE_OPENAI, 'gpt-4')?.name).toBe('GPT-4 Updated');
            const geminiModels = registry.getModelsForAuthType(AuthType.USE_GEMINI);
            expect(geminiModels.length).toBe(1);
        });
