...
import { AUTH_ENV_MAPPINGS, DEFAULT_MODELS, VIVEKMIND_OAUTH_ALLOWED_MODELS, MODEL_GENERATION_CONFIG_FIELDS, } from './constants.js';
export { validateModelConfig, } from '../core/contentGenerator.js';
/**
 * Applies VIVEKMIND_API_TIMEOUT_MS env override if modelProvider has not set a timeout.
...
function applyTimeoutEnvOverride(env, generationConfig, sources, modelProvider) {
    if (modelProvider?.generationConfig?.timeout !== undefined)
        return;
    const raw = env['VIVEKMIND_API_TIMEOUT_MS'] || env['VIVEKMIND_CODE_API_TIMEOUT_MS'];
    if (raw === undefined)
        return;
    ...
    if (Number.isFinite(parsed) && parsed > 0) {
        ...
        sources['timeout'] = {
            kind: 'env',
            envKey: env['VIVEKMIND_API_TIMEOUT_MS'] ? 'VIVEKMIND_API_TIMEOUT_MS' : 'VIVEKMIND_CODE_API_TIMEOUT_MS',
        };
    }
}
/**
...
export function resolveModelConfig(input) {
    ...
    // ---- API Key ----
    const apiKeyLayers = [];
    // 1. Highest priority: API key stored explicitly in settings.json
    if (settings?.apiKey) {
        apiKeyLayers.push(layer(settings.apiKey, settingsSource('security.auth.apiKey')));
    }
    // 2. Next: API key from modelProvider's specific envKey (handles both standard and generated names)
    ...
    // 4. Finally: Fallback to standard environment variables for the provider
    ...
    // ---- Env override: VIVEKMIND_CODE_API_TIMEOUT_MS ----
...
function resolveQwenOAuthConfig(input, warnings) {
    ...
    const sources = {};
    // VivekMind OAuth only allows specific models
    ...
    if (requestedModel && allowedModels.has(requestedModel)) {
        ...
        if (requestedModel) {
            ...
            const extraMessage = isVisionModel
                ...
                : '';
            warnings.push(`Warning: Unsupported VivekMind OAuth model '${requestedModel}', falling back to '${DEFAULT_VIVEKMIND_MODEL}'.${extraMessage}`);
        }
        resolvedModel = DEFAULT_VIVEKMIND_MODEL;
        modelSource = defaultSource(`fallback to '${DEFAULT_VIVEKMIND_MODEL}'`);
    }
    sources['model'] = modelSource;
    sources['apiKey'] = computedSource('VivekMind OAuth dynamic token');
    sources['authType'] = computedSource('provided by caller');
    if (proxy) {
        sources['proxy'] = computedSource('Config.getProxy()');
    }
    // Resolve generation config from settings and modelProvider
    const generationConfig = resolveGenerationConfig(settings?.generationConfig, modelProvider?.generationConfig, AuthType.VIVEKMIND_OAUTH, resolvedModel, sources);
    // ---- Env override: VIVEKMIND_CODE_API_TIMEOUT_MS ----
