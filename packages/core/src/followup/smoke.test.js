...
import { shouldFilterSuggestion, getFilterReason, } from './suggestionGenerator.js';
import { OverlayFs } from './overlayFs.js';
import { evaluateToolCall, rewritePathArgs } from './speculationToolGate.js';
import { saveCacheSafeParams, getCacheSafeParams, clearCacheSafeParams, } from '../utils/forkedAgent.js';
...
describe('SMOKE TESTS — E2E Verification', () => {
    ...
    describe('Smoke 3: ToolGate → OverlayFs integration', () => {
        it('write redirects to overlay, read resolves from overlay', async () => {
            ...
            const wr = await evaluateToolCall(ToolNames.EDIT, { file_path: filePath }, overlay, ApprovalMode.AUTO_EDIT);
