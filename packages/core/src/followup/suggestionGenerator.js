...
import { uiTelemetryService, EVENT_API_RESPONSE, } from '../telemetry/uiTelemetry.js';
...
export async function generatePromptSuggestion(config, conversationHistory, abortSignal, options) {
    ...
    try {
        ...
        debugLogger.debug(`Generating suggestion: cacheSharing=${!!cacheSafe}, model=${modelOverride || '(default)'}`);
        const raw = cacheSafe
...
async function generateViaForkedQuery(config, abortSignal, modelOverride) {
 ...
 */
function reportSuggestionUsage(model, usage, durationMs) {
    const event = new ApiResponseEvent('suggestion-' + Date.now(), model, durationMs, 'prompt_suggestion', undefined, {
