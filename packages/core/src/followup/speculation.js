...
import { getCacheSafeParams, createForkedChat, runForkedAgent, } from '../utils/forkedAgent.js';
...
async function runSpeculativeLoop(config, state, cacheSafe, modelOverride) {
    ...
    for (let turn = 0; turn < MAX_SPECULATION_TURNS; turn++) {
        ...
        for (const part of functionCalls) {
            ...
            const name = fc.name ?? '';
            const args = (fc.args ?? {});
            const gate = await evaluateToolCall(name, args, state.overlayFs, approvalMode);
            ...
            try {
                ...
                state.toolUseCount++;
                const responseContent = typeof result.llmContent === 'string'
