...
import { INITIAL_FOLLOWUP_STATE, createFollowupController, } from './followupState.js';
describe('createFollowupController', () => {
    ...
    it('accept recovers when onAccept callback throws', async () => {
        ...
        ctrl.accept();
        await Promise.resolve();
        expect(consoleErrorSpy).toHaveBeenCalledWith('[followup] onAccept callback threw:', expect.any(Error));
    ...
    it('accept with skipOnAccept skips onAccept callback but still clears state and fires telemetry', async () => {
        ...
        expect(onOutcome).toHaveBeenCalledWith(expect.objectContaining({ outcome: 'accepted', accept_method: 'enter' }));
...
//# sourceMappingURL=followupState.test.js.map