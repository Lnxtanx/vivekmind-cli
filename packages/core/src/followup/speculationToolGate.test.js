...
describe('speculationToolGate', () => {
    ...
    describe('SAFE_READ_ONLY_TOOLS', () => {
        it.each([
            ...
            const result = await evaluateToolCall(toolName, {}, overlayFs, ApprovalMode.DEFAULT);
            expect(result.action).toBe('allow');
        });
    });
    describe('WRITE_TOOLS', () => {
        it('redirects edit in auto-edit mode', async () => {
            const result = await evaluateToolCall(ToolNames.EDIT, {}, overlayFs, ApprovalMode.AUTO_EDIT);
            expect(result.action).toBe('redirect');
        });
        it('redirects write_file in yolo mode', async () => {
            const result = await evaluateToolCall(ToolNames.WRITE_FILE, {}, overlayFs, ApprovalMode.YOLO);
            expect(result.action).toBe('redirect');
        });
        it('hits boundary for edit in default mode', async () => {
            const result = await evaluateToolCall(ToolNames.EDIT, {}, overlayFs, ApprovalMode.DEFAULT);
            expect(result.action).toBe('boundary');
        });
        it('hits boundary for write_file in plan mode', async () => {
            const result = await evaluateToolCall(ToolNames.WRITE_FILE, {}, overlayFs, ApprovalMode.PLAN);
            expect(result.action).toBe('boundary');
        });
    });
    describe('SHELL', () => {
        it('allows read-only shell commands', async () => {
            const result = await evaluateToolCall(ToolNames.SHELL, { command: 'ls -la' }, overlayFs, ApprovalMode.DEFAULT);
            expect(result.action).toBe('allow');
        });
        it('hits boundary for non-read-only shell commands', async () => {
            const result = await evaluateToolCall(ToolNames.SHELL, { command: 'rm -rf /' }, overlayFs, ApprovalMode.DEFAULT);
            expect(result.action).toBe('boundary');
        });
        it('hits boundary for empty command', async () => {
            const result = await evaluateToolCall(ToolNames.SHELL, { command: '' }, overlayFs, ApprovalMode.DEFAULT);
            expect(result.action).toBe('boundary');
        });
    });
    ...
    describe('unknown tools', () => {
        it('hits boundary for unknown tool names', async () => {
            const result = await evaluateToolCall('mcp_custom_tool', {}, overlayFs, ApprovalMode.DEFAULT);
            expect(result.action).toBe('boundary');
    ...
    describe('read path resolution through overlay', () => {
        ...
        it('does not resolve read path when file was not written to overlay', async () => {
            ...
            await writeFile(filePath, 'content');
            const args = { file_path: filePath };
            await evaluateToolCall(ToolNames.READ_FILE, args, overlayFs, ApprovalMode.DEFAULT);
            // Path should remain unchanged
            expect(args['file_path']).toBe(filePath);
        });
    });
});
//# sourceMappingURL=speculationToolGate.test.js.map