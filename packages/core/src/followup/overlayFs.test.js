...
describe('OverlayFs', () => {
    ...
    describe('redirectWrite', () => {
        ...
        it('throws for paths outside cwd', async () => {
            await expect(overlay.redirectWrite('/etc/passwd')).rejects.toThrow('Cannot redirect write outside cwd');
        });
        it('throws for path traversal attempts', async () => {
            await expect(overlay.redirectWrite(join(testDir, '..', '..', 'etc', 'passwd'))).rejects.toThrow('Cannot redirect write outside cwd');
        });
    });
...
//# sourceMappingURL=overlayFs.test.js.map