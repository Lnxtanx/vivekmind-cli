...
import { SkillManager, watcherIgnored, WATCHER_MAX_DEPTH, } from './skill-manager.js';
...
describe('SkillManager', () => {
    ...
    beforeEach(() => {
        ...
        mockParseYaml.mockImplementation((yamlString) => {
            ...
            if (/^paths:/m.test(yamlString)) {
                ...
                const result = { name, description, paths };
            ...
            if (yamlString.includes('name: shared-skill')) {
                const desc = yamlString.includes('From vivekmind dir')
                    ? 'From vivekmind dir'
    ...
    const validSkillConfig = {
        ...
        description: 'A test skill',
        level: 'project',
        filePath: '/test/project/.vivekmind/skills/test-skill/SKILL.md',
...
`;
            const config = manager.parseSkillContent(markdownCrlf, validSkillConfig.filePath, 'project');
            expect(config.name).toBe('test-skill');
            expect(config.description).toBe('A test skill');
            expect(config.body).toBe('You are a helpful assistant with this skill.');
        });
...
You are a helpful assistant with this skill.
`;
            const config = manager.parseSkillContent(markdownWithBom, validSkillConfig.filePath, 'project');
            expect(config.name).toBe('test-skill');
            expect(config.description).toBe('A test skill');
        });
...
description: A test skill
---`;
            const config = manager.parseSkillContent(frontmatterOnly, validSkillConfig.filePath, 'project');
            expect(config.name).toBe('test-skill');
            expect(config.description).toBe('A test skill');
...
Skill body.
`;
            const config = manager.parseSkillContent(markdownWithArgumentHint, validSkillConfig.filePath, 'project');
...
Body.
`;
            const config = manager.parseSkillContent(markdown, validSkillConfig.filePath, 'project');
...
Body.
`;
            expect(() => manager.parseSkillContent(markdown, validSkillConfig.filePath, 'project')).toThrow(/"paths" must be an array/);
        });
        it('should determine level from file path', () => {
            const projectPath = '/test/project/.vivekmind/skills/test-skill/SKILL.md';
            const userPath = '/home/user/.vivekmind/skills/test-skill/SKILL.md';
...
`;
    ...
    describe('listSkills', () => {
        beforeEach(() => {
            ...
            const projectVivekMindSkillsDir = path.join('/test/project', '.vivekmind', 'skills');
            const userVivekMindSkillsDir = path.join('/home/user', '.vivekmind', 'skills');
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            vi.mocked(fs.readdir).mockImplementation((dirPath) => {
                const pathStr = String(dirPath);
                if (pathStr === projectVivekMindSkillsDir) {
                ...
                if (pathStr === userVivekMindSkillsDir) {
                    return Promise.resolve([
                        {
                            name: 'skill3',
                            isDirectory: () => true,
...
Skill 3 content`);
        ...
        it('should deduplicate same-name skills across provider dirs within a level', async () => {
            // Override readdir to return the same skill name from both .vivekmind and .agents dirs
            vi.mocked(fs.readdir).mockReset();
            const projectVivekMindDir = path.join('/test/project', '.vivekmind', 'skills');
            const projectAgentDir = path.join('/test/project', '.agents', 'skills');
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            vi.mocked(fs.readdir).mockImplementation((dirPath) => {
                const pathStr = String(dirPath);
                if (pathStr === projectVivekMindDir) {
                    return Promise.resolve([
                        {
                            name: 'shared-skill',
                            isDirectory: () => true,
            ...
            vi.mocked(fs.readFile).mockImplementation((filePath) => {
                const pathStr = String(filePath);
                if (pathStr.includes('.vivekmind') && pathStr.includes('shared-skill')) {
                    return Promise.resolve(`---\nname: shared-skill\ndescription: From vivekmind dir\n---\nVivekMind content`);
                }
            ...
            // Only one instance should remain, from .vivekmind (first in PROVIDER_CONFIG_DIRS)
            expect(skills).toHaveLength(1);
            expect(skills[0].name).toBe('shared-skill');
            expect(skills[0].description).toBe('From vivekmind dir');
        });
    ...
    describe('getSkillsBaseDirs', () => {
        it('should return all project-level base dirs', () => {
            ...
            expect(baseDirs).toContain(path.join('/test/project', '.vivekmind', 'skills'));
            expect(baseDirs).toContain(path.join('/test/project', '.agents', 'skills'));
        });
        it('should return all user-level base dirs', () => {
            const baseDirs = manager.getSkillsBaseDirs('user');
            expect(baseDirs).toHaveLength(2);
            expect(baseDirs).toContain(path.join('/home/user', '.vivekmind', 'skills'));
        ...
        it('should throw for extension level', () => {
            expect(() => manager.getSkillsBaseDirs('extension')).toThrow('Extension skills do not have a base directory');
        });
    });
    describe('bundled skills', () => {
        const bundledDirSegment = path.join('skills', 'bundled');
        const projectDirSegment = path.join('.vivekmind', 'skills');
        const userDirSegment = path.join('.vivekmind', 'skills');
        ...
        const emptyDir = [];
        function mockReaddirForLevels(levels) {
            vi.mocked(fs.readdir).mockImplementation((dirPath) => {
                const pathStr = String(dirPath);
                const isBundled = pathStr.endsWith(bundledDirSegment) && !pathStr.includes('.vivekmind');
                const isProject = pathStr.includes(projectDirSegment) &&
                    pathStr.startsWith(projectPrefix);
                const isUser = pathStr.includes(userDirSegment) && pathStr.startsWith(userPrefix);
                if ((levels.has('bundled') && isBundled) ||
                    (levels.has('project') && isProject) ||
                    (levels.has('user') && isUser)) {
                    return Promise.resolve([reviewDirEntry]);
                }
...
Review content`);
    ...
    describe('change listeners', () => {
        ...
        it('awaits async listeners before resolving', async () => {
            ...
            let resolveListener = () => { };
            const listenerSettled = new Promise((resolve) => {
                resolveListener = resolve;
            });
            let listenerObserved = false;
            manager.addChangeListener(() => listenerSettled.then(() => {
...
`);
        ...
        it('activates a conditional skill when a matching file path is touched', async () => {
            await loadConditionalFixture();
            const newly = await manager.matchAndActivateByPath('/test/project/src/App.tsx');
        ...
        it('does not re-notify listeners on subsequent matches of the same skill', async () => {
            ...
            const listener = vi.fn();
            manager.addChangeListener(listener);
            expect(await manager.matchAndActivateByPath('/test/project/src/A.tsx')).toEqual(['tsx-helper']);
            expect(listener).toHaveBeenCalledTimes(1);
            // Same pattern touched again — skill already active, no new
            // notification.
            expect(await manager.matchAndActivateByPath('/test/project/src/B.tsx')).toEqual([]);
            expect(listener).toHaveBeenCalledTimes(1);
        });
        it('does nothing for paths outside the project root', async () => {
            await loadConditionalFixture();
            expect(await manager.matchAndActivateByPath('/other/place/foo.tsx')).toEqual([]);
...
`);
            await manager.refreshCache();
            const newly = await manager.matchAndActivateByPath('/test/project/src/foo.ts');
...
`);
        ...
        it('does not activate a visible skill from a shadowed copy paths', async () => {
            ...
            vi.mocked(fs.readdir).mockImplementation((dirPath) => {
                const pathStr = String(dirPath);
                if (pathStr === projectVivekMindSkillsDir || pathStr === userVivekMindSkillsDir) {
                    return Promise.resolve([
                        {
                            name: 'foo',
                            isDirectory: () => true,
            ...
            vi.mocked(fs.readFile).mockImplementation((filePath) => {
                const pathStr = String(filePath);
                if (pathStr.startsWith(projectVivekMindSkillsDir)) {
                    return Promise.resolve(`---
name: foo
description: A test skill
paths:
...
`);
                ...
                if (pathStr.startsWith(userVivekMindSkillsDir)) {
                    return Promise.resolve(`---
name: foo
description: A test skill
paths:
...
`);
            ...
            expect(await manager.matchAndActivateByPath('/test/project/lib/x.ts')).toEqual([]);
            expect(manager.getActivatedSkillNames().has('foo')).toBe(false);
            // Touching `src/x.ts` (matches the visible project-foo's paths) does
            // activate it.
            expect(await manager.matchAndActivateByPath('/test/project/src/x.ts')).toEqual(['foo']);
        });
    });
...
Symlinked skill content`);
    ...
    describe('file watchers', () => {
        it('should pass ignored function and shallow depth to chokidar', async () => {
            const projectSkillsDir = path.join('/test/project', '.vivekmind', 'skills');
            vi.mocked(fsSync.existsSync).mockImplementation((p) => String(p) === projectSkillsDir);
            vi.mocked(fs.readdir).mockResolvedValue([]);
            mockWatch.mockClear();
        ...
        it('watcherIgnored should reject .git directories', () => {
            ...
            expect(watcherIgnored(path.join('/skills', 'my-skill', 'SKILL.md'))).toBe(false);
        });
...
---
Skill content`;
            const config = manager.parseSkillContent(markdown, '/test/skill/SKILL.md', 'user');
