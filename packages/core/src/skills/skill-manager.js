...
import { SkillError, SkillErrorCode, parseModelField, parsePathsField, validateSkillName, } from './types.js';
...
export class SkillManager {
    ...
    constructor(config) {
        this.config = config;
        this.bundledSkillsDir = path.join(path.dirname(fileURLToPath(import.meta.url)), 'bundled');
    }
    /**
    ...
    parseSkillContent(content, filePath, level) {
        try {
            ...
            if (frontmatterYaml.includes('hooks:')) {
                ...
                const fullFrontmatter = yaml.parse(frontmatterYaml);
                const hooksRaw = fullFrontmatter['hooks'];
    ...
    getSkillsBaseDirs(level) {
        switch (level) {
            ...
            case 'user':
                return SKILL_PROVIDER_CONFIG_DIRS.map((v) => path.join(os.homedir(), v, SKILLS_CONFIG_DIR));
            case 'bundled':
    ...
    async listSkillsAtLevel(level) {
        ...
        // so the order defines implicit precedence (.vivekmind > .agent > .cursor > ...).
    ...
    async loadSkillsFromDir(baseDir, level) {
        ...
        try {
            ...
            try {
                ...
                const errorMessage = error instanceof Error ? error.message : 'Unknown error';
                debugLogger.debug(`Cannot realpath skills baseDir ${baseDir}: ${errorMessage}`);
                return [];
            }
                ...
                if (isSymlink) {
                    ...
                    if (!check.ok) {
                        if (check.reason === 'escapes') {
                            debugLogger.warn(`Skipping symlink ${entry.name} that escapes ${baseDir}`);
                        }
