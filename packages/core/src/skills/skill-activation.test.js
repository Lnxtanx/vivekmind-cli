...
import { SkillActivationRegistry, resolveProjectRelativePath, splitConditionalSkills, } from './skill-activation.js';
...
describe('SkillActivationRegistry', () => {
    ...
    it('activates a conditional skill when a matching path is touched', () => {
        const reg = new SkillActivationRegistry([makeSkill({ name: 'tsx-helper', paths: ['src/**/*.tsx'] })], projectRoot);
    ...
    it('activates broad globs on dotfiles too (dot: true semantics)', () => {
        ...
        const reg = new SkillActivationRegistry([makeSkill({ name: 'lint-helper', paths: ['**/*.js'] })], projectRoot);
    ...
    it('rejects an absolute relative path (Windows cross-drive case)', () => {
        ...
        // normalization.
        const reg = new SkillActivationRegistry([makeSkill({ name: 'broad', paths: ['**/*.ts'] })], projectRoot);
...
describe('resolveProjectRelativePath', () => {
    ...
    it('returns the forward-slash-normalized relative path for in-project files (POSIX)', () => {
        expect(resolveProjectRelativePath('/project/src/App.tsx', '/project', path.posix)).toBe('src/App.tsx');
    });
    it('returns null for paths outside the project root (POSIX, `..` prefix)', () => {
        expect(resolveProjectRelativePath('/elsewhere/foo.ts', '/project', path.posix)).toBeNull();
    });
    it('returns null for Windows cross-drive paths (different drive letter)', () => {
        ...
        expect(resolveProjectRelativePath('D:\\other\\file.ts', 'C:\\project', path.win32)).toBeNull();
    });
    it('normalizes backslashes for in-project Windows paths', () => {
        expect(resolveProjectRelativePath('C:\\project\\src\\App.tsx', 'C:\\project', path.win32)).toBe('src/App.tsx');
    });
});
describe('extractToolFilePaths → SkillActivationRegistry integration', () => {
    ...
    it('activates a skill keyed on src/**/*.ts from glob({ path: "src", pattern: "**/*.ts" })', async () => {
        const { extractToolFilePaths } = await import('../core/coreToolScheduler.js');
        const candidates = extractToolFilePaths('glob', {
            path: 'src',
            pattern: '**/*.ts',
        });
        const reg = new SkillActivationRegistry([makeSkill({ name: 'tsx-helper', paths: ['src/**/*.ts'] })], '/project');
        // Hand each candidate the registry the way coreToolScheduler does;
        // collect the union.
        const activated = new Set();
        for (const c of candidates) {
            for (const n of reg.matchAndConsume(c))
                activated.add(n);
        }
    ...
    it('does NOT activate from external glob.path (project-root guard wins)', async () => {
        ...
        const reg = new SkillActivationRegistry([makeSkill({ name: 'broad', paths: ['**/*.ts'] })], '/project');
        const activated = new Set();
        for (const c of candidates) {
            for (const n of reg.matchAndConsume(c))
                activated.add(n);
...
//# sourceMappingURL=skill-activation.test.js.map