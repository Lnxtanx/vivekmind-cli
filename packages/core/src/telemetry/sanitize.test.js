...
describe('sanitizeHookName', () => {
    ...
    it('should extract command name from full path on Unix systems', () => {
        ...
        expect(sanitizeHookName('/path/to/.gemini/hooks/check-secrets.sh')).toBe('check-secrets.sh');
        expect(sanitizeHookName('/home/user/script.py --arg=value')).toBe('script.py');
    });
    it('should extract command name from full path on Windows systems', () => {
        ...
        expect(sanitizeHookName('C:\\Users\\User\\Documents\\test.bat /c')).toBe('test.bat');
    });
    it('should return the command name without arguments for simple commands', () => {
        ...
        expect(sanitizeHookName('python script.py --api-key=abc123')).toBe('python');
    });
    ...
    it('should handle complex command lines', () => {
        expect(sanitizeHookName('/path/to/.gemini/hooks/check-secrets.sh --api-key=abc123')).toBe('check-secrets.sh');
        expect(sanitizeHookName('python /home/user/script.py --token=xyz --verbose')).toBe('python');
    });
