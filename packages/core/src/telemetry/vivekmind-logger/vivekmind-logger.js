...
export class VivekMindLogger {
    ...
    constructor(config) {
        this.config = config;
        this.debugLogger = createDebugLogger('VIVEKMIND_LOGGER');
    ...
    static getInstance(config) {
        if (config === undefined || !config?.getUsageStatisticsEnabled())
            return undefined;
        if (!VivekMindLogger.instance) {
            VivekMindLogger.instance = new VivekMindLogger(config);
        }
        return VivekMindLogger.instance;
    }
    enqueueLogEvent(event) {
        try {
            ...
            if (wasAtCapacity) {
                this.debugLogger.debug(`VivekMindLogger: Dropped old event to prevent memory leak (queue size: ${this.events.size})`);
            }
        }
        catch (error) {
            this.debugLogger.error('VivekMindLogger: Failed to enqueue log event.', error);
        }
    }
    ...
    async flushToRum() {
        ...
        if (this.isFlushInProgress) {
            this.debugLogger.debug('VivekMindLogger: Flush already in progress, marking pending flush.');
    ...
    requeueFailedEvents(eventsToSend) {
        ...
        if (eventsToSend.length > MAX_RETRY_EVENTS) {
            this.debugLogger.warn(`VivekMindLogger: Dropping ${eventsToSend.length - MAX_RETRY_EVENTS} events due to retry queue limit. Total events: ${eventsToSend.length}, keeping: ${MAX_RETRY_EVENTS}`);
        }
        ...
        this.debugLogger.debug(`VivekMindLogger: Re-queued ${numEventsToRequeue} events for retry (queue size: ${this.events.size})`);
    }
}
...
//# sourceMappingURL=vivekmind-logger.js.map