...
export class VivekMindLogger {
  private static instance: VivekMindLogger;
  ...
  static getInstance(config?: Config): VivekMindLogger | undefined {
    ...
    try {
      ...
      if (wasAtCapacity) {
        this.debugLogger.debug(
          `VivekMindLogger: Dropped old event to prevent memory leak (queue size: ${this.events.size})`,
        );
      }
    } catch (error) {
  ...
  async flushToRum(): Promise<LogResponse> {
    ...
    if (this.isFlushInProgress) {
      this.debugLogger.debug(
        'VivekMindLogger: Flush already in progress, marking pending flush.',
      );
  ...
  private requeueFailedEvents(eventsToSend: RumEvent[]): void {
    ...
    this.debugLogger.debug(
      `VivekMindLogger: Re-queued ${numEventsToRequeue} events for retry (queue size: ${this.events.size})`,
    );
  }
}
