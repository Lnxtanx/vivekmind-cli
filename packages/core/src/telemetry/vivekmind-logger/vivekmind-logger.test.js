...
import { VivekMindLogger, TEST_ONLY } from './vivekmind-logger.js';
import { AuthType } from '../../core/contentGenerator.js';
import { StartSessionEvent, EndSessionEvent, IdeConnectionEvent, KittySequenceOverflowEvent, IdeConnectionType, HookCallEvent, } from '../types.js';
...
const makeFakeConfig = (overrides = {}) => {
    const defaults = {
        ...
        getAuthType: () => AuthType.VIVEKMIND_OAUTH,
        getMcpServers: () => ({}),
        getModel: () => 'test-model',
...
describe('VivekMindLogger', () => {
    ...
    describe('getInstance', () => {
        it('returns undefined when usage statistics are disabled', () => {
            ...
            const logger = VivekMindLogger.getInstance(config);
            expect(logger).toBeUndefined();
        });
        it('returns an instance when usage statistics are enabled', () => {
            const logger = VivekMindLogger.getInstance(mockConfig);
            expect(logger).toBeInstanceOf(VivekMindLogger);
        });
        it('is a singleton', () => {
            const logger1 = VivekMindLogger.getInstance(mockConfig);
            const logger2 = VivekMindLogger.getInstance(mockConfig);
    ...
    describe('createRumPayload', () => {
        ...
        it('includes source when source.json exists with valid source', async () => {
            ...
            expect(payload.app.channel === undefined ||
                typeof payload.app.channel === 'string').toBe(true);
        });
        it('caches source info and does not read file on every payload creation', async () => {
            ...
            const payload1 = await logger.createRumPayload();
            const payload2 = await logger.createRumPayload();
    ...
    describe('event queue management', () => {
        it('should handle event overflow gracefully', () => {
            ...
            const events = logger['events'].toArray();
            expect(logger['events'].size).toBe(TEST_ONLY.MAX_EVENTS);
            expect(events[0]?.name).toBe('test-event-10');
            expect(events[events.length - 1]?.name).toBe(`test-event-${TEST_ONLY.MAX_EVENTS + 9}`);
        });
    ...
    describe('event handlers', () => {
        ...
        it('should re-read source info when starting a new session', async () => {
            const logger = VivekMindLogger.getInstance(mockConfig);
            const readSourceInfoSpy = vi.spyOn(logger, 'readSourceInfo');
            const testConfig = makeFakeConfig({
                getModel: () => 'test-model',
                getEmbeddingModel: () => 'test-embedding',
    ...
    describe('logHookCallEvent', () => {
        it('should log a successful hook call event', () => {
            const logger = VivekMindLogger.getInstance(mockConfig);
            const enqueueSpy = vi.spyOn(logger, 'enqueueLogEvent');
            const event = new HookCallEvent('PreToolUse', 'command', 'check-secrets.sh', { tool_name: 'read_file' }, 150, true, { result: 'valid' }, 0, 'stdout', 'stderr', undefined);
        ...
        it('should log a failed hook call event with error when telemetry log prompts enabled', () => {
            ...
            const logger = VivekMindLogger.getInstance(configWithLogPrompts);
            const enqueueSpy = vi.spyOn(logger, 'enqueueLogEvent');
            const event = new HookCallEvent('PostToolUse', 'command', 'cleanup.sh', { tool_name: 'shell' }, 200, false, undefined, 1, '', 'error output', 'Command failed');
            logger.logHookCallEvent(event);
            expect(enqueueSpy).toHaveBeenCalledWith(expect.objectContaining({
                event_type: 'action',
                type: 'hook',
        ...
        it('should not include error when telemetry log prompts disabled', () => {
            ...
            VivekMindLogger.instance = undefined;
            const logger = VivekMindLogger.getInstance(configWithoutLogPrompts);
            const enqueueSpy = vi.spyOn(logger, 'enqueueLogEvent');
            const event = new HookCallEvent('PostToolUse', 'command', 'cleanup.sh', { tool_name: 'shell' }, 200, false, undefined, 1, '', 'error output', 'Command failed with sensitive data');
            logger.logHookCallEvent(event);
            expect(enqueueSpy).toHaveBeenCalledWith(expect.objectContaining({
                properties: expect.objectContaining({
                    hook_event_name: 'PostToolUse',
        ...
        it('should sanitize hook name to remove sensitive information', () => {
            ...
            const event = new HookCallEvent('PreToolUse', 'command', '/home/user/.vivekmind/hooks/check-secrets.sh --api-key=secret123', { tool_name: 'read_file' }, 100, true);
            logger.logHookCallEvent(event);
            expect(enqueueSpy).toHaveBeenCalledWith(expect.objectContaining({
                properties: expect.objectContaining({
        ...
        it('should sanitize hook name with Windows path', () => {
            const logger = VivekMindLogger.getInstance(mockConfig);
            const enqueueSpy = vi.spyOn(logger, 'enqueueLogEvent');
            const event = new HookCallEvent('Stop', 'command', 'C:\\Users\\user\\hooks\\cleanup.bat --token=xyz', {}, 50, true);
            logger.logHookCallEvent(event);
            expect(enqueueSpy).toHaveBeenCalledWith(expect.objectContaining({
                properties: expect.objectContaining({
        ...
        it('should handle empty hook name', () => {
            const logger = VivekMindLogger.getInstance(mockConfig);
            const enqueueSpy = vi.spyOn(logger, 'enqueueLogEvent');
            const event = new HookCallEvent('SessionStart', 'command', '', {}, 10, true);
            logger.logHookCallEvent(event);
            expect(enqueueSpy).toHaveBeenCalledWith(expect.objectContaining({
                properties: expect.objectContaining({
        ...
        it('should handle hook name with only whitespace', () => {
            const logger = VivekMindLogger.getInstance(mockConfig);
            const enqueueSpy = vi.spyOn(logger, 'enqueueLogEvent');
            const event = new HookCallEvent('SessionEnd', 'command', '   ', {}, 10, true);
            logger.logHookCallEvent(event);
            expect(enqueueSpy).toHaveBeenCalledWith(expect.objectContaining({
                properties: expect.objectContaining({
                    hook_name: 'unknown-command',
        ...
        it('should handle hook name that is just a command without path', () => {
            const logger = VivekMindLogger.getInstance(mockConfig);
            const enqueueSpy = vi.spyOn(logger, 'enqueueLogEvent');
            const event = new HookCallEvent('Notification', 'command', 'python --arg=value', {}, 100, true);
            logger.logHookCallEvent(event);
            expect(enqueueSpy).toHaveBeenCalledWith(expect.objectContaining({
                properties: expect.objectContaining({
        ...
        it('should call flushIfNeeded after logging', () => {
            ...
            const event = new HookCallEvent('PreToolUse', 'command', 'test-hook.sh', {}, 100, true);
            logger.logHookCallEvent(event);
            expect(flushSpy).toHaveBeenCalled();
        });
...
//# sourceMappingURL=vivekmind-logger.test.js.map