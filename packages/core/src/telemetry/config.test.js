...
describe('telemetry/config helpers', () => {
    ...
    describe('resolveTelemetrySettings', () => {
        ...
        it('uses env over settings and argv over env', async () => {
            ...
            const env = {
                VIVEKMIND_TELEMETRY_ENABLED: '1',
                VIVEKMIND_TELEMETRY_TARGET: 'gcp',
                VIVEKMIND_TELEMETRY_OTLP_ENDPOINT: 'http://env:4317',
                VIVEKMIND_TELEMETRY_OTLP_PROTOCOL: 'http',
                VIVEKMIND_TELEMETRY_LOG_PROMPTS: 'true',
                VIVEKMIND_TELEMETRY_OUTFILE: 'env.log',
                VIVEKMIND_TELEMETRY_USE_COLLECTOR: 'true',
            };
            const argv = {
                telemetry: false,
        ...
        it('throws on unknown protocol values', async () => {
            const env = { VIVEKMIND_TELEMETRY_OTLP_PROTOCOL: 'unknown' };
        ...
        it('throws on unknown target values', async () => {
            const env = { VIVEKMIND_TELEMETRY_TARGET: 'unknown' };
        ...
        it('VIVEKMIND_ env vars take precedence over OTEL_ vars for per-signal endpoints', async () => {
            const env = {
                VIVEKMIND_TELEMETRY_OTLP_TRACES_ENDPOINT: 'http://vivekmind-traces:4318/v1/traces',
                OTEL_EXPORTER_OTLP_TRACES_ENDPOINT: 'http://otel-traces:4318/v1/traces',
            };
            const resolved = await resolveTelemetrySettings({ env });
            expect(resolved.otlpTracesEndpoint).toBe('http://vivekmind-traces:4318/v1/traces');
        });
        it('resolves per-signal endpoints from settings', async () => {
            ...
            const resolved = await resolveTelemetrySettings({ settings });
            expect(resolved.otlpTracesEndpoint).toBe('http://traces-settings:4318/v1/traces');
            expect(resolved.otlpLogsEndpoint).toBeUndefined();
            expect(resolved.otlpMetricsEndpoint).toBe('http://metrics-settings:4318/v1/metrics');
        });
    });
});
