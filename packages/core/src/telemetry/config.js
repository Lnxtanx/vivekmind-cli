...
export async function resolveTelemetrySettings(options) {
    ...
    const enabled = argv.telemetry ??
        parseBooleanEnvFlag(env['VIVEKMIND_TELEMETRY_ENABLED']) ??
        settings.enabled;
    const rawTarget = argv.telemetryTarget ??
        env['VIVEKMIND_TELEMETRY_TARGET'] ??
        settings.target;
    ...
    const otlpEndpoint = argv.telemetryOtlpEndpoint ??
        env['VIVEKMIND_TELEMETRY_OTLP_ENDPOINT'] ??
        env['OTEL_EXPORTER_OTLP_ENDPOINT'] ??
        settings.otlpEndpoint;
    const rawProtocol = argv.telemetryOtlpProtocol ??
        env['VIVEKMIND_TELEMETRY_OTLP_PROTOCOL'] ??
    ...
    const logPrompts = argv.telemetryLogPrompts ??
        parseBooleanEnvFlag(env['VIVEKMIND_TELEMETRY_LOG_PROMPTS']) ??
        settings.logPrompts;
    const outfile = argv.telemetryOutfile ?? env['VIVEKMIND_TELEMETRY_OUTFILE'] ?? settings.outfile;
    const useCollector = parseBooleanEnvFlag(env['VIVEKMIND_TELEMETRY_USE_COLLECTOR']) ??
        settings.useCollector;
    // Per-signal endpoint overrides (HTTP only).
    // Priority: VIVEKMIND_ env var > standard OTEL_ env var > settings.json
    const otlpTracesEndpoint = env['VIVEKMIND_TELEMETRY_OTLP_TRACES_ENDPOINT'] ??
        env['OTEL_EXPORTER_OTLP_TRACES_ENDPOINT'] ??
        settings.otlpTracesEndpoint;
    const otlpLogsEndpoint = env['VIVEKMIND_TELEMETRY_OTLP_LOGS_ENDPOINT'] ??
        env['OTEL_EXPORTER_OTLP_LOGS_ENDPOINT'] ??
        settings.otlpLogsEndpoint;
    const otlpMetricsEndpoint = env['VIVEKMIND_TELEMETRY_OTLP_METRICS_ENDPOINT'] ??
