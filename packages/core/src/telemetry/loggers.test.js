...
import { AuthType, GeminiClient, ToolConfirmationOutcome, ToolErrorType, ToolRegistry, } from '../index.js';
...
import { logApiRequest, logApiResponse, logStartSession, logUserPrompt, logToolCall, logFlashFallback, logChatCompression, logMalformedJsonResponse, logFileOperation, logRipgrepFallback, logToolOutputTruncated, logExtensionEnable, logExtensionDisable, logExtensionInstallEvent, logExtensionUninstall, logHookCall, logApiError, } from './loggers.js';
import * as metrics from './metrics.js';
import { VivekMindLogger } from './vivekmind-logger/vivekmind-logger.js';
import * as sdk from './sdk.js';
import { ToolCallDecision } from './tool-call-decision.js';
import { ApiRequestEvent, ApiResponseEvent, FlashFallbackEvent, StartSessionEvent, ToolCallEvent, UserPromptEvent, RipgrepFallbackEvent, MalformedJsonResponseEvent, makeChatCompressionEvent, FileOperationEvent, ToolOutputTruncatedEvent, ExtensionEnableEvent, ExtensionDisableEvent, ExtensionInstallEvent, ExtensionUninstallEvent, HookCallEvent, ApiErrorEvent, } from './types.js';
...
describe('loggers', () => {
    ...
    describe('logChatCompression', () => {
        beforeEach(() => {
            ...
            vi.spyOn(VivekMindLogger.prototype, 'logChatCompressionEvent');
        });
        it('logs the chat compression event to VivekMindLogger', () => {
            ...
            expect(VivekMindLogger.prototype.logChatCompressionEvent).toHaveBeenCalledWith(event);
        });
    ...
    describe('logApiResponse skips chatRecordingService for internal prompt IDs', () => {
        it.each(['prompt_suggestion', 'forked_query', 'speculation'])('should not record to chatRecordingService when prompt_id is %s', (promptId) => {
            ...
            const event = new ApiResponseEvent('resp-id', 'test-model', 50, promptId);
        ...
        it('should record to chatRecordingService for normal prompt IDs', () => {
            ...
            const event = new ApiResponseEvent('resp-id', 'test-model', 50, 'user_query');
            logApiResponse(configWithRecording, event);
    ...
    describe('logRipgrepFallback', () => {
        ...
        beforeEach(() => {
            vi.spyOn(VivekMindLogger.prototype, 'logRipgrepFallbackEvent');
        });
        it('should log ripgrep fallback event', () => {
            ...
            expect(VivekMindLogger.prototype.logRipgrepFallbackEvent).toHaveBeenCalled();
    ...
    describe('logMalformedJsonResponse', () => {
        beforeEach(() => {
            vi.spyOn(VivekMindLogger.prototype, 'logMalformedJsonResponseEvent');
        });
        it('logs the event to Clearcut and OTEL', () => {
            ...
            expect(VivekMindLogger.prototype.logMalformedJsonResponseEvent).toHaveBeenCalledWith(event);
            expect(mockLogger.emit).toHaveBeenCalledWith({
    ...
    describe('logExtensionInstall', () => {
        ...
        beforeEach(() => {
            vi.spyOn(VivekMindLogger.prototype, 'logExtensionInstallEvent');
        });
        afterEach(() => {
            vi.resetAllMocks();
        });
        it('should log extension install event', () => {
            ...
            expect(VivekMindLogger.prototype.logExtensionInstallEvent).toHaveBeenCalledWith(event);
            expect(mockLogger.emit).toHaveBeenCalledWith({
    ...
    describe('logExtensionUninstall', () => {
        ...
        beforeEach(() => {
            vi.spyOn(VivekMindLogger.prototype, 'logExtensionUninstallEvent');
        });
        afterEach(() => {
            vi.resetAllMocks();
        });
        it('should log extension uninstall event', () => {
            ...
            expect(VivekMindLogger.prototype.logExtensionUninstallEvent).toHaveBeenCalledWith(event);
            expect(mockLogger.emit).toHaveBeenCalledWith({
    ...
    describe('logExtensionEnable', () => {
        ...
        beforeEach(() => {
            vi.spyOn(VivekMindLogger.prototype, 'logExtensionEnableEvent');
        });
        afterEach(() => {
            vi.resetAllMocks();
        });
        it('should log extension enable event', () => {
            ...
            expect(VivekMindLogger.prototype.logExtensionEnableEvent).toHaveBeenCalledWith(event);
            expect(mockLogger.emit).toHaveBeenCalledWith({
    ...
    describe('logExtensionDisable', () => {
        ...
        beforeEach(() => {
            vi.spyOn(VivekMindLogger.prototype, 'logExtensionDisableEvent');
        });
        afterEach(() => {
            vi.resetAllMocks();
        });
        it('should log extension disable event', () => {
            ...
            expect(VivekMindLogger.prototype.logExtensionDisableEvent).toHaveBeenCalledWith(event);
            expect(mockLogger.emit).toHaveBeenCalledWith({
    ...
    describe('logHookCall', () => {
        ...
        const mockVivekMindLogger = {
            logHookCallEvent: vi.fn(),
        };
        beforeEach(() => {
            vi.spyOn(VivekMindLogger, 'getInstance').mockReturnValue(mockVivekMindLogger);
            mockVivekMindLogger.logHookCallEvent.mockClear();
        });
        it('should log a successful hook call to VivekMindLogger', () => {
            const event = new HookCallEvent('UserPromptSubmit', 'command', 'check-secrets.sh', { prompt: 'test prompt' }, 150, true, { output: 'success' }, 0, 'stdout message', 'stderr message', undefined);
            logHookCall(mockConfig, event);
            // Should call VivekMindLogger
            expect(mockVivekMindLogger.logHookCallEvent).toHaveBeenCalledWith(event);
        });
        it('should log a failed hook call with error', () => {
            const event = new HookCallEvent('Stop', 'command', 'cleanup.sh', { last_assistant_message: 'final message' }, 200, false, undefined, 1, 'stdout message', 'stderr message', 'Error occurred');
            logHookCall(mockConfig, event);
            // Should call VivekMindLogger
            expect(mockVivekMindLogger.logHookCallEvent).toHaveBeenCalledWith(event);
        });
        it('should handle when VivekMindLogger is not available', () => {
            vi.spyOn(VivekMindLogger, 'getInstance').mockReturnValue(undefined);
            const event = new HookCallEvent('UserPromptSubmit', 'command', 'test-hook.sh', { prompt: 'test' }, 100, true);
            // Should not throw when VivekMindLogger is not available
            expect(() => logHookCall(mockConfig, event)).not.toThrow();
        });
        it('should log hook call with all optional fields', () => {
            const event = new HookCallEvent('PreToolUse', 'command', 'validator.sh', { tool_name: 'read_file', path: '/test/file.txt' }, 250, true, { decision: 'allow', reason: 'validated' }, 0, 'validation passed', '', undefined);
            logHookCall(mockConfig, event);
            expect(mockVivekMindLogger.logHookCallEvent).toHaveBeenCalledWith(event);
        });
        it('should log hook call with minimal fields', () => {
            const event = new HookCallEvent('SessionStart', 'command', 'init.sh', {}, 10, true);
            logHookCall(mockConfig, event);
            expect(mockVivekMindLogger.logHookCallEvent).toHaveBeenCalledWith(event);
        });
        it('should log hook call with exit code', () => {
            const event = new HookCallEvent('PostToolUseFailure', 'command', 'error-handler.sh', { tool_name: 'shell' }, 50, false, undefined, 1, '', 'error output', 'Command failed with exit code 1');
            logHookCall(mockConfig, event);
            expect(mockVivekMindLogger.logHookCallEvent).toHaveBeenCalledWith(event);
        });
        it('should log hook call with zero exit code on success', () => {
            const event = new HookCallEvent('PostToolUse', 'command', 'success-handler.sh', { tool_name: 'write_file' }, 100, true, { result: 'ok' }, 0, 'done', '', undefined);
            logHookCall(mockConfig, event);
            expect(mockVivekMindLogger.logHookCallEvent).toHaveBeenCalledWith(event);
        });
        it('should log hook call with non-zero exit code on failure', () => {
            const event = new HookCallEvent('PostToolUseFailure', 'command', 'failure-handler.sh', { tool_name: 'shell' }, 75, false, undefined, 127, '', 'command not found', 'Hook command not found');
            logHookCall(mockConfig, event);
            expect(mockVivekMindLogger.logHookCallEvent).toHaveBeenCalledWith(event);
        });
        it('should log all hook event types', () => {
            ...
            for (const eventType of eventTypes) {
                mockVivekMindLogger.logHookCallEvent.mockClear();
                const event = new HookCallEvent(eventType, 'command', 'test-hook.sh', {}, 100, true);
                logHookCall(mockConfig, event);
                expect(mockVivekMindLogger.logHookCallEvent).toHaveBeenCalledWith(event);
            }
        });
        it('should pass the exact event object to VivekMindLogger', () => {
            const event = new HookCallEvent('PreToolUse', 'command', 'test-hook.sh', { tool_name: 'read_file' }, 100, true);
            logHookCall(mockConfig, event);
            // Verify the exact event object is passed
            expect(mockVivekMindLogger.logHookCallEvent).toHaveBeenCalledTimes(1);
            const passedEvent = mockVivekMindLogger.logHookCallEvent.mock.calls[0][0];
