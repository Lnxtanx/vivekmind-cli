...
export function logStartSession(config, event) {
    VivekMindLogger.getInstance(config)?.logStartSessionEvent(event);
...
export function logUserPrompt(config, event) {
    VivekMindLogger.getInstance(config)?.logNewPromptEvent(event);
    if (!isTelemetrySdkInitialized())
        return;
    const attributes = {
        ...getCommonAttributes(config),
...
export function logUserRetry(config, event) {
    VivekMindLogger.getInstance(config)?.logRetryEvent(event);
    if (!isTelemetrySdkInitialized())
        return;
    const attributes = {
        ...getCommonAttributes(config),
...
export function logToolCall(config, event) {
    ...
    VivekMindLogger.getInstance(config)?.logToolCallEvent(event);
    if (!isTelemetrySdkInitialized())
        return;
    const attributes = {
        ...getCommonAttributes(config),
...
export function logToolOutputTruncated(config, event) {
    VivekMindLogger.getInstance(config)?.logToolOutputTruncatedEvent(event);
    if (!isTelemetrySdkInitialized())
        return;
    const attributes = {
        ...getCommonAttributes(config),
...
export function logFileOperation(config, event) {
    VivekMindLogger.getInstance(config)?.logFileOperationEvent(event);
    if (!isTelemetrySdkInitialized())
        return;
    const attributes = {
        ...getCommonAttributes(config),
...
export function logApiRequest(config, event) {
    // VivekMindLogger.getInstance(config)?.logApiRequestEvent(event);
    if (!isTelemetrySdkInitialized())
        return;
    const attributes = {
        ...getCommonAttributes(config),
...
export function logFlashFallback(config, event) {
    VivekMindLogger.getInstance(config)?.logFlashFallbackEvent(event);
    if (!isTelemetrySdkInitialized())
        return;
    const attributes = {
        ...getCommonAttributes(config),
...
export function logRipgrepFallback(config, event) {
    VivekMindLogger.getInstance(config)?.logRipgrepFallbackEvent(event);
    if (!isTelemetrySdkInitialized())
        return;
    const attributes = {
        ...getCommonAttributes(config),
...
export function logApiError(config, event) {
    ...
    VivekMindLogger.getInstance(config)?.logApiErrorEvent(event);
    if (!isTelemetrySdkInitialized())
        return;
    const attributes = {
        ...getCommonAttributes(config),
...
export function logApiCancel(config, event) {
    ...
    uiTelemetryService.addEvent(uiEvent);
    VivekMindLogger.getInstance(config)?.logApiCancelEvent(event);
    if (!isTelemetrySdkInitialized())
        return;
    const attributes = {
        ...getCommonAttributes(config),
...
export function logApiResponse(config, event) {
    ...
    VivekMindLogger.getInstance(config)?.logApiResponseEvent(event);
    if (!isTelemetrySdkInitialized())
        return;
    const attributes = {
        ...getCommonAttributes(config),
...
export function logLoopDetected(config, event) {
    VivekMindLogger.getInstance(config)?.logLoopDetectedEvent(event);
    if (!isTelemetrySdkInitialized())
        return;
    const attributes = {
        ...getCommonAttributes(config),
...
export function logLoopDetectionDisabled(config, _event) {
    VivekMindLogger.getInstance(config)?.logLoopDetectionDisabledEvent();
}
export function logNextSpeakerCheck(config, event) {
    VivekMindLogger.getInstance(config)?.logNextSpeakerCheck(event);
    if (!isTelemetrySdkInitialized())
        return;
    const attributes = {
        ...getCommonAttributes(config),
...
export function logSlashCommand(config, event) {
    VivekMindLogger.getInstance(config)?.logSlashCommandEvent(event);
    if (!isTelemetrySdkInitialized())
        return;
    const attributes = {
        ...getCommonAttributes(config),
...
export function logIdeConnection(config, event) {
    VivekMindLogger.getInstance(config)?.logIdeConnectionEvent(event);
    if (!isTelemetrySdkInitialized())
        return;
    const attributes = {
        ...getCommonAttributes(config),
...
export function logConversationFinishedEvent(config, event) {
    VivekMindLogger.getInstance(config)?.logConversationFinishedEvent(event);
    if (!isTelemetrySdkInitialized())
        return;
    const attributes = {
        ...getCommonAttributes(config),
...
export function logChatCompression(config, event) {
    VivekMindLogger.getInstance(config)?.logChatCompressionEvent(event);
    const attributes = {
        ...getCommonAttributes(config),
        ...event,
...
export function logKittySequenceOverflow(config, event) {
    VivekMindLogger.getInstance(config)?.logKittySequenceOverflowEvent(event);
    if (!isTelemetrySdkInitialized())
        return;
    const attributes = {
        ...getCommonAttributes(config),
...
export function logMalformedJsonResponse(config, event) {
    VivekMindLogger.getInstance(config)?.logMalformedJsonResponseEvent(event);
    if (!isTelemetrySdkInitialized())
        return;
    const attributes = {
        ...getCommonAttributes(config),
...
export function logInvalidChunk(config, event) {
    VivekMindLogger.getInstance(config)?.logInvalidChunkEvent(event);
    if (!isTelemetrySdkInitialized())
        return;
    const attributes = {
        ...getCommonAttributes(config),
...
export function logContentRetry(config, event) {
    VivekMindLogger.getInstance(config)?.logContentRetryEvent(event);
    if (!isTelemetrySdkInitialized())
        return;
    const attributes = {
        ...getCommonAttributes(config),
...
export function logContentRetryFailure(config, event) {
    VivekMindLogger.getInstance(config)?.logContentRetryFailureEvent(event);
    if (!isTelemetrySdkInitialized())
        return;
    const attributes = {
        ...getCommonAttributes(config),
...
export function logSubagentExecution(config, event) {
    VivekMindLogger.getInstance(config)?.logSubagentExecutionEvent(event);
    if (!isTelemetrySdkInitialized())
        return;
    const attributes = {
        ...getCommonAttributes(config),
...
export function logModelSlashCommand(config, event) {
    VivekMindLogger.getInstance(config)?.logModelSlashCommandEvent(event);
    if (!isTelemetrySdkInitialized())
        return;
    const attributes = {
        ...getCommonAttributes(config),
...
export function logExtensionInstallEvent(config, event) {
    VivekMindLogger.getInstance(config)?.logExtensionInstallEvent(event);
    if (!isTelemetrySdkInitialized())
        return;
    const attributes = {
        ...getCommonAttributes(config),
...
export function logExtensionUninstall(config, event) {
    VivekMindLogger.getInstance(config)?.logExtensionUninstallEvent(event);
    if (!isTelemetrySdkInitialized())
        return;
    const attributes = {
        ...getCommonAttributes(config),
...
export async function logExtensionUpdateEvent(config, event) {
    VivekMindLogger.getInstance(config)?.logExtensionUpdateEvent(event);
    const attributes = {
        ...getCommonAttributes(config),
        ...event,
...
export function logExtensionEnable(config, event) {
    VivekMindLogger.getInstance(config)?.logExtensionEnableEvent(event);
    if (!isTelemetrySdkInitialized())
        return;
    const attributes = {
        ...getCommonAttributes(config),
...
export function logExtensionDisable(config, event) {
    VivekMindLogger.getInstance(config)?.logExtensionDisableEvent(event);
    if (!isTelemetrySdkInitialized())
        return;
    const attributes = {
        ...getCommonAttributes(config),
...
export function logAuth(config, event) {
    VivekMindLogger.getInstance(config)?.logAuthEvent(event);
    if (!isTelemetrySdkInitialized())
        return;
    const attributes = {
        ...getCommonAttributes(config),
...
export function logUserFeedback(config, event) {
    ...
    uiTelemetryService.addEvent(uiEvent);
    config.getChatRecordingService()?.recordUiTelemetryEvent(uiEvent);
    VivekMindLogger.getInstance(config)?.logUserFeedbackEvent(event);
    if (!isTelemetrySdkInitialized())
        return;
    const attributes = {
        ...getCommonAttributes(config),
...
export function logArenaSessionStarted(config, event) {
    VivekMindLogger.getInstance(config)?.logArenaSessionStartedEvent(event);
    if (!isTelemetrySdkInitialized())
        return;
    const attributes = {
        ...getCommonAttributes(config),
...
export function logArenaAgentCompleted(config, event) {
    VivekMindLogger.getInstance(config)?.logArenaAgentCompletedEvent(event);
    if (!isTelemetrySdkInitialized())
        return;
    const attributes = {
        ...getCommonAttributes(config),
...
export function logArenaSessionEnded(config, event) {
    VivekMindLogger.getInstance(config)?.logArenaSessionEndedEvent(event);
    if (!isTelemetrySdkInitialized())
        return;
    const attributes = {
        ...getCommonAttributes(config),
