...
import { initializeTelemetry, isTelemetrySdkInitialized, shutdownTelemetry, resolveHttpOtlpUrl, } from './sdk.js';
...
describe('resolveHttpOtlpUrl', () => {
    it('appends signal path to base collector URL', () => {
        expect(resolveHttpOtlpUrl('http://collector:4318', 'traces')).toBe('http://collector:4318/v1/traces');
        expect(resolveHttpOtlpUrl('http://collector:4318', 'logs')).toBe('http://collector:4318/v1/logs');
        expect(resolveHttpOtlpUrl('http://collector:4318', 'metrics')).toBe('http://collector:4318/v1/metrics');
    });
    ...
    it('preserves explicit full signal path URL', () => {
        expect(resolveHttpOtlpUrl('http://collector:4318/v1/traces', 'traces')).toBe('http://collector:4318/v1/traces');
        expect(resolveHttpOtlpUrl('http://collector:4318/v1/logs', 'logs')).toBe('http://collector:4318/v1/logs');
        expect(resolveHttpOtlpUrl('http://collector:4318/v1/metrics', 'metrics')).toBe('http://collector:4318/v1/metrics');
    });
    it('appends signal path when URL has a non-signal custom path', () => {
        expect(resolveHttpOtlpUrl('http://collector:4318/custom/prefix', 'traces')).toBe('http://collector:4318/custom/prefix/v1/traces');
    });
    it('handles HTTPS URLs', () => {
        expect(resolveHttpOtlpUrl('https://otel.example.com', 'logs')).toBe('https://otel.example.com/v1/logs');
        expect(resolveHttpOtlpUrl('https://otel.example.com:4318', 'metrics')).toBe('https://otel.example.com:4318/v1/metrics');
    });
    it('preserves query strings when appending signal paths', () => {
        expect(resolveHttpOtlpUrl('https://host/otlp?token=abc', 'traces')).toBe('https://host/otlp/v1/traces?token=abc');
        expect(resolveHttpOtlpUrl('https://host/otlp?token=abc&foo=bar', 'logs')).toBe('https://host/otlp/v1/logs?token=abc&foo=bar');
    });
});
describe('Telemetry SDK', () => {
    ...
    it('should append signal paths to HTTP endpoint', () => {
        ...
        vi.spyOn(mockConfig, 'getTelemetryOtlpEndpoint').mockReturnValue('https://my-collector.com');
        initializeTelemetry(mockConfig);
        expect(OTLPTraceExporterHttp).toHaveBeenCalledWith(expect.objectContaining({ url: 'https://my-collector.com/v1/traces' }));
        expect(OTLPLogExporterHttp).toHaveBeenCalledWith(expect.objectContaining({ url: 'https://my-collector.com/v1/logs' }));
        expect(OTLPMetricExporterHttp).toHaveBeenCalledWith(expect.objectContaining({ url: 'https://my-collector.com/v1/metrics' }));
    });
    it('should use per-signal endpoint overrides when provided', () => {
        vi.spyOn(mockConfig, 'getTelemetryOtlpProtocol').mockReturnValue('http');
        vi.spyOn(mockConfig, 'getTelemetryOtlpEndpoint').mockReturnValue('http://default-collector:4318');
        vi.spyOn(mockConfig, 'getTelemetryOtlpTracesEndpoint').mockReturnValue('http://traces-collector:4318/v1/traces');
        initializeTelemetry(mockConfig);
    ...
    it('should use per-signal overrides without base endpoint', () => {
        ...
        vi.spyOn(mockConfig, 'getTelemetryOtlpTracesEndpoint').mockReturnValue('http://traces-host/token/api/otlp/traces');
        vi.spyOn(mockConfig, 'getTelemetryOtlpMetricsEndpoint').mockReturnValue('http://metrics-host/token/api/otlp/metrics');
    ...
    it('should warn and skip startup for gRPC per-signal endpoints without base endpoint', () => {
        ...
        try {
            ...
            vi.spyOn(mockConfig, 'getTelemetryOtlpTracesEndpoint').mockReturnValue('http://traces-host/token/api/otlp/traces');
            initializeTelemetry(mockConfig);
            expect(diagWarnSpy).toHaveBeenCalledWith(expect.stringContaining('Telemetry SDK startup was skipped'));
    ...
    it('should set service.version to the application version, not Node.js version', () => {
        ...
        const resource = constructorCall.resource;
    ...
    it('should complete shutdown within timeout when SDK shutdown hangs', async () => {
        ...
        try {
            ...
            expect(isTelemetrySdkInitialized()).toBe(false);
            expect(diagWarnSpy).toHaveBeenCalledWith(expect.stringContaining('Telemetry shutdown timed out'));
        }
        finally {
    ...
    it('should log error when sdk.shutdown() rejects', async () => {
        ...
        try {
            ...
            await shutdownTelemetry();
            expect(isTelemetrySdkInitialized()).toBe(false);
            expect(diagErrorSpy).toHaveBeenCalledWith('Error shutting down SDK:', expect.any(Error));
        }
        finally {
            shutdownSpy.mockRestore();
