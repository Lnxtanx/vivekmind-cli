...
export class TelegramChannel extends ChannelBase {
  ...
  /** Track pending approval requests awaiting user response (promise-based, for onToolCallApproval). */
  private pendingApprovals: Map<string, PendingApproval> = new Map();

  /** Phase 1: Track pending permission callbacks for event-based approval flow. */
  private pendingPermissionCallbacks: Map<
    string,
  ...
  async connect(): Promise<void> {
    ...
    // ─── Phase 1: Event-based approval/deny callbacks ─────────────
    this.bot.on('callback_query:data', async (ctx) => {
      const data = ctx.callbackQuery.data;
      const chatId = String(ctx.callbackQuery.message?.chat.id);

      try {
        if (data.startsWith('approve:') && !data.startsWith('approve_once') && !data.startsWith('approve_always')) {
          await this.handleApprovalCallback(data, chatId);
        } else if (data.startsWith('deny:') && !data.startsWith('deny_once') && !data.startsWith('deny_always')) {
  ...
  protected override onRequestPermission(
    ...
    this.bot.api
      ...
      .then((sent) => {
        this.pendingPermissionCallbacks.set(request.id, {
          chatId,
  ...
  private async handleApprovalCallback(
    ...
    const pending = this.pendingPermissionCallbacks.get(permissionId);
    if (!pending) return;
    ...
    try {
      ...
      this.pendingPermissionCallbacks.delete(permissionId);
    }
  }
