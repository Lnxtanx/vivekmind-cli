...
export class DingtalkChannel extends ChannelBase {
    ...
    async emotionApi(endpoint, msgId, conversationId) {
        ...
        try {
            ...
            if (!resp.ok) {
                const detail = await resp.text().catch(() => '');
                process.stderr.write(`[DingTalk:${this.name}] emotion/${endpoint} failed: ${resp.status} ${detail}\n`);
            }
        }
        catch {
    ...
    async attachMedia(envelope, downloadCode, mediaType, fileName) {
        ...
        const robotCode = this.config.clientId;
        if (!token || !robotCode) {
            process.stderr.write(`[DingTalk:${this.name}] Cannot download media: missing token or robotCode.\n`);
            return;
        }
