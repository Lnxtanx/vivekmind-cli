# @vivekmind/channel-plugin-example

A reference channel plugin for VivekMind. It connects to a WebSocket server and routes messages through the full channel pipeline (access control, session routing, agent bridge).
...
```bash
npm install @vivekmind/channel-plugin-example
```
...
Add a channel entry to `~/.vivekmind/settings.json`:
...
```bash
vivekmind channel start my-plugin-test
```
