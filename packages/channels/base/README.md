...
Base infrastructure for building VivekMind channel adapters. Provides the abstract base class, access control, session routing, and the ACP bridge that communicates with the agent.
...
```bash
npm install @vivekmind/channel-base
```
...
For a complete working example, see [`@vivekmind/channel-plugin-example`](../plugin-example/).
...
| `AcpBridge`     | Spawns and communicates with the `vivekmind --acp` agent process |
...
Manages the `vivekmind --acp` child process and ACP sessions.
...
Persists pairing state to `~/.vivekmind/channels/{channelName}-pairing.json` and `{channelName}-allowlist.json`.
...
- [`@vivekmind/channel-plugin-example`](../plugin-example/) — working reference implementation
