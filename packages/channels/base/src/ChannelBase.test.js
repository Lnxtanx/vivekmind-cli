...
describe('ChannelBase', () => {
    ...
    function createChannel(configOverrides = {}, options) {
        return new TestChannel('test-chan', defaultConfig(configOverrides), bridge, options);
    }
    ...
    describe('dispatch modes', () => {
        it('collect: buffers messages and coalesces into one followup prompt', async () => {
            ...
            let resolveFirst;
            const firstPrompt = new Promise((r) => {
                resolveFirst = r;
            });
            let callCount = 0;
        ...
        it('steer: cancels running prompt and re-prompts with cancellation note', async () => {
            ...
            const p2 = ch.handleInbound(envelope({ text: 'actually refactor billing' }));
            // Both should resolve
            await p1;
            await p2;
            ...
            expect(ch.sent).not.toEqual(expect.arrayContaining([
    ...
    describe('prompt lifecycle hooks', () => {
        ...
        it('does not call hooks for buffered messages in collect mode', async () => {
            ...
            const ch = createChannel({ dispatchMode: 'collect' });
            const p1 = ch.handleInbound(envelope({ text: 'first', messageId: 'msg-1' }));
            await new Promise((r) => setTimeout(r, 10));
        ...
        it('calls onPromptEnd even when prompt throws', async () => {
            bridge.prompt.mockRejectedValue(new Error('agent error'));
            const ch = createChannel();
...
//# sourceMappingURL=ChannelBase.test.js.map