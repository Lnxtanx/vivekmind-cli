...
export abstract class ChannelBase {
  ...
  protected instructedSessions: Set<string> = new Set();
  ...
  constructor(
    ...
    if (!options?.router) {
      ...
      bridge.on('requestPermission', (request: PendingPermissionRequest) => {
    ...
    bridge.setPermissionHandler(
      async (
        ...
        return this.onToolCallApproval(
          target.chatId,
          {
            ...
            rawInput: params.toolCall.rawInput as Record<string, unknown> | undefined,
          },
          params,
        );
      },
   ...
   */
  protected async onToolCallApproval(
    _chatId: string,
  ...
  private registerSharedCommands(): void {
    ...
    this.registerCommand('help', async (envelope) => {
      ...
      if (agentCommands.length > 0) {
        lines.push('', 'Agent commands (forwarded to VivekMind):');
    ...
    this.registerCommand('attach', async (envelope, args) => {
      ...
      try {
        ...
        this.router.registerExternalSession(
          sessionId,
          this.name,
          envelope.senderId,
          envelope.chatId,
        ...
        await this.sendMessage(
          envelope.chatId,
          `✅ Session attached. Continue chatting — the agent remembers everything from the terminal session.`,
        );
      } catch {
        await this.sendMessage(
          envelope.chatId,
          `❌ Could not attach session "${sessionId}". Make sure the session ID is valid and the CWD matches.`,
        );
      }
      return true;
    });
    ...
    this.registerCommand('detach', async (envelope) => {
      const sessionId = this.router.detachSession(
        this.name,
        envelope.senderId,
        envelope.chatId,
        envelope.threadId,
      ...
      if (sessionId) {
        await this.sendMessage(
          envelope.chatId,
          `✅ Session detached. Session ${sessionId} is still active in the terminal — continue there, or use /attach to reconnect.`,
        );
      } else {
  ...
  async handleInbound(envelope: Envelope): Promise<void> {
    ...
    if (handoff) {
      const handoffTopic = (handoff as Record<string, unknown>)['topic'] as
        | string
        | undefined;
  ...
  protected async onPairingRequired(
    ...
    if (code) {
      await this.sendMessage(
        chatId,
        `Your pairing code is: ${code}\n\nAsk the bot operator to approve you with:\n  vivekmind channel pairing approve ${this.name} ${code}`,
      );
    } else {
      await this.sendMessage(
        chatId,
