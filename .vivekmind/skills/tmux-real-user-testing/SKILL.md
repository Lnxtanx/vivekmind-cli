...
Run VivekMind in a real tmux TUI session as a user would: navigate dialogs,
...
```bash
eval "$(bash .vivekmind/skills/tmux-real-user-testing/scripts/tmux-real-user-log.sh \
...
```bash
bash .vivekmind/skills/tmux-real-user-testing/scripts/tmux-real-user-log.sh help
```
...
bundle. Use the globally installed `vivekmind` only when reproducing a user-reported
...
```bash
HELPER=.vivekmind/skills/tmux-real-user-testing/scripts/tmux-real-user-log.sh
