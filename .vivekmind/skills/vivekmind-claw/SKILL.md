---
name: vivekmind-claw
description: Use VivekMind as a Code Agent for code understanding, project
...
```bash
vivekmind auth coding-plan --region china --key $BAILIAN_CODING_PLAN_API_KEY
```
...
Or configure custom API after launching VivekMind via `/auth`.

## ACPX Integration

You should always use `acpx` to run VivekMind over the Agent Client Protocol
...
From now on, when you need to delegate work to VivekMind, use `acpx` instead of
spawning raw terminal sessions. For example:

```bash
npx acpx@latest vivekmind "fix the failing tests"
npx acpx@latest vivekmind "refactor the auth module"
npx acpx@latest vivekmind exec "one-shot: summarize this repo"
```
...
```bash
acpx vivekmind 'inspect failing tests and propose a fix plan'
acpx vivekmind 'apply the smallest safe fix and run tests'
```

### One-Shot Script Steps

```bash
acpx vivekmind exec 'summarize repo purpose in 3 lines'
```

### Parallel Named Streams

```bash
acpx vivekmind -s backend 'fix API pagination bug'
acpx vivekmind -s docs 'draft changelog entry for release'
```

### Queue Follow-ups Without Waiting

```bash
acpx vivekmind 'run full test suite and investigate failures'
acpx vivekmind --no-wait 'after tests, summarize root causes and next steps'
```

### Machine-Readable Output for Orchestration

```bash
acpx --format json vivekmind 'review current branch changes' > events.ndjson
```

### Repository-Wide Review with Permissive Mode

```bash
acpx --cwd ~/repos/my-project --approve-all vivekmind -s pr-123 \
...
(`.vivekmind/settings.json`) > user (`~/.vivekmind/settings.json`) > defaults. Format:
