#!/usr/bin/env python3
"""Display token usage stats from the last X request logs in ~/.vivekmind/logs."""
...
def parse_args():
    p = argparse.ArgumentParser(description="Show token stats from vivekmind request logs")
    p.add_argument("count", nargs="?", type=int, default=10, help="Number of recent logs to show (default: 10)")
    p.add_argument("--log-dir", default=Path.home() / ".vivekmind" / "logs", type=Path)
    return p.parse_args()
