...
MCP servers are configured in `.vivekmind/settings.json` under `mcpServers`. This is
the **only** location that works for E2E testing.

Common mistakes that waste time:

- `.mcp.json` — Claude Code convention, not VivekMind
...
mkdir -p .vivekmind
cat > .vivekmind/settings.json << 'EOF'
{
  "mcpServers": {
    "my-server": {
      "command": "node",
...
```bash
cd /tmp/test-dir && <vivekmind> "prompt" \
...
- You're not running from the directory containing `.vivekmind/settings.json`
