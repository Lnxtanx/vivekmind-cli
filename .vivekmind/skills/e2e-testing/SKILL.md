---
name: e2e-testing
description: Guide for running end-to-end tests of the VivekMind CLI, including
...
How to run the VivekMind CLI end-to-end, from building the bundle to inspecting
...
- **Reproducing bugs**: use the globally installed `vivekmind` command — this
...
Run the CLI non-interactively with JSON output (`<vivekmind>` = `vivekmind` or `node
dist/cli.js` per above):

```bash
<vivekmind> "your prompt here" \
  --approval-mode yolo \
  --output-format json \
  2>/dev/null
```
...
tmux new-session -d -s test -x 200 -y 50 \
  "cd /tmp/test-dir && <vivekmind> --approval-mode yolo"
...
```bash
python3 .vivekmind/skills/e2e-testing/scripts/token-stats.py 20  # last 20 requests
```
