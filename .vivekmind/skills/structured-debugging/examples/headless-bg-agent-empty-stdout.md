...
A short vivekmind case to illustrate two failure modes from `SKILL.md`:
...
`~/.vivekmind/logs` shows the model returned proper text. Stdout was empty.
...
Fix commit: vivekmind `feadf052f` — `fix(cli): append newline to text-mode
