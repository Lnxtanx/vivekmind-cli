...
Create `.vivekmind/issues/` if it doesn't exist, then pipe the issue directly into a
markdown file using `gh`:

```bash
mkdir -p .vivekmind/issues
...
_Pending — to be filled by the test engineer._
' > .vivekmind/issues/issue-<number>.md
```
...
`.vivekmind/issues/issue-<number>.md` for the issue details, then assess and
...
`.vivekmind/issues/issue-<number>.md`** to get the reproduction report. If the status
...
`test-engineer` agent again and tell it to read `.vivekmind/issues/issue-<number>.md`
