...
function convertItemDefinitionToJsonSchema(itemDef) {
    const schema = {};
    ...
    if (itemDef.type === 'object' && itemDef.additionalProperties !== undefined) {
        if (typeof itemDef.additionalProperties === 'boolean') {
            ...
            schema.additionalProperties = convertItemDefinitionToJsonSchema(itemDef.additionalProperties);
        }
    }
    if (itemDef.items) {
        schema.type = 'array';
...
function convertSettingToJsonSchema(setting) {
    const schema = {};
    ...
    switch (setting.type) {
        ...
        case 'enum':
            if (setting.options && setting.options.length > 0) {
                ...
                const optionsText = 'Options: ' + setting.options.map((o) => `${o.value}`).join(', ');
        ...
        case 'object':
            ...
            if (setting.properties) {
                ...
                for (const [key, childDef] of Object.entries(setting.properties)) {
                    schema.properties[key] = convertSettingToJsonSchema(childDef);
                }
            }
            else {
...
function generateJsonSchema(settingsSchema) {
    const jsonSchema = {
        $schema: 'http://json-schema.org/draft-07/schema#',
        type: 'object',
        description: 'VivekMind settings configuration',
        properties: {},
        additionalProperties: true,
    };
    for (const [key, setting] of Object.entries(settingsSchema)) {
        jsonSchema.properties[key] = convertSettingToJsonSchema(setting);
    }
...
const outputDir = path.resolve(__dirname, '../packages/vscode-ide-companion/schemas');
