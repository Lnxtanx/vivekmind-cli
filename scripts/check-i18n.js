 ...
 */
function checkKeyMatching(enTranslations, localeTranslations, localeLabel) {
    const errors = [];
...
async function checkI18n() {
    ...
    try {
        ...
        errors.push(`Failed to load zh-TW.js: ${error instanceof Error ? error.message : String(error)}`);
        return {
            success: false,
            errors,
            warnings,
    ...
    const matchingErrors = checkKeyMatching(enTranslations, zhTranslations, 'zh.js');
    errors.push(...matchingErrors);
    // Check key matching between en and zh-TW
    const matchingTWErrors = checkKeyMatching(enTranslations, zhTWTranslations, 'zh-TW.js');
