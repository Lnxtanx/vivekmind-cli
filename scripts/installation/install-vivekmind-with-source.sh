...
# VivekMind Installation Script
# This script installs Node.js (via NVM) and VivekMind CLI
# Supports Linux and macOS
#
# Usage: install-vivekmind-with-source.sh --source [github|npm|internal|local-build]
#        install-vivekmind-with-source.sh -s [github|npm|internal|local-build]
...
# ============================================
echo "=========================================="
echo "   VivekMind Installation Script"
echo "=========================================="
echo ""
...
install_nvm() {
    ...
    if "${DOWNLOAD_CMD}" "${DOWNLOAD_ARGS}" "https://vivekmind-assets.oss-cn-hangzhou.aliyuncs.com/installation/install_nvm.sh" > "${NVM_INSTALL_TEMP}"; then
    ...
    # Check if profile file is writable
        ...
        {
            echo ""
            echo "# NVM configuration (added by VivekMind installer)"
...
fix_npm_permissions() {
    ...
    if [[ "${use_user_dir}" == true ]]; then
        ...
        if [[ -n "${PROFILE_FILE}" ]] && ! grep -q '.npm-global/bin' "${PROFILE_FILE}" 2>/dev/null; then
            {
                echo ""
                echo "# NPM global bin (added by VivekMind installer)"
...
install_vivekmind_code() {
    ...
    # Install VivekMind
    log_info "Installing VivekMind..."
    if npm install -g vivekmind@latest --registry https://registry.npmmirror.com; then
        log_success "VivekMind installed successfully!"
...
EOF
    ...
    log_success "Installation source saved to ~/.vivekmind/source.json"
}
...
main() {
    ...
    # Check if vivekmind is immediately available
    if command_exists vivekmind; then
        log_success "VivekMind is ready to use!"
        echo ""
        ...
        log_warning "VivekMind command not found in current session"
        echo ""
        echo "To use VivekMind immediately without restarting your terminal,"
        echo "run the following command in your current shell:"
        echo "  eval \$(${0} --print-env)"
        echo ""
        log_info "Or simply restart your terminal, then run: vivekmind"
    fi
}
