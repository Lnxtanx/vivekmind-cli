@echo off
REM Script to install Node.js and VivekMind with source information
REM This script handles the installation process and sets the installation source
REM
REM Usage: install-vivekmind-with-source.bat --source <source>
REM        install-vivekmind-with-source.bat -s <source>
REM
...
echo ===========================================
echo VivekMind Installation Script with Source Tracking
echo ===========================================
echo.
...
if !ERRORLEVEL! EQU 0 (
    ...
    if !MAJOR_VERSION! GEQ 20 (
        ...
        if !ERRORLEVEL! NEQ 0 (
            echo ERROR: Failed to install Node.js. Cannot continue with VivekMind installation.
            exit /b 1
        )
    )
) else (
...
REM Install VivekMind with source information
echo INFO: Installing VivekMind with source: %SOURCE%
echo INFO: Running: %NPM_CMD% install -g vivekmind@latest --registry https://registry.npmmirror.com
call "%NPM_CMD%" install -g vivekmind@latest --registry https://registry.npmmirror.com

if %ERRORLEVEL% EQU 0 (
    echo SUCCESS: VivekMind installed successfully!
) else (
    echo ERROR: Failed to install VivekMind.
    exit /b 1
)
...
if %ERRORLEVEL% EQU 0 (
    ...
    echo.
    echo INFO: Starting VivekMind...
    echo.
    call vivekmind
) else (
    echo WARNING: VivekMind may not be in PATH. Please check your npm global bin directory.
    echo.
    echo ===========================================
    echo SUCCESS: Installation completed!
    echo The source information is stored in %USERPROFILE%\.vivekmind\source.json
    echo.
    echo ===========================================
)
...
set "TEMP_DIR=%TEMP%\vivekmind-nodejs-install"
